...
# ==========================================
# PRE-LOAD ORB + FIRST 15 MIN FROM KITE API (LIVE MODE)
# ==========================================
def load_orb_and_first15_from_kite():
    """
    Fetches TODAY's 1-minute candles from Kite historical API for each symbol
    and builds orb_data + first15_data BEFORE the WebSocket connects.

    This is the correct approach for live paper trading where no local CSV
    exists yet for today.

    - ORB     = High/Low of all 1-min candles from 09:15 to 10:14 (60-min range)
    - First15 = High/Low of all 1-min candles from 09:15 to 09:29 (15-min range)

    If the market is still inside the ORB / First-15 window when this runs,
    the ranges will be partial — but build_context() will continue extending
    them tick-by-tick until the window closes, so that is perfectly fine.
    """

    today = datetime.now().date()
    now_time = datetime.now().strftime("%H:%M")

    print(f"📡 Fetching today's ORB & First-15 from Kite API for {today}...")

    # Reverse map: symbol → token  (instrument_tokens is token → symbol)
    ...
    for symbol in SYMBOLS:
        ...
        try:
            # Fetch only today's minute candles — very fast, ~1-2 candles per call
            ...
            if not orb_df.empty:
                orb_ready = now_time >= "10:15"   # freeze if window already closed
                orb_data[symbol] = {
            ...
            if not f15_df.empty:
                f15_ready = now_time >= "09:30"   # freeze if window already closed
                first15_data[symbol] = {
                    "open":  f15_df.iloc[0]["open"],
                    "high":  f15_df["high"].max(),
                    "low":   f15_df["low"].min(),
                    "close": f15_df.iloc[-1]["close"],
                    "ready": f15_ready
                }
    ...
    print(f"✅ ORB & First-15 loaded from Kite API for {loaded} symbols")
