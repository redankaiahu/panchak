...
def load_ema_data_from_local(replay_date=None):
    ...
    for symbol in SYMBOLS:
        ...
        if replay_date:
            df = df[df["date"] < _safe_to_naive(pd.Series([pd.to_datetime(replay_date)])).iloc[0]]
...
def paper_position_manager():
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            # Use candle low/high — catches breaches between ticks
            if side == "BUY":
...
def _extra(ctx):
    ...
    return {
        ...
        "t_open":     f.get("open", 0),          # today's actual open (09:15 first tick)
        "t_high":     latest_highs.get(symbol, orb.get("high", 0)),   # true intraday high
        "t_low":      latest_lows.get(symbol,  orb.get("low", 0)),    # true intraday low
    }
...
def strategy_orb(ctx):
    ...
    if vol_percent < -40:
        _scan_record(_SNAME, symbol, f"Vol {vol_percent:.0f}% < 40%")
        return
...
def strategy_open_low(ctx):
    ...
    # ── Context filter: Open=Low BUY requires bullish gap context ────────
    # If stock gapped DOWN below yesterday's low → bearish context → no BUY
    # If stock gapped UP above yesterday's high → bullish but already extended → no BUY
    # Valid BUY zone: today opened between yesterday's low and yesterday's high (or slight gap up ≤1%)
    #
    # Open=High SELL requires bearish gap context:
    # If stock gapped UP above yesterday's high → bullish context → no SELL
    # Valid SELL zone: today opened between yesterday's low and yesterday's high (or slight gap down ≤1%)

    today_open = f["open"]
    gap_from_yl = ((today_open - yest["low"]) / yest["low"]) * 100    # +ve = above YL (good for BUY)
    gap_from_yh = ((yest["high"] - today_open) / yest["high"]) * 100  # +ve = below YH (good for SELL)

    # For SELL: also block if today opened BELOW yesterday's low already
    # (stock already in breakdown — chasing it further is risky)
    ...
    buy_signal = (
        ...
        gap_from_yl >= -1.0    # today open must be at or above yesterday low (not gap-down context)
    )
    sell_signal = (
        299 <= ltp <= 6999 and
        open_is_high and
        ...
        gap_from_yh >= -1.0 and            # today open must be at or below yesterday high
        not today_opened_below_yl          # today must NOT have opened already below YL
    )
    ...
    if buy_signal:
        ...
        if not open_is_low and not open_is_high:
            ...
            reason = f"SELL: today opened {today_open:.2f} already below YL {yest['low']:.2f} — breakdown already done"
...
def strategy_ema_pullback(ctx):
    ...
    if vol_percent < -25:
        _scan_record(_SNAME, symbol, f"Vol {vol_percent:.0f}% < 25%")
        return
...
def strategy_inside_bar(ctx):
    ...
    # Use ORB high/low (covers full day so far) as proxy for today's range.
    orb = orb_data.get(symbol, {})
    today_high = orb.get("high", 0)
    today_low  = orb.get("low", 999999)

    if today_high == 0 or today_low == 999999:
        _scan_record(_SNAME, symbol, "ORB data not available for today range check")
        return
...
def strategy_gap_first15(ctx):
    ...
    # ── Gap filter: must be between 0.1% and 1.5% (both directions) ─────
    abs_gap = abs(true_gap_pct)
    if abs_gap < 0.1 or abs_gap > 1.5:
        _scan_record(_SNAME, symbol,
            f"Gap {true_gap_pct:.2f}% outside 0.1–1.5% range")
        return

    # ── Move from yesterday close to first15 extreme must be < 2% ────────
    # BUY  side: how far did price rally from yclose to f15 high?
    # SELL side: how far did price drop  from yclose to f15 low?
    move_to_f15_high = ((f["high"] - yest_close) / yest_close) * 100   # +ve = gap+rally
    move_to_f15_low  = ((yest_close - f["low"])  / yest_close) * 100   # +ve = gap+drop

    # For gap-up  BUY  → total move (yclose → f15 high) must be < 2%
    # For gap-down SELL → total move (yclose → f15 low)  must be < 2%
    if true_gap_pct > 0 and move_to_f15_high >= 2.0:
        _scan_record(_SNAME, symbol,
            f"Gap-up: yclose→f15_high {move_to_f15_high:.2f}% >= 2%")
        return
    if true_gap_pct < 0 and move_to_f15_low >= 2.0:
        _scan_record(_SNAME, symbol,
            f"Gap-down: yclose→f15_low {move_to_f15_low:.2f}% >= 2%")
        return
    ...
    buy_signal = (
        true_gap_pct > 0 and              # gap UP (actual 09:15 open vs yclose)
        candle_is_green and               # first 15min closed green
        ltp > f["high"] and               # price broke above first15 high
        dist_from_yh_pct <= MAX_OVEREXTENSION_PCT   # not already overextended
    )

    # ── SELL: Gap Down + Red First15 + Break below First15 Low ──────────
    sell_signal = (
        true_gap_pct < 0 and              # gap DOWN (actual 09:15 open vs yclose)
        candle_is_red and                 # first 15min closed red
        ltp < f["low"] and                # price broke below first15 low
        dist_from_yl_pct <= MAX_OVEREXTENSION_PCT   # not already overextended
    )
    ...
    if buy_signal:
        ...
        if abs_gap < 0.1 or abs_gap > 1.5:
            ...
            reason = (f"No setup: gap={true_gap_pct:.2f}% "
                      f"f15={f['open']:.2f}/{f['high']:.2f}/{f['low']:.2f}/{f['close']:.2f} "
                      f"ltp={ltp}")
        _scan_record(_SNAME, symbol, reason)
