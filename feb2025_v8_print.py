...
def evaluate_buy_condition_1(data,ltp,open_price,high_price,low_price,volume, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,Y_H_C_d,Y_H_L_d,Y_C_L_d,YHCon,YLCon,latest_max_5,latest_max_120,SMA_Volume_5, ema_200, ema_50, ema_20, ema_13,conditionB1,conditionB2,conditionB3,conditionB4,conditionI1,conditionI2,conditionSB,conditionS1,conditionS2,conditionS3,conditionS4,YCCon):
    try:
        if (
            ...
            data["yesterday_close"] > data["yesterday_open"]  
            #ltp > open_price and
            ...
            data["daybefore_low"] < data["yesterday_low"]
            #ltp > open_price and
            #high_price < YHCon and
            #ltp < YHCon
            ...
            ltp > data["yesterday_high"] and
            ltp > data["yesterday_close"] and
            data["daybefore_high"] > data["yesterday_high"] 
            #ltp > open_price and
            #high_price < YHCon and
            #ltp < YHCon
        ):
            print(f"BUY cond 1 -- {symbol_name}: day's Open: {open_price}, High: {high_price}, Low: {low_price}, LTP: {ltp}")
            ...
            print(f"Y_H_C_d: {Y_H_C_d}, Y_H_L_d: {Y_H_L_d}, Y_C_L_d: {Y_C_L_d}\n")
...
def evaluate_buy_condition_2(data,ltp,open_price,high_price,low_price,volume, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,Y_H_C_d,Y_H_L_d,Y_C_L_d,YHCon,YLCon,latest_max_5,latest_max_120,SMA_Volume_5, ema_200, ema_50, ema_20, ema_13,conditionB1,conditionB2,conditionB3,conditionB4,conditionI1,conditionI2,conditionSB,conditionS1,conditionS2,conditionS3,conditionS4,YH3Con,YCCon):
    try:
        if (
            ...
            ltp > data["yesterday_high"] and
            ltp > data["three_days_ago_high"] 
            #open_price < YH3Con and
            #high_price < YH3Con and
            #ltp < YH3Con
        ):
            print(f"BUY cond 2 -- {symbol_name}: day's Open: {open_price}, High: {high_price}, Low: {low_price}, LTP: {ltp}")
            print(f"Y_Open: {data['yesterday_open']}, High: {data['yesterday_high']}, Low: {data['yesterday_low']}, Close: {data['yesterday_close']}, volume: {volume}, yesterday_vol:{data['yesterday_vol']}, daybefore_vol: {data['daybefore_vol']}")
            print(f"Y_H_C_d: {Y_H_C_d}, Y_H_L_d: {Y_H_L_d}, Y_C_L_d: {Y_C_L_d}\n")
...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    if symbol not in modified_symbols:
        modified_symbols[symbol] = {"0.5%": False, "1%": False, "2%": False, "3%": False}
    ...
    try:
        ...
        sl_move_0_002_percent = executed_price * 0.0009  # 0.002% SL move when LTP moves 0.5%
        sl_move_0_5_percent = executed_price * 0.005  # 0.5% SL move when LTP moves 1%
...
def should_place_order():
    current_time = datetime.now().time()
    cutoff_time_start = datetime.strptime('00:18:05', '%H:%M:%S').time()  # After 9:21 AM
    cutoff_time_end = datetime.strptime('23:59:00', '%H:%M:%S').time()    # Before 2:50 PM
    ...
    with websocket_lock:
        ...
        try:
            while not terminate_program:
                now = datetime.now()
                current_time = datetime.now().time()
                market_close_time = now.replace(hour=23, minute=59, second=0, microsecond=0)
                ...
                while time.time() - start_time < 1240:
