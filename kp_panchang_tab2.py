...
def _tg_open(slot, p1,p2,p3,p4, nifty_ltp=0, bnf_ltp=0, interp=None):
    ...
    msg = (f"🟡🟡🟡🟡🟡🟡🟡🟡🟡🟡\n"
           ...
           f"   ({pnames})\n"
           f"📡 Signal  : <b>{sig}</b>\n"
           f"📊 NIFTY spot: <b>{nifty_ltp:,.0f}</b>\n"
           f"🏦 BNIFTY spot: <b>{bnf_ltp:,.0f}</b>\n"
    ...
    if today_df.empty:
        ...
        for _, row in today_df.iterrows():
            ...
            if _need_fetch_o and not ss_o.get("auto_fetched"):
                for sym_f, hk_f, lk_f in [("NIFTY","nh","nl"),("BANKNIFTY","bh","bl")]:
            ...
            def _slot_candles(sym_c, odf, sh, sm2, eh, em2):
                if odf.empty or sym_c not in odf["symbol"].values:
                    return pd.DataFrame()
                return odf[
                    (odf["symbol"] == sym_c) &
                    (odf["datetime"].dt.hour*60 + odf["datetime"].dt.minute >= sh*60+sm2) &
                    (odf["datetime"].dt.hour*60 + odf["datetime"].dt.minute <  eh*60+em2)
            ...
            for cdf_x, hk_x, lk_x in [(n_cdf,"nh","nl"),(bn_cdf,"bh","bl")]:
                if not cdf_x.empty:
            ...
            all_times_o = sorted(set(n_times_o + bn_times_o)) or [
                s_str_o,
            ...
            with fc1:
                if st.button("🔄 Fetch OHLC", key=f"fetch_top_{slot_o}"):
                    with st.spinner("Fetching..."):
                        for sym_f2,hk_f2,lk_f2 in [("NIFTY","nh","nl"),("BANKNIFTY","bh","bl")]:
            ...
            st.markdown(
                ...
                f'<thead>{hdr_n}</thead>'
                f'<tbody>{rows_n}{spacer}{rows_bn}</tbody>'
                f'</table></div>',
                unsafe_allow_html=True,
            )
            ...
            if tb5.button("📋 Sum",  key=f"btntg_sum_top_{slot_o}"):
