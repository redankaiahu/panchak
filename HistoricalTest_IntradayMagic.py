    ...
    try:
        ...
        if isinstance(historical_data, pd.DataFrame) and not historical_data.empty:
            ...
            if historical_data.shape[0] >= 3:
                ...
                yesterday_data = historical_data.iloc[-1] # Testing with days change
                ...
                #yesterday_data = historical_data.iloc[-1] #original
                #day_before_data = historical_data.iloc[-2] #original
                #three_days_ago = historical_data.iloc[-3]   #original
                #four_days_ago = historical_data.iloc[-4]    #original
                ...
                return {
                    ...
                    "five_days_ago_low": float(five_days_ago['low']),
                    "five_days_ago_high": float(five_days_ago['high']),
                    "five_days_ago_open": float(five_days_ago['open']),
                    "five_days_ago_close": float(five_days_ago['close']),
                    "five_days_ago_vol": float(five_days_ago['volume']),
...
def evaluate_buy_condition_1(historical_data,symbol,Intraday_Magic_list):
    ...
    if historical_data:
        ...
        five_days_ago_open = round(float(historical_data.get("five_days_ago_open")),2)
        five_days_ago_close = round(float(historical_data.get("five_days_ago_close")),2)
        five_days_ago_high = round(float(historical_data.get("five_days_ago_high")),2)
        five_days_ago_low = round(float(historical_data.get("five_days_ago_low")),2)
        five_days_ago_vol = round(float(historical_data.get("five_days_ago_vol")),2)
        ...
        SMA_5 = round(float(historical_data_one['SMA_5'].iloc[-1]),2)
        ...
        #low_values = historical_data.iloc[:60]['low'].tolist()
        #high_values = historical_data.iloc[:60]['high'].tolist()
        #low_values = historical_df.iloc[:60]['low'].tolist()
        #high_values = historical_df.iloc[:60]['high'].tolist()
        #low_values = historical_df['low'].iloc[:60].tolist()
        #high_values = historical_df['high'].iloc[:60].tolist()
        ...
        ORBL15_Sell_diff = round(float(((ORBL15_one - yesterday_close) / ORBL15_one) * 100),2)# caluculating percentage, when sell from ORBL15 to ltp
        ORBL15_Sell_diff1 = round(float(ORBL15_one - yesterday_close),2)                        # caluculating in numbers , when sell from ORBL15 to ltp

        ORBH15_buy_diff = round(float((( yesterday_close - ORBH15_one) / yesterday_close) * 100),2) # caluculating percentage, when buy from ORBH15 to ltp
        ORBH15_buy_diff1 = round(float( yesterday_close - ORBH15_one),2)                            # caluculating in numbers , when buy from ORBH15 to ltp

        ORBH15_ORBL15_diff = round(float((( ORBH15_one - ORBL15_one)/ORBH15_one) * 100),2) #  calculating ORBH15 and ORBL15 range in %
        ...
        if  (            
            #high_low_diff <= 3 and
            #yesterday_high > SMA_5 and
            daybefore_close <= daybefore_open and 
            three_days_ago_close <= three_days_ago_open and 
            four_days_ago_close <= four_days_ago_open and
            five_days_ago_close <= five_days_ago_open and
            yesterday_open >=  daybefore_close and
            daybefore_low < three_days_ago_low and
            daybefore_vol > 100000 
            #volume > data["daybefore_vol"] * 1.5 and
            ...
            #ltp > open_price
            #yesterday_open >  daybefore_close and
            #yesterday_open >  daybefore_low
        ):

            Intraday_Magic_list.append({
                "symbol": symbol_name,
                "open": yesterday_open,
                "high": yesterday_high,
                "low": yesterday_low,
                ...
                "high_close_diff": high_close_diff,
                "high_low_diff": high_low_diff,
                "ORBH15_buy_diff1": ORBH15_buy_diff1,
                "ORBH15_buy_diff": ORBH15_buy_diff,
                "ORBH15_ORBL15_diff": ORBH15_ORBL15_diff,
                "SMA_5": SMA_5
            })
...
def main_loop():
    ...
    for symbol in symbols:
        historical_data = fetch_and_filter_symbol(symbol)
        if historical_data:
            evaluate_buy_condition_1(historical_data,symbol,Intraday_Magic_list)
    #evaluate_buy_condition_1()
    # Print results for OPEN == LOW
    
    if Intraday_Magic_list:
        print("\nSymbols with Intraday_Magic_list:")
        for item in Intraday_Magic_list:
            print(f"{item['symbol']}: SMA_5: {item['SMA_5']},Open: {item['open']}, High: {item['high']}, Low: {item['low']}, Close: {item['close']}, day2_open: {item['day2_open']}, day2_high: {item['day2_high']}, day2_low: {item['day2_low']}, day2_close: {item['day2_close']},ORBH15:{item['ORBH15_one']},ORBL15:{item['ORBL15_one']},day1_V: {item['yesterday_vol']},day2_V:{item['daybefore_vol']},high_close_diff: {item['high_close_diff']},high_low_diff: {item['high_low_diff']},ORBH15_buy_diff1: {item['ORBH15_buy_diff1']},ORBH15_buy_diff: {item['ORBH15_buy_diff']},ORBH15_ORBL15_diff: {item['ORBH15_ORBL15_diff']}\n")
    
    #if high_orbh15_list:
     #   print("\nSymbols with high == orbh15_list:")
      #  for item in high_orbh15_list:
       #     print(f"{item['symbol']}:Open:{item['open']},High:{item['high']},Low:{item['low']},Close:{item['close']},d2_O:{item['day2_open']},d2_H:{item['day2_high']},d2_L:{item['day2_low']},d2_C:{item['day2_close']},ORBH15:{item['ORBH15_one']},ORBL15:{item['ORBL15_one']},day1_V: {item['yesterday_vol']},day2_V:{item['daybefore_vol']},high_close_diff: {item['high_close_diff']},high_low_diff: {item['high_low_diff']},ORBL15_Sell_diff1: {item['ORBL15_Sell_diff1']},ORBL15_Sell_diff: {item['ORBL15_Sell_diff']},ORBH15_ORBL15_diff: {item['ORBH15_ORBL15_diff']}\n")
