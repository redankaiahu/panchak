...
# ═══════════════════════════════════════════════════════════════════
# 🔢 NUMEROLOGY ENGINE — @pankajkummar369 system
# Root number = sum all digits of DD+MM+YYYY → reduce to 1-9
# Each root = a planet → bullish/bearish market signal
# ═══════════════════════════════════════════════════════════════════
_NUMEO_PLANET = {
...
_NUMEO_SIGNAL = {
    1:("🟢 BULLISH",   "Buy dips — Sun energy, new beginnings, leadership"),
    2:("🟡 NEUTRAL",   "Wait & watch — Moon emotional swings"),
    3:("🟢 BULLISH",   "Growth & expansion — Jupiter blesses longs"),
    4:("🔴 BEARISH",   "Rahu volatile — extreme swings, beware false rallies"),
    5:("🟡 VOLATILE",  "Mercury choppy — IT/banking two-sided, no bias"),
    6:("🟢 BULLISH",   "Venus rally — buy dips, FMCG/banks supportive"),
    7:("🔴 BEARISH",   "Ketu detachment — avoid longs, losses likely"),
    8:("🔴 BEARISH",   "Saturn slowdown — sell on rise, restrict longs"),
    9:("🔴 EXTREME",   "Mars CRASH risk — war energy, strong sell, biggest falls"),
}
_NUMEO_ACTION = {
    1:"✅ BUY dips at open, hold till 14:00",
    2:"⚠️ WAIT till 10:30 for direction",
    3:"✅ BUY dips, targets 0.5–1% above open",
    4:"⛔ SHORT SELL rallies — volatile, traps likely",
    5:"⚠️ SCALP only — avoid overnight",
    6:"✅ BUY dips, FMCG/banks/silver outperform",
    7:"❌ AVOID longs — short weak stocks",
    8:"❌ SELL on rise — targets 1–2% below open",
    9:"❌ STRONG SELL — biggest crash days, no longs",
}

def _numeo_root(d) -> int:
    """Compute numerology root number for a date object."""
    s = sum(int(c) for c in d.strftime("%d%m%Y"))
    while s > 9:
        s = sum(int(c) for c in str(s))
    return s

def _numeo_day(d) -> dict:
    """Return full numerology reading for a date."""
    root   = _numeo_root(d)
    planet = _NUMEO_PLANET[root]
    sig, desc = _NUMEO_SIGNAL[root]
    action    = _NUMEO_ACTION[root]
    digits    = list(d.strftime("%d%m%Y"))
    digit_sum = sum(int(c) for c in digits)
    calc      = "+".join(digits) + "=" + str(digit_sum)
    if digit_sum > 9:
        calc += "=" + str(sum(int(c) for c in str(digit_sum)))
    return {
        "date": d, "root": root, "planet": planet,
        "signal": sig, "desc": desc, "action": action,
        "calc": calc, "digit_sum": digit_sum,
    }
...
<div style="max-width:700px;margin:auto;background:#111;border-radius:10px;overflow:hidden">
  ...
  <div style="background:#0a0a0a;padding:10px 20px">
    ...
    # ═══════════════════════════════════════════════════════════════
    # 🔢 NUMEROLOGY PANEL — @pankajkummar369 system
    # ═══════════════════════════════════════════════════════════════
    st.divider()
    st.subheader("🔢 Numerology Signal — Date Digit Sum System")
    st.caption(
        "System used by @pankajkummar369 on X. "
        "Sum all digits of DD+MM+YYYY → root number → planet ruler → market signal. "
        "Tested on multiple 2025–2026 predictions with good accuracy. **Not financial advice.**"
    )

    _nc1, _nc2, _nc3 = st.columns([2, 3, 3])
    with _nc1:
        _numeo_sel_date = st.date_input(
            "Date", value=datetime.now(IST).date(),
            min_value=date(2020,1,1), max_value=date(2030,12,31),
            key="numeo_date_sel"
        )
    _nd = _numeo_day(_numeo_sel_date)
    _nr = _nd["root"]
    _np = _nd["planet"]
    _ns = _nd["signal"]
    _na = _nd["action"]
    _nc = _nd["calc"]
    _ncol = "#00C851" if "BULLISH" in _ns else ("#FF4444" if "BEARISH" in _ns or "EXTREME" in _ns else "#FFD700")

    with _nc2:
        st.markdown(f"""
<div style="background:#111;border-radius:10px;padding:14px 18px;margin-top:4px">
  <div style="font-size:13px;color:#aaa;margin-bottom:4px">🔢 Digit calculation</div>
  <div style="font-size:18px;font-weight:700;color:#fff;letter-spacing:1px">{_nc}</div>
  <div style="font-size:22px;font-weight:800;color:{_ncol};margin-top:6px">
    Root {_nr} — {_np}
  </div>
  <div style="font-size:15px;font-weight:600;color:{_ncol};margin-top:4px">{_ns}</div>
  <div style="font-size:13px;color:#ccc;margin-top:6px">{_nd["desc"]}</div>
</div>""", unsafe_allow_html=True)

    with _nc3:
        st.markdown(f"""
<div style="background:#111;border-radius:10px;padding:14px 18px;margin-top:4px">
  <div style="font-size:13px;color:#aaa;margin-bottom:6px">💡 Trading action</div>
  <div style="font-size:15px;font-weight:700;color:{_ncol}">{_na}</div>
  <div style="margin-top:14px;font-size:12px;color:#888">
    Source: @pankajkummar369 numerology system<br>
    Root 1/3/6 = Bullish · Root 4/7/8 = Bearish · Root 9 = Extreme crash risk
  </div>
</div>""", unsafe_allow_html=True)

    # Next 10 trading days numerology forecast
    st.markdown("##### 📅 Next 10 Trading Days — Numerology Forecast")
    _numeo_rows = []
    _nd_check = datetime.now(IST).date()
    _nd_found = 0
    for _ndi in range(1, 30):
        _nfd = _nd_check + timedelta(days=_ndi)
        if _nfd.weekday() >= 5 or _nfd in NSE_HOLIDAYS:
            continue
        _ndr = _numeo_day(_nfd)
        _numeo_rows.append({
    ...
    if _numeo_rows:
        import pandas as _pd_n
        _ndf = _pd_n.DataFrame(_numeo_rows)
        def _numeo_sig_style(val):
            v = str(val)
            if "BULLISH" in v: return "background-color:#003300;color:#00ff88;font-weight:700"
            if "EXTREME" in v: return "background-color:#220000;color:#ff2222;font-weight:700"
            if "BEARISH" in v: return "background-color:#1a0000;color:#ff6666;font-weight:700"
            return "background-color:#222200;color:#ffdd00"
        def _numeo_root_style(val):
            v = int(val)
            if v in (1,3,6): return "color:#00ff88;font-weight:700"
            if v == 9:        return "color:#ff2222;font-weight:700"
            if v in (4,7,8):  return "color:#ff8888;font-weight:600"
            return "color:#ffdd00"
        st.dataframe(
            _ndf.style
                .map(_numeo_sig_style, subset=["Signal"])
                .map(_numeo_root_style, subset=["Root"])
                .set_properties(**{"font-size":"12px","text-align":"left"}),
            use_container_width=True, height=380
        )

    # Combine with today's Vedic astro score
    try:
        _today_vedic = _vedic_day_analysis(datetime.now(IST).date())
        _vedic_score = _today_vedic["net_score"]
        _today_numeo = _numeo_day(datetime.now(IST).date())
        _tn_root     = _today_numeo["root"]

        # Agreement check
        _numeo_bull  = _tn_root in (1, 3, 6)
        _numeo_bear  = _tn_root in (4, 7, 8, 9)
        _vedic_bull  = _vedic_score >= 2
        _vedic_bear  = _vedic_score <= -2

        if _numeo_bull and _vedic_bull:
            _combo = ("🟢 DOUBLE CONFIRMED BULLISH", "#00C851",
                      "Both Vedic astro AND numerology agree: BUY day. High confidence.")
        elif _numeo_bear and _vedic_bear:
            _combo = ("🔴 DOUBLE CONFIRMED BEARISH", "#FF4444",
                      "Both Vedic astro AND numerology agree: SELL day. High confidence.")
        elif _numeo_bull or _vedic_bull:
            _combo = ("🟢 MILD BULLISH BIAS", "#88ff88",
                      f"One signal bullish (Numerology root {_tn_root} / Vedic score {_vedic_score:+d}).")
        elif _numeo_bear or _vedic_bear:
            _combo = ("🔴 MILD BEARISH BIAS", "#ff8888",
                      f"One signal bearish (Numerology root {_tn_root} / Vedic score {_vedic_score:+d}).")
        else:
            _combo = ("🟡 MIXED / NEUTRAL", "#FFD700",
                      "Numerology and Vedic astro give conflicting or neutral signals.")

        st.markdown(f"""
<div style="background:#111;border-radius:8px;padding:12px 16px;margin-top:8px">
  <div style="font-size:13px;color:#aaa">🔗 Numerology + Vedic Astro Combined</div>
  <div style="font-size:18px;font-weight:700;color:{_combo[1]};margin-top:4px">{_combo[0]}</div>
  <div style="font-size:13px;color:#ccc;margin-top:4px">{_combo[2]}</div>
  <div style="font-size:12px;color:#888;margin-top:6px">
    Numerology root {_tn_root} ({_today_numeo["planet"]}) · Vedic score {_vedic_score:+d}
  </div>
</div>""", unsafe_allow_html=True)
    except Exception:
        pass
