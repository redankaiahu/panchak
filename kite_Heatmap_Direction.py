...
LOG_TXT_FILE = f"heatmap_nifty_execution_log_{today_str}.txt"

paper_positions = {}
paper_trade_log_file = f"heatmap_nifty_paper_trades_{today_str}.csv"
...
# ==========================================
TRADES_CACHE_FILE   = f"heatmap_nifty_trades_cache_{today_str}.csv"
POSITIONS_CACHE_FILE = f"heatmap_nifty_positions_cache_{today_str}.csv"
...
_logged_triggers    = set()  # symbols already logged as TRIGGER today — prevents repeated "🔴 S9/S10" prints
...
DAILY_MAX_LOSS_PERCENT = 100       # 👈 Effectively disable %-based halt
HARD_LOSS_LIMIT = -1500            # 👈 Fixed Rupee Limit active
...
PROFIT_LOCK_FLOOR    = 1000     # ₹ — minimum profit to preserve once locked (₹1 500 guaranteed)
HARD_LOSS_LIMIT      = -1500    # ₹ — square off ALL positions + halt when total PnL hits this
                                #      Works for both PAPER and LIVE. Change here to adjust.
GROSS_WIN_TARGET     = 5000     # ₹ — close ALL positions + halt when gross winning (sum of all profitable
...
def _update_nifty_bias(new_ltp):
    ...
    with _nifty_lock:
        ...
        if _nifty_pending_count >= NIFTY_REVERSAL_TICKS:
            ...
            # NIFTY REVERSAL suppressed for Strategy C (Heatmap Engine)
            ...
            # cost-to-cost reversal thread also disabled for Strategy C
            # threading.Thread(
...
# ── Heatmap Direction Thresholds ──────────────────────────────────────────────
HM_BULL_THRESHOLD = 65.0      # % bullishness required for BUY strategies
HM_BEAR_THRESHOLD = 35.0      # % bullishness below which SELL strategies trigger

# ── Nifty 50 Weights for Heatmap ─────────────────────────────────────────────
N50_WEIGHTS = {
...
# Heatmap State
n50_bull_pct = 50.0  # global tracking
n50_impact   = 0.0
heatmap_bias = "NEUTRAL"
heatmap_color = Fore.WHITE

def update_heatmap_logic():
    """
    Background thread: periodically computes Nifty 50 heatmap % based on
    live stock prices vs yesterday's close.
    """
    global n50_bull_pct, n50_impact, heatmap_bias, heatmap_color
    while True:
        try:
            bull_wt = 0.0
            bear_wt = 0.0
            ...
            for sym, wt in N50_WEIGHTS.items():
                ltp = latest_prices.get(sym, 0)
                # In this script, yesterday_data[sym]["close"] is yesterday's close
                y_data = yesterday_data.get(sym)
                if not y_data: continue
                yc = y_data.get("close", 0)
                if ltp > 0 and yc > 0:
                    chg = (ltp - yc) / yc * 100
                    total_impact += (wt * chg / 100)
                    if chg > 0.25:
                        bull_wt += wt
                    elif chg < -0.25:
                        bear_wt += wt
            
            if bull_wt + bear_wt > 0:
                old_bias = heatmap_bias
                n50_bull_pct = (bull_wt / (bull_wt + bear_wt)) * 100
                n50_impact   = total_impact
                
                if n50_bull_pct > HM_BULL_THRESHOLD:
                    heatmap_bias = "BULLISH (BUY)"
                    heatmap_color = Fore.GREEN
                elif n50_bull_pct < HM_BEAR_THRESHOLD:
                    heatmap_bias = "BEARISH (SELL)"
                    heatmap_color = Fore.RED
                else:
                    heatmap_bias = "NEUTRAL"
                    heatmap_color = Fore.YELLOW
                
                if heatmap_bias != old_bias:
                    msg = f"🔥 HEATMAP BIAS CHANGE: {heatmap_bias} | Bullish: {n50_bull_pct:.1f}% | Impact: {n50_impact:+.2f}"
                    print(heatmap_color + msg + ts())
                    write_log(msg)
        except Exception:
            pass
        time.sleep(10)

def is_high_conviction_inside_bar(symbol, ltp, side):
    """
    Checks if an inside bar setup is High Conviction (near the breakout point).
    """
    y_data = yesterday_data.get(symbol)
    if not y_data: return False
    yh = y_data.get("high", 0)
    yl = y_data.get("low", 0)
    if yh == 0 or yl == 0: return False
    
    if side == "BUY":
        if ltp >= yh * 0.998 and ltp <= yh * 1.002:
            return True
    else: # SELL
        if ltp >= yl * 0.998 and ltp <= yl * 1.002:
            return True
    return False
...
def place_trade(symbol, side, ltp, tick, condition_name, extra_info=None, custom_sl=None, tsl_only=False, tsl_step=None, bypass_nifty_filter=False):
    ...
    # BUY only if Heatmap is BULLISH
    # SELL only if Heatmap is BEARISH
    if side == "BUY" and heatmap_bias != "BULLISH (BUY)":
        _block_key = (symbol, side)
        if _block_key not in _nifty_bias_blocked:
            _nifty_bias_blocked.add(_block_key)
            print(Fore.YELLOW + f"⛔ HEATMAP BIAS BLOCK | {symbol} | Wanted:BUY | Current:{heatmap_bias} ({n50_bull_pct:.1f}%)" + ts())
        return
    if side == "SELL" and heatmap_bias != "BEARISH (SELL)":
        _block_key = (symbol, side)
        if _block_key not in _nifty_bias_blocked:
            _nifty_bias_blocked.add(_block_key)
            print(Fore.YELLOW + f"⛔ HEATMAP BIAS BLOCK | {symbol} | Wanted:SELL | Current:{heatmap_bias} ({n50_bull_pct:.1f}%)" + ts())
        return

    # ── Inside Bar High Conviction Guard ──────────────────────────────────
    if "Inside Bar" in condition_name:
        if not is_high_conviction_inside_bar(symbol, ltp, side):
            return
...
def check_and_add_pyramid(symbol, ltp):
    ...
    while True:
        ...
        # ── Early-exit if near-target guard already fired for this symbol ──
        # Prevents the "too close to target" message from spamming every tick.
        if pyr.get("pyramiding_done"):
            break
        ...
        if base_target is not None:
            ...
            if _remaining <= _sl_width:
                ...
                # ── FIX: mark pyramiding permanently done so this message
                # doesn't spam every tick. check_and_add_pyramid() is called
                # on every price update; without this flag the trigger_hit
                # condition stays True and re-evaluates endlessly.
                pyr["pyramiding_done"] = True
...
# ==========================================
YESTERDAY_CACHE_FILE = "heatmap_yesterday_ohlc_cache.csv"
...
def load_yesterday_from_local(replay_date):
    ...
    for symbol in SYMBOLS:
        ...
        file_path = os.path.join(DAILY_DIR, f"heatmap_{symbol}_daily.csv")
...
def strategy_open_high_breakdown(ctx):
    ...
    # BUG FIX: snap to tick_size so Kite does not reject the SL-M order in LIVE mode.
    _s9_tick = tick_sizes.get(symbol, 0.05)
    _sl_raw  = day_high * 1.003   # 0.3% above open/high
    ...
    if sell_signal:
        ...
        # Only print trigger once — skip if already traded this symbol today or already logged
        if symbol not in trades_taken and symbol not in _logged_triggers:
            _logged_triggers.add(symbol)
            print(Fore.RED +
    ...
    if ctx is None:
        ...
        if symbol and symbol in yesterday_data:
            ...
            if "09:18" <= current_time < "09:31":
                ...
                # S10 DISABLED — strategy_open_high_sell_at_919 not called at 09:18
                ...
                if _gross_win_live >= GROSS_WIN_TARGET:
                    ...
                    def _gwt_squareoff():
                        try:
                            ...
                            time.sleep(1)
                            _pos_list = kite.positions().get("day", [])
...
def run_market_replay_proper(replay_date):
    ...
    first5_data.clear()     # BUG FIX: S9/S10 used stale F5 candle from previous replay day
    ...
    _nifty_minute_data = {}
    _nifty_minute_file = os.path.join(MINUTE_DIR, "heatmap_NIFTY.csv")
    if os.path.exists(_nifty_minute_file):
        ...
        print(Fore.YELLOW +
              "⚠️  [REPLAY] heatmap_NIFTY.csv not found — bias will use static opening state. "
...
def fresh_daily_download():
    ...
    try:
        ...
        if nifty_daily:
            ...
            ndf.to_csv(os.path.join(DAILY_DIR, "heatmap_NIFTY_daily.csv"), index=False)
...
def download_today_minute_data():
    ...
    try:
        ...
        if _nifty_min:
            ...
            _nifty_path = os.path.join(MINUTE_DIR, "heatmap_NIFTY.csv")
...
def download_6_month_data():
    ...
    try:
        ...
        if _nifty_minute_all:
            ...
            _nmdf.to_csv(os.path.join(MINUTE_DIR, "heatmap_NIFTY.csv"), index=False)
    ...
    for token, symbol in instrument_tokens.items():
        ...
        try:
            ...
            if daily_data:
                ...
                daily_df.to_csv(
                    os.path.join(DAILY_DIR, f"heatmap_{symbol}_daily.csv"),
                    index=False
                )
...
<div class="tabs">
  ...
  <div class="tab" data-tab="hs" onclick="st('hs')">HISTORY <span class="tc" id="tc-hs">H</span></div>
</div>
<div class="main">
  ...
  <div class="pane" id="pane-hs">
    <div style="display:grid;grid-template-columns:250px 1fr;gap:20px">
      <div>
        <div class="sh">Past Trading Days</div>
        <div class="tw" style="max-height:600px;overflow-y:auto"><table id="thl"><thead><tr><th>Date</th><th>Day P&amp;L</th></tr></thead><tbody id="tbh-list"></tbody></table></div>
      </div>
      <div>
        <div class="sh" id="h-day-title">Trade Details (Select a Date)</div>
        <div class="tw"><table><thead><tr><th>Time</th><th>Symbol</th><th>Side</th><th>Entry</th><th>Exit</th><th>Qty</th><th>PnL</th><th>Reason</th></tr></thead><tbody id="tbh-trades"></tbody></table></div>
      </div>
    </div>
  </div>
...
async function fdh(){
  try{
    const r=await fetch(SRV+'/history?'+Date.now(),{cache:'no-store'});
    const d=await r.json();
    histData = d;
    renderHistory();
    document.getElementById('tc-hs').textContent=d.length;
  }catch(e){}
}
function renderHistory(){
  const tbh=document.getElementById('tbh-list');
  tbh.innerHTML=histData.length?histData.map((h,i)=>'<tr onclick="shd('+i+')" style="cursor:pointer"><td>'+h.date+'</td><td class="'+(h.pnl>=0?'pp':'pn')+'">₹'+h.pnl.toFixed(2)+'</td></tr>').join(''):'<tr><td colspan="2">No history files found</td></tr>';
}
function shd(i){
  const h=histData[i];
  document.getElementById('h-day-title').textContent='Trade Details ('+h.date+') | Day PnL: ₹'+h.pnl.toFixed(2);
  const tbt=document.getElementById('tbh-trades');
  tbt.innerHTML=h.trades.map(t=>{
    const pnl=parseFloat(t.PnL)||0;
    return '<tr><td>'+(t.Time?t.Time.slice(11,19):'—')+'</td><td class="sym">'+t.Symbol+'</td><td class="'+(t.Side==='BUY'?'buy':'sell')+'">'+t.Side+'</td><td>'+fn(t.Entry)+'</td><td>'+fn(t.Exit)+'</td><td>'+t.Qty+'</td><td class="'+pc(t.PnL)+'">₹'+pnl.toFixed(2)+'</td><td>'+et(t.Reason)+'</td></tr>'
  }).join('');
}
...
def _dashboard_get_json():
    ...
    summary = {
        ...
        "heatmap_pct":   round(n50_bull_pct, 1),
    }
    return _json.dumps({"summary": summary, "positions": positions,
                        "trades": trades, "log": log_lines}).encode()

def _dashboard_get_history():
    import json as _json, csv as _csv, os as _os, glob as _glob
    # Find both legacy and heatmap-prefixed files
    files = _glob.glob("*_paper_trades_*.csv")
    history = []
    for f in files:
        fname = _os.path.basename(f)
        # Extract date string (last part before .csv, e.g. 18-04-2026)
        date_str = fname.split("_")[-1].replace(".csv", "")
        try:
        ...
        try:
            if _os.path.exists(f):
                with open(f, newline='', encoding='utf-8', errors='replace') as fh:
                    ...
                    for t in trades:
                        # Sum PnL for base positions only
                        if "__PYR" not in str(t.get("Symbol", "")):
                            day_pnl += float(t.get("PnL", 0) or 0)
        except:
            pass
...
def _pnl_guard_thread():
    ...
    def _close_all_live(reason):
        try:
            _open = [p for p in kite.positions().get("day", [])
    ...
    while True:
        try:
            ...
            # ── Profit Target check ──────────────────────────────────────
            if trading_enabled and PROFIT_TARGET > 0 and _total_pnl >= PROFIT_TARGET:
                trading_enabled = False
                _msg = (f"🎯 PNL_GUARD PROFIT TARGET | PnL:₹{_total_pnl:.2f} "
                        f">= Target:₹{PROFIT_TARGET} | Closing ALL + HALTED")
                print(Fore.GREEN + _msg + ts())
                write_log(_msg)
                if TRADING_MODE == "PAPER":
                    _close_all_paper("PNL_GUARD_PROFIT_TARGET")
                else:
                    _close_all_live("PNL_GUARD_PROFIT_TARGET")
            ...
            # ── Profit Lock exit check ───────────────────────────────────
            if profit_lock_armed and trading_enabled and _total_pnl <= PROFIT_LOCK_FLOOR:
                trading_enabled = False
                _msg = (f"🛑 PNL_GUARD PROFIT LOCK HIT | PnL:₹{_total_pnl:.2f} "
                        f"<= Floor:₹{PROFIT_LOCK_FLOOR} | Closing ALL + HALTED")
                print(Fore.YELLOW + _msg + ts())
                write_log(_msg)
                if TRADING_MODE == "PAPER":
                    _close_all_paper("PNL_GUARD_PROFIT_LOCK")
                else:
                    _close_all_live("PNL_GUARD_PROFIT_LOCK")
...
if __name__ == "__main__":
    ...
    def _daily_data_is_fresh():
        ...
        for sym in SYMBOLS[:10]:   # check first 10 as a sample
            fp = os.path.join(DAILY_DIR, f"heatmap_{sym}_daily.csv")
            if not os.path.exists(fp):
    ...
    def _load_nifty_prev_close():
        ...
        _nifty_csv = os.path.join(DAILY_DIR, "heatmap_NIFTY_daily.csv")
        ...
        if os.path.exists(_nifty_csv):
            try:
                ...
                if not _past.empty:
                    ...
                    print(Fore.YELLOW + "⚠️  heatmap_NIFTY_daily.csv exists but no rows before cutoff — will fetch from API" + ts())
            except Exception as _fe:
                print(Fore.YELLOW + f"⚠️  heatmap_NIFTY_daily.csv read error ({_fe}) — falling back to API" + ts())
        else:
            print(Fore.YELLOW + "⚠️  heatmap_NIFTY_daily.csv not found — fetching from API (run fresh_daily_download to avoid this)" + ts())
        ...
        try:
            ...
            if _past_api:
                ...
                try:
                    ...
                    print(Fore.CYAN + "📂 heatmap_NIFTY_daily.csv saved for future use" + ts())
                except Exception:
                    pass
                return
            else:
    ...
    threading.Thread(target=update_heatmap_logic,     daemon=True).start()  # ← Heatmap Direction calculator
