...
def verify_restored_positions():
    ...
    if TRADING_MODE == "LIVE":
        ...
        # ── Sync daily_pnl with broker actual P&L ────────────────────
        # If algo was restarted mid-day and OCO monitors missed closes,
        # daily_pnl in memory is stale. Recompute from broker positions.
        if TRADING_MODE == "LIVE":
            try:
                broker_pos = kite.positions()["net"]
                algo_syms  = set(trades_taken.keys())
                broker_pnl = sum(
                    p["pnl"] for p in broker_pos
                    if p.get("exchange") == "NSE"
                    and p["tradingsymbol"] in algo_syms
                )
                if abs(broker_pnl - daily_pnl) > 1:   # meaningful difference
                    print(Fore.CYAN + f"📊 Syncing daily_pnl from broker: "
                          f"memory=₹{daily_pnl:.2f} broker=₹{broker_pnl:.2f} → using broker" + ts())
                    daily_pnl = broker_pnl
            except Exception as be:
                write_log(f"daily_pnl broker sync failed: {be}")
...
def load_trades_cache():
    ...
    try:
        ...
        trade_statistics["total_trades"] = restored
        ...
        if now_hhmm >= SQUARE_OFF_TIME:
            ...
            if TRADING_MODE == "LIVE":
                ...
                try:
                    ...
                    # Normalise to str — Kite API returns int, trades_cache stores str
                    ...
                    for symbol, trade in list(trades_taken.items()):
                        ...
                        for oid_key in ("sl", "target"):
                            ...
                            if order and order["status"] in ("OPEN", "TRIGGER PENDING"):
                                try:
                                    kite.cancel_order(variety="regular", order_id=oid)
                                    print(Fore.YELLOW + f"Cancelled {oid_key} for {symbol}" + ts())
                                    cancelled += 1
                                    time.sleep(0.1)
                                except Exception as ce:
                                    print(Fore.RED + f"Could not cancel {oid_key} for {symbol}: {ce}" + ts())

                    # Wait for cancellations to settle before placing exits
                    wait_secs = min(3, 1 + cancelled * 0.2)
                    ...
                    for pos in positions:
                        ...
                        time.sleep(0.3)   # let margin free up between exits
