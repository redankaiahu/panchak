...
def place_trade(symbol, side, ltp, tick, condition_name):
    ...
    sl = round(ltp * (1 - STOP_LOSS_PERCENT/100), 2) if side == "BUY" else round(ltp * (1 + STOP_LOSS_PERCENT/100), 2)
    target = round(ltp * (1 + TARGET_PERCENT/100), 2) if side == "BUY" else round(ltp * (1 - TARGET_PERCENT/100), 2)

    color = Fore.GREEN if side == "BUY" else Fore.RED

    print(color + "-> " * 20)
    print(color + f"{TRADING_MODE} {side} | {symbol} | Entry:{ltp} SL:{sl} Target:{target} Qty:{qty}")

    write_log(f"{TRADING_MODE} {side} {symbol} Entry:{ltp} SL:{sl} Target:{target}")
    ...
    # Paper & Live both tracked internally
...
def load_yesterday_levels():
    ...
    if os.path.exists(YESTERDAY_CACHE_FILE):
        print("Loading Yesterday OHLC from local cache...")
        with open(YESTERDAY_CACHE_FILE, "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
                yesterday_data[row["symbol"]] = {
                    "high": float(row["high"]),
                    "low": float(row["low"]),
                    "close": float(row["close"]),
                    "volume": float(row["volume"])
                }
                live_day_extremes[row["symbol"]] = {
                    "high": 0,
                    "low": 999999
                }
        return

    print("Fetching Yesterday OHLC from Kite...")
    with open(YESTERDAY_CACHE_FILE, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["symbol", "high", "low", "close", "volume"])
...
def load_ema_data():
    ...
    for token, symbol in instrument_tokens.items():
        try:
            data = kite.historical_data(
                token,
                datetime.now().replace(day=datetime.now().day-60),
                datetime.now(),
                "day"
            )
...
# ==========================================
# ==========================================
# INSTITUTIONAL GRADE STRATEGY (FIXED)
# ==========================================
def strategy(token, tick):
    ...
    if symbol not in yesterday_data or symbol not in ema_cache:
        return

    # Prevent multiple entries per symbol
    if symbol in trades_taken:
        return
    ...
    ema7 = ema_cache[symbol]["ema7"]
    ema20 = ema_cache[symbol]["ema20"]
    ema50 = ema_cache[symbol]["ema50"]
    ...
    dist_from_day_high = ((day_high - ltp) / day_high) * 100 if day_high else 0
    dist_from_day_low = ((ltp - day_low) / day_low) * 100 if day_low else 0
    ...
    if (
        ...
        ltp >= ema7 >= ema20 #> ema50 and
        #vol_percent >= 30
    ):
        print(Fore.GREEN +
              f"BUY SIGNAL | {symbol} | YH:{yest_high} YC:{yest_close} "
              f"EMA7:{ema7:.2f} EMA20:{ema20:.2f} EMA50:{ema50:.2f} "
              f"VOL%:{vol_percent:.1f}")

        place_trade(symbol, "BUY", ltp, tick, "YH + EMA + VOL")
    ...
    if (
        ...
        print(Fore.RED +
              f"SELL SIGNAL | {symbol} | YL:{yest_low} YC:{yest_close} "
              f"EMA7:{ema7:.2f} EMA20:{ema20:.2f} EMA50:{ema50:.2f} "
              f"VOL%:{vol_percent:.1f}")

        place_trade(symbol, "SELL", ltp, tick, "YL + EMA + VOL")
...
def run_market_replay(date):

    print(f"\n🔁 STARTING MARKET REPLAY FOR {date}")
...
def download_6_month_data():
    ...
    print("📥 Downloading 6 Months Historical Data...")
...
if __name__ == "__main__":
    ...
    if TRADING_MODE == "PAPER" and input("Replay Mode? (y/n): ") == "y":
        ...
        from datetime import datetime
        replay_date = datetime(2025, 2, 27)   # Change date here
        run_market_replay(replay_date)
