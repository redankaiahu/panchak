...
def paper_position_summary():
    ...
    while True:
        ...
        print("="*80)
        print(Fore.YELLOW + f"   Open  positions : {open_count:>3}  |  Open PnL   : {open_pnl:>+10.2f}" + ts())
        print(Fore.YELLOW + f"   Closed positions: {closed_count:>3}  |  Closed PnL : {closed_pnl:>+10.2f}" + ts())
        total_color = Fore.GREEN if total_pnl >= 0 else Fore.RED
        print(total_color  + f"   ── TOTAL PnL (Open + Closed) ──────────  {total_pnl:>+10.2f}" + ts())
        print("="*80)
