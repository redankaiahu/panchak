...
with st.expander("💾 Data Storage Status (click to expand)", expanded=False):
    ...
    def _color_status(val):
        ...
        return ""
    st.dataframe(
        _status_df.style.applymap(_color_status, subset=["status"]),
...
def fetch_nifty_oi_intelligence():
    ...
    try:
        ...
        near_atm_strikes = [s for s in chain if abs(s - atm) <= step]
        ...
        with open(OI_INTEL_CACHE_PATH, "w") as f:
...
with tabs[5]:   # 👈 replace with correct index
    ...
    if oi_intel is None:
        ...
        if "WATERFALL" in pattern_label or "BEARISH" in direction:
            trade_bias   = "BEARISH"
            entry_strike = atm
            # SL: just above nearest call wall (real resistance ceiling)
            sl_spot      = int(real_sl_call) + step
            # Target: just below put floor (real support floor)
            ...
            trade_bias   = "BULLISH"
            entry_strike = atm
            # SL: just below nearest put floor
            sl_spot      = int(real_tgt_put) - step
            # Target: just above nearest call wall
            ...
            risk_pts     = 0
            reward_pts   = 0
            setup_line   = f"WAIT — No clear edge. Range: {int(real_tgt_put)} – {int(real_sl_call)}"
            sl_line      = "—"
            target_line  = "—"
            bias_color   = "#e65100"
            bias_icon    = "⚠️"
        ...
        if os.path.exists(prev_direction_file):
            with open(prev_direction_file) as f:
                prev_direction = f.read().strip()
        with open(prev_direction_file, "w") as f:
            f.write(direction)
