...
def fetch_oi_intelligence(index_name="NIFTY"):
    ...
    try:
        ...
        # Ratio of Volume/OI: High ratio (> 10) indicates aggressive churn
        ...
        if pcr > 1.25 and pe_add > ce_add * 2: direction = "🟢 STRONG BULLISH (PE Writing)"
        elif pcr < 0.75 and ce_add > pe_add * 2: direction = "🔴 STRONG BEARISH (CE Writing)"
        elif pe_add > ce_add: direction = "🟢 BULLISH BIAS"
        elif ce_add > pe_add: direction = "🔴 BEARISH BIAS"

        # 9. Result Object
        res = {
            "index": index_name, "spot": spot, "atm": atm, "pcr": pcr, "max_pain": max_pain,
            "direction": direction, "vol_oi_ratio": vol_oi_ratio,
            "battle_ce": battle_ce, "battle_pe": battle_pe,
            "ce_add": ce_add, "pe_add": pe_add, "expiry": expiry.strftime("%d-%b"),
            "timestamp": datetime.now(IST).strftime("%H:%M:%S")
        }

        # 10. Save Snapshot for 15-min Delta
        try:
