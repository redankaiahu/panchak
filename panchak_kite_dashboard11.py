# ==========================================
# PANCHAK + BREAKOUT DASHBOARD (FINAL FIXED)
# ==========================================
...
# ---------------- PERIOD OHLC ----------------
def build_period_ohlc(filename, start, end):
...
DISPLAY_COLS = [c for c in df.columns if c != "Symbol"]
df_display = df.copy()
...
with tabs[0]:
    styled = df_display.style.apply(row_style, axis=1).map(near_style, subset=["NEAR"])
    st.dataframe(styled, width="stretch")

with tabs[1]:
    st.dataframe(df_display[df_display.LTP >= df_display.Y_HIGH], width="stretch")
    st.dataframe(df_display[df_display.LTP <= df_display.Y_LOW], width="stretch")

with tabs[2]:
    st.dataframe(df_display[df_display.LTP >= df_display.W_HIGH], width="stretch")
    st.dataframe(df_display[df_display.LTP <= df_display.W_LOW], width="stretch")

with tabs[3]:
    st.dataframe(df_display[df_display.LTP >= df_display.M_HIGH], width="stretch")
    st.dataframe(df_display[df_display.LTP <= df_display.M_LOW], width="stretch")

with tabs[4]:
    st.dataframe(df_display[df_display.LTP >= df_display.TOP_HIGH], width="stretch")

with tabs[5]:
    st.dataframe(df_display[df_display.LTP <= df_display.TOP_LOW], width="stretch")
