...
def send_telegram(message: str, dedup_key: str = None, parse_mode: str = "HTML") -> bool:
    ...
    if not TG_BOT_TOKEN:
        return False
    # ── Block on holidays and weekends ───────────────────────────────
...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    # 10-minute dedup slot: alerts fire at most once per 10 min per stock
...
def detect_new_entries(name, current_symbols):
    # ── Block on holidays / weekends — no new entries on closed days ──
    if not is_market_hours():
        return []
...
# ── Universal number + colour helpers ────────────────────────────────
def _fmt_num(v):
    try:
        f = float(v)
