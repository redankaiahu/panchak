...
PAPER_MAX_TRADES = 20        # max simultaneous paper base positions (new stocks only)
...
PYRAMID_MAX_PCT      = 0.90   # S1–S8: stop pyramiding after 0.90% move (3 legs max)
...
CANDLE15_DIR = os.path.join(HIST_DIR, "candles_15m")  # daily 15m OHLC snapshots

os.makedirs(MINUTE_DIR,   exist_ok=True)
os.makedirs(DAILY_DIR,    exist_ok=True)
os.makedirs(CANDLE15_DIR, exist_ok=True)
...
_ema_missing_warned = set()  # symbols warned once about missing EMA cache — prevents flood of silent skips
...
def save_paper_positions_cache():
    ...
    with _save_cache_lock:
        try:
            ...
            df["gross_win_target_hit"]  = int(gross_win_target_hit)
...
def load_paper_positions_cache():
    ...
    try:
        ...
        if "gross_win_target_hit" in df.columns:
            global gross_win_target_hit
            _gwt_val = df["gross_win_target_hit"].iloc[-1]
            gross_win_target_hit = bool(int(_gwt_val)) if pd.notna(_gwt_val) else False
            if gross_win_target_hit:
                write_log("gross_win_target_hit RESTORED from cache — trading remains halted")
        ...
        for _, row in df.iterrows():
            ...
            # Initialize _last_saved_sl so paper_position_manager LIVE path
            # doesn't trigger a spurious save on the very first iteration.
            pos["_last_saved_sl"] = float(row["sl"]) if pd.notna(row["sl"]) else None
...
PROFIT_LOCK_FLOOR    = 1000     # ₹ — minimum profit to preserve once locked (₹1 000 guaranteed)
HARD_LOSS_LIMIT      = -2200    # ₹ — square off ALL positions + halt when total PnL hits this
                                #      Works for both PAPER and LIVE. Change here to adjust.
GROSS_WIN_TARGET     = 4578     # ₹ — close ALL positions + halt when gross winning (sum of all profitable
                                #      closed trades) crosses this level. Minimum effective: 3500.
                                #      Set 0 to disable. Works for both PAPER and LIVE.
PROFIT_TARGET        = 5000     # ₹ — once profit lock is armed AND total closed PnL crosses this,
                                #      no new base entries are allowed even if all positions are cleared.
PNL_MONITOR_INTERVAL = 60      # seconds — dedicated PnL guard thread poll interval (PAPER + LIVE)
...
gross_win_target_hit = False  # True once gross winning crosses GROSS_WIN_TARGET — halt + close all
...
def _update_nifty_bias(new_ltp):
    ...
    with _nifty_lock:
        ...
        if _nifty_pending_count >= NIFTY_REVERSAL_TICKS:
            ...
            # ── Reversal action: move opposing positions to breakeven
            # and unlock the new direction for fresh entries.
            # Run in a background thread so _nifty_lock is released first
            # (paper_positions + trades_taken are not lock-protected, so no
            # deadlock risk; the thread-start just avoids any latency spike
            # on the WebSocket tick thread).
            threading.Thread(
                target=_handle_nifty_reversal,
                args=(nifty_bias, tick_color),
                daemon=True
            ).start()


def _handle_nifty_reversal(new_bias, new_color):
    """
    Called immediately after a confirmed NIFTY colour flip.

    TWO actions on reversal:
      1. COST-TO-COST: move every open position that is now AGAINST the new
         bias to breakeven SL (entry price).  This protects the capital locked
         in those trades while letting the new direction play out.
         - BUY positions → SL moved to entry when NIFTY flips RED  (new_bias="SELL")
         - SELL positions → SL moved to entry when NIFTY flips GREEN (new_bias="BUY")
         In LIVE mode the broker SL-M order is also modified.

      2. UNLOCK OPPOSITE SIDE: clear _nifty_bias_blocked for the newly-allowed
         side so the strategy can immediately start looking for entries in the
         new direction (the per-symbol block was set when the side was wrong —
         now it is correct, so we wipe the set).

    Thread-safe: reads/writes paper_positions while holding _nifty_lock is
    NOT done here (caller already released it).  paper_positions mutations are
    safe because paper_position_manager holds no lock either.
    """
    global _nifty_bias_blocked

    old_side = "BUY" if new_bias == "SELL" else "SELL"   # side now AGAINST bias

    be_count  = 0   # positions moved to breakeven
    skip_count = 0  # positions already at/past breakeven — no change needed
    ...
    for symbol, pos in list(paper_positions.items()):
        ...
        side = pos.get("side")
        if side != old_side:
            continue   # position is aligned with new bias — leave it alone
        ...
        be_sl     = round(round(entry / _tick_sz) * _tick_sz, 10)
        cur_sl    = float(pos.get("sl", 0))

        # Only tighten — never loosen.  If SL is already past breakeven (profit),
        # leave it alone (TSL has already pushed it into profit territory).
        if side == "BUY":
            if cur_sl >= be_sl:
                skip_count += 1
                continue
            ltp_now = latest_prices.get(real_sym, 0)
            if ltp_now > 0 and be_sl >= ltp_now:
                # LTP already below entry — position is at a loss; instant trigger
                # risk is high, so just push SL as close to current LTP as safe.
                safe_be = round(round((ltp_now * 0.9995) / _tick_sz) * _tick_sz, 10)
                be_sl = min(be_sl, safe_be)
            pos["sl"]       = be_sl
            pos["_sl_floor"] = be_sl   # lock floor so TSL won't trail below entry
        else:  # SELL
            if cur_sl <= be_sl:
                skip_count += 1
                continue
            ltp_now = latest_prices.get(real_sym, 0)
            if ltp_now > 0 and be_sl <= ltp_now:
                safe_be = round(round((ltp_now * 1.0005) / _tick_sz) * _tick_sz, 10)
                be_sl = max(be_sl, safe_be)
            pos["sl"]       = be_sl
            pos["_sl_floor"] = be_sl

        be_count += 1
        ltp_display = latest_prices.get(real_sym, entry)

        print(Fore.YELLOW +
              f"🔄 NIFTY REVERSAL → COST-TO-COST | {symbol} | {side} | "
              f"Entry:{entry} | OldSL:{cur_sl:.2f} → NewSL(BE):{be_sl:.2f} | "
              f"LTP:{ltp_display:.2f}" + ts())
        write_log(
            f"NIFTY REVERSAL C2C {symbol} {side} Entry:{entry} "
            f"SL:{cur_sl:.2f}→{be_sl:.2f} (breakeven)"
        )

        # ── LIVE: push the modified SL to broker ────────────────────────
        if TRADING_MODE == "LIVE":
            is_pyr = "__PYR" in symbol
            if is_pyr:
                # Pyramid leg — SL order ID stored on the position itself
                leg_sl_oid = pos.get("_live_sl_oid")
                if leg_sl_oid:
                    try:
                        kite.modify_order(
                            variety=kite.VARIETY_REGULAR,
                            order_id=str(leg_sl_oid),
                            trigger_price=be_sl
                        )
                        pos["_last_live_sl"] = be_sl
                        print(Fore.YELLOW +
                              f"  ↳ LIVE SL-M modified (PYR) | {symbol} | {leg_sl_oid} → {be_sl}" + ts())
                        write_log(f"REVERSAL C2C LIVE PYR SL MODIFIED {symbol} → {be_sl}")
                    except Exception as _me:
                        write_log(f"REVERSAL C2C PYR modify_order failed {symbol}: {_me}")
                        print(Fore.YELLOW +
                              f"  ⚠️  PYR SL modify failed for {symbol}: {_me}" + ts())
            else:
                # Base position — SL order ID stored in trades_taken
                trade = trades_taken.get(symbol)
                if isinstance(trade, dict) and trade.get("sl"):
                    try:
                        kite.modify_order(
                            variety=kite.VARIETY_REGULAR,
                            order_id=str(trade["sl"]),
                            trigger_price=be_sl
                        )
                        trade["_last_live_sl"] = be_sl
                        print(Fore.YELLOW +
                              f"  ↳ LIVE SL-M modified | {symbol} | {trade['sl']} → {be_sl}" + ts())
                        write_log(f"REVERSAL C2C LIVE SL MODIFIED {symbol} → {be_sl}")
                    except Exception as _me:
                        write_log(f"REVERSAL C2C modify_order failed {symbol}: {_me}")
                        print(Fore.YELLOW +
                              f"  ⚠️  SL modify failed for {symbol}: {_me}" + ts())

    if be_count or skip_count:
        print(Fore.CYAN +
              f"🔄 NIFTY REVERSAL C2C summary | "
              f"Moved to BE: {be_count} | Already past BE: {skip_count}" + ts())
        save_paper_positions_cache()

    # ── Unlock the new bias side ─────────────────────────────────────────
    # _nifty_bias_blocked holds (symbol, side) pairs that were logged as
    # blocked.  After a reversal, the previously-blocked side is now ALLOWED.
    # Clear all entries whose side matches the new allowed direction so those
    # symbols can enter fresh trades in the new direction.
    newly_allowed_side = new_bias   # "BUY" or "SELL"
    before = len(_nifty_bias_blocked)
    _nifty_bias_blocked = {
        pair for pair in _nifty_bias_blocked
        if pair[1] != newly_allowed_side
    }
    after = len(_nifty_bias_blocked)
    if before != after:
        print(Fore.CYAN +
              f"🔓 NIFTY REVERSAL → {newly_allowed_side} side unblocked "
              f"({before - after} symbols now eligible)" + ts())
        write_log(f"REVERSAL UNLOCK {newly_allowed_side} side ({before - after} symbols)")
...
# BUG FIX: save_paper_positions_cache and save_trades_cache are called from
# multiple threads (oco_monitor, paper_position_manager, auto_square_off).
# Without a file-level lock, concurrent writes produce truncated/corrupt CSVs.
_save_cache_lock = threading.Lock()
...
def _chg(price, y_close):
    ...
    if not y_close or y_close == 0 or not price:
        return ""
...
def place_trade(symbol, side, ltp, tick, condition_name, extra_info=None, custom_sl=None, tsl_only=False, tsl_step=None, bypass_nifty_filter=False):
    ...
    # Block new base entries when gross win target already hit — no new
    # trades for the rest of the session once the daily target is reached.
    if gross_win_target_hit:
        return
    ...
    if TRADING_MODE == "LIVE":
        ...
        if (open_count + pending_count) >= LIVE_MAX_TRADES:
            ...
            return
        # LIVE: block new BASE entries when profit lock is armed and positions still open,
        # or when the profit target is already banked — mirrors PAPER logic.
        if profit_lock_armed and "__PYR" not in symbol:
            _live_open_base = sum(
                1 for k, p in paper_positions.items()
                if p.get("status") == "OPEN" and "__PYR" not in str(k)
            )
            if _live_open_base > 0 or daily_pnl >= PROFIT_TARGET:
                return

    # PAPER mode: cap simultaneous open base positions
    # Pyramid legs (__PYR) are NOT counted — they are add-ons to existing trades.
    if TRADING_MODE == "PAPER":
        if profit_lock_armed and "__PYR" not in symbol:
            # Profit lock is armed. Allow new entries ONLY if:
            #   1. ALL existing base positions are closed (none open), AND
            #   2. Total closed PnL has NOT yet crossed PROFIT_TARGET.
            # If either condition fails — block the entry.
            _open_base_count = sum(
                1 for k, p in paper_positions.items()
                if p.get("status") == "OPEN" and "__PYR" not in str(k)
            )
            if _open_base_count > 0 or daily_pnl >= PROFIT_TARGET:
                # Still have open positions (SL would go to breakeven) OR
                # target already banked — no new entries needed
                return
        _open_paper = sum(
            1 for k, p in paper_positions.items()
            if p.get("status") == "OPEN" and "__PYR" not in str(k)
        )
        if _open_paper >= PAPER_MAX_TRADES:
            if symbol not in _max_trades_warned:
                _max_trades_warned.add(symbol)
                print(Fore.YELLOW + f"⚠️  Max paper trades ({PAPER_MAX_TRADES}) reached — skipping {symbol}" + ts())
            return
    ...
    if TRADING_MODE == "LIVE":
        ...
        try:
            ...
            # ── Fetch actual fill price from broker (with retry) ──────────
            ...
            # BUG FIX: single fetch with 0.5s delay was insufficient — busy
            # markets can take 1-3s for the order to show COMPLETE in Kite's
            # order book. Retry up to 4 times (total ~4s) before falling back.
            actual_entry_price = ltp   # fallback: use signal price if fetch fails
            for _fill_attempt in range(4):
                try:
                    ...
                    if _filled and _filled.get("average_price"):
                        ...
                        break   # got a COMPLETE fill — stop retrying
                    else:
                        # Order not yet COMPLETE — wait and retry
                        if _fill_attempt < 3:
                            time.sleep(1.0)
                except Exception as _fp_e:
                    write_log(f"Fill price fetch attempt {_fill_attempt+1} failed {symbol}: {_fp_e}")
                    if _fill_attempt < 3:
                        time.sleep(1.0)
            else:
                write_log(f"Fill price fetch gave up after 4 attempts {symbol} — using signal LTP {ltp}")
...
#   • Maximum move for pyramiding: PYRAMID_MAX_PCT (0.90%).
#     → 3 legs maximum (at +0.30%, +0.60%, +0.90%). Leg 4 is disabled.
#     Cap is enforced by: max_legs = int(round(PYRAMID_MAX_PCT / PYRAMID_STEP_PCT)) = 3
...
def _calc_ema(closes, period):
    """Compute EMA of `period` over the list `closes` (oldest first).
...
def _get_ema15m_7(symbol):
    """Return current 15m EMA7 for symbol, or None if not enough data.
    Uses the completed-candle close buffer (ema15m_closes[symbol]).
    """
    from collections import deque
    closes = list(ema15m_closes.get(symbol, deque()))
    return _calc_ema(closes, 7)

def _update_ema15m_closes(symbol, slot_15, close_price, prev_candle=None):
    """Call this when a new 15m slot starts — saves the COMPLETED candle close.
    Keeps a rolling window of last 20 closes (enough for EMA7 with warmup).
    Also appends the full OHLC to ema15m_today for end-of-day persistence.
    """
    from collections import deque
    if symbol not in ema15m_closes:
        ema15m_closes[symbol] = deque(maxlen=20)
    prev = ema15m_prev_slot.get(symbol)
    if prev is not None and prev != slot_15:
        ema15m_closes[symbol].append(close_price)
        if prev_candle:
            if symbol not in ema15m_today:
                ema15m_today[symbol] = []
            ema15m_today[symbol].append({
                "slot": prev,
                "open":  prev_candle.get("open",  close_price),
                "high":  prev_candle.get("high",  close_price),
                "low":   prev_candle.get("low",   close_price),
                "close": prev_candle.get("close", close_price),
            })
    ema15m_prev_slot[symbol] = slot_15
...
def check_and_add_pyramid(symbol, ltp):
    ...
    while True:
        ...
        # ── Guard A: Base position must be in profit before adding Leg 2+ ──
        # Adding size into a losing base means two positions suffer together.
        # Require that the base is at least breakeven (unrealised PnL >= 0)
        # before firing any pyramid leg. Leg 1 (first add) is also gated.
        _base_ltp_now = ltp
        _base_entry   = base_pos.get("entry", 0)
        _base_side    = base_pos.get("side", side)
        _base_unreal  = (_base_ltp_now - _base_entry) if _base_side == "BUY" else (_base_entry - _base_ltp_now)
        if _base_unreal <= 0:
            write_log(
                f"PYRAMID SKIPPED {symbol} Leg{leg_num}: "
                f"base not profitable ({_base_unreal:.2f} pts unrealised)"
            )
            break

        # ── Guard B: 15m EMA7 trend filter ──────────────────────────────
        # For SELL pyramids: price must be BELOW the 15m EMA7 (downtrend confirmed).
        # For BUY  pyramids: price must be ABOVE the 15m EMA7 (uptrend confirmed).
        # If fewer than 7 completed 15m candles exist yet (early session),
        # the filter is skipped so pyramiding is not blocked at market open.
        _ema15m = _get_ema15m_7(symbol)
        if _ema15m is not None:
            _ema_ok = (
                (side == "SELL" and ltp < _ema15m) or
                (side == "BUY"  and ltp > _ema15m)
            )
            if not _ema_ok:
                write_log(
                    f"PYRAMID SKIPPED {symbol} Leg{leg_num}: "
                    f"15m EMA7={_ema15m:.2f} filter failed "
                    f"(side={side} ltp={ltp:.2f})"
                )
                break
        ...
        if TRADING_MODE == "LIVE":
            try:
                ...
                # BUG FIX L4: fetch actual pyramid leg fill price from broker (with retry).
                ...
                for _pyr_attempt in range(3):
                    try:
                        ...
                        write_log(f"PYR leg fill fetch attempt {_pyr_attempt+1} failed {symbol} Leg{leg_num}: {_pfe}")
                        if _pyr_attempt < 2:
                            time.sleep(1.0)
...
def _cascade_cancel_live_pyramid_legs(base_symbol, exit_price=None):
    ...
    for leg_key, leg_pos in list(paper_positions.items()):
        ...
        # Use explicit None check — net_pnl of 0.0 (breakeven) is valid and must not be recomputed.
        if leg_pos.get("net_pnl") is None:
...
def _order_cache_fetcher():
    ...
    while True:
        try:
            ...
            _errors = 0   # reset on success
        except Exception as _oe:
            _errors += 1
            write_log(f"order_cache_fetcher error #{_errors}: {_oe}")
            # Exponential backoff: 2s → 4s → 8s → cap at 30s
            _t.sleep(min(2 ** _errors, 30))
            continue
        _t.sleep(2)
            ...
            if tgt_id:
                ...
                if tgt_order is None:
                    ...
                    fallback_t  = next((
                        ...
                        and (tgt_price is None or
                             abs(float(o.get("price", 0)) - float(tgt_price)) / max(float(tgt_price), 1) < 0.005)
                    ), None)
                    if fallback_t:
            ...
            if base_pos.get("status") == "OPEN":
                ...
                if (current_sl and
                    ...
                    try:
                        kite.modify_order(
                            variety=kite.VARIETY_REGULAR,
                            order_id=str(sl_id),   # always str — cache restore may give int
    ...
    for symbol in SYMBOLS:
        ...
        if orb and f15:
            ...
            rows.append({
                ...
                # F5 (09:15–09:19) — S10 Open=High Instant SELL (legacy; S9 now uses F10)
                ...
                # F10 (09:15–09:24) — S9 Open=High 10-min Breakdown
                "f10_open":  f10.get("open",  0),
                "f10_high":  f10.get("high",  0),
                "f10_low":   f10.get("low",   999999),
                "f10_close": f10.get("close", 0),
                "f10_ready": f10.get("ready", False),
                "f10_o_eq_h": (
                    True if (
                        f10.get("ready", False) and f10.get("open", 0) > 0 and
                        f10.get("high", 0) == f10.get("open", 0)
                    ) else ""
                ),
            })
    if rows:
...
def _load_orb_cache():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            # F5 (09:15–09:19) — S10 Open=High Instant SELL (legacy, kept for S10)
            ...
            # F10 (09:15–09:24) — S9 Open=High 10-min Breakdown
            # Only restore if high is real (> 0) and low is valid (< 999999)
            if ("f10_high" in row and pd.notna(row["f10_high"]) and float(row["f10_high"]) > 0
                    and float(row.get("f10_low", 999999)) < 999999):
                first10_data[symbol] = {
                    "open":  float(row["f10_open"]),
                    "high":  float(row["f10_high"]),
                    "low":   float(row["f10_low"]),
                    "close": float(row["f10_close"]),
                    "ready": str(row["f10_ready"]).lower() == "true"
                }
...
def _refresh_orb_cache_loop():
    ...
    while True:
        try:
            ...
            for symbol in SYMBOLS:
                ...
                try:
                    ...
                    # S10 needs this candle. _build_first5() only runs via WebSocket
                    # ticks, so on restart after 09:20 first5_data is empty.
                    ...
                    # ── F10 (09:15–09:24): seed/correct first10_data from API ──
                    # S9 (Open=High 10-min Breakdown) needs this candle.
                    # _build_first10() only runs via WebSocket ticks, so on restart
                    # after 09:25, first10_data is empty. Seed it here so S9 works
                    # post-restart and the ORB cache CSV stores valid F10 values.
                    f10_df = df[(df["time_str"] >= "09:15") & (df["time_str"] < "09:25")]
                    if not f10_df.empty:
                        existing_f10 = first10_data.get(symbol, {})
                        first10_data[symbol] = {
                            "open":  existing_f10.get("open") or float(f10_df.iloc[0]["open"]),
                            "high":  max(existing_f10.get("high", 0),      float(f10_df["high"].max())),
                            "low":   min(existing_f10.get("low",  999999), float(f10_df["low"].min())),
                            "close": float(f10_df.iloc[-1]["close"]),
                            "ready": now_time >= "09:25",
                        }
        ...
        # ── Smart sleep: wake at 09:25 if we're still pre-F10-freeze ────────
        # The first10_data candle (S9) freezes at 09:25. If the current cycle
        # finished before 09:25, sleep only until 09:25:05 so we immediately
        # seed F10 and save a fresh ORB cache with F10 values.
        _now = datetime.now()
        _now_time = _now.strftime("%H:%M")
        if _now_time < "09:25":
            _target = _now.replace(hour=9, minute=25, second=5, microsecond=0)
...
def paper_position_manager():
    ...
    global daily_pnl, profit_lock_armed, trading_enabled, gross_win_target_hit
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            # BUG FIX 10 (corrected placement): clear _max_trades_warned when a LIVE
            # base position just closed (status==CLOSED set by oco_monitor).
            # Must run BEFORE the `if status != "OPEN": continue` skip so it actually
            # fires. The _live_cleared sentinel prevents repeated clears on every loop.
            if (TRADING_MODE == "LIVE" and
                    pos.get("status") == "CLOSED" and
                    "__PYR" not in symbol and
                    not pos.get("_live_cleared")):
                pos["_live_cleared"] = True
            ...
            # BUG FIX: guard against entry=0 (corrupt cache restore) → ZeroDivisionError
            ...
            # ── Daily Max Loss Circuit Breaker (PAPER) ───────────────────
            if trading_enabled:
                # Hard rupee limit — mirrors LIVE position_manager check
                if daily_pnl <= HARD_LOSS_LIMIT:
                    trading_enabled = False
                    msg = (
                        f"🚨 PAPER HARD LOSS LIMIT | PnL:{daily_pnl:.2f} | "
                        f"Limit:{HARD_LOSS_LIMIT:.2f} | Closing ALL positions + Trading HALTED"
                    )
                    print(Fore.RED + msg + ts())
                    write_log(msg)
                    # Close every remaining open paper position at current LTP
                    for _hl_sym, _hl_pos in list(paper_positions.items()):
                        if _hl_pos.get("status") != "OPEN":
                            continue
                        _hl_base = _hl_sym.split("__PYR")[0]
                        _hl_ltp  = latest_prices.get(_hl_base, _hl_pos["entry"])
                        _hl_side = _hl_pos["side"]
                        _hl_pts  = (_hl_ltp - _hl_pos["entry"]) if _hl_side == "BUY" else (_hl_pos["entry"] - _hl_ltp)
                        _hl_net  = round(_hl_pts * int(_hl_pos.get("qty", FIXED_QTY)) - BROKERAGE_PER_ORDER * 2, 2)
                        _hl_pos["status"]  = "CLOSED"
                        _hl_pos["net_pnl"] = _hl_net
                        with _daily_pnl_lock:
                            daily_pnl += _hl_net
                        write_log(f"HARD_LOSS CLOSE {_hl_sym} {_hl_side} "
                                  f"Entry:{_hl_pos['entry']} Exit:{_hl_ltp:.2f} Net:{_hl_net:.2f}")
                    save_paper_positions_cache()
                    pyramid_state.clear()
                else:
                    # Percentage-based backstop
                    ref_capital = sum(
                        p2["entry"] * p2["qty"]
        ...
        # ── Gross Win Target check (PAPER) ──────────────────────────────
        # When the sum of all profitable closed trades crosses GROSS_WIN_TARGET,
        # immediately close every open position at market and halt trading.
        # This locks in a strong winning day rather than risking a reversal.
        if (TRADING_MODE == "PAPER" and GROSS_WIN_TARGET > 0
                and trading_enabled and not gross_win_target_hit):
            _gross_win = sum(
                p["net_pnl"] for p in paper_positions.values()
                if p.get("status") == "CLOSED" and p.get("net_pnl", 0) > 0
            )
            if _gross_win >= GROSS_WIN_TARGET:
                gross_win_target_hit = True
                trading_enabled      = False
                _gwt_msg = (
                    f"🏆 GROSS WIN TARGET HIT (PAPER) | "
                    f"Gross winning: ₹{_gross_win:.2f} >= Target: ₹{GROSS_WIN_TARGET:.0f} | "
                    f"Closing all positions + Trading HALTED"
                )
                print(Fore.GREEN + _gwt_msg + ts())
                write_log(_gwt_msg)
                # Close every open paper position at current market price
                for _gs, _gp in list(paper_positions.items()):
                    if _gp.get("status") != "OPEN":
                        continue
                    _gbase  = _gs.split("__PYR")[0]
                    _g_ltp  = latest_prices.get(_gbase, _gp["entry"])
                    _g_side = _gp["side"]
                    _g_pts  = (_g_ltp - _gp["entry"]) if _g_side == "BUY" else (_gp["entry"] - _g_ltp)
                    _g_net  = _g_pts * int(_gp.get("qty", FIXED_QTY)) - BROKERAGE_PER_ORDER * 2
                    _gp["status"]   = "CLOSED"
                    _gp["net_pnl"]  = _g_net
                    with _daily_pnl_lock:
                        daily_pnl += _g_net
                    write_log(
                        f"GROSS WIN CLOSE {_gs} {_g_side} "
                        f"Entry:{_gp['entry']} Exit:{_g_ltp:.2f} NetPnL:{_g_net:.2f}"
                    )
                save_paper_positions_cache()
                pyramid_state.clear()
...
def replay_update_intraday_state(symbol, candle_time, open_p, high, low, close_p, volume):
    ...
    if symbol not in replay_15m_data or replay_15m_data[symbol]["start_min"] != slot_15:
        # Slot rolled over — save the completed candle close for EMA15m
        _prev_c15r = replay_15m_data.get(symbol, {})
        if _prev_c15r and "close" in _prev_c15r:
            _update_ema15m_closes(symbol, slot_15, _prev_c15r["close"],
                                  prev_candle=_prev_c15r)
...
def build_context(token, tick):
    ...
    if not ema.get("ema7") or not ema.get("ema20"):
        # One-time warning — EMA missing means ALL strategies silently skip this symbol all day.
        # Root cause: daily CSV missing/corrupt or insufficient history (< 50 bars).
        if symbol not in _ema_missing_warned:
            _ema_missing_warned.add(symbol)
            print(Fore.YELLOW + f"⚠️  EMA cache missing for {symbol} — all strategies will skip it. "
                  f"Check historical_data/daily/{symbol}_daily.csv" + ts())
            write_log(f"EMA MISSING {symbol} — strategies blocked for entire session")
        return None

    # BUG FIX: guard against yest["close"] == 0 (missing CSV data) → ZeroDivisionError crash
    _yc = yest["close"] if yest.get("close") and yest["close"] != 0 else None
    change_percent = ((ltp - _yc) / _yc * 100) if _yc else 0.0
    ...
    gap_percent = ((_gap_base - _yc) / _yc * 100) if _yc else 0.0
    ...
    # BUG FIX: guard against yest["low"]/yest["high"] == 0 → ZeroDivisionError crash
    _yl = yest["low"]  if yest.get("low")  and yest["low"]  != 0 else None
    _yh = yest["high"] if yest.get("high") and yest["high"] != 0 else None
    dist_from_yl_pct = (((_yl - ltp) / _yl) * 100) if _yl else 0.0   # +ve = below YL
    dist_from_yh_pct = (((ltp - _yh) / _yh) * 100) if _yh else 0.0  # +ve = above YH
...
def strategy_orb(ctx):
    ...
    if buy_signal:
        ...
        elif ltp >= orb_high and day_high_past_orb_pct > MAX_STALE_PCT:
            reason = f"BUY stale: day_high already {day_high_past_orb_pct:.2f}% past ORB high (max {MAX_STALE_PCT}%)"
        elif ltp <= orb_low and day_low_past_orb_pct > MAX_STALE_PCT:
            reason = f"SELL stale: day_low already {day_low_past_orb_pct:.2f}% past ORB low (max {MAX_STALE_PCT}%)"
        ...
        elif ltp >= orb_high and dist_from_yh_pct > MAX_OVEREXTENSION_PCT:
            reason = f"BUY overextended {dist_from_yh_pct:.2f}% above YH (max {MAX_OVEREXTENSION_PCT}%)"
        elif ltp <= orb_low and dist_from_yl_pct > MAX_OVEREXTENSION_PCT:
            reason = f"SELL overextended {dist_from_yl_pct:.2f}% below YL (max {MAX_OVEREXTENSION_PCT}%)"
...
ema15m_closes      = {}   # symbol → deque of last 20 completed 15m candle closes
                          #           used to compute rolling EMA7 on 15m timeframe
ema15m_prev_slot   = {}   # symbol → last slot_15 value seen (detects slot rollover)
ema15m_today       = {}   # symbol → list of {slot, open, high, low, close} dicts
                          #           full today 15m OHLC — saved to disk at squareoff
...
def strategy_15m_inside_break(ctx):
    ...
    yest = ctx["yest"]
    # BUG FIX: guard against yest["close"] == 0 (missing data) → ZeroDivisionError crash
    _yc_s8 = yest.get("close")
    if not _yc_s8:
        _scan_record(_SNAME, symbol, "Yesterday close missing — cannot compute gap")
        return
    true_gap_pct = ((c1["open"] - _yc_s8) / _yc_s8) * 100
    ...
    # BUG FIX: guard against zero c1_range (flat open candle) → ZeroDivisionError crash
    if c1_range <= 0:
        _scan_record(_SNAME, symbol, "C1 range=0 — flat candle, skip")
        return
...
first5_data = {}   # symbol → {open, high, low, close, ready}  (legacy — kept for S10 compatibility)

# ==========================================
# STRATEGY 9 — FIRST 10-MIN CANDLE (09:15–09:24)
# ==========================================
# Replaces first5_data for Strategy 9.
# Window: 09:15–09:24 (10 minutes), freezes at 09:25.
# day_open = first tick of first10_data (09:15 open)
# day_high (within the window) = first10_data["high"]
# Entry fires when ltp breaks below first10_data["low"] after 09:25.
# ==========================================
first10_data = {}  # symbol → {open, high, low, close, ready}

def _build_first10(symbol, ltp, current_time):
    """Build the 09:15–09:24 candle (first 10-min candle) using ltp.
    Freezes at 09:25. Used exclusively by Strategy 9.
    """
    if symbol not in first10_data:
...
def _build_first5(symbol, ltp, current_time):
    ...
    Kept for S10 compatibility; S9 now uses _build_first10."""
...
def strategy_open_high_breakdown(ctx):
    """
    Strategy 9 — Open=High 10-Min Breakdown (SELL only)
    =====================================================
    Setup window  : 09:15 – 09:24  (10 minutes)
    Qualification : Day open == Day high within the 10-min window
                    (price never traded above the opening tick — sellers in control)
    Candle used   : first10_data  (09:15–09:24, freezes at 09:25)
    Entry trigger : After 09:25 — when LTP breaks below the 09:25 candle low
                    (i.e. first10_data["low"])
    SL            : 0.3% above day open/high (snapped to tick size)
    Exit          : TSL-only (trailed out — no fixed target)
    """
    ...
    # Build first-10 candle on every tick (09:15–09:24 window)
    _build_first10(symbol, ltp, current_time)

    # ── Time gate: only after first-10 candle is complete (≥ 09:25) ────
    if current_time < "09:25":
        _scan_record(_SNAME, symbol, "Before 09:25 (first-10 building 09:15–09:24)")
        return
    if current_time > "14:20":
        _scan_record(_SNAME, symbol, "After 14:20 cutoff")
        return

    # ── First-10 candle must be ready ───────────────────────────────────
    if not first10_data.get(symbol, {}).get("ready"):
        _scan_record(_SNAME, symbol, "First-10 not ready")
        return

    f10 = first10_data[symbol]

    if f10["open"] == 0 or f10["high"] == 0 or f10["low"] == 999999:
        _scan_record(_SNAME, symbol, "First-10 data invalid")
        return

    # ── Volume filter: REMOVED for S9 ──────────────────────────────────
    # S9 fires at 09:25 — only 10 minutes of trading have elapsed.
    # At 09:25: 10min/375min * burst = ~2.7%+burst → still unlikely to
    # reach any meaningful vol threshold. Price structure is the signal.
    _vol_pct_log = vol_percent  # available for logging only

    # ── Condition A: Day Open == Day High (exact match within 10-min window)
    # The first10_data open IS the 09:15 market open price.
    # The first10_data high is the highest price traded 09:15–09:24.
    # Open == High means price NEVER traded above the opening tick for 10 mins.
    # This is a strong seller signal — every attempt to rally was rejected.
    # No tolerance: even 1 paisa above open disqualifies the setup.
    day_open = f10["open"]   # 09:15 market open
    day_high = f10["high"]   # highest tick 09:15–09:24

    if day_high != day_open:
        _scan_record(_SNAME, symbol,
            f"Open≠High: open={day_open:.2f} f10_high={day_high:.2f} "
            f"diff={day_high - day_open:.2f} (must be exactly equal — O=H in 10-min window)")
        return

    # ── Condition B: First-10 candle range < 3.0% ──────────────────────
    # 10-min window allows a wider range than the old 5-min (2.5%).
    # Capped at 3.0% to filter out erratic gap-and-crash opens.
    f10_range_pct = ((f10["high"] - f10["low"]) / f10["open"]) * 100
    if f10_range_pct >= 3.0:
        _scan_record(_SNAME, symbol,
            f"First-10 range {f10_range_pct:.2f}% >= 3.0% (too wide)")
        return

    # ── Entry: LTP breaks below first-10 candle low (the 09:25 low) ────
    entry_level = f10["low"]

    # SL = 0.3% above day open/high (snapped to tick size for LIVE order validity)
    _s9_tick = tick_sizes.get(symbol, 0.05)
    _sl_raw  = day_high * 1.003
    sl_level = round(round(_sl_raw / _s9_tick) * _s9_tick, 10)
    ...
    # Stale guard — same logic as before:
    # latest_lows is updated before strategy() fires, so on the exact
    # breakdown tick stale_pct ≈ slippage_pct.  Allow up to MAX_SLIPPAGE_PCT.
    # A re-test (bounced down 1%+ earlier then recovered) → stale_pct > 0.3% → blocked.
    stale_pct = ((f10["low"] - latest_lows.get(symbol, ltp)) / f10["low"] * 100)
    MAX_STALE_PCT = MAX_SLIPPAGE_PCT   # 0.3%

    sell_signal = (
        399 <= ltp <= 6999               and   # price range filter
        ltp < entry_level                and   # broke below first-10 low
        slippage_pct <= MAX_SLIPPAGE_PCT and   # not chasing
        stale_pct    <= MAX_STALE_PCT    and   # fresh breakdown (not re-test)
        dist_from_yl_pct <= MAX_OVEREXTENSION_PCT
    )

    if sell_signal:
        _scan_record(_SNAME, symbol,
            f"✅ SIGNAL SELL ltp={ltp} f10_low={entry_level:.2f} "
            f"sl={sl_level:.2f} range={f10_range_pct:.2f}% TSL-only")
        if symbol not in trades_taken:
            print(Fore.RED +
                f"🔴 S9 SELL TRIGGER | {symbol} | "
                f"LTP:{ltp:.2f} broke F10-Low:{entry_level:.2f} | "
                f"F10 O:{f10['open']:.2f} H:{f10['high']:.2f} L:{f10['low']:.2f} | "
                f"Open==High (10-min):{day_open:.2f} | SL:{sl_level:.2f} | TSL-step:1.5%" + ts())
        ...
        if symbol in trades_taken:
            return   # already traded — suppress skip-reason spam
        if not (399 <= ltp <= 6999):
            reason = f"Price {ltp:.2f} out of 399–6999 range"
        elif day_high != day_open:
            reason = f"Open≠High in 10-min window: open={day_open:.2f} high={day_high:.2f}"
        elif f10_range_pct >= 3.0:
            reason = f"F10 range {f10_range_pct:.2f}% >= 3.0% (too wide)"
        elif ltp >= entry_level:
            reason = f"ltp {ltp:.2f} not below first-10 low {entry_level:.2f}"
        elif slippage_pct > MAX_SLIPPAGE_PCT:
            reason = f"Slippage {slippage_pct:.2f}% > {MAX_SLIPPAGE_PCT}% — chasing"
        elif stale_pct > MAX_STALE_PCT:
            reason = f"Stale: day low already {stale_pct:.2f}% below f10_low — re-test"
        elif dist_from_yl_pct > MAX_OVEREXTENSION_PCT:
            reason = f"Overextended {dist_from_yl_pct:.2f}% below YL"
        else:
            reason = f"No breakdown: ltp={ltp:.2f} f10={f10['low']:.2f}–{f10['high']:.2f}"
        _scan_record(_SNAME, symbol, reason)
...
def strategy_open_high_sell_at_919(ctx):
    ...
    # ── LTP must be BELOW day_open — confirms bearish price action ────────
    # At 09:18 the stock must actually be trading below its opening price.
    # If LTP == day_open the move has not started; if LTP > day_open the
    # O=H pattern is already broken.  Require at least 0.05% below open.
    _min_drop = round(day_open * 0.0005, 2)   # 0.05% of open
    if ltp >= day_open - _min_drop:
        _scan_record(_SNAME, symbol,
            f"LTP {ltp:.2f} not yet below open {day_open:.2f} "
            f"(need < {day_open - _min_drop:.2f}) — no confirmed breakdown")
        return

    # ── SL = max(day_open + 1.0, day_open * 1.005) snapped to tick grid ──
    # Dynamic SL: 0.5% of open, floored at ₹1 above open, then tick-snapped.
    # A fixed ₹1 SL is too tight for stocks >₹500 and too loose for <₹100.
    _s10_tick = tick_sizes.get(symbol, 0.05)
    _raw_sl   = max(day_open + 1.0, day_open * 1.005)
    sl_level  = round(round(_raw_sl / _s10_tick) * _s10_tick, 10)
...
#   ✅ S9  Open=High 10-min Breakdown (SELL only)
#            Window  : 09:15–09:24 (10 minutes, freezes at 09:25)
#            Trigger : ltp breaks below F10 low (09:25 low)
#            Qualify : Day open == Day high within 10-min window (strict exact match)
#            SL      : 0.3% above day open/high | Exit: TSL-only
#   ❌ S10 Open=High Instant SELL @ 09:18 — DISABLED
#
...
def strategy(token, tick):
    ...
    # ==========================================
    # build_context() returns None when current_time < ENTRY_START_TIME ("09:18")
    # or when ema_cache / yesterday_data is not yet populated.
    #
    # S9 needs to BUILD the 10-min candle from 09:15 and FIRE from 09:25.
    # Since build_context blocks anything before 09:18, ticks 09:15–09:17
    # would be dropped entirely without this bypass.
    # Ticks 09:18–09:24 DO get a real ctx (ENTRY_START_TIME passes), so
    # strategy_open_high_breakdown(ctx) is called normally — the internal
    # time gate (< "09:25") prevents entry during that window while still
    # building first10_data on every tick.
    #
    # This bypass handles: 09:15–09:17 (before ENTRY_START_TIME).
    # S9 internal gate handles: 09:18–09:24 (building) and ≥09:25 (entry).
    if ctx is None:
        symbol = instrument_tokens.get(token)
        if symbol and symbol in yesterday_data:
            candle_time = tick.get("date", datetime.now())
            current_time = candle_time.strftime("%H:%M")
            # Cover 09:15–09:17 (before ENTRY_START_TIME="09:18") so F10 builds
            # from the very first tick of the day.
            if "09:15" <= current_time < "09:31":
                ltp = tick["last_price"]
                yest = yesterday_data[symbol]
                live_volume = tick.get("volume_traded", tick.get("volume", 0))
                if symbol in replay_cum_volume:
                ...
                # S9: build the 10-min candle continuously 09:15–09:24;
                # entry fires from 09:25 when ltp breaks below F10-low.
    ...
    # ✅ Strategy 9: Open=High 10-min Breakdown (SELL — open==high within 09:15–09:24, break f10 low after 09:25)
    #    Early window 09:15–09:30 handled above via early_ctx bypass (builds F10 candle + fires entry from 09:25).
    strategy_open_high_breakdown(ctx)

    # ❌ Strategy 10: Open=High Instant SELL @ 09:19  — DISABLED (hard limit hit on crash days)
...
def position_manager():
    ...
    while True:
        ...
        try:
            ...
            # ── Gross Win Target (LIVE) ───────────────────────────────────
            global gross_win_target_hit
            if (GROSS_WIN_TARGET > 0 and trading_enabled
                    and not gross_win_target_hit):
                try:
                    _gwt_closed = kite.trades()
                    _gross_win_live = sum(
                        float(t.get("profit", 0))
                        for t in _gwt_closed
                ...
                if _gross_win_live >= GROSS_WIN_TARGET:
                    gross_win_target_hit = True
                    trading_enabled      = False
                    _gwt_msg = (
                        f"🏆 GROSS WIN TARGET HIT (LIVE) | "
                        f"Gross winning: ₹{_gross_win_live:.2f} >= Target: ₹{GROSS_WIN_TARGET:.0f} | "
                        f"Squaring off all positions + Trading HALTED"
                    )
                    print(Fore.GREEN + _gwt_msg + ts())
                    write_log(_gwt_msg)
                    import threading as _thr_gwt
                    def _gwt_squareoff():
                        try:
                            write_log("GROSS WIN SQUAREOFF triggered")
                            global pyramid_state
                            pyramid_state.clear()
                            try:
                                _open_ords = kite.orders()
                                _algo_syms = set(trades_taken.keys())
                                for _o in _open_ords:
                                    _base = _o.get("tradingsymbol", "").split("__PYR")[0]
                                    if (_base in _algo_syms and
                                            _o.get("product") == "MIS" and
                                            _o.get("status") in ("OPEN", "TRIGGER PENDING")):
                                        try:
                                            kite.cancel_order(
                                ...
                                write_log(f"GWT cancel sweep failed: {_ce}")
                            time.sleep(1)
                            _pos_list = kite.positions().get("net", [])   # BUG FIX: "day" includes closed; "net" is open only
                            for _p2 in _pos_list:
                                if _p2.get("quantity", 0) == 0:
                                    continue
                                _s2 = ("SELL" if _p2["quantity"] > 0 else "BUY")
                                try:
                                    kite.place_order(
                                        variety=kite.VARIETY_REGULAR,
                                        exchange=kite.EXCHANGE_NSE,
                                    ...
                                    write_log(f"GWT exit failed {_p2['tradingsymbol']}: {_me}")
                        except Exception as _ex:
                            write_log(f"GWT squareoff thread error: {_ex}")
                    _thr_gwt.Thread(target=_gwt_squareoff, daemon=True).start()
...
# ==========================================
# ==========================================
# 15m CANDLE PERSISTENCE — SAVE & LOAD
# ==========================================

def _candle15_filepath(date_obj):
    return os.path.join(CANDLE15_DIR, f"candles_15m_{date_obj.strftime('%Y-%m-%d')}.csv")


def save_15m_candles_to_disk():
    """Save today's completed 15m OHLC candles to disk at squareoff."""
    today_date = datetime.now().date()
    filepath   = _candle15_filepath(today_date)
    rows = []
    for sym, candles in ema15m_today.items():
        for c in candles:
            rows.append({"symbol": sym, "slot": c["slot"],
                         "open": c["open"], "high": c["high"],
                         "low": c["low"], "close": c["close"]})
    if not rows:
        write_log("save_15m_candles: no data to save")
        return
    df = pd.DataFrame(rows)
    df.to_csv(filepath, index=False)
    msg = (f"💾 15m candles saved → {filepath} "
           f"({df['symbol'].nunique()} symbols, {len(df)} candles)")
    print(Fore.CYAN + msg + ts())
    write_log(msg)


def load_15m_candles_from_disk(target_date=None):
    """Seed ema15m_closes with last 20 completed 15m candle closes.

    Three paths tried in order:
      1. Pre-built candles_15m_YYYY-MM-DD.csv  — saved at squareoff, fastest.
         Available from Day 2 onwards.
      2. Build from MINUTE_DIR CSVs — works if minute CSVs already contain
         recent data (i.e. user ran download_today_minute_data yesterday).
      3. Fetch previous trading day 1-min bars from Kite API and resample
         on-the-fly. Same kite object already authenticated. Updates the
         local MINUTE_DIR CSVs as a side-effect so future runs use Path 2.

    Result: EMA7 is accurate from tick one on every run.
    """
    from collections import deque
    ...
    # ── Path 1: pre-built candles_15m file (squareoff snapshot) ─────────
    for days_back in range(1, 8):
        candidate_date = target_date - timedelta(days=days_back)
        candidate_path = _candle15_filepath(candidate_date)
            ...
            try:
                ...
                if not df.empty:
                    ...
                    for sym, grp in df.groupby("symbol"):
                        closes = grp.sort_values("slot")["close"].tolist()[-20:]
                        ema15m_closes[sym] = deque(closes, maxlen=20)
                        loaded += 1
                    msg = (f"📂 15m EMA seed loaded from {candidate_path} "
                           f"({loaded} symbols, up to 20 candles each)")
                    print(Fore.CYAN + msg + ts())
                    write_log(msg)
                    return   # done
            except Exception as e:
                write_log(f"load_15m_candles path1 error: {e}")

    # ── Path 2: build from existing MINUTE_DIR CSVs ──────────────────────
    # Only reliable if minute CSVs have been kept up to date.
    ref_date = None
    # Find the most recent trading day with 1-min data in MINUTE_DIR
    for days_back in range(1, 8):
        check_date = target_date - timedelta(days=days_back)
        found_recent = False
        for sym in SYMBOLS[:5]:   # spot-check 5 symbols
            fp = os.path.join(MINUTE_DIR, f"{sym}.csv")
            if not os.path.exists(fp):
                continue
            try:
                df_chk = pd.read_csv(fp, usecols=["date"])
                df_chk["date"] = pd.to_datetime(df_chk["date"])
                last_date = df_chk["date"].dt.date.max()
    ...
    if ref_date is not None:
        ...
        for sym in SYMBOLS:
            ...
            try:
                ...
                day_df = df[df["date"].dt.date == ref_date].copy()
                if day_df.empty:
                    skipped += 1
                    continue
                day_df = day_df.sort_values("date")
                day_df = day_df[
                    (day_df["date"].dt.time >= pd.Timestamp("09:15").time()) &
                    (day_df["date"].dt.time <= pd.Timestamp("15:30").time())
                ]
                if day_df.empty:
                    skipped += 1
                    continue
                day_df["slot"] = (
                    (day_df["date"].dt.hour * 60 + day_df["date"].dt.minute) // 15 * 15
                )
                c15 = day_df.groupby("slot").agg(
                    close=("close", "last")
                ).reset_index()
                closes = c15["close"].tolist()[-20:]
                if not closes:
                    skipped += 1
                    continue
                ema15m_closes[sym] = deque(closes, maxlen=20)
                ...
                write_log(f"load_15m_candles path2 error {sym}: {e}")
                skipped += 1

        if loaded > 0:
            msg = (f"📂 15m EMA seed built from MINUTE_DIR ({ref_date}) — "
                   f"{loaded} symbols loaded, {skipped} skipped")
            print(Fore.CYAN + msg + ts())
            write_log(msg)
            return   # done

    # ── Path 3: fetch previous trading day bars from Kite API ────────────
    # Minute CSVs are stale or missing. Use the already-authenticated kite
    # object to fetch yesterday's 1-min bars and resample to 15m.
    # Also saves results back to MINUTE_DIR so Path 2 works next time.
    print(Fore.YELLOW +
          "⚠️  Minute CSVs stale or missing — fetching previous day 1-min "
          "bars from Kite API to build 15m EMA seed..." + ts())

    # Find the previous trading day: yesterday going back up to 7 days
    prev_date = None
    for days_back in range(1, 8):
        candidate = target_date - timedelta(days=days_back)
        # Weekday only (Mon=0..Fri=4); skip Sat/Sun
    ...
    if prev_date is None:
        print(Fore.YELLOW +
              "⚠️  Could not determine previous trading day — "
              "EMA7 will build intraday from ~11:00am" + ts())
        return

    from_dt = datetime(prev_date.year, prev_date.month, prev_date.day, 9, 15)
    to_dt   = datetime(prev_date.year, prev_date.month, prev_date.day, 15, 30)
    ...
    for sym in SYMBOLS:
        ...
        try:
            ...
            # Resample 1-min → 15-min
            df_day = df.sort_values("date")
            df_day = df_day[
                (df_day["date"].dt.time >= pd.Timestamp("09:15").time()) &
                (df_day["date"].dt.time <= pd.Timestamp("15:30").time())
            ]
            df_day["slot"] = (
                (df_day["date"].dt.hour * 60 + df_day["date"].dt.minute) // 15 * 15
            )
            c15 = df_day.groupby("slot").agg(close=("close", "last")).reset_index()
            closes = c15["close"].tolist()[-20:]
            if not closes:
                skipped += 1
                continue
            ...
            # Save to MINUTE_DIR so future runs use Path 2 (no API call)
            fp = os.path.join(MINUTE_DIR, f"{sym}.csv")
            if os.path.exists(fp):
                existing = pd.read_csv(fp)
                existing["date"] = _safe_to_naive(existing["date"])
                existing = existing[existing["date"].dt.date != prev_date]
            ...
            api_errors += 1
            write_log(f"load_15m_candles path3 error {sym}: {e}")
            skipped += 1

    if loaded > 0:
        msg = (f"📂 15m EMA seed fetched from Kite API ({prev_date}) — "
               f"{loaded} symbols loaded, {skipped} skipped"
...
def auto_square_off():
    ...
    while True:
        ...
        # ── HALT new entries IMMEDIATELY before any cancel/exit ──────────
        # Without this, strategy signals firing during the 5-10s cancel+exit
        # sequence can place fresh orders that get abandoned with no SL.
        global trading_enabled
        trading_enabled = False
        write_log("SQUAREOFF: trading_enabled=False — no new entries will fire")
        ...
        # =================================================================
            ...
            for sym_key in list(paper_positions.keys()):
                ...
                # Guard: entry=0 means corrupt restore — close at breakeven
                if not entry or entry == 0:
                    pos["status"]  = "CLOSED"
                    pos["net_pnl"] = round(-(BROKERAGE_PER_ORDER * 2), 2)
                    write_log(f"PAPER SQ OFF entry=0 {sym_key} — closed at breakeven")
                    continue
...
def paper_position_summary():
    ...
    while True:
        ...
        gwt_status   = (Fore.GREEN + f"🏆 GROSS WIN TARGET HIT (₹{GROSS_WIN_TARGET:,.0f})"
                        if gross_win_target_hit else
                        Fore.CYAN  + f"   Gross win target: ₹{GROSS_WIN_TARGET:,.0f}")
...
def run_market_replay_proper(replay_date):
    ...
    latest_highs.clear()
    ema15m_closes.clear()
    ema15m_prev_slot.clear()
    ema15m_today.clear()
    latest_lows.clear()
    ema_pullback_state.clear()
    vwap_state.clear()
    pyramid_state.clear()   # ← reset pyramiding state for each replay day
    first5_data.clear()     # BUG FIX: S10 used stale F5 candle from previous replay day
    first10_data.clear()    # BUG FIX: S9 used stale F10 candle from previous replay day
    _ema_missing_warned.clear()  # reset so EMA warnings fire fresh for each replay day
    # BUG FIX: _nifty_bias_blocked carries (symbol, side) pairs between replay days.
    # Without reset, symbols blocked on day N can never fire in the correct direction
    # on day N+1 (even if bias has completely flipped), corrupting multi-day replay PnL.
    global _nifty_bias_blocked
    _nifty_bias_blocked = set()
    # BUG FIX: scan diagnostic buffers carry last minute's data between replay days.
    # _scan_last_minute retains HH:MM from previous day → first minute of new day
    # is silently dropped (minute matches the "already printed" key) and the
    # _scan_flush_worker never prints it. Reset all three dicts together.
    with _scan_lock:
        _scan_last_minute.clear()
        _scan_buffer.clear()
        _scan_printed.clear()
    ...
    # ── Load NIFTY minute data for replay bias simulation ────────────────
    # Without this, nifty_bias is set once from nifty_prev_close and never
    # updated intraday. Real reversal signals never fire in replay — the bias
    # stays stuck at the opening state all day, causing wrong filter decisions.
    # NIFTY minute CSV is stored as "NIFTY.csv" by download_today_minute_data.
    _nifty_minute_data = {}
    _nifty_minute_file = os.path.join(MINUTE_DIR, "NIFTY.csv")
    if os.path.exists(_nifty_minute_file):
        try:
            _ndf_m = pd.read_csv(_nifty_minute_file)
            _ndf_m["date"] = pd.to_datetime(_ndf_m["date"])
            _ndf_m["date_only"] = _ndf_m["date"].dt.date
            _nifty_day = _ndf_m[_ndf_m["date_only"] == replay_date].sort_values("date")
            if not _nifty_day.empty:
                for _, _nr in _nifty_day.iterrows():
                    _nifty_minute_data[_nr["date"]] = float(_nr["close"])
                print(Fore.CYAN +
                      f"📊 [REPLAY] NIFTY minute data loaded: "
                      f"{len(_nifty_minute_data)} bars for {replay_date}" + ts())
        except Exception as _nme:
            print(Fore.YELLOW + f"⚠️  [REPLAY] NIFTY minute load failed: {_nme}" + ts())
    else:
        print(Fore.YELLOW +
              "⚠️  [REPLAY] NIFTY.csv not found — bias will use static opening state. "
              "Run download_today_minute_data() to fix." + ts())
    ...
    for current_time in all_times:
        ...
        # ── Feed NIFTY price for this minute to bias filter ───────────────
        if _nifty_minute_data:
            _nifty_close = _nifty_minute_data.get(current_time)
            if _nifty_close:
                _update_nifty_bias(_nifty_close)
        ...
        for symbol, df in symbol_minute_data.items():
            ...
            # ── Pre-seed first10_data (S9 10-min candle, 09:15–09:24) ───────
            if "09:15" <= candle_time_str_pre < "09:25":
                if symbol not in first10_data:
                    first10_data[symbol] = {
                        "open":  open_p,
                        "high":  high,
                    ...
                    first10_data[symbol]["high"]  = max(first10_data[symbol]["high"],  high)
                    first10_data[symbol]["low"]   = min(first10_data[symbol]["low"],   low)
                    first10_data[symbol]["close"] = close_p
                # Freeze at 09:25
                if candle_time_str_pre >= "09:25":
                    c10 = first10_data[symbol]
                    if c10["high"] != 0 and c10["low"] != 999999:
                        c10["ready"] = True
...
def download_today_minute_data():
    ...
    # ── Download NIFTY 50 index minute data first ─────────────────────────
    # Required for replay bias simulation. Saved as NIFTY.csv in MINUTE_DIR.
    try:
        _nifty_min = kite.historical_data(NIFTY_TOKEN, from_dt, to_dt, "minute")
        if _nifty_min:
            _ndf_new = pd.DataFrame(_nifty_min)
            _ndf_new["date"] = _safe_to_naive(_ndf_new["date"])
            _nifty_path = os.path.join(MINUTE_DIR, "NIFTY.csv")
            if os.path.exists(_nifty_path):
                _ndf_ex = pd.read_csv(_nifty_path)
                _ndf_ex["date"] = _safe_to_naive(_ndf_ex["date"])
                _ndf_ex = _ndf_ex[_ndf_ex["date"].dt.date != today]
                _ndf_new = pd.concat([_ndf_ex, _ndf_new], ignore_index=True)
            _ndf_new.drop_duplicates(subset=["date"], inplace=True)
            _ndf_new.sort_values("date", inplace=True)
            _ndf_new.to_csv(_nifty_path, index=False)
            print(Fore.CYAN + f"📊 NIFTY minute data saved ({len(_ndf_new)} bars)" + ts())
        time.sleep(0.1)
    except Exception as _ne:
        print(Fore.YELLOW + f"⚠️  NIFTY minute download failed: {_ne}" + ts())
...
def download_6_month_data():
    ...
    # ── Download NIFTY 50 minute data in 60-day chunks ────────────────────
    # Required for replay bias simulation (_update_nifty_bias per minute bar).
    try:
        print("📥 Downloading NIFTY 50 minute data..." + ts())
        _nifty_minute_all = []
        _chunk = from_date
        while _chunk < to_date:
            _chunk_end = min(_chunk + timedelta(days=60), to_date)
            _bars = kite.historical_data(NIFTY_TOKEN, _chunk, _chunk_end, "minute")
            if _bars:
                _nifty_minute_all.extend(_bars)
            _chunk = _chunk_end + timedelta(days=1)
            time.sleep(0.3)
        if _nifty_minute_all:
            _nmdf = pd.DataFrame(_nifty_minute_all)
            _nmdf["date"] = _safe_to_naive(_nmdf["date"])
            _nmdf.drop_duplicates(subset=["date"], inplace=True)
            _nmdf.sort_values("date", inplace=True)
            _nmdf.to_csv(os.path.join(MINUTE_DIR, "NIFTY.csv"), index=False)
            print(Fore.CYAN + f"📊 NIFTY minute data saved ({len(_nmdf)} bars)" + ts())
    except Exception as _nme2:
        print(Fore.YELLOW + f"⚠️  NIFTY minute download failed: {_nme2}" + ts())
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if symbol not in replay_15m_data or replay_15m_data[symbol]["start_min"] != slot_15:
            # Slot rolled over — save the completed candle close for EMA15m
            _prev_c15 = replay_15m_data.get(symbol, {})
            if _prev_c15 and "close" in _prev_c15:
                _update_ema15m_closes(symbol, slot_15, _prev_c15["close"],
                                      prev_candle=_prev_c15)
            replay_15m_data[symbol] = {
        ...
        if not ws_ready:
            ...
            # ── S9/S10 first-candle build: must run during warmup (09:15–09:24) ──
            # build_context() returns None before ENTRY_START_TIME, so
            # _build_first10 (S9) and _build_first5 (S10) inside strategy() never
            # run during the 09:15–09:24 window when warmup blocks strategy().
            # We call them here directly before the warmup gate.
            # latest_highs is already updated above so S10's O=H check sees
            # the correct day high on the very first 09:19 tick.
            current_time_str = tick_time.strftime("%H:%M")
            if "09:15" <= current_time_str < "09:25":
                _build_first10(symbol, ltp, current_time_str)  # S9: 10-min candle
            if "09:15" <= current_time_str < "09:20":
                _build_first5(symbol, ltp, current_time_str)   # S10: 5-min candle
            ...
            def _retighten_on_restart():
                ...
                try:
                    for _sym, _pos in list(paper_positions.items()):
                        ...
                        if _side == "SELL":
                            if _cur_sl <= _new_sl:   # SL already at/below entry → skip (was backwards)
                                continue
                        else:  # BUY
...
def on_reconnect(ws, attempt):
    global ws_ready, ws_connect_time
    # BUG FIX: reset ws_ready so warmup fires again after reconnect.
    # Without this, strategies run immediately after reconnect during the
    # price-catch-up window, potentially firing on stale tick data.
    ws_ready        = False
    ws_connect_time = datetime.now()
...
# ==========================================
# DEDICATED PnL GUARD THREAD
# ==========================================
def _pnl_guard_thread():
    """Standalone PnL watchdog — fires every PNL_MONITOR_INTERVAL seconds."""
    global daily_pnl, trading_enabled, gross_win_target_hit, profit_lock_armed

    import time as _t

    def _close_all_paper(reason):
        global daily_pnl
        for _gs, _gp in list(paper_positions.items()):
            if _gp.get("status") != "OPEN":
                continue
            _base  = _gs.split("__PYR")[0]
            _ltp   = latest_prices.get(_base, _gp["entry"])
            _side  = _gp["side"]
            _pts   = (_ltp - _gp["entry"]) if _side == "BUY" else (_gp["entry"] - _ltp)
            _net   = round(_pts * int(_gp.get("qty", FIXED_QTY)) - BROKERAGE_PER_ORDER * 2, 2)
            _gp["status"]  = "CLOSED"
            _gp["net_pnl"] = _net
            with _daily_pnl_lock:
                daily_pnl += _net
            write_log(f"PNL_GUARD CLOSE {_gs} {_side} Entry:{_gp['entry']} "
                      f"Exit:{_ltp:.2f} Net:{_net:.2f} Reason:{reason}")
        save_paper_positions_cache()
        pyramid_state.clear()

    def _close_all_live(reason):
        try:
            # ── Step 1: Cancel all open SL-M/target orders to prevent
            # dangling orders that would create reverse positions after market exit.
            try:
                _all_ords = kite.orders()
                _algo_syms = set(trades_taken.keys())
                for _o in _all_ords:
                    _base = _o.get("tradingsymbol", "").split("__PYR")[0]
                    if (_base in _algo_syms and
                ...
                write_log(f"PNL_GUARD cancel sweep failed: {_ce}")
            _t.sleep(1)  # let cancellations propagate

            # ── Step 2: Market-exit all open net positions ──────────────
            # Use ["net"] not ["day"] — "day" includes closed positions (qty=0)
            # which would cause phantom market orders for already-flat symbols.
            _open = [p for p in kite.positions().get("net", [])
                     if p.get("quantity", 0) != 0]
            for _p in _open:
                _side = "SELL" if _p["quantity"] > 0 else "BUY"
                try:
                    kite.place_order(
                        variety=kite.VARIETY_REGULAR,
                        exchange=kite.EXCHANGE_NSE,
                        tradingsymbol=_p["tradingsymbol"],
                        transaction_type=_side,
                        quantity=abs(_p["quantity"]),
                        order_type=kite.ORDER_TYPE_MARKET,
                        product=kite.PRODUCT_MIS,
                    )
                    _t.sleep(0.3)
                    write_log(f"PNL_GUARD LIVE EXIT {_p['tradingsymbol']} qty={abs(_p['quantity'])} Reason:{reason}")
                except Exception as _e:
                    write_log(f"PNL_GUARD LIVE EXIT FAIL {_p['tradingsymbol']}: {_e}")
        except Exception as _ex:
            write_log(f"PNL_GUARD live close_all error: {_ex}")

    _t.sleep(120)
    while True:
        try:
            _now_hm = datetime.now().strftime("%H:%M")
            if _now_hm < "09:15" or _now_hm > "15:30":
                _t.sleep(PNL_MONITOR_INTERVAL)
                continue

            with _daily_pnl_lock:
                _closed_pnl = daily_pnl

            _open_pnl = 0.0
            for _sym, _pos in list(paper_positions.items()):
                if _pos.get("status") != "OPEN":
                    continue
                _base = _sym.split("__PYR")[0]
                _ltp  = latest_prices.get(_base)
                if _ltp:
                    _pts = (_ltp - _pos["entry"]) if _pos["side"] == "BUY" else (_pos["entry"] - _ltp)
                    _open_pnl += _pts * int(_pos.get("qty", FIXED_QTY))
            _total_pnl = _closed_pnl + _open_pnl

            # ── Hard Loss check ──────────────────────────────────────────
            if trading_enabled and _total_pnl <= HARD_LOSS_LIMIT:
                trading_enabled = False
                _msg = (f"🚨 PNL_GUARD HARD LOSS | Total PnL:₹{_total_pnl:.2f} "
                        f"<= Limit:₹{HARD_LOSS_LIMIT} | Closing ALL + HALTED")
                print(Fore.RED + _msg + ts())
                write_log(_msg)
                if TRADING_MODE == "PAPER":
                    _close_all_paper("PNL_GUARD_LOSS_LIMIT")
                else:
                    _close_all_live("PNL_GUARD_LOSS_LIMIT")

            # ── Gross Win Target check ───────────────────────────────────
            if trading_enabled and not gross_win_target_hit and GROSS_WIN_TARGET > 0:
                _gross_win = sum(
                    p["net_pnl"] for p in paper_positions.values()
                    if p.get("status") == "CLOSED" and p.get("net_pnl", 0) > 0
                )
                if _gross_win >= GROSS_WIN_TARGET:
                    gross_win_target_hit = True
                    trading_enabled = False
                    _msg = (f"🏆 PNL_GUARD GROSS WIN TARGET | Gross:₹{_gross_win:.2f} "
                            f">= Target:₹{GROSS_WIN_TARGET} | Closing ALL + HALTED")
                    print(Fore.GREEN + _msg + ts())
                    write_log(_msg)
                    if TRADING_MODE == "PAPER":
                        _close_all_paper("PNL_GUARD_GROSS_WIN")
                    else:
                        _close_all_live("PNL_GUARD_GROSS_WIN")

            # ── Profit Lock arm check ────────────────────────────────────
            if not profit_lock_armed and _total_pnl >= PROFIT_LOCK_TRIGGER:
                profit_lock_armed = True
                _msg = (f"🔒 PNL_GUARD PROFIT LOCK ARMED | PnL:₹{_total_pnl:.2f} "
                        f">= Trigger:₹{PROFIT_LOCK_TRIGGER} | Floor:₹{PROFIT_LOCK_FLOOR}")
                print(Fore.CYAN + _msg + ts())
                write_log(_msg)

        except Exception as _e:
            write_log(f"pnl_guard_thread error: {_e}")
...
if __name__ == "__main__":
    ...
    threading.Thread(target=_pnl_guard_thread,         daemon=True).start()  # ← dedicated PnL watchdog (every 60 s)
