...
H1_TRACK_CSV     = os.path.join(CACHE_DIR, "1h_hilo_track.csv")
...
def _load_raw_5min_today():
    """
    Load today's 5-min candles from CSV.
    FIX BUG 6: filter to today only — multi-day CSV would mix slots.
    FIX BUG 9: uses real 'volume' column; falls back to 0 if absent.
    Returns raw DataFrame or None on failure.
    """
...
def build_15m_tracker():
    """
    FIX BUG 1,2,3,6,9: Correct sequential 15-min tracker.
...
def build_1h_tracker():
    """
    FIX BUG 4,5,6: Correct sequential 1-hour tracker.
...
_15m_high_df, _15m_low_df, _vol_df = build_15m_tracker()
_h1_high_df,  _h1_low_df           = build_1h_tracker()
