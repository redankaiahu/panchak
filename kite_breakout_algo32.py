...
def _flush_scan(strategy_name, minute_str):
    ...
    with _scan_lock:
        ...
        buf = list(_scan_buffer.get(strategy_name, []))
...
def strategy_inside_bar(ctx):
    ...
    if buy_signal:
        ...
        if not (ltp > c1["high"]) and not (ltp < c1["low"]):
...
# AUTO SQUARE OFF
# ==========================================
_STARTUP_TIME = datetime.now()   # recorded when program starts

def auto_square_off():
    """
    Waits until SQUARE_OFF_TIME then closes all open positions once.
    Guards:
      1. Never fires before SQUARE_OFF_TIME
      2. If restarted AFTER square off time and no OPEN positions remain → skip silently
      3. Waits for WebSocket to connect before checking (min 30s after startup)
    """
    # Wait at least 60 seconds after startup before checking
    # This ensures WebSocket connects, positions restore, and prompts complete
    time.sleep(60)
    ...
    while True:
        ...
        if datetime.now().strftime("%H:%M") >= SQUARE_OFF_TIME:
            ...
            # If no open positions, nothing to do — skip silently
...
# NOTE: auto_square_off and position manager threads are started
# inside if __name__ == "__main__" after all prompts complete.
...
if __name__ == "__main__":
    ...
    if TRADING_MODE == "PAPER" and input("Replay Mode? (y/n): ") == "y":
        ...
        # ── Start WebSocket ───────────────────────────────────────────────
        # After connect, verify_restored_positions() fires once prices arrive
        kws.connect()