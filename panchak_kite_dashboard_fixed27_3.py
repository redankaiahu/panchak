...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    if _up_rows and not _is_tg_disabled("TOP_HIGH"):
        ...
        try:
            ...
            _pstart_key = "?"
        send_telegram_bg(_msg, dedup_key=f"PANCHAK_UP_{_syms_key}_{_pstart_key}_{_now_str[:5]}")
    ...
    if _down_rows and not _is_tg_disabled("TOP_LOW"):
        ...
        try:
            ...
            _pstart_key = "?"
        send_telegram_bg(_msg, dedup_key=f"PANCHAK_DOWN_{_syms_key}_{_pstart_key}_{_now_str[:5]}")
...
def _vedic_day_analysis(d_date):
    ...
    # Jupiter retrograde — computed dynamically from actual ephemeris
    # (previous code had hardcoded Mar 11 – Jul 2026 which was WRONG — Jupiter is DIRECT in this period)
    # We compute daily motion: if tomorrow's longitude < today's → retrograde
    _j_dt1 = datetime(d_date.year, d_date.month, d_date.day, 3, 45, 0)
    _j_dt2 = datetime((d_date + timedelta(days=1)).year,
                      (d_date + timedelta(days=1)).month,
                      (d_date + timedelta(days=1)).day, 3, 45, 0)
    _j_jd1 = _jd(_j_dt1)
    _j_jd2 = _jd(_j_dt2)
    _j_lon1 = _sid(_jupiter_trop(_j_jd1), _j_jd1)
    _j_lon2 = _sid(_jupiter_trop(_j_jd2), _j_jd2)
    _j_motion = _j_lon2 - _j_lon1
    if abs(_j_motion) > 1: _j_motion = _j_motion - 360 if _j_motion > 0 else _j_motion + 360
    _j_is_retro = _j_motion < 0

    if _j_is_retro:
        if sun_nak_lord == "Jupiter" or moon_nak_lord == "Jupiter":
            bearish_pts += 1
            signals.append(("Jupiter retrograde + Sun/Moon in Jupiter nakshatra — correction amplifier", "B"))
    ...
    # ══════════════════════════════════════════════════════════════
    # VISHNU BHASKAR — Advanced Techniques of Predictive Astrology
    # Vol 1 (2012) + Vol 2 Ch48-50: Mundane + Commodity Pricing
    # New rules: VB1-VB14 add ~6-10 pts to scoring accuracy
    # ══════════════════════════════════════════════════════════════

    # VB1: Govardhan Padhati — Auspicious vs Adverse Nakshatras for Finance (Ch50)
    _VB_BUY_NAKS  = {"Ashwini","Mrigasira","Punarvasu","Pushya","Hasta","Chitra",
                     "Swati","Anuradha","Shravana","Dhanishtha","Shatabhisha","Revati"}
    _VB_SELL_NAKS = {"Bharani","Krittika","Rohini","Ardra","Ashlesha","Magha",
                     "Purva Phalguni","Purvashadha","Purva Bhadrapada",
                     "Uttara Phalguni","Uttarashadha","Uttara Bhadrapada",
                     "Vishakha","Mula"}
    if moon_nak in _VB_BUY_NAKS:
        bullish_pts += 1
        signals.append((f"VB Ch50: {moon_nak} = Auspicious Financial Nakshatra (Govardhan) — BUY signal", "G"))
    elif moon_nak in _VB_SELL_NAKS:
        bearish_pts += 1
        signals.append((f"VB Ch50: {moon_nak} = Adverse Financial Nakshatra (Govardhan) — SELL signal", "B"))

    # VB2: Govardhan Padhati — Weekday bias (Ch50)
    # Wed & Sun = Buying days; Thu & Mon = Selling days
    _vb_weekday = d_date.weekday()   # 0=Mon...6=Sun
    if _vb_weekday in (2, 6):
        bullish_pts += 1
        signals.append((f"VB Ch50: {d_date.strftime('%A')} = Govardhan Buying Vaar — bullish bias", "G"))
    elif _vb_weekday in (3, 0):
        bearish_pts += 1
        signals.append((f"VB Ch50: {d_date.strftime('%A')} = Govardhan Selling Vaar — bearish bias", "B"))

    # VB3: Dhruvanka Method — Shares (Dhruvanka=86) price prediction (Ch50)
    # Remainder of (Sum of Dhruvankas)/9: 1/3/4/6/8=Rise, 2/5/9=Fall, 7=No change
    _VB_NAK_DH = {
        "Ashwini":10,"Bharani":10,"Krittika":96,"Rohini":56,"Mrigasira":20,
    ...
    _VB_RASHI_DH = {
        "Aries":37,"Taurus":84,"Gemini":66,"Cancer":109,"Leo":125,"Virgo":102,
        "Libra":140,"Scorpio":144,"Sagittarius":144,"Capricorn":198,
        "Aquarius":190,"Pisces":180,
    }
    _VB_TITHI_DH = {
        "Pratipada":25,"Dwitiya":23,"Tritiya":21,"Chaturthi":19,"Panchami":17,
    ...
    _VB_VAAR_DH = {0:103,1:70,2:62,3:212,4:112,5:542,6:40}
    _dh_total = (86
                 + _VB_NAK_DH.get(moon_nak, 50)
                 + _VB_RASHI_DH.get(moon_rashi, 100)
                 + _VB_TITHI_DH.get(tithi_name, 15)
                 + _VB_VAAR_DH.get(_vb_weekday, 100))
    _dh_rem   = _dh_total % 9 or 9
    _VB_DH_RESULT = {1:"RISE",2:"FALL",3:"RISE",4:"SHARP RISE",
                     5:"FALL",6:"RISE",7:"NO CHANGE",8:"RISE",9:"FALL"}
    _dh_outcome = _VB_DH_RESULT.get(_dh_rem, "MIXED")
    if _dh_outcome in ("RISE","SHARP RISE"):
        _dh_pts = 2 if _dh_outcome == "SHARP RISE" else 1
        bullish_pts += _dh_pts
        signals.append((f"VB Dhruvanka Shares: {_dh_total}%9=R{_dh_rem} → {_dh_outcome} (Govardhan Padhati)", "G"))
    elif _dh_outcome == "FALL":
        bearish_pts += 1
        signals.append((f"VB Dhruvanka Shares: {_dh_total}%9=R{_dh_rem} → {_dh_outcome} (Govardhan Padhati)", "B"))
    else:
        signals.append((f"VB Dhruvanka Shares: {_dh_total}%9=R{_dh_rem} → {_dh_outcome}", "M"))

    # VB4: Mundane Planetary Conjunctions (Ch48) — Market crash triggers
    ...
    if mars_jupiter < 10:
        bearish_pts += 2
        signals.append((f"VB Ch48: Mars+Jupiter {mars_jupiter:.0f}° — financial/religious troubles (Mundane)", "B"))
    elif mars_jupiter < 20:
        bearish_pts += 1
        signals.append((f"VB Ch48: Mars+Jupiter {mars_jupiter:.0f}° approaching conj — volatility warning", "B"))
    if jup_saturn < 5:
        signals.append((f"VB Ch48: Jupiter+Saturn {jup_saturn:.1f}° — Great Mutation (20yr cycle); trend shift", "M"))

    # VB5: Planet strength → commodity/market effect (Ch50)
    # Strong Mercury (own/exalt sign) = REDUCES share prices
    if mercury_rashi in ("Gemini","Virgo"):
        bearish_pts += 1
        signals.append((f"VB Ch50: Mercury strong in {mercury_rashi} — REDUCES share prices (planet-commodity rule)", "B"))
    elif mercury_rashi == "Pisces":
        bullish_pts += 1
        signals.append(("VB Ch50: Mercury debil in Pisces — RAISES share prices", "G"))
    if jupiter_rashi == "Cancer":
        bullish_pts += 1
        signals.append(("VB Ch50: Jupiter exalted Cancer — commodity prices fall; equities supported", "G"))
    elif jupiter_rashi == "Capricorn":
        bearish_pts += 1
        signals.append(("VB Ch50: Jupiter debil Capricorn — markets depressed", "B"))
    if saturn_rashi == "Aries":
        bullish_pts += 1
        signals.append(("VB Ch50: Saturn debil Aries — metal scarcity; metal stocks rally", "G"))

    # VB6: Retrograde planets — bearish for their sector (Ch50)
    _RETRO_PERIODS = [
        ("Venus",   "IT/FMCG/Banks",  date(2025,3,1),   date(2025,4,13)),
        ("Mercury", "IT/Telecom",     date(2025,3,15),  date(2025,4,7)),
        ("Mercury", "IT/Telecom",     date(2025,7,18),  date(2025,8,11)),
        ("Mercury", "IT/Telecom",     date(2025,11,9),  date(2025,11,29)),
        ("Mercury", "IT/Telecom",     date(2026,3,15),  date(2026,4,7)),
        ("Jupiter", "Banks/Gold",     date(2025,11,11), date(2026,3,10)),
        ("Saturn",  "Metals/Infra",   date(2025,7,13),  date(2025,11,28)),
        ("Mars",    "Metals/Auto",    date(2024,12,6),  date(2025,2,24)),
    ]
    for _rp, _rs, _rb, _re in _RETRO_PERIODS:
        if _rb <= d_date <= _re:
            bearish_pts += 1
            signals.append((f"VB Ch50: {_rp} RETROGRADE — {_rs} bearish (strong retro = increased price fall)", "B"))
            break

    # VB7: Moon+Jupiter aspect — auspicious (Ch48 Mundane)
    if moon_jupiter < 8:
        bullish_pts += 1
        signals.append((f"VB Ch48: Moon+Jupiter {moon_jupiter:.0f}° — auspicious Mundane; bullish", "G"))
    elif abs(moon_jupiter - 180) < 5:
        bullish_pts += 1
        signals.append((f"VB Ch48: Moon opposite Jupiter {moon_jupiter:.0f}° — Full Moon on Jupiter; positive", "G"))

    # VB8: Murthy Nirnaya — Jupiter transit quality by rashi (Ch50)
    _JUPITER_MURTHY = {
        "Aries":("Copper","G","Jupiter in Aries = Copper Murthy — Good transit"),
        "Taurus":("Silver","G","Jupiter in Taurus = Silver Murthy — BEST transit for markets"),
        "Gemini":("Iron","B","Jupiter in Gemini = Iron Murthy — Bad; equities under pressure"),
        "Cancer":("Gold","M","Jupiter in Cancer = Gold Murthy — Average (exalted but mixed)"),
        "Leo":("Silver","G","Jupiter in Leo = Silver Murthy — BEST transit for equities"),
        "Virgo":("Copper","G","Jupiter in Virgo = Copper Murthy — Good transit"),
        "Libra":("Iron","B","Jupiter in Libra = Iron Murthy — Bad; market bearish bias"),
        "Scorpio":("Gold","M","Jupiter in Scorpio = Gold Murthy — Average transit"),
        "Sagittarius":("Copper","G","Jupiter in Sagittarius = Copper Murthy — Good (own sign)"),
        "Capricorn":("Iron","B","Jupiter in Capricorn = Iron Murthy — Bad; debilitated"),
        "Aquarius":("Silver","G","Jupiter in Aquarius = Silver Murthy — BEST for tech/markets"),
        "Pisces":("Gold","M","Jupiter in Pisces = Gold Murthy — Average (own sign)"),
    }
    _jm = _JUPITER_MURTHY.get(jupiter_rashi)
    if _jm:
        _jm_murthy, _jm_type, _jm_text = _jm
        if _jm_type == "G":
            bullish_pts += 1
            signals.append((f"VB Murthy Nirnaya: {_jm_text}", "G"))
        elif _jm_type == "B":
            bearish_pts += 1
            signals.append((f"VB Murthy Nirnaya: {_jm_text}", "B"))
        else:
            signals.append((f"VB Murthy Nirnaya: {_jm_text}", "M"))

    # VB9: Sun+Rahu conjunction — share prices bearish (Ch50 + Mundane)
    if sun_rahu < 10:
        bearish_pts += 2
        signals.append((f"VB: Sun+Rahu {sun_rahu:.0f}° — share prices bearish (Ch50 Planet rule + Mundane)", "B"))

    # VB10: Mercury+Venus conjunction — prices fall (VR2+VB agree) 
    if mercury_rashi == venus_rashi and merc_venus < 15:
        bearish_pts += 1
        signals.append((f"VB+VR: Mercury+Venus conjunct in {mercury_rashi} ({merc_venus:.0f}°) — PRICES FALL (both texts)", "B"))

    # VB11: Moon+Mars conjunction — volatile/sudden selloff (Ch48)
    if moon_mars < 8:
        bearish_pts += 1
        signals.append((f"VB Ch48: Moon+Mars {moon_mars:.0f}° — aggressive market; sudden selloff risk", "B"))

    # VB12: Sun+Mars conjunction — fires/market crash (Ch48 Group I adversity)
    if sun_mars < 5:
        bearish_pts += 2
        signals.append((f"VB Ch48: Sun+Mars {sun_mars:.0f}° — Mundane adversity Group I; crash risk", "B"))
    elif sun_mars < 12:
        bearish_pts += 1
        signals.append((f"VB Ch48: Sun+Mars {sun_mars:.0f}° — selling pressure building", "B"))

    # VB13: Jupiter+Rahu conjunction — Gold/Silver hyperinflation (Ch17+Ch50)
    if jup_rahu < 15:
        signals.append((f"VB+Banerjee: Jupiter+Rahu {jup_rahu:.0f}° — Gold/Silver HYPERINFLATION (Govardhan+Ch17)", "M"))

    # VB14: Tithi-based enhanced market direction (Ch50 Govardhan)
    if paksha == "Shukla" and tithi_name in ("Panchami","Dashami","Ekadashi","Dwadashi"):
        bullish_pts += 1
        signals.append((f"VB Ch50: Shukla {tithi_name} — auspicious tithi for markets (Govardhan Padhati)", "G"))
    elif paksha == "Krishna" and tithi_name in ("Ashtami","Navami","Dashami"):
        bearish_pts += 1
        signals.append((f"VB Ch50: Krishna {tithi_name} — inauspicious tithi for markets", "B"))
    ...
    return {
        ...
        "dhruvanka_total": _dh_total,
        "dhruvanka_rem":   _dh_rem,
        "dhruvanka_result":_dh_outcome,
...
<div class="{_card_cls}">
  ...
  <div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:8px">
    ...
    <span>♃ <b>Jupiter:</b> {_snap["jupiter_rashi"]} · <b style="color:{'#FFD700' if _snap.get('jupiter_murthy') else '#aaa'}">{_snap.get("jupiter_murthy","—")} Murthy</b></span>
  ...
  <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:8px;font-size:12px;color:#aaa">
    <span>🔢 <b>VB Dhruvanka:</b> <span style="color:{'#00C851' if _snap.get('dhruvanka_result') in ('RISE','SHARP RISE') else '#FF4444' if _snap.get('dhruvanka_result')=='FALL' else '#FFD700'}">{_snap.get("dhruvanka_result","—")}</span> (Total={_snap.get("dhruvanka_total","—")} R={_snap.get("dhruvanka_rem","—")})</span>
    <span>⚡ Mars-Rahu: <b style="color:{'#FF4444' if _snap['mars_rahu_deg']<15 else '#aaa'}">{_snap["mars_rahu_deg"]}°</b></span>
    <span>☿ Merc-Venus: <b>{_snap.get("merc_venus_deg","—")}°</b></span>
    <span>♃ Mars-Jupiter: <b>{_snap.get("mars_jupiter_deg","—")}°</b></span>
    <span>☀ Sun-Rahu: <b style="color:{'#FF4444' if (_snap.get('sun_rahu_deg') or 180)<10 else '#aaa'}">{_snap.get("sun_rahu_deg","—")}°</b></span>
  </div>
</div>
""", unsafe_allow_html=True)
...
with tabs[8]:
    ...
    if not _orb_range_ready:
        ...
        if not _h1_up.empty:
            ...
            st.dataframe(_h1_up.sort_values("POST_BREAK_MOVE_%", ascending=False), width="stretch")
        if not _h1_down.empty:
            st.markdown("#### 🔴 Breakdowns Below 1H Low")
            st.dataframe(_h1_down.sort_values("POST_BREAK_MOVE_%", ascending=False), width="stretch")
...
with tabs[18]:
    ...
    _BEARISH_NAK_RULES = {
        "Shatabhisha":       ("BEARISH", "HIGH",   "Rahu nakshatra — most bearish. Combo with Amavasya = extreme fall."),
        "Ashlesha":          ("BEARISH", "HIGH",   "Mercury+Saturn nakshatra — institutional selling, IT weak."),
        "Bharani":           ("BEARISH", "HIGH",   "Venus nakshatra — debilitating, FMCG/auto weak."),
        "Krittika":          ("BEARISH", "HIGH",   "Sun nakshatra — sell into any rally, ignore gap-ups."),
        "Uttara Phalguni":   ("BEARISH", "HIGH",   "Sun nakshatra — selling after brief morning rally."),
        "Mula":              ("BEARISH", "EXTREME","Ketu nakshatra — sudden gap-downs, crude bullish. Sell all rallies."),
        "Purvashadha":       ("BEARISH", "HIGH",   "Venus nakshatra — sustained fall, metals weak."),
        "Dhanishtha":        ("BEARISH", "HIGH",   "Mars nakshatra — volatile bearish, metals/auto weak."),
        "Ardra":             ("BEARISH", "HIGH",   "Rahu nakshatra — sharp volatile fall."),
        "Purva Bhadrapada":  ("BEARISH", "HIGH",   "Jupiter+Saturn — financial stocks weak."),
        "Chitra":            ("BEARISH", "MEDIUM", "Mars nakshatra — mild bearish, realty weak."),
        "Vishakha":          ("BEARISH", "MEDIUM", "Jupiter+Saturn nakshatra — slight selling bias."),
    }
    _BULLISH_NAK_RULES = {
        "Pushya":            ("BULLISH", "HIGH",   "Most auspicious nakshatra — BUY at open, add on dips. Banks/FMCG lead."),
        "Punarvasu":         ("BULLISH", "HIGH",   "Jupiter nakshatra — broad rally, IT/Pharma lead."),
        "Anuradha":          ("BULLISH", "HIGH",   "Saturn stable nakshatra — buy dips, holds well."),
        "Hasta":             ("BULLISH", "HIGH",   "Moon's own nakshatra — gap-down then 200pt recovery. Buy gap-down."),
        "Uttara Bhadrapada": ("BULLISH", "HIGH",   "Saturn auspicious — steady bullish, banking leads."),
        "Shravana":          ("BULLISH", "MEDIUM", "Moon nakshatra — positive close, buy 09:30 after open."),
        "Uttarashadha":      ("BULLISH", "MEDIUM", "Sun nakshatra — moderate bullish. Metals/infra lead."),
        "Revati":            ("BULLISH", "MEDIUM", "Mercury nakshatra — IT/telecom bullish. Steady uptrend."),
    }
    _TRAP_NAK_RULES = {
        "Rohini": ("TRAP", "HIGH",
                   "Classic TRAP day — opens bullish, reverses afternoon. "
                   "BUY puts after 10:30 when rally fails. Do NOT buy calls at open."),
    }
    _SPECIAL_TITHI_RULES = {
        ...
        "Purnima":           ("MIXED",   "MEDIUM", "Full moon = volatile. Watch direction at 09:30, trade with momentum."),
    }

    # ── SUN TRANSIT RULES (computed from rashi) ─────────────────────────
    _SUN_RASHI_RULES = {
        "Aries":       ("BULLISH", "HIGH",   "Sun in Aries = strong bullish month. Buy breakouts."),
        "Taurus":      ("MIXED",   "MEDIUM", "Sun in Taurus = neutral. Trade setups only."),
        "Gemini":      ("BEARISH", "HIGH",   "Sun in Gemini = bearish month. Sell rallies."),
        "Cancer":      ("BEARISH", "MEDIUM", "Sun in Cancer = IT/banks weak. Mild bearish."),
        "Leo":         ("BULLISH", "HIGH",   "Sun in Leo = bullish month. Buy dips."),
        "Virgo":       ("MIXED",   "MEDIUM", "Sun in Virgo = selective. FMCG/Pharma ok."),
        "Libra":       ("MIXED",   "MEDIUM", "Sun in Libra = neutral. Trade both sides."),
        "Scorpio":     ("BULLISH", "MEDIUM", "Sun in Scorpio = mild bullish. Bank Nifty leads."),
        "Sagittarius": ("BULLISH", "HIGH",   "Sun in Sagittarius = strong bull. Buy on every dip."),
        "Capricorn":   ("BULLISH", "HIGH",   "Sun in Capricorn = bull run month. All sectors rise."),
        "Aquarius":    ("BEARISH", "HIGH",   "Sun in Aquarius = bearish month. Avoid longs."),
        "Pisces":      ("MIXED",   "MEDIUM", "Sun in Pisces = transition. Wait for clarity."),
    }
    ...
    for _cdi in range(0, _cal_horizon_days + 5):   # +5 buffer so last day is always included
        ...
        # ── Nakshatra event ──────────────────────────────────────────
        if _nak in _BEARISH_NAK_RULES:
            _sig, _int, _rule = _BEARISH_NAK_RULES[_nak]
        elif _nak in _BULLISH_NAK_RULES:
            _sig, _int, _rule = _BULLISH_NAK_RULES[_nak]
        elif _nak in _TRAP_NAK_RULES:
            _sig, _int, _rule = _TRAP_NAK_RULES[_nak]
        else:
            _sig, _int, _rule = "MIXED", "LOW", f"Moon in {_nak} — neutral day"
        ...
        if _tithi_override:
            ...
            if _tsig == "BEARISH" and _sig == "BEARISH":
                _int = "EXTREME"
                _rule = f"DOUBLE BEARISH — {_nak} + {_tithi_override[0]}. {_rule} AND {_trule}"
                _sig = "BEARISH"
            elif _tsig == "BEARISH":
        ...
        _event = {
            ...
            "signal":      _sig,
            "intensity":   _int,
            "nakshatra":   f"{_nak} ({_cr['moon_nak_lord']})",
            "tithi":       _tithi_str,
...
<div style="max-width:700px;margin:auto;background:#111;border-radius:10px;overflow:hidden">
  ...
  <div style="background:#0a0a0a;padding:10px 20px">
    ...
    if _cal_events_filtered:
        ...
        st.caption(
            f"Rolling window: today → {_end_date} ({len(_cal_events)} events total) | "
            f"{_nb} Bearish | {_ng} Bullish | {_nt} Trap | {_ns} Sun transits | "
    ...
    with st.expander("📚 Static Reference — Batch 1: Planetary Signal Rules (click to expand)", expanded=False):
        ...
        st.dataframe(
            _astro_df.style
                ...
                .set_properties(**{"font-size": "12px", "text-align": "left"})
                .set_table_styles([{"selector": "th", "props": [("font-weight", "bold"), ("background-color", "#1a2255"), ("color", "#ffffff")]}]),
            width="stretch",
            height=900,
        )
