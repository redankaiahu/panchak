...
alerts_df = pd.DataFrame(st.session_state.get("alerts", []))
...
if alerts_df.empty:
    ...
    # Sort latest first (if TS exists)
    ...
    existing_cols = [c for c in desired_cols if c in alerts_df.columns]
    alerts_df = alerts_df[existing_cols]

    # Optional: High volume filter toggle
    high_vol_only = st.checkbox("Show High Volume Alerts Only (VOL_% > 50 & CHANGE_% > 1.5)")

    if high_vol_only and "VOL_%" in alerts_df.columns:
        alerts_df = alerts_df[
            (alerts_df["VOL_%"] > 50) &
            (alerts_df["CHANGE_%"] > 1.5)
        ]
