...
# ================= BUILD CONTROL SWITCH =================
FULL_BUILD_MODE = False   # 🔥 Set True only when you want to rebuild all historical data
...
# -------- BUILD OR LOAD FILES (CONTROLLED) --------
if FULL_BUILD_MODE:
    DAILY_FILE   = build_daily_ohlc()
    WEEKLY_FILE  = build_weekly_ohlc()
    MONTHLY_FILE = build_monthly_ohlc()
else:
    DAILY_FILE   = dated_file("daily_ohlc")
    WEEKLY_FILE  = dated_file("weekly_ohlc")
    MONTHLY_FILE = dated_file("monthly_ohlc")
...
# 🔥 HARD DISABLE PANCHAK BUILD
# build_panchak_files()
if not os.path.exists(PANCHAK_DATA_FILE):
    st.warning("Panchak file missing. Run once with FULL_BUILD_MODE=True")
    st.stop()
