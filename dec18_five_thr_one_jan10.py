...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    try:
        ...
        if trantype == "S":
            executed_price1 = executed_price - 5
            if ltp < executed_price1:
                new_stop_loss = float(executed_price - 0.50) #ltp + 5
                new_trigger_price = float(new_stop_loss - 0.30) #ltp + 3
        else:  # Buy Case
            executed_price1 = executed_price + 5
            if ltp > executed_price1:
                new_stop_loss = float(executed_price + 0.50) #ltp - 5
        ...
        if (trantype == "S" and ltp < executed_price1) or (trantype == "B" and ltp > executed_price1):
            #print("sl_order_id:",sl_order_id)
            #print(f"DEBUG: SL Order ID: {sl_order_id}")
            #print(f"DEBUG: Instrument: {instrument}")
            #print(f"DEBUG: Quantity: {qty}")
            #print(f"DEBUG: New Stop Loss: {new_stop_loss}")
            #print(f"DEBUG: New Trigger Price: {new_trigger_price}")
            try:
                modify_order_response = alice.modify_order(
...
def place_order_with_prevention( instrument, transaction_type, price,  symbol_name,condition,open_price,high_price,low_price,volume,ORBL5,ORBH5,ORBL15,ORBH15,yesterday_open,yesterday_high,yesterday_low,yesterday_close,ORBL15_one,ORBH15_one,ORBL15_two,ORBH15_two,ORBL15_thr,ORBH15_thr,ORBL15_four,ORBH15_four,ORBH5Con, ORBL5Con, ORBH15Con, ORBL15Con, ORBHP, ORBLP,SMA_5 ):   
    global orders_placed, order_tracking, order_count, sl_modify_tracking, current_time1
    ...
    if order_count >= 10:
       ...
       print(f"=======>>>>Order count limit reached -- not placing order -- {condition} - {transaction_type} - {symbol_name} at {price} -  Time: [{current_time}] ")
       return
