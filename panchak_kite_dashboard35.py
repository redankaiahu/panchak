...
ENABLE_LIVE_TRADING = False     # 🔴 CHANGE TO True ONLY WHEN READY
...
st_autorefresh(interval=8000_000, key="refresh")
...
#MONTHLY EXPIRY HELPER (ROBUST)

def get_monthly_expiry(index):
    today = date.today()
    year = today.year
    month = today.month
    ...
    return f"{str(year)[-2:]}{calendar.month_abbr[month].upper()}{expiry_day:02d}"

#STRIKE SELECTION (EXACT)

def round_to_step(price, step):
    return int(round(price / step) * step)

def get_itm_strike(spot, step, offset, opt_type):
    atm = round_to_step(spot, step)
    return atm - offset if opt_type == "CE" else atm + offset

#SIGNAL ENGINE (FINAL LOGIC)

def check_trade_signal(row):
    open_ = row["LIVE_OPEN"]
    ltp = row["LTP"]
    yh = row["YEST_HIGH"]
    yl = row["YEST_LOW"]
    ...
    # BUY CALL
    if open_ < yh and ltp >= yh:
        return "CE"

    # BUY PUT
    if open_ > yl and ltp <= yl:
        return "PE"

    return None

#OPTION SYMBOL BUILDER (KITE FORMAT)
def build_option_symbol(index, strike, opt_type):
    expiry = get_monthly_expiry(index)
    return f"{index}{expiry}{strike}{opt_type}"

#PLACE ORDER (PAPER + LIVE SAFE)

def place_trade(index, opt_type, row):

    cfg = TRADE_CONFIG[index]

    strike = get_itm_strike(
        spot=row["LTP"],
        step=cfg["step"],
        offset=cfg["itm_offset"],
        opt_type=opt_type
    )

    symbol = build_option_symbol(index, strike, opt_type)
    qty = cfg["lot"]
    ...
    trade = {
        ...
        "TYPE": opt_type,
        "QTY": qty,
        "STATUS": "PAPER" if not ENABLE_LIVE_TRADING else "ORDER_PLACED",
        "ENTRY_PRICE": None,
        "SL": None,
        "TARGET": None,
        "ORDER_ID": None
        ...
        for o in orders:
            if o["order_id"] == trade["ORDER_ID"]:
                ...
                if o["status"] == "COMPLETE":
                    trade["ENTRY_PRICE"] = o["average_price"]
                    trade["SL"] = round(o["average_price"] * 0.75, 2)
                    trade["TARGET"] = round(o["average_price"] * 1.50, 2)
...
##SL / TARGET MANAGEMENT (PRICE BASED)

def manage_trade(entry_price, ltp):
...
## ONE TRADE PER INDEX PER DAY (CRITICAL)
...
# =========================================================
# ONE TRADE PER INDEX PER DAY (SAFE & CORRECT)
# =========================================================
...
for index in ["NIFTY", "BANKNIFTY", "SENSEX"]:
    ...
    if index in TRADED_TODAY:
        continue

    row_df = df[df["Symbol"] == index]
    if row_df.empty:
        continue

    row = row_df.iloc[0]

    signal = check_trade_signal(row)
    if not signal:
        continue

    # ✅ Place trade
    place_trade(index, signal, row)

    # ✅ Mark as traded
    TRADED_TODAY.add(index)
