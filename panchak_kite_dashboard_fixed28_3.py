...
# ═══════════════════════════════════════════════════════════════════
# 🔢 AUTO-START NUMEROLOGY MONITOR (background thread)
# Monitors @pankajkummar369 on X and sends TG alerts for new posts.
# Runs in background — dashboard works normally even if monitor fails.
# ═══════════════════════════════════════════════════════════════════
def _start_numerology_monitor():
    """Launch numerology_monitor.py as a background subprocess if not running."""
    import subprocess, os
    monitor_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "numerology_monitor.py")
    if not os.path.exists(monitor_script):
        return  # monitor script not found — silent skip
    pid_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CACHE", "numeo_monitor.pid")
    ...
    try:
        proc = subprocess.Popen(
            ...
            stdout=open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "CACHE", "numeo_monitor.log"), "a"),
...
# Auto-start monitor in background thread (non-blocking)
...
def send_telegram(message: str, dedup_key: str = None, parse_mode: str = "HTML") -> bool:
    ...
    dedup_key: if provided, message is sent only ONCE per 10-min slot per key.
    """
    if not TG_BOT_TOKEN:
        return False
...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    for _, row in df_live.iterrows():
        ...
        if pd.notna(top_high) and ltp > 0 and ltp >= float(top_high):
            if not _panchak_tg_already_sent(sym, "UP"):
                ...
                _panchak_tg_mark_sent(sym, "UP")

        # ── DOWN break ────────────────────────────────────────
        if pd.notna(top_low) and ltp > 0 and ltp <= float(top_low):
            if not _panchak_tg_already_sent(sym, "DOWN"):
                chg_pct = row.get("CHANGE_%", "")
                _entry = {
                    "sym":     sym,
                    "ltp":     ltp,
                ...
                _panchak_tg_mark_sent(sym, "DOWN")
    ...
    # ── Special stocks UP: fire toast + email too ──────────────
    if _special_up and st.session_state.get("special_stock_alerts", True):
        ...
        if not _is_email_disabled("TOP_HIGH"):
    ...
    # ── Special stocks DOWN: fire toast + email too ─────────────
    if _special_down and st.session_state.get("special_stock_alerts", True):
        ...
        if not _is_email_disabled("TOP_LOW"):
...
def _vedic_day_analysis(d_date, hour_ist=9, minute_ist=15):
    ...
    _EXTREME_NAKS = {"Mula","Ardra","Ashlesha","Shatabhisha"}
    ...
    if dig["Jupiter"] == "EXALTED":
        G += 1; signals.append(("Jupiter exalted (Cancer) — strong market protection", "G"))
    ...
    elif net >= -3:overall, intensity = "🔴 BEARISH", "Mild"
...
_ALERT_TOGGLE_DEFAULTS = {
    ...
    "toast_WEEKLY_UP":      True,
    "toast_WEEKLY_DOWN":    True,
    "toast_MONTHLY_UP":     True,
    "toast_MONTHLY_DOWN":   True,
    "toast_EMA20_50":       False,
    "toast_THREE_GREEN_15M":False,
    "toast_SEQ_15M_HIGH":   True,
    "toast_SEQ_15M_LOW":    True,
    "toast_SEQ_1H_HIGH":    True,
    "toast_SEQ_1H_LOW":     True,
    "toast_VOL_SURGE_15M":  True,
    "email_TOP_HIGH":       False,
    "email_TOP_LOW":        False,
    "email_DAILY_UP":       False,
    "email_DAILY_DOWN":     False,
    "email_WEEKLY_UP":      True,
    "email_WEEKLY_DOWN":    True,
    "email_MONTHLY_UP":     True,
    "email_MONTHLY_DOWN":   True,
    "email_EMA20_50":       False,
    "email_THREE_GREEN_15M":False,
    "email_SEQ_15M_HIGH":   True,
    "email_SEQ_15M_LOW":    True,
    "email_SEQ_1H_HIGH":    True,
    "email_SEQ_1H_LOW":     True,
    "email_VOL_SURGE_15M":  True,
    "tg_TOP_HIGH":          False,
    "tg_TOP_LOW":           False,
    "tg_DAILY_UP":          False,
    "tg_DAILY_DOWN":        False,
    ...
    "tg_WEEKLY_DOWN":       True,
    "tg_MONTHLY_UP":        True,
    "tg_MONTHLY_DOWN":      True,
    "tg_EMA20_50":          False,
    "tg_THREE_GREEN_15M":   False,
    "tg_SEQ_15M_HIGH":      True,
    "tg_SEQ_15M_LOW":       True,
    "tg_SEQ_1H_HIGH":       True,
    "tg_SEQ_1H_LOW":        True,
...
# Combines: date root number + live planetary positions
# Research basis:
#   • @pankajkummar369 system (date digit sum → root → planet)
#   • Vedic astro-numerology: each root = a planet ruler
#   • Planetary combinations: Mars+Rahu/Saturn = extreme bearish
#   • Saturn: slow grind down; Rahu: volatile traps; Mars: crashes
#   • Jupiter/Venus: rally energy; Sun: govt/infra strength
#   • Mercury retrograde: IT/banking weak; own sign = bearish for price
#   • Moon/Jupiter trine: Gajakesari = strong rally
#   • 50yr sector analysis: Root 1=govt, 6=FMCG, 9=defence/energy
#   Sources: Stock Lord numerology study, AstroSanhita market analysis,
#            Sheelaa.com financial numerology, @pankajkummar369 system
# ═══════════════════════════════════════════════════════════════════

# Root → ruling planet (standard Vedic numerology)
_NR_PLANET = {1:"Sun",2:"Moon",3:"Jupiter",4:"Rahu",
              5:"Mercury",6:"Venus",7:"Ketu",8:"Saturn",9:"Mars"}

# Root base signal (from research + tweet validation)
# Format: (signal_label, base_score, base_desc, sector_strength, sector_weak)
_NR_BASE = {
    1: ("🟢 BULLISH",   +2, "Sun energy — leadership, new initiatives, govt sector strength",
        "PSU, infra, metals, govt stocks", "banking"),
    2: ("🟡 NEUTRAL",   0,  "Moon energy — emotional swings, consumer/FMCG supported",
        "FMCG, consumer, agri, dairy", "large caps"),
    3: ("🟢 BULLISH",   +2, "Jupiter energy — expansion, banking, finance positive",
        "banking, finance, gold, pharma", "none"),
    4: ("🔴 BEARISH",   -2, "Rahu energy — illusion traps, false rallies, extreme volatility",
        "IT, tech (volatile)", "most sectors"),
    5: ("🟡 VOLATILE",  -1, "Mercury energy — choppy, two-sided; IT/telecom mixed",
        "IT, telecom (short only)", "banking"),
    6: ("🟢 BULLISH",   +2, "Venus energy — rally expected; FMCG, jewellery, silver strong",
        "FMCG, silver, jewellery, banks", "none"),
    7: ("🔴 BEARISH",   -2, "Ketu energy — detachment, losses; market loses direction",
        "nothing clear", "most sectors"),
    8: ("🔴 BEARISH",   -2, "Saturn energy — slowdown, restriction; steady downtrend risk",
        "realty (long term only)", "banking, IT"),
    9: ("🔴 EXTREME",   -4, "Mars energy — aggression, crash risk; war/geopolitical fear",
        "defence, crude oil (inverse)", "Nifty, BankNifty"),
}

# Planetary combination amplifiers — computed dynamically from live positions
# Each returns (score_delta, description) or None if not active
def _nr_combo_amplifiers(vedic: dict, root: int) -> list:
    """
    Dynamically compute planetary combination effects.
    Uses live planetary positions from _vedic_day_analysis output.
    All rules sourced from research + verified tweet predictions.
    """
    amps = []
    mr  = vedic.get("mars_rahu_deg",   99)
    ss  = vedic.get("sun_saturn_deg",  99)
    ms  = vedic.get("moon_saturn_deg", 99)
    mk  = vedic.get("moon_ketu_deg",   99)
    mj  = vedic.get("moon_jupiter_deg",99)
    mra = vedic.get("mars_rahu_deg",   99)  # same as mr
    p   = _NR_PLANET.get(root, "")
    retro = vedic.get("retro", {})
    moon_nak_lord  = vedic.get("moon_nak_lord","")
    saturn_rashi   = vedic.get("saturn_rashi","")
    moon_rashi     = vedic.get("moon_rashi","")
    venus_rashi    = vedic.get("venus_rashi","")
    mercury_rashi  = vedic.get("mercury_rashi","")

    # ── Mars-Rahu combo (most dangerous per research & tweets) ────────
    if mr < 5:
        amps.append((-4, f"⚡ Mars–Rahu EXACT {mr:.1f}° — EXTREME crash energy (research: biggest falls)"))
    elif mr < 10:
        amps.append((-3, f"⚡ Mars–Rahu {mr:.1f}° — very dangerous, crude bullish, Nifty bearish"))
    elif mr < 15:
        amps.append((-2, f"⚡ Mars–Rahu {mr:.1f}° active — bearish amplifier"))

    # ── Root planet amplifier: if root planet is also active ──────────
    if p == "Mars" and mr < 15:
        amps.append((-2, f"🔴 Root {root} (Mars) + Mars–Rahu active = DOUBLE crash signal"))
    if p == "Saturn" and ss < 8:
        amps.append((-2, f"🔴 Root {root} (Saturn) + Sun–Saturn {ss:.1f}° = DOUBLE bearish"))
    if p == "Rahu" and mr < 15:
        amps.append((-2, f"🔴 Root {root} (Rahu) + Mars–Rahu active = Rahu amplified crash"))

    # ── Sun-Saturn combo ──────────────────────────────────────────────
    if ss < 3:
        amps.append((-3, f"☀–♄ Sun–Saturn EXACT {ss:.1f}° — Nifty+Gold+Silver all fall (research confirmed)"))
    elif ss < 8:
        amps.append((-2, f"☀–♄ Sun–Saturn {ss:.1f}° — strong bearish, Gold/Silver weak"))

    # ── Moon-Saturn (Vish Yoga) ───────────────────────────────────────
    if ms < 8:
        amps.append((-2, f"🌙–♄ Moon–Saturn {ms:.1f}° Vish Yoga — psychological pressure, sharp fall risk"))

    # ── Moon-Ketu ─────────────────────────────────────────────────────
    if mk < 12:
        amps.append((-2, f"🌙–Ketu {mk:.1f}° — HDFC/Reliance weak, BankNifty bears more"))

    # ── Gajakesari (Moon-Jupiter) — bullish amplifier ─────────────────
    if mj < 10:
        amps.append((+2, f"🌙–♃ Moon–Jupiter {mj:.1f}° Gajakesari — strong rally amplifier"))
    elif mj < 15:
        amps.append((+1, f"🌙–♃ Moon–Jupiter {mj:.1f}° — mild rally support"))

    # ── Venus in Aries = bottom zone per research ─────────────────────
    if venus_rashi == "Aries":
        amps.append((+3, f"♀ Venus in Aries — BOTTOM ZONE signal; heavy long bias (research: reversal point)"))

    # ── Mercury in own sign = bearish for price per VB Ch50 ───────────
    if mercury_rashi in ("Gemini","Virgo"):
        amps.append((-1, f"☿ Mercury in {mercury_rashi} (own sign) — REDUCES share prices (VB astro research)"))

    # ── Retrograde planet vs root number ─────────────────────────────
    if p in retro:
        amps.append((-1, f"🔄 Root planet {p} is RETROGRADE — root energy weakened/reversed"))
    if "Jupiter" in retro and p in ("Jupiter",):
        amps.append((-1, f"♃ Jupiter retrograde + Root 3 = expansion energy inverted"))
    if "Saturn" in retro and root != 8:
        amps.append((+1, f"♄ Saturn retrograde — restriction easing, mild relief"))

    # ── Root-planet in bearish rashi (Saturn in Aries = 12th from Taurus/market lagna) ──
    if saturn_rashi == "Aries":
        amps.append((-2, f"♄ Saturn in Aries → aspects market's 7th house, destroys trade confidence (research)"))

    # ── Moon in barren signs (Gemini/Virgo/Capricorn/Aquarius) ───────
    if moon_rashi in ("Aquarius","Capricorn"):
        amps.append((-1, f"🌙 Moon in {moon_rashi} (Saturn-ruled) — psychological drag on markets"))

    return amps


def _nr_full_analysis(d, vedic: dict = None) -> dict:
    """
    Full dynamic numerology analysis for a date.
    Optionally takes vedic dict (from _vedic_day_analysis) for planet combos.
    Returns: root, planet, base_signal, combo_amps, final_score, final_signal,
             digit_calc, action, sectors_strong, sectors_weak
    """
    # 1. Date root number
    digit_str = d.strftime("%d%m%Y")
    ...
    calc_str  = "+".join(str(c) for c in digits) + "=" + str(raw_sum)
    if raw_sum != root:
        calc_str += "→" + str(root)

    # 2. Base signal from root
    planet = _NR_PLANET[root]
    sig_label, base_score, base_desc, sec_strong, sec_weak = _NR_BASE[root]

    # 3. Planetary combination amplifiers (dynamic, from live positions)
    amps = []
    amp_score = 0
    if vedic:
        amps = _nr_combo_amplifiers(vedic, root)
        amp_score = sum(a[0] for a in amps)

    # 4. Final composite score
    total = base_score + amp_score

    # 5. Final signal label
    if total >= 4:   final_sig = "🟢 STRONG BULLISH"
    elif total >= 2: final_sig = "🟢 BULLISH"
    elif total >= 1: final_sig = "🟢 MILD BULLISH"
    elif total == 0: final_sig = "🟡 NEUTRAL / MIXED"
    elif total >= -1: final_sig = "🔴 MILD BEARISH"
    elif total >= -3: final_sig = "🔴 BEARISH"
    elif total >= -5: final_sig = "🔴 STRONG BEARISH"
    else:            final_sig = "🔴 EXTREME BEARISH — CRASH RISK"

    # 6. Dynamic action (generated from score + planet combo)
    if total >= 3:
        action = f"✅ BUY dips at open — Root {root} ({planet}) + amplifiers confirm bullish. Target 0.5–1% above open."
    elif total >= 1:
        action = f"✅ LEAN LONG — Root {root} ({planet}). Buy on morning dip after 09:45. Tight stop."
    elif total == 0:
        action = f"⚠️ WAIT — Root {root} ({planet}) neutral. Trade only confirmed breakouts after 10:30."
    elif total >= -2:
        action = f"❌ SELL RALLIES — Root {root} ({planet}) bearish. Short on bounces, target -0.5% to -1%."
    elif total >= -4:
        action = f"❌ STRONG SHORT — Root {root} ({planet}) + {len([a for a in amps if a[0]<0])} negative amplifiers. Short at open."
    else:
        action = f"🚨 DO NOT BUY — Root {root} ({planet}) EXTREME. Sit out or strong short only. Crash risk day."
    ...
    return {
        ...
        "action":        action,
        "sectors_strong":sec_strong,
        "sectors_weak":  sec_weak,
    }
...
if is_market_hours(): notify_browser("🟢 New TOP LIVE_HIGH", new_TOP_HIGH)
...
if is_market_hours(): notify_browser("🔴 New TOP LIVE_LOW", new_TOP_LOW)
...
def _is_toast_disabled(category: str) -> bool:
    ...
    return not st.session_state.get(f"toast_{category}", True)
...
with tabs[1]:
    ...
    _pv_styled = panchak_view.style.apply(_style_panchak, axis=1)
...
with tabs[2]:
    ...
    st.dataframe(TOP_HIGH_df, use_container_width=True, height=7000)
...
with tabs[3]:
    ...
    st.dataframe(TOP_LOW_df, use_container_width=True, height=7000)
...
with tabs[5]:   # 👈 replace with correct index
    ...
    if _15m_low_df.empty:
        ...
        st.dataframe(
            _15m_low_df[_show].style.background_gradient(subset=["BELOW_%"], cmap="Reds"), use_container_width=True
        )
    ...
    if _vol_df.empty:
        ...
        st.dataframe(
            _vol_df[_show].style.background_gradient(subset=["VOL_SURGE_%"], cmap="Blues"), use_container_width=True
        )
    ...
    if _h1_high_df.empty:
        ...
        st.dataframe(
            _h1_high_df[_show].style.background_gradient(subset=["ABOVE_%"], cmap="Greens"), use_container_width=True
        )
    ...
    if _h1_low_df.empty:
        ...
        st.dataframe(
            _h1_low_df[_show].style.background_gradient(subset=["BELOW_%"], cmap="Reds"), use_container_width=True
        )
...
with tabs[8]:
    ...
    if not _orb_range_ready:
        ...
        if not _h1_up.empty:
            ...
            st.dataframe(_h1_up.sort_values("POST_BREAK_MOVE_%", ascending=False), use_container_width=True)
        if not _h1_down.empty:
            st.markdown("#### 🔴 Breakdowns Below 1H Low")
            st.dataframe(_h1_down.sort_values("POST_BREAK_MOVE_%", ascending=False), use_container_width=True)
...
with tabs[17]:
    ...
    def _alert_btn(label, key):
        is_on = st.session_state.get(key, True)
        btn_label = f"{'✅' if is_on else '❌'} {label}"
        btn_style = "🟢 ON" if is_on else "🔴 OFF"
        if st.button(f"{btn_style}  {label}", key=f"btn_{key}"):
            st.session_state[key] = not is_on
            _save_alert_toggles()
            st.rerun()

    # Table: one row per category, 3 toggle buttons per row (Toast | Email | TG)
    ...
    for _cat_label, _cat_key in _ALERT_CATEGORIES_UI:
        ...
        for _col, _prefix, _emoji in [(_c2, "toast", "🍞"), (_c3, "email", "📧"), (_c4, "tg", "✈️")]:
            _key = f"{_prefix}_{_cat_key}"
            _on  = st.session_state.get(_key, True)
            with _col:
                if st.button(
                    "🟢 ON" if _on else "🔴 OFF",
                    key=f"_btn_{_prefix}_{_cat_key}",
                    help=f"{'Disable' if _on else 'Enable'} {_emoji} for {_cat_label}",
                    use_container_width=True,
                ):
                    st.session_state[_key] = not _on
                    _save_alert_toggles()   # ✅ persist to CACHE/alert_toggles.json
                    st.rerun()
    ...
    _sp_on = st.session_state.get("special_stock_alerts", True)
    _sp_col1, _sp_col2 = st.columns([1, 4])
    with _sp_col1:
        if st.button(
            "🟢 ON — All-3 Alerts" if _sp_on else "🔴 OFF — All-3 Alerts",
            key="_btn_special_stock_alerts",
            use_container_width=True,
        ):
            st.session_state["special_stock_alerts"] = not _sp_on
            _save_alert_toggles()   # ✅ persist to CACHE/alert_toggles.json
            st.rerun()
    with _sp_col2:
        st.info(
...
<div style="max-width:700px;margin:auto;background:#111;border-radius:10px;overflow:hidden">
  ...
  <div style="background:#0a0a0a;padding:10px 20px">
    ...
    # All computed live — date root + planetary amplifiers
    # ═══════════════════════════════════════════════════════════════
    st.divider()
    st.subheader("🔢 Numerology Signal — Dynamic Date + Planetary Engine")
    st.caption(
        "Date root number (DD+MM+YYYY digit sum) × live planetary positions = composite signal. "
        "Research basis: @pankajkummar369 system, 50-yr sector study, Vedic astro-numerology. "
        "**Not financial advice.**"
    )

    _nc1, _nc2 = st.columns([1, 3])
    with _nc1:
        _numeo_sel = st.date_input("Date", value=datetime.now(IST).date(),
            min_value=date(2020,1,1), max_value=date(2030,12,31), key="numeo_date_v2")

    # Run full analysis using TODAY's live vedic snapshot
    try:
        _numeo_vedic = _vedic_day_analysis(_numeo_sel)
    except Exception:
        _numeo_vedic = None

    _nr = _nr_full_analysis(_numeo_sel, _numeo_vedic)
    _nr_col = ("#00C851" if "BULLISH" in _nr["final_signal"]
               else "#FF4444" if "BEARISH" in _nr["final_signal"] or "EXTREME" in _nr["final_signal"]
               else "#FFD700")
    _nr_bg  = "#001a00" if "BULLISH" in _nr["final_signal"] else ("#1a0000" if "BEARISH" in _nr["final_signal"] else "#1a1a00")

    with _nc2:
        # Main signal card
        st.markdown(f"""
<div style="background:{_nr_bg};border:1px solid {_nr_col}33;border-radius:10px;padding:14px 18px">
  <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:center">
    <div>
      <div style="font-size:12px;color:#aaa">🔢 {_nr["calc"]} = Root {_nr["root"]}</div>
      <div style="font-size:22px;font-weight:800;color:{_nr_col}">{_nr["final_signal"]}</div>
      <div style="font-size:14px;color:{_nr_col};opacity:0.8">Ruled by: <b>{_nr["planet"]}</b></div>
    </div>
    <div style="flex:1;min-width:240px">
      <div style="font-size:13px;color:#ccc">{_nr["base_desc"]}</div>
      <div style="font-size:13px;color:{_nr_col};margin-top:6px;font-weight:600">{_nr["action"]}</div>
    </div>
    <div style="min-width:160px;font-size:12px;color:#aaa">
      <div>📈 Strong: <span style="color:#88ff88">{_nr["sectors_strong"]}</span></div>
      <div>📉 Weak: <span style="color:#ff8888">{_nr["sectors_weak"]}</span></div>
    </div>
  </div>
</div>""", unsafe_allow_html=True)

    # Planetary amplifiers (all dynamic, computed live)
    if _nr["amplifiers"]:
        st.markdown("**⚡ Active Planetary Amplifiers** (live position-based):")
        _amp_cols = st.columns(2)
        for _ai, (_ascore, _adesc) in enumerate(_nr["amplifiers"]):
            _acol = "#00ff88" if _ascore > 0 else "#ff6666"
            _aicon = "🟢" if _ascore > 0 else "🔴"
            _amp_cols[_ai % 2].markdown(
                f'<div style="font-size:12px;color:{_acol};padding:2px 0">'
                f'{_aicon} ({_ascore:+d}) {_adesc}</div>',
                unsafe_allow_html=True
            )

    # Score breakdown
    _nb1, _nb2, _nb3, _nb4 = st.columns(4)
    _nb1.metric("Root Number",    f"{_nr['root']} ({_nr['planet']})")
    _nb2.metric("Base Score",     f"{_nr['base_score']:+d}")
    _nb3.metric("Amplifiers",     f"{_nr['amp_score']:+d}" if _nr["amplifiers"] else "0 (no live data)")
    _nb4.metric("Final Score",    f"{_nr['total_score']:+d}")

    st.divider()

    # ── 10-day dynamic forecast ───────────────────────────────────
    st.markdown("##### 📅 Next 10 Trading Days — Dynamic Numerology Forecast")
    _numeo_rows = []
    _base_date  = datetime.now(IST).date()
    _nfound     = 0
    for _ndi in range(1, 35):
        _nfd = _base_date + timedelta(days=_ndi)
        if _nfd.weekday() >= 5 or _nfd in NSE_HOLIDAYS:
            continue
        # Use lightweight analysis (no vedic for forecast speed; base root only)
        _nfr = _nr_full_analysis(_nfd)  # no vedic arg = base signal only
        _numeo_rows.append({
            "Date":          _nfd.strftime("%a %d %b"),
            "Root":          _nfr["root"],
            "Planet":        _nfr["planet"],
            "Signal":        _nfr["final_signal"],
            "Calc":          _nfr["calc"],
            "Strong Sectors":_nfr["sectors_strong"],
            "Action":        _nfr["action"][:60] + "...",
        })
        _nfound += 1
        if _nfound >= 10:
            break
    ...
    if _numeo_rows:
        ...
        def _nr_root_style(v):
            n = int(v)
            if n in (1,3,6): return "color:#00ff88;font-weight:700"
            if n == 9:       return "color:#ff2222;font-weight:800"
            if n in (4,7,8): return "color:#ff8888;font-weight:600"
            return "color:#ffdd00"
        st.dataframe(
            _ndf.style
                .map(_nr_sig_style,  subset=["Signal"])
                .map(_nr_root_style, subset=["Root"])
                .set_properties(**{"font-size":"12px"}),
            use_container_width=True, height=400
        )

    # ── Combined Numerology + Vedic score ────────────────────────
    if _numeo_vedic:
        _vs = _numeo_vedic.get("net_score", 0)
        _ts = _nr["total_score"]
        _combined = _ts + _vs
        if _combined >= 5:   _cv, _cc = "🟢 VERY STRONG BULLISH — Double confirmed", "#00C851"
        elif _combined >= 2: _cv, _cc = "🟢 BULLISH — Aligned", "#00C851"
        elif _combined >= 0: _cv, _cc = "🟡 NEUTRAL — Mixed signals", "#FFD700"
        elif _combined >= -3:_cv, _cc = "🔴 BEARISH — Aligned", "#FF4444"
        else:                _cv, _cc = "🔴 EXTREME BEARISH — Both systems confirm crash risk", "#FF4444"
        st.markdown(f"""
<div style="background:#111;border-radius:8px;padding:10px 16px;margin-top:6px">
  <div style="font-size:13px;color:#aaa">🔗 Numerology ({_ts:+d}) + Vedic Astro ({_vs:+d}) = Combined</div>
  <div style="font-size:18px;font-weight:700;color:{_cc}">{_cv}</div>
  <div style="font-size:12px;color:#888;margin-top:4px">
    Root {_nr["root"]} ({_nr["planet"]}) · Vedic {_vs:+d} pts · Combined {_combined:+d}
  </div>
</div>""", unsafe_allow_html=True)
