...
def _fetch(key, url):
    try:
        ...
        with _lock:
            _cache[key]          = data   # update only on success
            _cache[f"{key}_err"] = None
    except URLError as e:
        with _lock:
            # Keep last good data — only mark error, don't wipe cache
            _cache[f"{key}_err"] = f"Engine offline or not yet started ({e.reason})"
    except (OSError, ConnectionRefusedError, ConnectionResetError, TimeoutError) as e:
        with _lock:
            _cache[f"{key}_err"] = f"Connection error: {e}"
...
def _poll_loop():
    while True:
        ...
        t1.start(); t2.start()
        t1.join(timeout=8)   # don't block forever if engine hangs
        t2.join(timeout=8)
...
function renderEngine(prefix, data, errMsg) {
  ...
  // No data at all yet (first startup)
  if (!data && !errMsg) {
  ...
  // Engine erroring but we have stale cached data — show data with a warning banner
  if (!data && errMsg) {
    sumEl.innerHTML = buildOffline(errMsg);
    posEl.innerHTML = ''; trdEl.innerHTML = ''; logEl.innerHTML = '';
    return;
  }
  // Show stale warning banner above data if errMsg present but cached data exists
  const staleBar = errMsg
    ? `<div style="background:#2a1a0e;border:1px solid #8b5e1a;border-radius:6px;padding:7px 12px;font-size:12px;color:#e3a04f;margin-bottom:10px;">⚠ Engine unreachable — showing last known data</div>`
    : '';
  const s = data.summary || {};
  document.getElementById('mode-badge').textContent = s.mode || 'PAPER';
  sumEl.innerHTML = staleBar + buildSummary(s);
...
function renderSide(id, pnlId, data, errMsg) {
  ...
  if (!data && !errMsg) {
    el.innerHTML = `<div style="color:var(--muted);font-size:12px;">Waiting for first data…</div>`;
    pnlEl.innerHTML = '—';
    return;
  }
  if (!data && errMsg) {
  ...
  // summary mini bar
  const staleBanner = errMsg
    ? `<div style="font-size:11px;color:#e3a04f;margin-bottom:6px;">⚠ Showing last known data</div>`
    : '';
  el.innerHTML = `
    ${staleBanner}
