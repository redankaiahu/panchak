...
def _fire_heatmap_confluence_alert():
    ...
    try:
        ...
        # Sensex weighted score
        _snx_bull_wt = _snx_bear_wt = _snx_impact = 0.0
        ...
        _snx_top_gainers = _snx_top_losers = []
    ...
    _tg_msg = (
        ...
        f"🌙 <b>Astro:</b> {_as_sig} | 🔑 <b>KP:</b> {_kp_sub}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
...
with tabs[0]:
    ...
    from astro_logic import get_kp_reversals
    from astro_engine import get_gann_square_9

    with st.expander("🔮 MAGIC LINES & REVERSALS (High Conviction)", expanded=True):
        col_m1, col_m2 = st.columns(2)
        with col_m1:
            st.markdown("#### 🧱 Gann Magic Lines (Nifty Targets)")
            # Using a static reference for Nifty, in a real scenario would use live LTP
            nifty_ref = 24358.60 
            gann_levels = get_gann_square_9(nifty_ref)
            st.write(f"**Base Price:** {nifty_ref}")
            m_rows = []
...
<div class="mom-wrap">
  ...
  <div style="margin-top:5px;font-size:11px;color:{sig_col}">
    ...
    # ════════════════ INDEX CARDS ════════════════
    _ni_row  = _indices_df[_indices_df["Index"] == "NIFTY 50"] if "Index" in _indices_df.columns else pd.DataFrame()
    _bnk_row = _indices_df[_indices_df["Index"] == "BANK NIFTY"] if "Index" in _indices_df.columns else pd.DataFrame()
...
with tabs[20]:
    ...
    c1, c2, c3 = st.columns(3)
    c1.metric("♃ Jupiter", "Exalted in Cancer", "Bullish Banks")
    c2.metric("♀ Venus", "Exalted in Pisces", "Bullish FMCG")
    c3.metric("♄ Saturn", "Own Sign Aquarius", "Stable Energy")
    ...
    from astro_logic import SECTOR_SIGNIFICATORS
    from astro_engine import get_dignities
