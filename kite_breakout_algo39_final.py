    ...
    while True:
        ...
        if TRADING_MODE == "LIVE":
            try:
                ...
                break   # stop adding more legs if one LIVE order fails
...
def _cascade_cancel_live_pyramid_legs(base_symbol):
    """
    ...
    for leg_key, leg_pos in list(paper_positions.items()):
        ...
        if leg_pos.get("status") != "OPEN":
            continue
        _cancel_live_order_safe(leg_pos.get("_live_sl_oid"),  f"PYR SL  {leg_key}")
        _cancel_live_order_safe(leg_pos.get("_live_tgt_oid"), f"PYR TGT {leg_key}")
        leg_pos["status"] = "CLOSED"
        print(Fore.YELLOW + f"🔺 PYRAMID LEG CANCELLED (cascade) | {leg_key}" + ts())
        write_log(f"PYRAMID CASCADE CANCEL {leg_key}")
...
def oco_monitor(symbol):
    ...
    MAX_NOT_FOUND = 5   # allow a few polls for Kite order list to propagate
        ...
        try:
            ...
            if sl_order is None:
                ...
                if sl_not_found_count <= MAX_NOT_FOUND:
                    ...
                    continue
                print(Fore.RED +
                      f"⚠️  OCO: SL permanently missing for {symbol} — stopping monitor" + ts())
            ...
            if sl_order["status"] == "COMPLETE":
                ...
                _cascade_cancel_live_pyramid_legs(symbol)
                break
            ...
            if tgt_id:
                ...
                if tgt_order is None:
                    ...
                    if tgt_not_found_count <= MAX_NOT_FOUND:
                        ...
                        continue
                    print(Fore.RED +
                          f"⚠️  OCO: target permanently missing for {symbol} — stopping monitor" + ts())
                    write_log(f"OCO target permanently missing {symbol}")
                    break
                tgt_not_found_count = 0
        ...
        if TRADING_MODE == "LIVE":
            ...
            def _sq_cancel_all_orders():
                ...
                try:
                    ...
                    # ── Cancel pyramid leg SL and target orders ───────────
                    for leg_key, leg_pos in list(paper_positions.items()):
                ...
                try:
                    ...
                    for pos in positions:
                        ...
                        try:
                            ...
                            print(Fore.YELLOW + msg + ts())
                            write_log(msg)
                            # Mark local tracking as closed
                            if sym in paper_positions:
                                paper_positions[sym]["status"] = "CLOSED"
