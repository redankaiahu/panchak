...
def _build_html_email(title, symbols_with_ltp, df_table=None, color="#1a237e", icon="🚨"):
    ...
    symbols_with_ltp: list of (symbol, ltp) tuples for the banner
    df_table: optional DataFrame to render as table (new entries)
    """
...
def notify_all(category, title, symbols, ltp_map=None):
    ...
    if email_symbols and is_email_allowed():
        ...
        def _send_bg(subj=subject, ttl=title, swl=syms_with_ltp,
            ...
            html = _build_html_email(ttl, swl, df_table=ndf, color=col, icon=ic)
...
def fetch_nifty_oi_intelligence():
    ...
    try:
        ...
        # Direction logic
        # Call wall > spot → resistance; Put support > spot → floor
        call_walls_above = [s for s in ce_oi_map if s > spot]
        put_floors_below  = [s for s in pe_oi_map if s < spot]

        nearest_call_wall = min(call_walls_above, key=lambda x: x - spot) if call_walls_above else None
        nearest_put_floor = min(put_floors_below, key=lambda x: spot - x) if put_floors_below else None
        ...
        if pcr >= 1.3:
            direction = "🟢 BULLISH"
            direction_reason = f"PCR={pcr} (Strong Put Support)"
        elif pcr <= 0.7:
            direction = "🔴 BEARISH"
            direction_reason = f"PCR={pcr} (Strong Call Pressure)"
        elif strongest_ce > spot and dist_to_call < dist_to_put:
            direction = "🔴 BEARISH BIAS"
            direction_reason = f"Call Wall at {strongest_ce} is closer"
        elif strongest_pe < spot and dist_to_put < dist_to_call:
            direction = "🟢 BULLISH BIAS"
            direction_reason = f"Put Floor at {strongest_pe} is closer"
        else:
            direction = "⚠️ SIDEWAYS/NEUTRAL"
            direction_reason = f"PCR={pcr}, balanced OI"

        # Spot vs Max Pain direction
        ...
        # ── 13. Advice ──────────────────────────────────────────
        if "BEARISH" in direction and not "BIAS" in direction:
            advice = "⚠️ Strong BEARISH. Avoid buying calls. Consider PUT buying."
            setup = f"Buy {atm} PE | SL above {strongest_ce}"
        elif "BULLISH" in direction and not "BIAS" in direction:
            advice = "✅ Strong BULLISH. Avoid selling calls aggressively. Consider CALL buying."
            setup = f"Buy {atm} CE | SL below {strongest_pe}"
        elif "BEARISH BIAS" in direction:
            advice = "Mild bearish. Sell rallies, avoid aggressive longs."
            setup = f"Sell on rise near {strongest_ce} CE wall"
        elif "BULLISH BIAS" in direction:
            advice = "Mild bullish. Buy dips, avoid aggressive shorts."
            setup = f"Buy dips near {strongest_pe} PE floor"
        else:
            advice = "Range-bound. Trade breakouts only. Avoid directional bets."
            setup = f"Wait for breakout above {strongest_ce} or below {strongest_pe}"
...
def build_15m_tracker(raw=None):
    ...
    # 15-min slots
    raw["slot15"] = raw["datetime"].dt.floor("15T").dt.strftime("%H:%M")
...
def build_1h_tracker(raw=None):
    ...
    raw["slot60"] = (
        ...
        .dt.floor("60T") + pd.Timedelta(minutes=15)
    ).dt.strftime("%H:%M")
...
with tabs[5]:   # 👈 replace with correct index
    ...
    if oi_intel is None:
        ...
        # ── Pattern detection — fully from live data ──────────
        chain_rows_raw = oi_intel.get("chain_rows", [])
        pattern_label  = ""
        pattern_detail = ""

        # Thresholds derived from actual data magnitude
        # ce_building: near-ATM CE OI added ≥ 5% of near-ATM CE total OI
        # pe_crumbling: near-ATM PE OI dropped ≥ 3% (near_pe_drop_pct is negative)
        ce_building  = near_ce_pct >= 5
        pe_crumbling = near_pe_drop_pct <= -3
        pe_building  = near_pe_pct >= 5
        ce_crumbling = oi_intel.get("most_dropped_ce", 0) != 0 and near_ce_pct <= -3

        if ce_building and pe_crumbling:
            pattern_label = "🚨 WATERFALL TRAP: CALL WALL BUILDING + PUT SUPPORT CRUMBLING"
            ce_detail = (f"Call writing at {shifting_ce} CE — "
                         f"OI added {shifting_ce_add:,} ({shifting_ce_pct}% of strike OI in session), "
                         f"premium ₹{shifting_ce_ltp:.1f}")
            pe_detail = (f"Put support crumbling at {most_dropped_pe} PE — "
                         f"OI dropped {abs(most_dropped_pe_val):,} ({abs(most_dropped_pe_pct)}% of strike OI in session), "
                         f"premium ₹{most_dropped_pe_ltp:.1f}")
            pattern_detail = f"{ce_detail}. {pe_detail}."

        elif pe_building and ce_crumbling:
            pattern_label = "🚀 SHORT SQUEEZE TRAP: PUT WALL BUILDING + CALL RESISTANCE CRUMBLING"
            pe_detail = (f"Put writing at {shifting_pe} PE — "
                         f"OI added {shifting_pe_add:,} ({shifting_pe_pct}% of strike OI in session), "
                         f"premium ₹{shifting_pe_ltp:.1f}")
            ce_detail = (f"Call resistance crumbling at {most_dropped_ce} CE — "
                         f"OI dropped {abs(near_ce_added):,} ({abs(near_ce_pct):.1f}% in session)")
            pattern_detail = f"{pe_detail}. {ce_detail}."

        elif ce_building and not pe_crumbling:
            pattern_label  = "⚠️ CALL WALL BUILDING — Put support still intact"
            pattern_detail = (f"Call writing at {shifting_ce} CE — OI added {shifting_ce_add:,} "
                              f"({shifting_ce_pct}% of strike OI), premium ₹{shifting_ce_ltp:.1f}. "
                              f"Put floor at {s_pe} holding ({pe_oi_val:,} OI).")

        elif pe_building and not ce_crumbling:
            pattern_label  = "✅ PUT FLOOR BUILDING — Call resistance still intact"
            pattern_detail = (f"Put writing at {shifting_pe} PE — OI added {shifting_pe_add:,} "
                              f"({shifting_pe_pct}% of strike OI), premium ₹{shifting_pe_ltp:.1f}. "
                              f"Call wall at {s_ce} holding ({ce_oi_val:,} OI).")

        elif "BEARISH" in direction:
            pattern_label  = "🔴 BEARISH REGIME — Call pressure dominant"
            pattern_detail = (f"Strongest call wall: {s_ce} CE ({ce_oi_val:,} OI). "
                              f"Nearest wall {d_call} pts above spot. PCR={pcr} (bearish).")

        elif "BULLISH" in direction:
            pattern_label  = "🟢 BULLISH REGIME — Put support dominant"
            pattern_detail = (f"Strongest put floor: {s_pe} PE ({pe_oi_val:,} OI). "
                              f"Nearest floor {d_put} pts below spot. PCR={pcr} (bullish).")

        else:
            pattern_label  = "📊 OI REGIME: NEUTRAL / BALANCED"
            pattern_detail = (f"No dominant OI shift near ATM. "
                              f"CE OI added: {near_ce_pct:+.1f}% | PE OI change: {near_pe_pct:+.1f}% | "
                              f"PE drop: {near_pe_drop_pct:+.1f}% | PCR={pcr}")

        # ── Compute trade setup — fully from live chain data ──
        # BEARISH: SL = nearest call wall above spot + step (buffer)
        #          Target = strongest put floor below spot - step
        # BULLISH: SL = nearest put floor below spot - step
        #          Target = nearest call wall above spot + step

        if "WATERFALL" in pattern_label or "BEARISH" in direction:
            trade_bias   = "BEARISH"
            entry_strike = atm
            # SL: nearest call wall above spot + 1 step buffer
            # real_sl_call is nearest_call_wall (dynamic from live chain)
            sl_spot      = int(real_sl_call) + step
            # Target: strongest put floor below spot - 1 step buffer
            # Use 2nd put floor if nearest is too close (< 50 pts away)
            if isinstance(real_tgt_put, (int, float)) and (spot - real_tgt_put) < step:
                # nearest floor too close — use next floor down
                floors = oi_intel.get("put_floors_below", [])
                target_spot = int(floors[1][0]) - step if len(floors) > 1 else int(real_tgt_put) - step * 2
            else:
                target_spot  = int(real_tgt_put) - step
            risk_pts     = max(1, round(abs(sl_spot - spot), 0))
            reward_pts   = max(1, round(abs(spot - target_spot), 0))
            setup_line   = f"Buy {entry_strike} PE"
            sl_line      = f"Above {sl_spot} ({int(real_sl_call)} call wall)"
            target_line  = str(int(target_spot))
            bias_color   = "#b71c1c"
            bias_icon    = "🔴"
            advice_text  = "Strong BEARISH setup. Do NOT buy call options here."

        elif "SHORT SQUEEZE" in pattern_label or "BULLISH" in direction:
            trade_bias   = "BULLISH"
            entry_strike = atm
            # SL: nearest put floor below spot - 1 step buffer
            sl_spot      = int(real_tgt_put) - step
            # Target: nearest call wall above spot + 1 step buffer
            if isinstance(real_sl_call, (int, float)) and (real_sl_call - spot) < step:
                walls = oi_intel.get("call_walls_above", [])
                target_spot = int(walls[1][0]) + step if len(walls) > 1 else int(real_sl_call) + step * 2
            else:
                target_spot  = int(real_sl_call) + step
            risk_pts     = max(1, round(abs(spot - sl_spot), 0))
            reward_pts   = max(1, round(abs(target_spot - spot), 0))
            setup_line   = f"Buy {entry_strike} CE"
            sl_line      = f"Below {sl_spot} ({int(real_tgt_put)} put floor)"
            target_line  = str(int(target_spot))
            bias_color   = "#1b5e20"
            bias_icon    = "🟢"
            advice_text  = "Strong BULLISH setup. Do NOT sell call options aggressively."
            ...
            risk_pts     = 0
            reward_pts   = 0
            # Show the live range between put floor and call wall
            range_low  = int(real_tgt_put) if isinstance(real_tgt_put, (int,float)) else atm - step*2
            range_high = int(real_sl_call) if isinstance(real_sl_call, (int,float)) else atm + step*2
            setup_line   = f"WAIT — No clear edge. Live range: {range_low} – {range_high}"
            sl_line      = "—"
            target_line  = "—"
            bias_color   = "#e65100"
            bias_icon    = "⚠️"
...
with tabs[17]:
    ...
    styled_df = future_df.style.map(color_signal, subset=["signal"])
