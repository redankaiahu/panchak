...
# =========================================================
# EMA20–EMA50 + TOP HIGH / TOP LOW (SINGLE SOURCE OF TRUTH)
# =========================================================
...
#ohlc_full = pd.read_csv(DAILY_FILE)
#ohlc_full["date"] = pd.to_datetime(ohlc_full["date"])
# ================= LOAD FULL OHLC (60D) FOR PATTERN SETUPS =================
ohlc_full = pd.read_csv(OHLC_FILE)

ohlc_full["date"] = pd.to_datetime(ohlc_full["date"])
ohlc_full = ohlc_full.sort_values(["Symbol", "date"])
...
for sym, g in ohlc_full.groupby("Symbol"):
    ...
    last4 = g.iloc[-5:-1]   # previous 4 completed days
    today = g.iloc[-1]

    # 4 red candles
    if not all(last4["close"] < last4["open"]):
        continue

    prev_day = g.iloc[-2]

    live = df[df.Symbol == sym]
    if live.empty:
        continue
    ...
    if (
        live["LIVE_HIGH"] >= prev_day["close"] * 1.025 and   # 2.5% breakout
        live["LTP"] < prev_day["close"] * 1.025 and          # failed follow-through
        live["OPEN"] < prev_day["high"]                      # open below prev high
    ):
        pattern2_rows.append({
            "Symbol": sym,
            "Prev Close": prev_day["close"],
            "Prev High": prev_day["high"],
            "OPEN": live["OPEN"],
            "LIVE_HIGH": live["LIVE_HIGH"],
            "LTP": live["LTP"],
            "Gap %": round((live["LIVE_HIGH"] - prev_day["close"]) / prev_day["close"] * 100, 2)
        })
...
###########     SETUP 2: FAKE 2.5% BREAKOUT (TRAP MOVE) ##################################
fake_break_rows = []
...
for _, row in df.iterrows():
    ...
    if (
        row["LIVE_HIGH"] >= yc * 1.025 and
        row["LTP"] < yc * 1.025 and
        row["OPEN"] < row["YEST_HIGH"]
    ):
        fake_break_rows.append({
            "Symbol": row["Symbol"],
            "YEST_CLOSE": yc,
            "LIVE_HIGH": row["LIVE_HIGH"],
            "LIVE_OPEN": row["OPEN"],
            "LTP": row["LTP"],
            "FAIL_%": round((row["LIVE_HIGH"] - row["LTP"]) / yc * 100, 2)
        })

fake_break_df = pd.DataFrame(fake_break_rows)
...
for sym, g in intraday_15m_df.groupby("Symbol"):
...
inside_break_df = pd.DataFrame(inside_break_rows)
...
with tabs[3]:
    st.markdown(
        '<div class="section-yellow"><b>🟡 NEAR – Watch Zone</b></div>',
        unsafe_allow_html=True
    )
    ...
    if near_base.empty:
        ...
        # distances
        near_base["DIST_HIGH"] = (near_base["TOP_HIGH"] - near_base["LTP"]).round(2)
        near_base["DIST_LOW"]  = (near_base["LTP"] - near_base["TOP_LOW"]).round(2)

        # split based on *nearest*
        near_buy_df = near_base[
            near_base["DIST_HIGH"] <= near_base["DIST_LOW"]
        ].copy()
        ...
        near_buy_df["NEAR"]  = "🟢 ↑ " + near_buy_df["DIST_HIGH"].astype(str)
        near_sell_df["NEAR"] = "🔴 ↓ " + near_sell_df["DIST_LOW"].astype(str)

        # sorting: closest first
        near_buy_df  = near_buy_df.sort_values("DIST_HIGH")
        near_sell_df = near_sell_df.sort_values("DIST_LOW")
...
with tabs[11]:  # 4 BAR
    ...
    ###############################
    st.markdown("### ⚠️ Fake 2.5% Breakout (Bull Trap)")

    if fake_break_df.empty:
        st.info("No fake breakouts today")
    else:
        st.dataframe(fake_break_df, width="stretch")
    ##############################################
    st.markdown("### 🔔 15-Min Inside Range Break")

    if inside_break_df.empty:
        st.info("No 15-min inside breakouts yet")
    else:
        st.dataframe(inside_break_df, width="stretch")
