...
_ema_missing_warned = set()  # symbols warned once about missing EMA cache — prevents silent strategy skips
...
# multiple threads. Without a file-level lock, concurrent writes produce
# truncated/corrupt CSVs.
_save_cache_lock = threading.Lock()
# (symbol, side) pairs already logged once — suppresses repeated NIFTY BIAS BLOCK prints
...
def place_trade(symbol, side, ltp, tick, condition_name, extra_info=None, custom_sl=None, tsl_only=False, tsl_step=None, bypass_nifty_filter=False):
    # bypass_nifty_filter: ignored in this file (no NIFTY bias system), accepted for API compatibility with S9/S10.
    ...
    if TRADING_MODE == "LIVE":
        ...
        try:
            ...
            # Write local position tracking using actual fill price
            paper_positions[symbol] = {
                "side":        side,
            ...
            if profit_lock_armed:
                ...
                _be_sl   = round(round(ltp / _tick_ne) * _tick_ne, 10)
                try:
                    kite.modify_order(
                        variety=kite.VARIETY_REGULAR,
...
def check_and_add_pyramid(symbol, ltp):
    ...
    while True:
        ...
        _base_ltp_now = ltp   # ltp already reflects current price
        ...
        if _ema15m is not None:
            ...
            if not _ema_ok:
                ...
                break   # price not aligned with 15m trend — skip this leg
        ...
        if TRADING_MODE == "LIVE":
            try:
                ...
                # Fetch actual pyramid leg fill price (with retry) so SL/target
                # are placed at real fill, not signal trigger price.
                    ...
                    try:
                        ...
                        if _pyr_filled and _pyr_filled.get("average_price"):
                            ...
                            if pyr_actual_fill != leg_entry:
                                ...
                                def _prt(p): return round(round(p / _pyr_ts) * _pyr_ts, 10)
                                leg_sl     = _prt(pyr_actual_fill * (1 - PYRAMID_SL_PCT / 100)) if side == "BUY" else _prt(pyr_actual_fill * (1 + PYRAMID_SL_PCT / 100))
                                if base_target is not None:
    ...
    for symbol in SYMBOLS:
        ...
        if orb and f15:
            ...
            rows.append({
                ...
                # O==H flag now checks the 10-min candle (F10) for S9 accuracy.
                # F5 o_eq flags kept for backward compatibility with existing CSVs.
                "f5_o_eq_h": (
                    True if (
        ...
        try:
            ...
            for symbol in SYMBOLS:
                ...
                try:
                    ...
                    # first5_data is empty. Seed it here so S10 works post-restart
...
def paper_position_manager():
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            if trading_enabled:
                # Hard rupee limit (HARD_LOSS_LIMIT) — checked first, exact
...
# day_high = highest price traded 09:15–09:24
...
def strategy_open_high_breakdown(ctx):
    ...
    # At 09:25: 10min/375min * burst is still too small for any vol threshold.
    # Price structure is the qualification; volume confirmation is not applicable.
    ...
    # BUG FIX: using round(x, 2) is wrong for stocks with tick_size != 0.05;
    # Kite rejects SL-M orders where price is not a multiple of tick_size.
    ...
    # latest_lows is updated BEFORE strategy() fires so on the breakdown tick
    # stale_pct ≈ slippage_pct. Allow up to MAX_SLIPPAGE_PCT (0.3%).
    # A re-test (bounced 1%+ below earlier then recovered) → stale_pct > 0.3% → blocked.
...
def strategy_open_high_sell_at_919(ctx):
    ...
    # ── SL = day_open + 0.5% (tighter than flat +1 rupee for high-priced stocks) ──
    # A fixed ₹1 SL is too tight for stocks above ₹500 and too loose below ₹100.
    # Use 0.5% of day_open, floored at ₹1 above open.
    sl_level = round(max(day_open + 1.0, day_open * 1.005), 2)
...
#            SL      : 0.3% above day open/high snapped to tick | Exit: TSL-only
#
...
def position_manager():
    ...
    while True:
        ...
        try:
            ...
            # When gross winning (sum of all profitable closed trades) crosses
            # GROSS_WIN_TARGET, immediately square off all positions and halt.
            global gross_win_target_hit
            if (GROSS_WIN_TARGET > 0 and trading_enabled
                    and not gross_win_target_hit):
                try:
                    _gwt_closed = kite.trades()  # broker realised trades today
                    _gross_win_live = sum(
                    ...
                    # Fallback: use paper_positions closed PnL cache
                    _gross_win_live = sum(
                        p["net_pnl"] for p in paper_positions.values()
                        if p.get("status") == "CLOSED" and p.get("net_pnl", 0) > 0
                    )
...
# ==========================================
# save_15m_candles_to_disk(): called at squareoff. Writes today's completed
#   15m OHLC candles for every symbol to CANDLE15_DIR/YYYY-MM-DD.csv.
#   Format: symbol, slot, open, high, low, close
#
# load_15m_candles_from_disk(): called at startup. Reads yesterday's file,
#   seeds ema15m_closes with the last 20 closes per symbol so EMA7 is
#   accurate from the very first pyramid trigger of the day.
# ==========================================

def _candle15_filepath(date_obj):
    """Return path to the 15m candle file for a given date."""
    return os.path.join(CANDLE15_DIR, f"candles_15m_{date_obj.strftime('%Y-%m-%d')}.csv")


def save_15m_candles_to_disk():
    """Save today's completed 15m OHLC candles to disk.
    Called at squareoff so tomorrow's session can seed the EMA buffer.
    Each row: symbol, slot (minutes since midnight), open, high, low, close.
    """
    ...
    msg = (f"💾 15m candles saved → {filepath} "
           f"({n_syms} symbols, {n_rows} candles)")
...
def auto_square_off():
    ...
    while True:
        ...
        save_15m_candles_to_disk()   # persist today's 15m OHLC for tomorrow's EMA seed
        daily_summary()
        break
...
def run_market_replay_proper(replay_date):
    ...
    # Without reset, symbols blocked on day N can never enter on day N+1.
    _nifty_bias_blocked.clear()
    ...
    # is silently dropped from scan logs. Reset all three dicts together.
    with _scan_lock:
    ...
    for current_time in all_times:
        ...
        for symbol, df in symbol_minute_data.items():
            ...
            # Freeze at 09:25
            if candle_time_str_pre >= "09:25" and symbol in first10_data:
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if not ws_ready:
            ...
            # ── First-candle builds during warmup (09:15–09:24) ─────────────
            # build_context() returns None before ENTRY_START_TIME, so
            # _build_first10 (S9) and _build_first5 (S10) inside strategy()
            # never run during the early window when warmup blocks strategy().
            # We call them here directly before the warmup gate fires.
...
class _DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            pass   # browser closed/refreshed before response completed — harmless, suppress
...
# ==========================================
# Runs every PNL_MONITOR_INTERVAL seconds (default 60 s) in BOTH PAPER and
# LIVE mode.  Acts as a safety net independent of oco_monitor / position_manager:
#   • HARD_LOSS_LIMIT  → close ALL open positions + halt trading
#   • GROSS_WIN_TARGET → close ALL open positions + halt trading
# Both checks mirror the logic in paper_position_manager / position_manager
# but run on a fixed clock so they fire even when no new tick arrives.
# ==========================================
def _pnl_guard_thread():
    ...
    def _close_all_paper(reason):
        """Close every open paper position at current LTP and halt."""
        global daily_pnl
        for _gs, _gp in list(paper_positions.items()):
            if _gp.get("status") != "OPEN":
                continue
    ...
    def _close_all_live(reason):
        """Cancel all open orders then market-exit every open LIVE position."""
        try:
            # ── Step 1: Cancel all open SL-M/target orders first ──────────
            # Prevents dangling orders creating reverse positions after exit.
            try:
            ...
            # ── Step 2: Market-exit open net positions (NOT "day") ─────────
            # "net" = only currently open positions with non-zero quantity.
            # "day" includes already-closed positions (qty=0) causing phantom orders.
    ...
    _t.sleep(120)   # allow warmup before first check
    while True:
        try:
            ...
            # Compute total PnL (closed + unrealised open)
            with _daily_pnl_lock:
            ...
            # ── Profit Lock arm check (independent of position manager) ──
...
if __name__ == "__main__":
    ...
    load_15m_candles_from_disk()   # seed 15m EMA7 from yesterday's candle file
