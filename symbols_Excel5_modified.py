
import os
import pandas as pd
import numpy as np
from datetime import datetime, timedelta
from alice_blue import AliceBlue
from pya3 import *

alice = Aliceblue(user_id='1660575', api_key='WaXdEOLI4VWzn5bUfDpmBMDSmBhN91v7nQjEx8WPBV4iQQHB2MIE6XWFuTNMxFcstK6z4QTVVgksD24KEUi0fZhuLAJYCVXAuqWluNdSke02Ssz9a37u9nyf2aiAk3eB')
session_id = alice.get_session_id()
if not session_id:
    print("Failed to establish session.")
    exit()

symbols = [
    'NIFTY', 'BANKNIFTY', 'RELIANCE', 'INFY', 'HCLTECH', 'TVSMOTOR',
    'BHARATFORG', 'JUBLFOOD', 'LAURUSLABS', 'SUNPHARMA', 'TATACONSUM',
    'COFORGE', 'ASIANPAINT', 'MUTHOOTFIN', 'CHOLAFIN', 'BSE', 'GRASIM',
    'ACC', 'ADANIENT', 'BHARTIARTL', 'BIOCON', 'BRITANNIA', 'DIVISLAB',
    'ESCORTS', 'JSWSTEEL', 'M&M', 'PAGEIND', 'SHREECEM', 'BOSCHLTD',
    'DIXON', 'MARUTI', 'ULTRACEMCO', 'APOLLOHOSP', 'MCX', 'POLYCAB',
    'PERSISTENT', 'TRENT', 'EICHERMOT', 'HAL', 'TIINDIA', 'SIEMENS',
    'GAIL', 'NATIONALUM', 'TATASTEEL', 'MOTHERSON', 'JSL', 'VEDL', 'VBL',
    'GRANULES', 'LICHSGFIN', 'UPL'
]

# ✅ Dates for OHLC - includes today's date dynamically
today = datetime.now().date()
specific_dates = ['20-05-2025', '21-05-2025', '22-05-2025', '23-05-2025', '24-05-2025']
specific_dates = [datetime.strptime(date, "%d-%m-%Y").date() for date in specific_dates]
if today not in specific_dates:
    specific_dates.append(today)

def fetch_ohlc(symbol, from_date, to_date):
    try:
        if symbol == 'NIFTY':
            instrument = alice.get_instrument_by_token('NSE', 26000)
        elif symbol == 'BANKNIFTY':
            instrument = alice.get_instrument_by_token('NSE', 26009)
        else:
            instrument = alice.get_instrument_by_symbol('NSE', symbol)

        if not instrument or not hasattr(instrument, 'token'):
            print(f"⚠️ Invalid instrument for {symbol}")
            return pd.DataFrame()

        data = alice.get_historical(
            instrument=instrument,
            from_datetime=datetime.combine(from_date, datetime.min.time()),
            to_datetime=datetime.combine(to_date, datetime.max.time()),
            interval='D',
            indices=symbol in ['NIFTY', 'BANKNIFTY']
        )

        df = pd.DataFrame(data)
        df['symbol'] = symbol
        return df

    except Exception as e:
        print(f"⚠️ Error fetching data for {symbol}: {e}")
        return pd.DataFrame()

final_df = pd.DataFrame()
for symbol in symbols:
    df_symbol = fetch_ohlc(symbol, min(specific_dates), max(specific_dates))
    if not df_symbol.empty:
        df_symbol['date'] = pd.to_datetime(df_symbol['datetime']).dt.date
        df_filtered = df_symbol[df_symbol['date'].isin(specific_dates)]
        print(f"✅ {symbol}: Data available for dates: {df_filtered['date'].unique()}")
        final_df = pd.concat([final_df, df_filtered], ignore_index=True)

if not final_df.empty:
    final_output = []
    for symbol in final_df['symbol'].unique():
        sub_df = final_df[final_df['symbol'] == symbol].copy()
        sub_df = sub_df[['symbol', 'date', 'close', 'open', 'high', 'low']]

        top_high = sub_df['high'].max()
        top_low = sub_df['low'].min()
        diff = top_high - top_low
        bt = top_high + diff
        st = top_low - diff

        sub_df['TOP HIGH'] = ''
        sub_df['TOP LOW'] = ''
        sub_df['DIFF'] = ''
        sub_df['BT'] = ''
        sub_df['ST'] = ''
        sub_df.iloc[0, sub_df.columns.get_loc('TOP HIGH')] = round(top_high, 2)
        sub_df.iloc[0, sub_df.columns.get_loc('TOP LOW')] = round(top_low, 2)
        sub_df.iloc[0, sub_df.columns.get_loc('DIFF')] = round(diff, 2)
        sub_df.iloc[0, sub_df.columns.get_loc('BT')] = round(bt, 2)
        sub_df.iloc[0, sub_df.columns.get_loc('ST')] = round(st, 2)

        final_output.append(sub_df)
        final_output.append(pd.DataFrame([[''] * len(sub_df.columns)], columns=sub_df.columns))

    formatted_df = pd.concat(final_output, ignore_index=True)

    output_filename = f"OHLC_Data_{datetime.now().strftime('%d%m%Y_%H%M%S')}.xlsx"
    output_path = os.path.join(os.getcwd(), output_filename)
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        formatted_df.to_excel(writer, index=False, sheet_name='OHLC')

    print(f"✅ OHLC data saved to: {output_path}")
else:
    print("⚠️ No OHLC data found for specified symbols and dates.")
