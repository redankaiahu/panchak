...
def _refresh_orb_cache_loop():
    ...
    while True:
        try:
            now_time = datetime.now().strftime("%H:%M")
            if now_time < "09:15" or now_time > "15:30":
...
#   ✅ S9  Open=High 5-min Breakdown (SELL only, fires after 09:20)
#
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if not ws_ready:
            ...
            # build_context() returns None before ENTRY_START_TIME (09:31), so
