...
HARD_LOSS_LIMIT      = -2500    # ₹ — square off + halt if total PnL hits this
GROSS_WIN_TARGET     = 3500     # ₹ — close ALL positions + halt when gross winning (sum of all profitable
                                #      closed trades) crosses this level. Set 0 to disable.
            ...
            if trading_enabled:
                ...
                if daily_pnl <= HARD_LOSS_LIMIT:
                    ...
                    msg = (
                        ...
                        f"Limit:{HARD_LOSS_LIMIT:.2f} | Trading HALTED"
                    )
...
def strategy_open_high_sell_at_919(ctx):
    ...
    # BUG FIX: snap to tick_size so Kite does not reject the SL-M order in LIVE mode.
    # day_open + 1.0 may land off the tick grid for stocks with tick_size=0.05.
    _s10_tick = tick_sizes.get(symbol, 0.05)
    sl_level  = round(round((day_open + 1.0) / _s10_tick) * _s10_tick, 10)
...
/* ── HISTORY TAB ── */
.heng-row{display:flex;background:#070a0e;border-bottom:1px solid var(--border);overflow:hidden}
.heng-btn{padding:8px 20px;font-family:var(--mono);font-size:11px;font-weight:600;cursor:pointer;border:none;background:transparent;color:var(--dim);border-bottom:2px solid transparent;letter-spacing:.06em;transition:all .15s}
.heng-btn.hact{color:var(--yellow);border-bottom-color:var(--yellow);background:#1a1300}
.hper-row{display:flex;gap:6px;padding:10px 20px 12px}
.hper-btn{padding:4px 14px;font-family:var(--mono);font-size:10px;font-weight:600;cursor:pointer;border:1px solid var(--border2);border-radius:20px;background:var(--bg2);color:var(--dim);letter-spacing:.06em;transition:all .15s}
.hper-btn.hpact{background:#1a1300;color:var(--yellow);border-color:var(--yellow)}
.hstats{display:flex;gap:1px;background:var(--border);margin-bottom:14px}
.hstat{flex:1;background:var(--bg2);padding:10px 14px}
.hstat .hl{font-family:var(--mono);font-size:9px;color:var(--dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px}
.hstat .hv{font-family:var(--mono);font-size:18px;font-weight:600;color:var(--bright)}
.hstat .hv.pos{color:var(--green)}.hstat .hv.neg{color:var(--red)}
.hlist{background:var(--bg2);border:1px solid var(--border);border-radius:4px;overflow:hidden;margin-bottom:14px}
.hrow{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid var(--border);cursor:pointer;transition:background .12s}
.hrow:last-child{border-bottom:none}
.hrow:hover{background:var(--bg3)}
.hrow.hexp{background:#0d1018;border-bottom-color:var(--border2)}
.hrow-l{display:flex;flex-direction:column;gap:3px}
.hrow-date{font-family:var(--mono);font-size:12px;font-weight:600;color:var(--bright)}
.hrow-meta{font-family:var(--mono);font-size:10px;color:var(--dim)}
.hrow-r{display:flex;align-items:center;gap:16px}
.hpnl{font-family:var(--mono);font-size:15px;font-weight:600}
.harrow{font-size:11px;color:var(--dim);transition:transform .2s}
.hrow.hexp .harrow{transform:rotate(90deg)}
.htrades{display:none;background:#070a0e;border-bottom:1px solid var(--border)}
.htrades.hopen{display:block}
.htrades table{font-size:11px}
.htrades thead th{background:#070a0e;font-size:9px}
.htrades tbody td{border-bottom-color:#0d1018;font-size:11px}
.hempty{padding:14px;color:var(--dim);font-size:11px;text-align:center;font-family:var(--mono)}
.bpos{display:inline-block;height:6px;background:var(--green);border-radius:1px;vertical-align:middle;opacity:.7}
.bneg{display:inline-block;height:6px;background:var(--red);border-radius:1px;vertical-align:middle;opacity:.7}
</style>
</head>
<body>
<div class="hdr">
...
<div class="tabs">
  ...
  <div class="tab" data-tab="hs" id="histTab" onclick="st('hs');loadHist()">📅 HISTORY</div>
</div>
<div class="main">
  ...
  <div class="pane" id="pane-hs">
    <div class="heng-row">
      <button class="heng-btn hact" id="hE_B" onclick="setHEng('B')">Strategy B — NIFTY Bias</button>
    </div>
    <div class="hper-row">
      <button class="hper-btn hpact" id="hP_D" onclick="setHPer('daily')">DAILY</button>
      <button class="hper-btn" id="hP_W" onclick="setHPer('weekly')">WEEKLY</button>
      <button class="hper-btn" id="hP_M" onclick="setHPer('monthly')">MONTHLY</button>
    </div>
    <div class="hstats" id="hStats"></div>
    <div id="hList"><div class="hempty">Click HISTORY tab to load…</div></div>
  </div>
</div>
<script>
const SRV='';
...
/* ── HISTORY ── */
let _hData=null,_hPer='daily',_hLoaded=false;
function setHEng(e){renderHist();}
function setHPer(p){
  _hPer=p;
  [{k:'D',v:'daily'},{k:'W',v:'weekly'},{k:'M',v:'monthly'}].forEach(x=>
    document.getElementById('hP_'+x.k).classList.toggle('hpact',x.v===p));
  renderHist();
}
async function loadHist(force=false){
  if(_hLoaded&&!force){renderHist();return;}
  document.getElementById('hList').innerHTML='<div class="hempty">⏳ Loading history…</div>';
  try{
    const r=await fetch('/history_data');_hData=await r.json();_hLoaded=true;renderHist();
  }catch(e){document.getElementById('hList').innerHTML='<div class="hempty">Failed to load history: '+e+'</div>';}
}
function renderHist(){
  if(!_hData)return;
  const src=_hData.B;
  if(!src||!(src[_hPer]||[]).length){
    document.getElementById('hList').innerHTML='<div class="hempty">No history files found — run the algo at least once to generate trade logs.</div>';
    document.getElementById('hStats').innerHTML='';return;
  }
  const entries=[...(src[_hPer]||[])].reverse();
  const tot=entries.reduce((s,e)=>s+(e.pnl||0),0);
  const wins=entries.filter(e=>e.pnl>0).length;
  const loss=entries.filter(e=>e.pnl<0).length;
  const best=entries.reduce((m,e)=>e.pnl>m?e.pnl:m,-Infinity);
  const wrst=entries.reduce((m,e)=>e.pnl<m?e.pnl:m,Infinity);
  const ttrades=entries.reduce((s,e)=>s+(e.trades||[]).length,0);
  const wr=entries.length?((wins/entries.length)*100).toFixed(0):0;
  document.getElementById('hStats').innerHTML=`
    <div class="hstat"><div class="hl">Total PnL</div><div class="hv ${tot>=0?'pos':'neg'}">₹${tot>=0?'+':''}${tot.toFixed(2)}</div></div>
    <div class="hstat"><div class="hl">Win Rate</div><div class="hv ${wr>=50?'pos':'neg'}">${wr}%</div></div>
    <div class="hstat"><div class="hl">Profitable</div><div class="hv pos">${wins}</div></div>
    <div class="hstat"><div class="hl">Loss Periods</div><div class="hv neg">${loss}</div></div>
    <div class="hstat"><div class="hl">Best</div><div class="hv pos">${isFinite(best)?'₹+'+best.toFixed(2):'—'}</div></div>
    <div class="hstat"><div class="hl">Worst</div><div class="hv neg">${isFinite(wrst)?'₹'+wrst.toFixed(2):'—'}</div></div>
    <div class="hstat"><div class="hl">Total Trades</div><div class="hv">${ttrades}</div></div>`;
  const maxA=Math.max(1,...entries.map(e=>Math.abs(e.pnl||0)));
  let html='<div class="hlist">';
  entries.forEach((e,i)=>{
    const pnl=e.pnl||0;const pcls=pnl>0?'pos':pnl<0?'neg':'';
    const lbl=_hPer==='daily'?_fmtDay(e.date):(e.label||e.date||'—');
    const bw=Math.round(Math.abs(pnl)/maxA*70);
    const bar=pnl>=0?`<span class="bpos" style="width:${bw}px"></span>`:`<span class="bneg" style="width:${bw}px"></span>`;
    const tc=(e.trades||[]).length;
    const dw=(e.trades||[]).filter(t=>parseFloat(t.pnl||0)>0).length;
    const dl=(e.trades||[]).filter(t=>parseFloat(t.pnl||0)<0).length;
    const meta=tc?`${tc} trade${tc!==1?'s':''} · <span style="color:var(--green)">${dw}W</span>/<span style="color:var(--red)">${dl}L</span> &nbsp;${bar}`:'No trades';
    html+=`<div class="hrow" id="hr_${i}" onclick="togH(${i})">
      <div class="hrow-l"><div class="hrow-date">${lbl}</div><div class="hrow-meta">${meta}</div></div>
      <div class="hrow-r"><span class="hpnl ${pcls}">₹${pnl>=0?'+':''}${pnl.toFixed(2)}</span><span class="harrow">▶</span></div>
    </div>
    <div class="htrades" id="ht_${i}">${_hTbl(e.trades||[])}</div>`;
  });
  html+='</div>';
  document.getElementById('hList').innerHTML=html;
}
function togH(i){
  document.getElementById('hr_'+i).classList.toggle('hexp');
  document.getElementById('ht_'+i).classList.toggle('hopen');
}
function _hTbl(trades){
  if(!trades.length)return'<div class="hempty">No trades recorded for this period</div>';
  const rows=trades.map(t=>{
    const pv=parseFloat(t.pnl||0);const pcls=isNaN(pv)?'':pv>0?'pp':'pn';
    const side=(t.side||'').toUpperCase();
    return`<tr><td class="sym">${t.symbol||'—'}</td>
      <td class="${side==='BUY'?'buy':'sell'}">${side||'—'}</td>
      <td><span class="stag">${t.strategy||'—'}</span></td>
      <td>${t.entry||'—'}</td><td>${t.exit||'—'}</td><td>${t.qty||'—'}</td>
      <td class="${pcls}">₹${isNaN(pv)?'—':(pv>=0?'+':'')+pv.toFixed(2)}</td>
      <td>${et(t.reason)||'—'}</td>
      <td style="color:var(--dim)">${t.time||'—'}</td></tr>`;
  }).join('');
  return`<table><thead><tr><th>Symbol</th><th>Side</th><th>Strategy</th><th>Entry</th><th>Exit</th><th>Qty</th><th>Net PnL</th><th>Reason</th><th>Time</th></tr></thead><tbody>${rows}</tbody></table>`;
}
function _fmtDay(ds){
  if(!ds)return'—';const[d,m,y]=ds.split('-');const dt=new Date(`${y}-${m}-${d}`);if(isNaN(dt))return ds;
  const days=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  const mons=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  return`${days[dt.getDay()]} ${d} ${mons[dt.getMonth()]} ${y}`;
}
document.getElementById('histTab').addEventListener('dblclick',()=>loadHist(true));
</script>
</body>
</html>"""

def _build_history_payload(engine_key="B"):
    """Scan dated CSV trade logs and return daily/weekly/monthly aggregates."""
    import glob as _glob, csv as _hcsv, re as _re, os as _os
    from datetime import timedelta as _td

    if engine_key == "A":
        _glob_pat = "paper_trades_*.csv"
        _date_re  = _re.compile(r"paper_trades_(\d{2}-\d{2}-\d{4})\.csv")
    else:
        _glob_pat = "nifty_paper_trades_*.csv"
        _date_re  = _re.compile(r"nifty_paper_trades_(\d{2}-\d{2}-\d{4})\.csv")
    ...
    def _read_t(path):
        ...
        try:
            with open(path, newline="", encoding="utf-8", errors="replace") as fh:
                for row in _hcsv.DictReader(fh):
                    rows.append({
                        "time": row.get("Time",""), "symbol": row.get("Symbol",""),
                        "side": row.get("Side",""), "entry": row.get("Entry",""),
                        "exit": row.get("Exit",""), "qty": row.get("Qty",""),
                        "pnl": row.get("PnL","0"), "reason": row.get("Reason",""),
    ...
    files = []
    for path in _glob.glob(_glob_pat):
        m = _date_re.search(_os.path.basename(path))
        if m:
            files.append((m.group(1), path))
    files.sort(key=lambda x: datetime.strptime(x[0], "%d-%m-%Y"))

    daily, weekly, monthly = [], {}, {}
    for date_str, path in files:
        trades = [t for t in _read_t(path) if "__PYR" not in t.get("symbol","")]
        day_pnl = round(sum(float(t["pnl"]) for t in trades if _is_f(t["pnl"])), 2)
        daily.append({"date": date_str, "pnl": day_pnl, "trades": trades})
        ...
        if wk not in weekly:
            weekly[wk] = {"label": f"{mon.strftime('%d %b')} – {fri.strftime('%d %b %Y')}",
                          "pnl": 0.0, "trades": [], "_s": mon}
        weekly[wk]["pnl"]    = round(weekly[wk]["pnl"] + day_pnl, 2)
        weekly[wk]["trades"] += trades

        mk = dt.strftime("%Y-%m")
        if mk not in monthly:
            monthly[mk] = {"label": dt.strftime("%B %Y"), "pnl": 0.0,
                           "trades": [], "_s": dt.replace(day=1)}
        monthly[mk]["pnl"]    = round(monthly[mk]["pnl"] + day_pnl, 2)
        monthly[mk]["trades"] += trades

    def _strip(lst):
        for d in lst: d.pop("_s", None)
        return lst

    return {
        "daily":   daily,
        "weekly":  _strip(sorted(weekly.values(),  key=lambda x: x["_s"])),
        "monthly": _strip(sorted(monthly.values(), key=lambda x: x["_s"])),
    }
    ...
    def do_GET(self):
        try:
            if self.path.startswith("/data"):
                ...
                ct   = "application/json"
            elif self.path.startswith("/history_data"):
                import json as _hj
                body = _hj.dumps({"B": _build_history_payload("B")},
                                 default=str).encode()
                ct   = "application/json"
            else:
