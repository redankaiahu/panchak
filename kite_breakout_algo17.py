...
FIXED_QTY = 10
STOP_LOSS_PERCENT = 0.5        # 1%
TARGET_PERCENT = 0.5         # 2.5%
TRAIL_STEP_PERCENT = 0.4       # trail every 1% move
...
def strategy(token, tick):
    ...
    # ==========================================================
    # ================= STRATEGY 1: ORB + PULLBACK ============
    # ==========================================================

    # Capture first 30 minutes range
    ...
    if current_time <= "09:45":
        orb_data[symbol]["high"] = max(orb_data[symbol]["high"], live_high)
        orb_data[symbol]["low"] = min(orb_data[symbol]["low"], live_low)
        return
    ...
    # BUY ORB Break + Pullback
    if (
        orb_data[symbol]["ready"] and
        3999999 <= ltp <= 399999999 and
        ltp > orb_high and
        ema7 > ema20 > ema50
    ):
        place_trade(symbol, "BUY", ltp, tick, "ORB Break")

    # SELL ORB Break + Pullback
    if (
        orb_data[symbol]["ready"] and
        39999999 <= ltp <= 39999999 and
        ltp < orb_low and
        ema7 < ema20 < ema50
    ):
        place_trade(symbol, "SELL", ltp, tick, "ORB Breakdown")

    # ==========================================================
    # ===== STRATEGY 2: OPEN=LOW 15 MIN BREAKOUT ==============
    # ==========================================================
    ...
    if current_time <= "09:30":
        first15_data[symbol]["high"] = max(first15_data[symbol]["high"], live_high)
        first15_data[symbol]["low"] = min(first15_data[symbol]["low"], live_low)
        first15_data[symbol]["close"] = ltp
        return

    if not first15_data[symbol]["range_ready"]:
        first15_data[symbol]["range_ready"] = True

    f_open = first15_data[symbol]["open"]
    f_high = first15_data[symbol]["high"]
    f_low = first15_data[symbol]["low"]
    f_close = first15_data[symbol]["close"]

    range_percent = ((f_high - f_low) / f_open) * 100 if f_open != 0 else 999

    # BUY CONDITION
    if (
        39999999 <= ltp <= 399999999 and
        f_open == f_low and
        f_close > f_open and
        range_percent < 1 and
        ltp > f_high
    ):
        place_trade(symbol, "BUY", ltp, tick, "Open=Low Break")
    ...
    if (
        ...
        place_trade(symbol, "SELL", ltp, tick, "Open=High Break")

    # ==========================================================
    # ===== STRATEGY 3: EXISTING YESTERDAY BREAKOUT ===========
    # ==========================================================

    if (
        399999999 <= ltp <= 399999999 and
        yest_volume >= 100000 and
        live_open <= yest_high and
        ltp >= yest_high and
        1 <= change_percent <= 2 and
        ltp >= ema7 >= ema20 >= ema50
    ):
        place_trade(symbol, "BUY", ltp, tick, "YH Break")
    ...
    if (
        ...
        ltp <= yest_low and
        -2 <= change_percent <= -1 and
        ltp <= ema7 <= ema20 <= ema50
    ):
        place_trade(symbol, "SELL", ltp, tick, "YL Break")

    # ==========================================================
    # ===== HIGH WIN% PULLBACK SCALP STRATEGY =================
    # ==========================================================

    # Only trade after 9:35
    if current_time < "09:35":
        return

    # NIFTY Trend Filter
    nifty_ema7 = ema_cache.get("NIFTY", {}).get("ema7")
    nifty_ema20 = ema_cache.get("NIFTY", {}).get("ema20")

    if nifty_ema7 is None or nifty_ema20 is None:
        return

    # ================= BUY SETUP =================
    if (
        nifty_ema7 > nifty_ema20 and
        ema7 > ema20 > ema50 and
        abs((ltp - ema7) / ema7) * 100 < 0.2 and   # Pullback near EMA7
        tick["ohlc"]["close"] > tick["ohlc"]["open"]  # Bullish candle
    ):
        place_trade(symbol, "BUY", ltp, tick, "Trend Pullback Buy")
    ...
    if (
        nifty_ema7 < nifty_ema20 and
        ema7 < ema20 < ema50 and
        abs((ltp - ema7) / ema7) * 100 < 0.2 and
        tick["ohlc"]["close"] < tick["ohlc"]["open"]
    ):
        place_trade(symbol, "SELL", ltp, tick, "Trend Pullback Sell")
