...
QTabBar::tab:hover { color: #dce6f5; background: #0f1f35; }
/* ── Tables ── */
QTableWidget {
    background-color: #090e18;
    gridline-color: #131f30;
    color: #dce6f5;
    border: none;
    font-size: 12px;
    selection-background-color: #1a3a5c44;
}
QTableWidget::item { padding: 3px 7px; border-bottom: 1px solid #0f1a28; }
QHeaderView::section {
...
QPushButton:hover   { background: #1a4a72; color: #ffffff; border-color: #4fc3f7; }
QPushButton:pressed { background: #0a3355; }
QPushButton:disabled { color: #2a4060; border-color: #1a2a3a; }
/* ── Misc ── */
QScrollArea { border: none; }
QLabel      { color: #dce6f5; }
QStatusBar  { background: #0d1829; color: #4a6a8a; border-top: 1px solid #1e2d45; font-size: 10px; }
QProgressBar {
    background: #0f1829;
    border: 1px solid #1e3550;
    border-radius: 3px;
...
QProgressBar::chunk { background: qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 #1565c0,stop:1 #4fc3f7); border-radius: 3px; }
QGroupBox {
    border: 1px solid #1e3550;
    border-radius: 6px;
    margin-top: 8px;
...
QGroupBox::title { subcontrol-origin: margin; padding: 0 6px; color: #4fc3f7; }
QSplitter::handle { background: #1e3550; width: 1px; }
"""


C_GREEN  = "#00e676"; C_RED    = "#ff5252"; C_YELLOW = "#ffca28"
C_BLUE   = "#4fc3f7"; C_PURPLE = "#ce93d8"; C_DIM    = "#4a6a8a"
C_BG_CARD= "#0d1829"; C_BORDER = "#1e3550"
C_BG_BULL= "#002a15"; C_BG_BEAR= "#2a0008"
C_GLOW_G = "#00e67633"; C_GLOW_R = "#ff525233"; C_GLOW_B = "#4fc3f722"
C_TEXT   = "#dce6f5"; C_MUTED  = "#8aaccc"
...
def card_frame(layout_cls=QVBoxLayout, pad=(12, 10, 12, 10), glow=None):
    frame = QFrame()
    border = glow if glow else C_BORDER
    bg_extra = f"background:{C_BG_CARD};"
    frame.setStyleSheet(
        f"QFrame {{ {bg_extra} border:1px solid {border}; border-radius:8px; }}")
    lay = layout_cls()
    ...
    def run(self):
        ...
        for symbol in self.symbols:
            try:
                ...
                # OB retest check for prior setups
                try:
                    prior = _todays_bos_setups.get(symbol)
                    if prior:
                        check_ob_retest(symbol, prior, ltp,
                                        lambda msg, **kw: None, tg_enabled=False)
                except Exception:
                    pass
            except Exception:
                continue
        ...
        try:
            hourly_events = scan_hourly_breaks(
                ...
                lambda msg, **kw: None,   # no TG from worker thread
...
class OIPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Direction card (stored ref so we can dynamically change glow)
        df, dl = card_frame(QVBoxLayout, (12, 10, 12, 10))
        self._dir_card = df
        row1 = QHBoxLayout()
        self.lbl_dir  = make_label("—", 16, bold=True, color=C_YELLOW)
        self.lbl_dr   = make_label("—", 11, color=C_DIM)
        row1.addWidget(self.lbl_dir)
        row1.addWidget(make_label("  │  ", 12, color=C_BORDER))
        row1.addWidget(self.lbl_dr); row1.addStretch()
        dl.addLayout(row1)
        row2 = QHBoxLayout()
        self.lbl_advice = make_label("—", 11, color=C_MUTED)
        self.lbl_setup  = make_label("—", 12, bold=True, color=C_GREEN)
        ...
        self.chain_tbl.setStyleSheet("""
            QTableWidget {
                alternate-background-color: #080e1a;
                background-color: #090e18;
                border: none;
                font-size: 12px;
            }
        ...
        # ── 30-Min OI Summary ────────────────────────────────────
        sum_hdr = QHBoxLayout()
        sum_hdr.addWidget(make_label("📊  OI Summary (30-min snapshots)", 11,
                                     bold=True, color=C_BLUE))
        sum_hdr.addStretch()
        self.btn_clear_snap = QPushButton("Clear")
        self.btn_clear_snap.setFixedWidth(60)
        self.btn_clear_snap.setStyleSheet(
            "QPushButton{background:#0f1f35;color:#4a6a8a;border:1px solid #1e3550;"
        ...
        self.lbl_oi_summary.setStyleSheet(f"""
            QLabel {{
                background: #050e1a;
                border: 1px solid #1e3550;
                border-radius: 6px;
    ...
    def _pill(self, label, value, color, width=160):
        frame = QFrame(); frame.setFixedWidth(width)
        frame.setStyleSheet(f"""
            QFrame {{
                background: qlineargradient(x1:0,y1:0,x2:0,y2:1,
                    stop:0 {color}22, stop:1 {color}08);
                border: 1px solid {color}66;
                border-radius: 8px;
            }}
        """)
        lay = QVBoxLayout(frame); lay.setContentsMargins(8, 6, 8, 6); lay.setSpacing(2)
        lbl_label = make_label(label, 8, color=C_DIM,
                               align=Qt.AlignmentFlag.AlignCenter, letter_spacing=True)
        lbl_value = make_label(value, 15, bold=True, color=color,
                               align=Qt.AlignmentFlag.AlignCenter)
        lay.addWidget(lbl_label); lay.addWidget(lbl_value)
        frame._v = lbl_value; return frame
    ...
    def _badge(self, label, value, color):
        ...
        """)
        lay = QHBoxLayout(frame); lay.setContentsMargins(10, 5, 10, 5); lay.setSpacing(6)
        lay.addWidget(make_label(label + ":", 9, color=C_DIM, letter_spacing=True))
        lbl = make_label(value, 14, bold=True, color=color)
...
class BosPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # ── Header ────────────────────────────────────────────────
        hdr = QHBoxLayout()
        hdr.addWidget(make_label("📐  BOS / CHoCH + Hourly Break Scanner  v2",
                                  14, bold=True, color=C_BLUE))
        hdr.addStretch()
        ...
        # ── Caption ───────────────────────────────────────────────
        cap = make_label(
            "BOS/CHoCH: candle quality + volume + daily EMA filter  |  "
            "Hourly Break: prev-1H high/low breach  |  OB Retest: B-entry alert zone",
            10, color=C_DIM)
        ...
        # ── Summary pills ─────────────────────────────────────────
        sr = QHBoxLayout()
        self.p_total  = self._pill("BOS Events",    "0", C_BLUE)
        self.p_bull   = self._pill("Bull 🚀",       "0", C_GREEN)
        self.p_bear   = self._pill("Bear 💥",       "0", C_RED)
        self.p_choch  = self._pill("CHoCH 🔄",      "0", C_PURPLE)
        self.p_hrly   = self._pill("Hrly Breaks ⚡","0", C_YELLOW)
        self.p_ema_f  = self._pill("EMA Filtered",  "0", C_DIM)
        ...
        # ── Filters row ───────────────────────────────────────────
        ff, fl = card_frame(QHBoxLayout, (8,6,8,6))
    ...
    def _make_tbl(self, cols):
        ...
        t.setShowGrid(False)
        t.setStyleSheet(t.styleSheet() + "QTableWidget{alternate-background-color:#0a1525;}")
        return t
...
class AutoRefreshBar(QWidget):
    ...
    def __init__(self, interval_secs=AUTO_REFRESH_SECS):
        ...
        self.setStyleSheet(f"background:{C_BG_CARD};border-top:1px solid {C_BORDER};")
...
class MoversPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Header
        hdr = QHBoxLayout()
        hdr.addWidget(make_label("🔥  Top Movers — Live", 14, bold=True, color=C_BLUE))
        hdr.addStretch()
        self.btn = QPushButton("⟳  Fetch Movers")
        self.btn.setFixedWidth(140); self.btn.clicked.connect(self._fetch)
        hdr.addWidget(self.btn)
        root.addLayout(hdr)

        # Summary pills
        pills = QHBoxLayout()
        self.p_g = self._pill("Gainers >2%",    "0", C_GREEN)
        self.p_l = self._pill("Losers <-2%",    "0", C_RED)
        self.p_v = self._pill("Vol Surge Top20","0", C_BLUE)
        self.p_t = self._pill("Updated",        "—", C_DIM)
    ...
    def _pill(self, label, value, color):
        f = QFrame()
        f.setStyleSheet(f"QFrame{{background:{color}22;border:1px solid {color}66;border-radius:4px;}}")
        l = QVBoxLayout(f); l.setContentsMargins(8, 3, 8, 3); l.setSpacing(1)
        l.addWidget(make_label(label, 9, color=C_DIM, align=Qt.AlignmentFlag.AlignCenter))
        lv = make_label(value, 14, bold=True, color=color, align=Qt.AlignmentFlag.AlignCenter)
        l.addWidget(lv); f._v = lv; return f
    ...
    def _make_tbl(self, cols):
        ...
        t.verticalHeader().setVisible(False)
        t.setAlternatingRowColors(True)
        t.setStyleSheet(t.styleSheet() + "QTableWidget{alternate-background-color:#111820;}")
        return t
...
class KPPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Header
        hdr = QHBoxLayout()
        hdr.addWidget(make_label("🪐  KP Panchang — Trading Windows", 14, bold=True, color=C_BLUE))
        hdr.addStretch()
        self.btn = QPushButton("⟳  Refresh")
        self.btn.setFixedWidth(100); self.btn.clicked.connect(self._fetch)
        hdr.addWidget(self.btn)
        root.addLayout(hdr)
    ...
    def _pill(self, label, value, color):
        f = QFrame()
        f.setStyleSheet(f"QFrame{{background:{color}18;border:1px solid {color}55;border-radius:5px;}}")
        l = QVBoxLayout(f); l.setContentsMargins(6, 3, 6, 3); l.setSpacing(1)
        l.addWidget(make_label(label, 9, color=C_DIM, align=Qt.AlignmentFlag.AlignCenter))
        lv = make_label(value, 12, bold=True, color=color, align=Qt.AlignmentFlag.AlignCenter)
        l.addWidget(lv); f._v = lv; return f
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        header.setStyleSheet("""
            QFrame {
                background: qlineargradient(x1:0,y1:0,x2:1,y2:0,
                    stop:0 #050c18, stop:0.5 #0a1828, stop:1 #050c18);
                border-bottom: 1px solid #1e3a5f;
            }
        """)
        hl = QHBoxLayout(header); hl.setContentsMargins(18, 0, 18, 0); hl.setSpacing(14)

        # Logo + title
        logo_lbl = make_label("📈", 18)
        title_lbl = make_label("PANCHAK DASHBOARD", 14, bold=True, color="#4fc3f7",
                                letter_spacing=True)
        ...
        sep1.setFixedHeight(30); sep1.setStyleSheet("color:#1e3a5f;")
        hl.addWidget(sep1)

        # NIFTY spot — large, prominent
        self.lbl_spot = QLabel("NIFTY: —")
        self.lbl_spot.setFont(QFont("Segoe UI", 16, QFont.Weight.Bold))
        self.lbl_spot.setStyleSheet(f"color:{C_YELLOW};")
        hl.addWidget(self.lbl_spot)

        # Change indicator
        self.lbl_chg = QLabel("")
        self.lbl_chg.setFont(QFont("Segoe UI", 11))
        self.lbl_chg.setStyleSheet(f"color:{C_DIM};")
        hl.addWidget(self.lbl_chg)

        hl.addStretch()

        # Status pills
        self.lbl_kite = make_label("🔴 Kite: disconnected", 10, color=C_RED)
        self.lbl_kite.setStyleSheet(f"""
            color:{C_RED}; background:#2a0a0a; border:1px solid #5a1a1a;
            border-radius:10px; padding:2px 10px; font-weight:600;
        """)
        hl.addWidget(self.lbl_kite)

        self.lbl_market = make_label("—", 10, color=C_DIM)
        self.lbl_market.setStyleSheet(f"""
            color:{C_DIM}; background:#0a1525; border:1px solid #1e3550;
            border-radius:10px; padding:2px 10px; font-weight:600;
        """)
        hl.addWidget(self.lbl_market)

        sep2 = QFrame(); sep2.setFrameShape(QFrame.Shape.VLine)
        sep2.setFixedHeight(30); sep2.setStyleSheet("color:#1e3a5f;")
        hl.addWidget(sep2)

        self.lbl_clock = make_label("—", 12, bold=True, color=C_YELLOW)
        hl.addWidget(self.lbl_clock)

        # Separator
        sep3 = QFrame(); sep3.setFrameShape(QFrame.Shape.VLine)
        sep3.setFixedHeight(30); sep3.setStyleSheet("color:#1e3a5f;")
        hl.addWidget(sep3)

        # Last refreshed — always visible in header
        ...
        self.statusBar().showMessage(
            "Phase 4 — OI | SMC | BOS | Screeners | Movers | KP | Alerts | Astro | Time | DB | Session")
    ...
    def _on_kite_connected(self):
        ...
        self.lbl_kite.setStyleSheet(f"color:{C_GREEN};font-size:11px;font-weight:bold;")
    ...
    def _on_spot(self, data):
        ...
        self.lbl_chg.setStyleSheet(f"color:{chg_color}; font-size:11px;")

    def _auto_refresh(self):
        """Fires every AUTO_REFRESH_SECS. Refreshes OI + all live panels + updates refresh timestamps."""
        ts = datetime.now().strftime("%H:%M:%S")
        self.oi_panel.trigger_refresh()
        if SCREENER_OK:
        ...
        self.movers_panel.trigger_fetch()
        self.kp_panel.trigger_refresh()
        # Update both refresh indicators
    ...
    def _mark_refreshed(self, ts: str = ""):
        ...
        self.lbl_last_refresh.setStyleSheet(f"""
            color:{C_GREEN}; background:#002a15;
            border:1px solid #006030; border-radius:8px;
            padding:2px 10px; font-size:10px; font-weight:700;
        """)
        # Fade header badge back to neutral after 4 seconds
        QTimer.singleShot(4000, lambda: self.lbl_last_refresh.setStyleSheet("""
            color: #4a6a8a; background: #0a1525;
            border: 1px solid #1e3550; border-radius: 8px;
            padding: 2px 10px; font-size: 10px;
        """))
        # AutoRefreshBar label
    ...
    def _tick_clock(self):
        ...
        if mo:
            ...
            self.lbl_market.setStyleSheet(f"""
                color:{C_GREEN}; background:#002a15;
                border:1px solid #006030; border-radius:10px;
                padding:2px 10px; font-weight:700; font-size:10px;
            """)
        else:
            self.lbl_market.setText("● MARKET CLOSED")
            self.lbl_market.setStyleSheet(f"""
                color:{C_RED}; background:#2a0008;
                border:1px solid #600020; border-radius:10px;
                padding:2px 10px; font-weight:700; font-size:10px;
            """)
