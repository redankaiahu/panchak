...
FIXED_QTY = 1                # ⚠️  SET TO 1 FOR LIVE TEST — change to 10 only after full validation
LIVE_MAX_TRADES = 15         # max simultaneous live trades (safety cap)
...
def paper_position_manager():
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            # Mirrors the live-mode check in position_manager() so paper PnL
            # accurately reflects what live mode would do.
            # Reference capital = total open position value; fallback ₹50,000.
            if trading_enabled:
                ref_capital = sum(
                    p["entry"] * p["qty"]
