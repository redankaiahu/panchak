...
def _run_bos_scan_safe(symbols, ltp_dict=None):
    """
    Scan for BOS/CHoCH — reads from local OHLC DB, no live API call.
    Routes alerts via send_alert_routed so channel routing (ch1/ch2/both)
    is respected per the Alert Control tab settings.
    """
...
# ── Second Telegram channel (Point 12): AutoBotTest123 ────────────────────
# Uses a DIFFERENT bot token (as specified in alerts_modifications.docx)
TG_CHAT_ID_2   = "-1002360390807"                           # AutoBotTest123 supergroup
TG_BOT_TOKEN_2 = "8183479936:AAHyIgC1zyGOy-yoSAidDcO5KJ9nReaPP78"  # AutoBotTest123 bot

def send_telegram2(message: str, dedup_key: str = None) -> bool:
    """Send to second Telegram channel (AutoBotTest123).
    Uses TG_BOT_TOKEN_2 (separate bot) as per alerts_modifications.docx.
    Only BOS UP/DOWN, CHoCH UP/DOWN, SMC+OI Confluence go here.
    """
    if not is_market_hours():          # ← same gate as send_telegram ch1
        return False
    _tok2 = globals().get("TG_BOT_TOKEN_2", "") or TG_BOT_TOKEN
    if not _tok2 or not TG_CHAT_ID_2:
        return False
...
def _load_panchak_tg_dedup():
    ...
    try:
        with open(_panchak_tg_dedup_path(), "r") as f:
            return json.load(f)
    except Exception:
        return {}
...
def _save_panchak_tg_dedup(d):
    ...
    try:
        with open(_panchak_tg_dedup_path(), "w") as f:
            json.dump(d, f)
    except Exception:
        pass
...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    if _special_up and st.session_state.get("special_stock_alerts", True):
        ...
        # Use per-stock panchak-period dedup — fires once per stock per panchak period
        _sp_any_new_up = False
        for _sp_s in _sp_syms:
            if _panchak_tg_check_and_mark(_sp_s, "UP_SPECIAL"):
                _sp_any_new_up = True
        if _sp_any_new_up:
            _sp_key = "SPECIAL_UP_" + "_".join(sorted(_sp_syms[:3])) + "_" + datetime.now(IST).strftime("%Y%m%d_%H%M")
    ...
    if _special_down and st.session_state.get("special_stock_alerts", True):
        ...
        for _sp_s in _sp_syms:
            if _panchak_tg_check_and_mark(_sp_s, "DOWN_SPECIAL"):
                _sp_any_new = True
        if _sp_any_new:
            _sp_key = "SPECIAL_DOWN_" + "_".join(sorted(_sp_syms[:3])) + "_" + datetime.now(IST).strftime("%Y%m%d_%H%M")
            send_telegram_bg(_sp_tg_msg, dedup_key=_sp_key)
...
with st.expander("💾 Data Storage Status (click to expand)", expanded=False):
    ...
    def _color_status(val):
        ...
        return ""
    st.dataframe(
        _status_df.style.map(_color_status, subset=["status"]), use_container_width=True,
        hide_index=True,
    )
...
def fire_combined_engine_tg_alert():
    ...
    msg = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💹 NIFTY: <b>{_nifty_ltp:,}</b>   BANKNIFTY: <b>{_bnifty_ltp:,}</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        if not TG_CHAT_ID:
            ...
            if st.button("💾 Save Chat ID") and _manual_cid.strip():
                try:
                    ...
                    with open(os.path.join(_cache_dir, "tg_chat_id.txt"), "w") as _cf: _cf.write(_manual_cid.strip())
...
with tabs[2]:
    ...
    st.dataframe(_pv_styled, use_container_width=True, height=7800)
    ...
    st.dataframe(_apply_fmt(TOP_HIGH_df), use_container_width=True, height=7000)
...
with tabs[4]:
    ...
    st.dataframe(_apply_fmt(TOP_LOW_df), use_container_width=True, height=7000)
    ...
    if near_base.empty:
        ...
        with col1:
            ...
            if near_buy_df.empty:
                ...
                st.dataframe(
                    near_buy_df, use_container_width=True,
        ...
        with col2:
            ...
            if near_sell_df.empty:
                ...
                st.dataframe(
                    near_sell_df, use_container_width=True,
                    height=min(4200, 60 + len(near_buy_df) * 35)
                )
...
if _raw_5min_today is None or (_raw_5min_today is not None and hasattr(_raw_5min_today, "empty") and _raw_5min_today.empty):
    try:
        import glob as _g5
        _5m_files = sorted(_g5.glob(os.path.join(CACHE_DIR, "five_min_*.csv")),
                           key=os.path.getmtime, reverse=True)
        if _5m_files:
            _raw_5min_today = pd.read_csv(_5m_files[0])
            _raw_5min_today["datetime"] = pd.to_datetime(_raw_5min_today["datetime"])
    except Exception:
        pass
...
with tabs[6]:   # 👈 replace with correct index
    ...
    def _css_gradient(series, cmap="Greens"):
        ...
        st.dataframe(
            _15m_high_df[_show].reset_index(drop=True).style.apply(_css_gradient, subset=["ABOVE_%"], cmap="Greens"), use_container_width=True
        )
    ...
    if _15m_low_df.empty:
        ...
        st.dataframe(
            _15m_low_df[_show].reset_index(drop=True).style.apply(_css_gradient, subset=["BELOW_%"], cmap="Reds"), use_container_width=True
        )
    ...
    if _vol_df.empty:
        ...
        st.dataframe(
            _vol_df[_show].reset_index(drop=True).style.apply(_css_gradient, subset=["VOL_SURGE_%"], cmap="Blues"), use_container_width=True
        )
    ...
    if _h1_high_df.empty:
        ...
        st.dataframe(
            _h1_high_df[_show].reset_index(drop=True).style.apply(_css_gradient, subset=["ABOVE_%"], cmap="Greens"), use_container_width=True
        )
    ...
    if _h1_low_df.empty:
        ...
        st.dataframe(
            _h1_low_df[_show].reset_index(drop=True).style.apply(_css_gradient, subset=["BELOW_%"], cmap="Reds"), use_container_width=True
        )
        ...
        if _long_df.empty:
            ...
            st.dataframe(
                _long_df.style
                    ...
                    .format(_fmt_long, na_rep="—"), use_container_width=True
            )
        ...
        if _short_df.empty:
            ...
            st.dataframe(
                _short_df.style
                    ...
                    .format(_fmt_short, na_rep="—"), use_container_width=True
            )
...
with tabs[7]:   # 👈 replace with correct index
    ...
    if table10.empty:
        ...
        st.dataframe(table10, use_container_width=True)
    ...
    if table11.empty:
        ...
        st.dataframe(table11, use_container_width=True)
...
with tabs[8]:   # change tab index if required
    ...
    if strong_high_df.empty:
        ...
        st.dataframe(
            styled_high, use_container_width=True,
            height=450
        )
    ...
    if strong_low_df.empty:
        ...
        st.dataframe(
            styled_low, use_container_width=True,
            height=450
        )

    st.subheader("📈 DAILY BREAKS – Above YEST HIGH")
    st.dataframe(_apply_fmt(daily_up), use_container_width=True)

    st.subheader("📉 DAILY BREAKS – Below YEST LOW")
    st.dataframe(_apply_fmt(daily_down), use_container_width=True)
...
with tabs[9]:
    ...
    if not _orb_range_ready:
        ...
        if not _h1_up.empty:
            ...
            st.dataframe(_h1_up.sort_values(_sort_col, ascending=False), use_container_width=True)
        if not _h1_down.empty:
            ...
            st.dataframe(_h1_down.sort_values(_sort_col, ascending=False), use_container_width=True)
    ...
    st.dataframe(_apply_fmt(weekly_up), use_container_width=True)

    st.subheader("📊 WEEKLY BREAKS – BeLIVE_LOW WEEK LOW")
    st.dataframe(_apply_fmt(weekly_down), use_container_width=True)
     ...
     #   st.markdown("🟢 BUY : WEEK HIGH + EMA20 > EMA50")
      #  if weekly_ema_buy.empty:
       #     st.info("No Weekly EMA BUY confirmations")
        #else:
         #   st.dataframe(weekly_ema_buy, use_container_width=True)
     ...
     #   st.markdown("🔴 SELL : WEEK LOW + EMA20 < EMA50")
      #  if weekly_ema_sell.empty:
       #     st.info("No Weekly EMA SELL confirmations")
        #else:
         #   st.dataframe(weekly_ema_sell, use_container_width=True)
...
if False:  # hidden tab[10] content
    ...
    st.subheader("📅 WEEKLY %")
    st.dataframe(weekly_break_df, use_container_width=True)

    st.subheader("📅 MONTHLY %")
    st.dataframe(monthly_break_df, use_container_width=True)

    st.subheader("📅 MONTHLY BREAKS – Above MONTH HIGH")
    monthly_up = df.loc[df.LTP >= df.HIGH_M, MONTHLY_COLUMNS]
    st.dataframe(_apply_fmt(monthly_up), use_container_width=True)

    st.subheader("📅 MONTHLY BREAKS – Below MONTH LOW")
    monthly_down = df.loc[df.LTP <= df.LOW_M, MONTHLY_COLUMNS]
    st.dataframe(_apply_fmt(monthly_down), use_container_width=True)
...
with tabs[11]:
    ...
    with col1:
        ...
        if LIVE_OPEN_LIVE_LOW_df.empty:
            ...
            st.dataframe(
                LIVE_OPEN_LIVE_LOW_df, use_container_width=True,
    ...
    with col2:
        ...
        if LIVE_OPEN_LIVE_HIGH_df.empty:
            ...
            st.dataframe(
                LIVE_OPEN_LIVE_HIGH_df, use_container_width=True,
...
if False:  # hidden tab[12] content
    ...
    if ema_buy_df.empty:
        ...
        st.dataframe(
            ema_buy_df, use_container_width=True,
    ...
    if ema_sell_df.empty:
        ...
        st.dataframe(
            ema_sell_df, use_container_width=True,
...
with tabs[14]:  # 4 BAR
    ...
    if four_bar_df.empty:
        ...
        st.dataframe(four_bar_df, use_container_width=True)
    ...
    with col1:
        ...
        if fake_bull_df.empty:
            ...
            st.dataframe(
                ...
                .style.apply(style_ltp_only, axis=1), use_container_width=True,
    ...
    with col2:
        ...
        if fake_bear_df.empty:
            ...
            st.dataframe(
                fake_bear_df
                .sort_values("FAIL_%", ascending=False)
                .style.apply(style_ltp_bear_only, axis=1), use_container_width=True,
    ...
    if inside_15m_df.empty:
        ...
        st.dataframe(
            inside_15m_df
                .sort_values("CHANGE_%", ascending=False)
                .style.apply(style_ltp_15min, axis=1), use_container_width=True,
...
if False:  # hidden tab[15] content
    ...
    if early_runner_df.empty:
        ...
        st.dataframe(
            early_runner_df, use_container_width=True
    )
    ...
    if fut_df.empty:
        ...
        st.dataframe(
            filtered_fut_df.sort_values("OI_SCORE", ascending=False), use_container_width=True
        )
...
with tabs[16]:
    ...
    st.dataframe(
        indices_df[_idx_display_cols] if _idx_display_cols else indices_df, use_container_width=True
    )
...
if False:  # hidden tab[17] content
    ...
    if continuation_df.empty:
        ...
        st.dataframe(
            continuation_df.sort_values("DIST_%", ascending=False), use_container_width=True
        )
    ...
    if three_green_15m_df.empty:
        ...
        st.dataframe(
            three_green_15m_df.sort_values("CANDLE_TIME", ascending=False), use_container_width=True
        )
...
with tabs[18]:
    ...
    with _tc3:
        if _toggles_exist:
            with open(_ALERT_TOGGLES_FILE, "r") as _f_show:
                st.download_button(
    ...
    _TG_ONLY_CATS = {
        ...
        "MACD_BULL", "MACD_BEAR", "INSIDE_BAR", "PANCHAK_RANGE",
        "EXPIRY_ALERT", "HOLIDAY_ALERT", "MTF_SEQ",
...
with tabs[20]:
    ...
    def _astro_cal_alert_sent(event_date, event_type):
        ...
        try:
            if os.path.exists(_alert_dedup_file):
                _dd = pd.read_csv(_alert_dedup_file)
    ...
    def _astro_cal_mark_sent(event_date, event_type):
        ...
        try:
            ...
            _dd.to_csv(_alert_dedup_file, index=False)
        except Exception:
            pass
  ...
  <div style="font-size:12px;color:#aaa;margin-top:6px">
    ...
    if _nrows:
        ...
        st.dataframe(_ndf.style.map(_nrs,subset=["Signal"])
            .set_properties(**{"font-size":"12px"}), use_container_width=True, height=390)
    if _nv:
    ...
    if _forecast_rows:
        ...
        st.dataframe(
            _fdf.style
                ...
                .set_properties(**{"font-size":"12px","text-align":"left"})
                .set_table_styles([{"selector":"th","props":[("font-weight","bold"),("background-color","#1a1a2e"),("color","white")]}]), use_container_width=True,
            height=420,
        )
    ...
    with st.expander("📚 Static Reference — Batch 1: Planetary Signal Rules (click to expand)", expanded=False):
        ...
        st.dataframe(
            _astro_df.style
                ...
                .set_properties(**{"font-size": "12px", "text-align": "left"})
                .set_table_styles([{"selector": "th", "props": [("font-weight", "bold"), ("background-color", "#1a2255"), ("color", "#ffffff")]}]), use_container_width=True,
            height=900,
        )
        ...
        if os.path.exists(_bos_log_path):
            try:
                ...
                if _bos_fallback_log:
                    ...
                    st.dataframe(_bpd.DataFrame(_blog_rows), use_container_width=True,
...
with tabs[22]:
    ...
    if _ib_ss_key not in st.session_state:
        with st.spinner("Scanning Inside Bar patterns… (loading daily candles)"):
            st.session_state[_ib_ss_key] = _load_inside_bars()
    _ib_df = st.session_state[_ib_ss_key]

    # ── Fire Telegram alerts for breakouts (once per stock per day) ──────────
    if not _ib_df.empty and is_market_hours():
        _today_str = date.today().strftime("%Y%m%d")
        for _, _ib_row in _ib_df.iterrows():
            _ib_sym    = _ib_row["Symbol"]
            _ib_status = _ib_row.get("Status", "")
            _ib_ltp    = _ib_row.get("LTP", 0)
            _ib_ph     = _ib_row.get("Parent High", 0)
            _ib_pl     = _ib_row.get("Parent Low", 0)
            _ib_comp   = _ib_row.get("Compression %", 0)
...
with tabs[23]:
    ...
    with _gc2:
        ...
        st.dataframe(pd.DataFrame(_sq9_rows), use_container_width=True, hide_index=True)
    ...
    st.dataframe(pd.DataFrame(_fan_rows), use_container_width=True, hide_index=True)
    ...
    if not _gd_upcoming.empty:
        ...
        st.dataframe(_gd_upcoming, use_container_width=True, hide_index=True)

    st.markdown("**📋 Full Gann Calendar:**")
    st.dataframe(_gd_df, use_container_width=True, hide_index=True)
    ...
    with _gw2:
        ...
        st.dataframe(_gw_show, use_container_width=True, hide_index=True)
...
""")
    ...
    st.markdown(
        ...
        f'&nbsp;&nbsp;|&nbsp;&nbsp;'
        f'{"<span style=\"color:#ff5252\">⚠️ Nifty WEAK — best time for relative strength scan</span>" if _nifty_c < -0.3 else "<span style=\"color:#aaa\">Nifty neutral/positive</span>"}'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    with _t1:
        ...
        if not _mtf_long_df.empty:
            ...
            st.dataframe(_mtf_long_df[_l15_cols], use_container_width=True, hide_index=True,
        ...
        if not _mtf_short_df.empty:
            ...
            st.dataframe(_mtf_short_df[_s15_cols], use_container_width=True, hide_index=True,
        ...
        if not _piind_log_today:
            ...
            st.dataframe(_log_df_show, use_container_width=True, hide_index=True,
    ...
    with _t2:
        ...
        if _mtf_watch_df.empty:
            ...
            st.dataframe(_mtf_watch_df[_watch_show_cols], use_container_width=True,
...
""")
        ...
        if not _cp_data:
            st.info(
                "Click **Run Pattern Scan** to start.  \n"
                "Results are cached for the day — re-runs are instant until midnight.  \n"
                "Scan covers all ~250 stocks on daily candles (4 months of data)."
            )
        else:
