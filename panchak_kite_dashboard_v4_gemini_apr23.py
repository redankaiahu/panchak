...
_LIVE_1H_HIST_N      = 20
_LIVE_1H_CACHE_KEY   = "_LIVE_1H_CANDLES"
...
def _run_smc_intelligence(_kite_inst, oi_intel_dict):
    if not _SMC_ENGINE_OK or not _kite_inst or not oi_intel_dict:
        return {}
    try:
        c15 = fetch_nifty_candles_kite(_kite_inst, interval="15minute", days=5)
        c1h = fetch_nifty_candles_kite(_kite_inst, interval="60minute", days=15)
        if not c15:
            return {}
...
# =============================================================================
# 🌟 PERSONAL FINANCE FORECAST ENGINE  — FULLY DYNAMIC
# Birth: 02-May-1985, 8:50 AM IST, Guntur, AP (16.3067°N, 80.4518°E)
# Sources: P.K. Vasudev "Vedic Astrology in Money Matters",
#          KP Krishnamurti System, Banerjee "Stock Market Astrology",
#          W.D. Gann "Master Commodities Course", Vimshottari Dasha system
#
# Dynamic behaviour:
#   • Today's forecast uses live Swiss Ephemeris positions (swe.calc_ut) —
#     updates every page refresh via st_autorefresh (same cadence as rest of dashboard)
#   • Moon nakshatra/sub-lord recomputed from CURRENT time, not fixed 9:15 AM
#   • Slow planets (Jupiter, Saturn, Rahu) cached 1 h — they barely move intraday
#   • Future-date table cached 1 h per date — daily positions change < 1° daily
#   • Natal chart uses @st.cache_resource — computed ONCE per process lifetime
#   • Score automatically changes when Moon changes nakshatra / KP sub-lord
# =============================================================================

_BIRTH_LAT = 16.3067   # Guntur, AP
_BIRTH_LON = 80.4518

@st.cache_resource
def _get_natal_chart():
    """Compute natal chart ONCE for 02-May-1985, 8:50 AM IST, Guntur.
    @st.cache_resource persists for the whole process — never recomputes."""
    birth_dt_utc = datetime(1985, 5, 2, 3, 20, 0)   # 8:50 IST = 3:20 UTC
    jd_b = _jd(birth_dt_utc)

    # Use Swiss Ephemeris for accurate natal positions
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    def _swe_sid(pid):
        r = swe.calc_ut(jd_b, pid, swe.FLG_SIDEREAL)[0]
        return r[0] % 360

    m_b  = _swe_sid(swe.MOON)
    s_b  = _swe_sid(swe.SUN)
    v_b  = _swe_sid(swe.VENUS)
    ma_b = _swe_sid(swe.MARS)
    j_b  = _swe_sid(swe.JUPITER)
    sa_b = _swe_sid(swe.SATURN)
    me_b = _swe_sid(swe.MERCURY)
    r_b  = _swe_sid(swe.MEAN_NODE)
    k_b  = (r_b + 180) % 360

    # Sidereal Ascendant at Guntur
    try:
        h_res  = swe.houses_ex(jd_b, _BIRTH_LAT, _BIRTH_LON, b'A')
        asc_lon = (h_res[0][0] - _lahiri(jd_b)) % 360
    except Exception:
        asc_lon = 66.0   # fallback ~Gemini
    lagna_idx = int(asc_lon / 30)

    def _house(plon):
        return (int(plon / 30) - lagna_idx) % 12 + 1

    natal_houses = {
        "Sun":     _house(s_b),  "Moon":    _house(m_b),
        "Mars":    _house(ma_b), "Mercury": _house(me_b),
        "Jupiter": _house(j_b),  "Venus":   _house(v_b),
        "Saturn":  _house(sa_b), "Rahu":    _house(r_b),
        "Ketu":    _house(k_b),
    }

    # Vimshottari dasha timeline
    _KP_ORD = ["Ketu","Venus","Sun","Moon","Mars","Rahu","Jupiter","Saturn","Mercury"]
    _KP_YRS = {"Ketu":7,"Venus":20,"Sun":6,"Moon":10,"Mars":7,
               "Rahu":18,"Jupiter":16,"Saturn":19,"Mercury":17}
    nak_idx      = int(m_b / (360/27))
    moon_nak_lord = _NAK_LORDS[nak_idx]
    nak_span     = 360.0/27.0
    frac_elapsed = (m_b - nak_idx * nak_span) / nak_span
    d_start_idx  = _KP_ORD.index(moon_nak_lord)
    remaining_yrs = _KP_YRS[moon_nak_lord] * (1.0 - frac_elapsed)

    dasha_timeline = []
    cur = date(1985, 5, 2)
    first_end = cur + timedelta(days=remaining_yrs * 365.25)
    dasha_timeline.append((moon_nak_lord, cur, first_end))
    cur = first_end
    for i in range(1, 10):
        p = _KP_ORD[(d_start_idx + i) % 9]
        end = cur + timedelta(days=_KP_YRS[p] * 365.25)
        dasha_timeline.append((p, cur, end))
        cur = end

    return {
        "jd_b": jd_b,
        "lagna_lon": asc_lon, "lagna_idx": lagna_idx, "lagna_rashi": _RASHIS[lagna_idx],
        "moon_lon": m_b, "sun_lon": s_b,  "venus_lon": v_b,  "mars_lon": ma_b,
        "jup_lon":  j_b, "sat_lon": sa_b, "merc_lon":  me_b, "rahu_lon": r_b, "ketu_lon": k_b,
        "natal_houses": natal_houses,
        "moon_nak": _NAKS[nak_idx], "moon_nak_lord": moon_nak_lord,
        "moon_rashi": _RASHIS[int(m_b/30)],
        "dasha_timeline": dasha_timeline,
        "kp_ord": _KP_ORD, "kp_yrs": _KP_YRS,
    }


def _get_current_dasha(ref_date=None):
    """Return (mahadasha, antardasha, pratyantar) lords for any date — computed fresh every call."""
    nc = _get_natal_chart()
    ref_date = ref_date or date.today()
    _KP_ORD = nc["kp_ord"]
    _KP_YRS = nc["kp_yrs"]
    maha = (nc["moon_nak_lord"], date(1985,5,2), date(1985,5,2))
    for (p, ds, de) in nc["dasha_timeline"]:
        if ds <= ref_date < de:
            maha = (p, ds, de)
            break
    m_lord, m_start, m_end = maha
    m_idx = _KP_ORD.index(m_lord)
    m_total_days = max((m_end - m_start).days, 1)
    m_elapsed    = (ref_date - m_start).days
    cum = 0.0
    antar_lord  = m_lord
    antar_start = m_start
    antar_end   = m_end
    for i in range(9):
        al   = _KP_ORD[(m_idx + i) % 9]
        frac = _KP_YRS[al] / 120.0
        ad   = m_total_days * frac
        if m_elapsed < cum + ad:
            antar_lord  = al
            antar_start = m_start + timedelta(days=cum)
            antar_end   = m_start + timedelta(days=cum + ad)
            break
        cum += ad
    a_total = max((antar_end - antar_start).days, 1)
    a_elapsed = (ref_date - antar_start).days
    a_idx = _KP_ORD.index(antar_lord)
    prat_lord = antar_lord
    cum2 = 0.0
    for i in range(9):
        pl   = _KP_ORD[(a_idx + i) % 9]
        frac = _KP_YRS[pl] / 120.0
        pd   = max(a_total * frac, 1)
        if a_elapsed < cum2 + pd:
            prat_lord = pl
            break
        cum2 += pd
    return m_lord, antar_lord, prat_lord


# Financial houses per P.K. Vasudev + KP system
_PROFIT_HOUSES = {2, 6, 10, 11}
_LOSS_HOUSES   = {8, 12}
_DASHA_FINANCE = {
    "Sun":0, "Moon":2, "Mars":-1, "Mercury":1,
    "Jupiter":3, "Venus":3, "Saturn":-2, "Rahu":-2, "Ketu":-1,
}


def _get_live_transit_positions():
    """Get LIVE sidereal planet positions via Swiss Ephemeris — called every refresh for today."""
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    jd_now = _get_jd_now()
    def _s(pid):
        return swe.calc_ut(jd_now, pid, swe.FLG_SIDEREAL)[0][0] % 360
    return {
        "Moon":    _s(swe.MOON),
        "Sun":     _s(swe.SUN),
        "Mercury": _s(swe.MERCURY),
        "Venus":   _s(swe.VENUS),
        "Mars":    _s(swe.MARS),
        "Jupiter": _s(swe.JUPITER),
        "Saturn":  _s(swe.SATURN),
        "Rahu":    _s(swe.MEAN_NODE),
        "jd":      jd_now,
    }


@st.cache_data(ttl=3600, show_spinner=False)
def _get_slow_transit_positions(d_date):
    """Slow-moving planets cached 1 h — accurate enough for daily forecast."""
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    dt_utc = datetime(d_date.year, d_date.month, d_date.day, 3, 45)
    jd_t   = _jd(dt_utc)
    def _s(pid):
        return swe.calc_ut(jd_t, pid, swe.FLG_SIDEREAL)[0][0] % 360
    return {
        "Jupiter": _s(swe.JUPITER),
        "Saturn":  _s(swe.SATURN),
        "Rahu":    _s(swe.MEAN_NODE),
...
def _kp_sublord_from_lon(lon):
    """Compute KP sub-lord for any sidereal longitude (real-time capable)."""
    _KP_ORDER = ["Ketu","Venus","Sun","Moon","Mars","Rahu","Jupiter","Saturn","Mercury"]
    _KP_YRS   = {"Ketu":7,"Venus":20,"Sun":6,"Moon":10,"Mars":7,
                 "Rahu":18,"Jupiter":16,"Saturn":19,"Mercury":17}
    _NAK_SPAN = 360.0 / 27.0
    nak_idx   = int(lon / _NAK_SPAN)
    nak_lord  = _NAK_LORDS[nak_idx]
    pos       = (lon - nak_idx * _NAK_SPAN) / _NAK_SPAN
    start     = _KP_ORDER.index(nak_lord)
    cum = 0.0
    sub = nak_lord
    for i in range(9):
...
def _score_from_transits(nc, tr_m, tr_j, tr_v, tr_sa, tr_r, tr_ma, tr_su, tr_me,
                          forecast_date, day_data=None):
    """
    Core scoring function — called by both live (today) and cached (future dates) paths.
    Returns (score, conviction_pts, factors, maha, antar, prat).
    """
    li = nc["lagna_idx"]
    def _h(lon): return (int(lon / 30) - li) % 12 + 1
    ...
    score = 0; cp = 0; factors = []

    # ── L1: Jupiter transit (Vasudev: biggest wealth indicator) ─────
    if tr_j_h in _PROFIT_HOUSES:
        pts = 18 if tr_j_h == 11 else 12
        score += pts; cp += 2
        factors.append(f"✅ L1-Jup: Transit Jupiter H{tr_j_h} ({_RASHIS[int(tr_j/30)]}) — financial expansion (+{pts})")
    elif tr_j_h in _LOSS_HOUSES:
        pts = -15 if tr_j_h == 12 else -12
        score += pts; cp += 2
        factors.append(f"⚠️ L1-Jup: Transit Jupiter H{tr_j_h} ({_RASHIS[int(tr_j/30)]}) — blocked gains ({pts})")

    # ── L1b: Venus transit ───────────────────────────────────────────
    if tr_v_h in _PROFIT_HOUSES:
        score += 10; cp += 1
        factors.append(f"✅ L1-Ven: Transit Venus H{tr_v_h} ({_RASHIS[int(tr_v/30)]}) — wealth inflow (+10)")
    elif tr_v_h in _LOSS_HOUSES:
        score += -8; cp += 1
        factors.append(f"⚠️ L1-Ven: Transit Venus H{tr_v_h} — financial drain (-8)")

    # ── L1c: Mars transit (Banerjee: Mars rules 3/8 = volatility) ───
    if tr_ma_h in {3, 6, 11}:
        score += 5
        factors.append(f"✅ L1-Mars: Transit Mars H{tr_ma_h} — assertive energy, momentum day (+5)")
    elif tr_ma_h in {8, 12}:
        score -= 7; cp += 1
        factors.append(f"⚠️ L1-Mars: Transit Mars H{tr_ma_h} — sudden loss risk (-7)")

    # ── L2: KP Moon sub-lord → natal house (core KP rule) ───────────
    live_moon_sub = _kp_sublord_from_lon(tr_m)
    msl_house = nc["natal_houses"].get(live_moon_sub, 0)
    if msl_house in _PROFIT_HOUSES:
        score += 14; cp += 2
        factors.append(f"✅ L2-KP: Moon sub-lord {live_moon_sub} (natal H{msl_house}) — KP GAIN signal (+14)")
    elif msl_house in _LOSS_HOUSES:
        score -= 14; cp += 2
        factors.append(f"⚠️ L2-KP: Moon sub-lord {live_moon_sub} (natal H{msl_house}) — KP LOSS signal (-14)")
    else:
        factors.append(f"🟡 L2-KP: Moon sub-lord {live_moon_sub} (natal H{msl_house}) — neutral")

    # ── L3: Transit Moon in natal houses (changes every ~2 hrs) ─────
    if tr_m_h in _PROFIT_HOUSES:
        score += 8; cp += 1
        factors.append(f"✅ L3-Moon: Transit Moon H{tr_m_h} ({_NAKS[int(tr_m/(360/27))]}) — positive daily pulse (+8)")
    elif tr_m_h in _LOSS_HOUSES:
        score -= 8
        factors.append(f"⚠️ L3-Moon: Transit Moon H{tr_m_h} ({_NAKS[int(tr_m/(360/27))]}) — negative pulse (-8)")

    # ── L4: Vimshottari Dasha ────────────────────────────────────────
    maha, antar, prat = _get_current_dasha(forecast_date)
    dasha_total = (_DASHA_FINANCE.get(maha,0)*5 +
                   _DASHA_FINANCE.get(antar,0)*3 +
                   _DASHA_FINANCE.get(prat,0)*2)
    score += dasha_total
    if abs(dasha_total) >= 5:
        cp += 2
        tag = "✅" if dasha_total > 0 else "⚠️"
        factors.append(f"{tag} L4-Dasha: {maha}/{antar}/{prat} — period quality ({dasha_total:+d})")

    # ── L5: Saturn / Rahu pressure on natal Moon (Sade Sati) ────────
    nat_m_lon = nc["moon_lon"]
    nat_m_sign_center = (int(nat_m_lon/30)*30 + 15)
    if _ang_dist(tr_sa, nat_m_sign_center) <= 30:
        score -= 10; cp += 2
        factors.append(f"⚠️ L5-SS: Saturn near natal Moon sign — Sade Sati active (-10)")
    if _ang_dist(tr_r, nat_m_lon) <= 12:
        score -= 8; cp += 1
        factors.append(f"⚠️ L5-Rahu: Rahu conjunct natal Moon ({round(_ang_dist(tr_r,nat_m_lon),1)}°) (-8)")

    # ── L5b: Jupiter aspect natal benefics ──────────────────────────
    for pname, pkey in [("Sun","sun_lon"),("Moon","moon_lon"),("Venus","venus_lon")]:
        nat = nc.get(pkey, 0)
        d   = _ang_dist(tr_j, nat)
        if d <= 8:
            score += 12; cp += 2
            factors.append(f"✅ L5b: Transit Jupiter conjunct natal {pname} ({round(d,1)}°) — wealth yoga (+12)")
            break
        if abs(d-120) <= 8:
            score += 8; cp += 1
            factors.append(f"✅ L5b: Transit Jupiter trine natal {pname} — supportive (+8)")
            break

    # ── L6: Gann time cycles from natal Sun/Moon (W.D. Gann) ────────
    for pname, nat_key, tr_lon in [("natal Sun","sun_lon",tr_su), ("natal Moon","moon_lon",tr_m)]:
        nat_lon = nc.get(nat_key, 0)
        ang = (tr_lon - nat_lon) % 360
        if any(abs(ang - a) <= 3 for a in [90, 180, 270]):
            score -= 6
            factors.append(f"⚠️ L6-Gann: Square/opposition to {pname} ({round(ang,1)}°) — reversal risk (-6)")
        elif any(abs(ang - a) <= 3 for a in [60, 120, 240, 300]):
            score += 4
            factors.append(f"✅ L6-Gann: Trine/sextile to {pname} ({round(ang,1)}°) — supportive cycle (+4)")

    # ── L7: General market quality (Vedic day engine) ────────────────
    if day_data:
        day_net = day_data.get("net_score", 0)
        day_pts = max(-10, min(10, day_net * 2))
        score  += day_pts
        if abs(day_pts) >= 4:
            factors.append(f"{'✅' if day_pts>0 else '⚠️'} L7-Market: {day_data.get('overall','')} — ({day_pts:+d})")

    score = max(-100, min(100, score))
    conviction = min(100, cp * 14 + abs(score) // 3)
    return score, conviction, factors, maha, antar, prat


def _make_label(score):
    if score >= 30:  return "💰 HIGH PROFIT DAY",  "#00c853"
    if score >= 12:  return "🟢 PROFIT DAY",        "#69f0ae"
    if score >= 0:   return "🟡 NEUTRAL / WATCH",   "#ffd600"
    if score >= -15: return "🟠 CAUTION/MILD LOSS", "#ff6d00"
    return              "🔴 LOSS DAY / AVOID",   "#d50000"


def _personal_finance_score_live():
    """
    TODAY's forecast — uses LIVE Swiss Ephemeris positions so score updates as
    Moon moves through nakshatras and KP sub-lords change.
    No cache — called fresh every Streamlit refresh.
    """
    nc  = _get_natal_chart()
    pos = _get_live_transit_positions()   # real-time Moon/all planets
    today = datetime.now(IST).date()
    day_data = _vedic_day_analysis(today)

    tr_m  = pos["Moon"];  tr_j  = pos["Jupiter"]; tr_v  = pos["Venus"]
    tr_sa = pos["Saturn"]; tr_r  = pos["Rahu"];   tr_ma = pos["Mars"]
    tr_su = pos["Sun"];   tr_me = pos["Mercury"]

    score, conviction, factors, maha, antar, prat = _score_from_transits(
        nc, tr_m, tr_j, tr_v, tr_sa, tr_r, tr_ma, tr_su, tr_me, today, day_data
    )
    label, color = _make_label(score)

    # Moon nakshatra change ETA (minutes)
    _nak_span = 360.0/27.0
    _m_nak_idx = int(tr_m / _nak_span)
    _m_nak_end = (_m_nak_idx + 1) * _nak_span
    _deg_left  = _m_nak_end - tr_m
    _mins_to_next = round(_deg_left / (13.2 / (24*60)))
    ...
    return {
        ...
        "factors":    factors[:10],
        "dasha":      f"{maha}/{antar}/{prat}",
        "moon_h":     (int(tr_m/30) - nc["lagna_idx"]) % 12 + 1,
        "jup_h":      (int(tr_j/30) - nc["lagna_idx"]) % 12 + 1,
        "day_signal": day_data.get("overall",""),
        "moon_nak":   _NAKS[_m_nak_idx],
        "moon_sub":   _kp_sublord_from_lon(tr_m),
        "mins_to_nak_change": _mins_to_next,
        "computed_at": datetime.now(IST).strftime("%H:%M:%S IST"),
        "is_live":    True,
    }


@st.cache_data(ttl=3600, show_spinner=False)
def _personal_finance_score(forecast_date):
    """
    Future-date forecast — cached 1 h.  Slow planets barely move, acceptable accuracy.
    Do NOT use for today — use _personal_finance_score_live() instead.
    """
    nc = _get_natal_chart()
    slow = _get_slow_transit_positions(forecast_date)
    day_data = _vedic_day_analysis(forecast_date)

    # Moon at market open (9:15 AM IST = 3:45 UTC) for future dates
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    dt_utc = datetime(forecast_date.year, forecast_date.month, forecast_date.day, 3, 45)
    jd_t   = _jd(dt_utc)
    tr_m   = swe.calc_ut(jd_t, swe.MOON, swe.FLG_SIDEREAL)[0][0] % 360

    score, conviction, factors, maha, antar, prat = _score_from_transits(
        nc,
        tr_m, slow["Jupiter"], slow["Venus"], slow["Saturn"], slow["Rahu"],
        slow["Mars"], slow["Sun"], slow["Mercury"],
        forecast_date, day_data,
    )
    label, color = _make_label(score)
    li = nc["lagna_idx"]
    return {
        ...
        "factors":    factors[:8],
        "dasha":      f"{maha}/{antar}/{prat}",
        "moon_h":     (int(tr_m/30) - li) % 12 + 1,
        "jup_h":      (int(slow["Jupiter"]/30) - li) % 12 + 1,
        "day_signal": day_data.get("overall",""),
        "moon_nak":   _NAKS[int(tr_m / (360/27))],
        "moon_sub":   _kp_sublord_from_lon(tr_m),
        "is_live":    False,
    }


def _build_personal_forecast_tg(ts):
    """Build Telegram message for personal finance forecast."""
    d   = ts["date"].strftime("%a %d %b %Y")
    tag = "🔴 LOSS" if ts["score"] < 0 else "🟢 PROFIT" if ts["score"] >= 12 else "🟡 NEUTRAL"
    msg = (
        f"🌟 <b>PERSONAL FINANCE FORECAST</b>\n"
        f"📅 {d}\n{'─'*28}\n"
        f"{tag}  Score: <b>{ts['score']:+d}</b>  Conviction: <b>{ts['conviction']}%</b>\n"
        f"<b>{ts['label']}</b>\n"
        f"Dasha: {ts['dasha']}\n"
        f"Moon → {ts.get('moon_nak','?')} (Sub: {ts.get('moon_sub','?')}) H{ts['moon_h']}\n"
        f"Jupiter H{ts['jup_h']} | Market: {ts['day_signal']}\n{'─'*28}\n"
    )
    for f in ts["factors"][:5]:
        msg += f"{f}\n"
    msg += "\n<i>⚠️ NOT financial advice. Astrology only.</i>"
    return msg


def fire_personal_forecast_alert():
    """Fire personal finance forecast Telegram alert once per calendar day at startup."""
    if _is_tg_disabled("PERSONAL_FORECAST"):
        return
    today = datetime.now(IST).date()
    dedup = f"personal_forecast_{today.strftime('%Y%m%d')}"
    with _TG_DEDUP_LOCK:
        d = _load_tg_dedup()
        if d.get(dedup):
            return
    try:
        ts  = _personal_finance_score_live()
        msg = _build_personal_forecast_tg(ts)
        send_alert_routed("PERSONAL_FORECAST", msg, dedup)
    except Exception as _e:
        print(f"Personal forecast alert error: {_e}")
...
_ALERT_TOGGLE_DEFAULTS = {
    # ── Toast (Browser) Alerts ──
    "toast_TOP_HIGH":       False,
    "toast_TOP_LOW":        False,
    "toast_DAILY_UP":       False,
    "toast_DAILY_DOWN":     False,
    "toast_WEEKLY_UP":      False,
    "toast_WEEKLY_DOWN":    False,
    "toast_MONTHLY_UP":     False,
    "toast_MONTHLY_DOWN":   False,
    "toast_EMA20_50":       False,
    "toast_THREE_GREEN_15M":False,
    "toast_INSIDE_15M_BREAK":False,
    "toast_SEQ_15M_HIGH":   False,
    "toast_SEQ_15M_LOW":    False,
    "toast_SEQ_1H_HIGH":    False,
    "toast_SEQ_1H_LOW":     False,
    "toast_VOL_SURGE_15M":  False,
    "toast_OI_INTEL":       False,
    "toast_OI_30M_SUMMARY": False,
    "toast_KP_ALERTS":      False,
    "toast_LONG_UNWIND":    False,
    "toast_PUT_CRUMBLE":    False,
    "toast_PUT_FLOOR":      False,
    ...
    "toast_ASTRO_ADVANCE":  False,
    "toast_ASTRO_UPDATE":   False,
    "toast_BOS_1H":         False,
    "toast_BOS_HC_ONLY":    False,
    "toast_CHART_PATTERN":  False,
    "toast_GREEN_OPEN_STRUCTURE": False,
    "toast_RED_OPEN_STRUCTURE":   False,
    "toast_BREAK_ABOVE_1H_HIGH":  False,
    "toast_BREAK_BELOW_1H_LOW":   False,
    "toast_YEST_GREEN_OPEN_BREAK":False,
    "toast_YEST_RED_OPEN_LOWER":  False,
    "toast_OEH_OEL_SETUPS":       False,
    "toast_KP_BREAK_15M":         False,
    "toast_COMBINED_ENGINE":      False,
    "toast_HEATMAP_REVERSAL":     False,
    "toast_HEATMAP_ALERT":        False,
    "toast_NIFTY_SLOT_BREAK":     False,
    "toast_BT_ST_TARGET":         False,
    "toast_MACD_BULL":            False,
    "toast_MACD_BEAR":            False,
    "toast_INSIDE_BAR":           False,
    "toast_PANCHAK_RANGE":        False,
    "toast_MTF_SEQ":              False,
    "toast_EXPIRY_ALERT":         False,
    "toast_HOLIDAY_ALERT":        False,
    "toast_tg2_BOS_UP_RAW":       False,
    "toast_tg2_BOS_DOWN_RAW":     False,
    "toast_tg2_CHOCH_UP_RAW":     False,
    "toast_tg2_CHOCH_DOWN_RAW":   False,
    "toast_tg2_SMC_OI_CONFLUENCE_RAW": False,
    "toast_PERSONAL_FORECAST":    False,
    ...
    "email_WEEKLY_UP":      False,
    "email_WEEKLY_DOWN":    False,
    "email_MONTHLY_UP":     False,
    "email_MONTHLY_DOWN":   False,
    "email_EMA20_50":       False,
    "email_THREE_GREEN_15M":False,
    "email_INSIDE_15M_BREAK":False,
    "email_SEQ_15M_HIGH":   False,
    "email_SEQ_15M_LOW":    False,
    "email_SEQ_1H_HIGH":    False,
    "email_SEQ_1H_LOW":     False,
    "email_VOL_SURGE_15M":  False,
    "email_OI_INTEL":       False,
    "email_OI_30M_SUMMARY": False,
    "email_KP_ALERTS":      False,
    "email_LONG_UNWIND":    False,
    "email_PUT_CRUMBLE":    False,
    "email_PUT_FLOOR":      False,
    ...
    "email_ASTRO_ADVANCE":  False,
    "email_ASTRO_UPDATE":   False,
    "email_BOS_1H":         False,
    "email_BOS_HC_ONLY":    False,
    "email_CHART_PATTERN":  False,
    "email_GREEN_OPEN_STRUCTURE": False,
    "email_RED_OPEN_STRUCTURE":   False,
    "email_BREAK_ABOVE_1H_HIGH":  False,
    "email_BREAK_BELOW_1H_LOW":   False,
    "email_YEST_GREEN_OPEN_BREAK":False,
    "email_YEST_RED_OPEN_LOWER":  False,
    "email_OEH_OEL_SETUPS":       False,
    "email_KP_BREAK_15M":         False,
    "email_COMBINED_ENGINE":      False,
    "email_HEATMAP_REVERSAL":     False,
    "email_HEATMAP_ALERT":        False,
    "email_NIFTY_SLOT_BREAK":     False,
    "email_BT_ST_TARGET":         False,
    "email_MACD_BULL":            False,
    "email_MACD_BEAR":            False,
    "email_INSIDE_BAR":           False,
    "email_PANCHAK_RANGE":        False,
    "email_MTF_SEQ":              False,
    "email_EXPIRY_ALERT":         False,
    "email_HOLIDAY_ALERT":        False,
    "email_tg2_BOS_UP_RAW":       False,
    "email_tg2_BOS_DOWN_RAW":     False,
    "email_tg2_CHOCH_UP_RAW":     False,
    "email_tg2_CHOCH_DOWN_RAW":   False,
    "email_tg2_SMC_OI_CONFLUENCE_RAW": False,
    "email_PERSONAL_FORECAST":    False,
    ...
    "tg_DAILY_DOWN":        False,
    "tg_WEEKLY_UP":         False,
    "tg_WEEKLY_DOWN":       False,
    "tg_MONTHLY_UP":        False,
    "tg_MONTHLY_DOWN":      False,
    "tg_EMA20_50":          False,
    "tg_THREE_GREEN_15M":   False,
    "tg_INSIDE_15M_BREAK":  True,
    "tg_SEQ_15M_HIGH":      False,
    "tg_SEQ_15M_LOW":       False,
    "tg_SEQ_1H_HIGH":       False,
    "tg_SEQ_1H_LOW":        True,
    "tg_VOL_SURGE_15M":     True,
    "tg_OI_INTEL":          False,
    "tg_OI_30M_SUMMARY":    False,
    "tg_KP_ALERTS":         True,
    ...
    "tg_KP_BREAK_15M":          False,
    "tg_COMBINED_ENGINE":       False,
    "tg_HEATMAP_ALERT":         True,
    ...
    "tg_tg2_BOS_UP_RAW":        True,
    "tg_tg2_BOS_DOWN_RAW":      True,
    "tg_tg2_CHOCH_UP_RAW":      True,
    "tg_tg2_CHOCH_DOWN_RAW":    True,
    "tg_tg2_SMC_OI_CONFLUENCE_RAW": True,
    "tg_PERSONAL_FORECAST":     True,
    ...
    "route_HEATMAP_ALERT":         "both",
    "route_HEATMAP_REVERSAL":      "both",
    "route_PERSONAL_FORECAST":     "ch1",
    "route_tg2_BOS_UP_RAW":        "ch2",
    "route_tg2_BOS_DOWN_RAW":      "ch2",
    "route_tg2_CHOCH_UP_RAW":      "ch2",
    "route_tg2_CHOCH_DOWN_RAW":    "ch2",
    "toast_YH15":           False,
    "email_YH15":           False,
    "tg_YH15":              True,
    "toast_FOUR_BAR_BUY":   False,
    "email_FOUR_BAR_BUY":   False,
    "tg_FOUR_BAR_BUY":      True,
    "toast_OEH_OEL_LONG":   False,
    "email_OEH_OEL_LONG":   False,
    "tg_OEH_OEL_LONG":      True,
    "toast_OEH_OEL_SHORT":  False,
    "email_OEH_OEL_SHORT":  False,
    "tg_OEH_OEL_SHORT":     True,
    "route_YH15":           "ch1",
    "route_FOUR_BAR_BUY":   "ch1",
    "route_OEH_OEL_LONG":   "ch1",
    "route_OEH_OEL_SHORT":  "ch1",
}
...
def _load_alert_toggles() -> dict:
    ...
    for k, v in saved.items():
        if k not in merged:
            # Dynamically pick up keys from saved file even if missing from defaults
        ...
        if isinstance(default_val, bool) and isinstance(v, bool):
            merged[k] = v
        elif isinstance(default_val, str) and isinstance(v, str):
            merged[k] = v
    return merged

def _save_alert_toggles():
    """
    Write all alert-related session_state values to CACHE/alert_toggles.json.
    Picks up toast_*, email_*, tg_*, and route_* keys.
    """
    try:
        os.makedirs(os.path.dirname(_ALERT_TOGGLES_FILE), exist_ok=True)
        snapshot = {}
        # 1. Start with defaults
        for k, v in _ALERT_TOGGLE_DEFAULTS.items():
            snapshot[k] = st.session_state.get(k, v)
        
        # 2. Pick up any dynamic keys in session_state that match the naming pattern
        for k in st.session_state.keys():
            if k.startswith(("toast_", "email_", "tg_", "route_", "special_stock_alerts")):
...
# ── Personal Finance Forecast: fire Telegram ONCE on first startup each day ──
if not st.session_state.get("_personal_forecast_startup_fired", False):
    st.session_state["_personal_forecast_startup_fired"] = True
    try:
        t = threading.Thread(target=fire_personal_forecast_alert, daemon=True)
...
def _is_email_disabled(category: str) -> bool:
    ...
    return not st.session_state.get(f"email_{category}", False)  # default OFF per doc
...
| 📊 NIFTY OI Intelligence | Spot + PCR + direction + walls + setup | Every 15-min slot |
        ...
        with _tg_test_col2:
            if st.button("📤 Send Test", key="btn_tg_test"):
...
<div class="{_vcrd3}" style="padding:16px">
  ...
  <div style="margin-top:10px;display:flex;gap:12px;flex-wrap:wrap;font-size:12px;color:#aaa">
    ...
    # ═══════════════════════════════════════════════════════════════════
    # 🌟 PERSONAL FINANCE FORECAST - 02 May 1985, 8:50 AM, Guntur AP
    # ═══════════════════════════════════════════════════════════════════
    # 🌟 PERSONAL FINANCE FORECAST — LIVE (updates every dashboard refresh)
    # ═══════════════════════════════════════════════════════════════════
    st.divider()
    st.subheader("🌟 Personal Finance Forecast — Aarya (02 May 1985, 8:50 AM, Guntur)")
    st.caption(
        "🔴 **LIVE** — recomputed on every refresh using Swiss Ephemeris real-time positions.  "
        "Score changes automatically as Moon transits nakshatras/KP sub-lords during the day.  "
        "**Sources:** P.K. Vasudev · KP Krishnamurti · W.D. Gann · Vimshottari Dasha.  "
        "**NOT financial advice.**"
    )

    # ── Natal chart info row ──────────────────────────────────────────
    try:
        _nc = _get_natal_chart()
        _nc_col1, _nc_col2, _nc_col3, _nc_col4 = st.columns(4)
        _nc_col1.metric("Natal Lagna", _nc["lagna_rashi"], "Rising Sign (birth)")
        _nc_col2.metric("Natal Moon", f"{_nc['moon_nak']}", _nc["moon_rashi"])
        _maha, _antar, _prat = _get_current_dasha()
        _nc_col3.metric("Mahadasha / Antar", f"{_maha} / {_antar}", f"Pratyantar: {_prat}")
        _nc_col4.metric("Natal Moon House", f"H{_nc['natal_houses'].get('Moon','?')}", "From lagna")
    except Exception as _ne:
        st.warning(f"Natal chart error: {_ne}")

    # ── TODAY'S LIVE FORECAST ─────────────────────────────────────────
    _today_pf = None
    try:
        _today_pf = _personal_finance_score_live()   # ← live Swiss Ephemeris, no cache
    except Exception as _pfe:
        st.error(f"Live forecast error: {_pfe}")

    if _today_pf:
        _pf_color = _today_pf["color"]
        _pf_score = _today_pf["score"]
        _pf_label = _today_pf["label"]
        _pf_conv  = _today_pf["conviction"]
        _pf_dasha = _today_pf["dasha"]
        _pf_nak   = _today_pf.get("moon_nak", "?")
        _pf_sub   = _today_pf.get("moon_sub", "?")
        _pf_mins  = _today_pf.get("mins_to_nak_change", 0)
        _pf_at    = _today_pf.get("computed_at", "")

        # Big live banner
        st.markdown(
            f"<div style='background:{_pf_color}22;border:2px solid {_pf_color};"
            f"border-radius:12px;padding:18px;margin:8px 0'>"
            f"<span style='color:#aaa;font-size:11px'>🔴 LIVE  {_pf_at}</span><br>"
            f"<h2 style='color:{_pf_color};margin:4px 0;font-size:26px'>{_pf_label}</h2>"
            f"<p style='color:#eee;margin:4px 0;font-size:15px'>"
            f"Score: <b style='color:{_pf_color};font-size:20px'>{_pf_score:+d}</b>/100 &nbsp;|&nbsp; "
            f"Conviction: <b>{_pf_conv}%</b> &nbsp;|&nbsp; Dasha: <b>{_pf_dasha}</b>"
            f"</p>"
            f"<p style='color:#ccc;margin:4px 0;font-size:13px'>"
            f"Moon: <b>{_pf_nak}</b> · KP Sub-lord: <b>{_pf_sub}</b> · "
            f"Nakshatra change in <b>~{_pf_mins} min</b>"
            f"</p>"
            f"</div>",
            unsafe_allow_html=True,
        )
        ...
        # Live factors
        with st.expander("📋 Live Astrological Factors — updates each refresh", expanded=True):
            _fac_cols = st.columns(2)
            for _fi, _fac in enumerate(_today_pf["factors"]):
                _fac_cols[_fi % 2].markdown(f"- {_fac}")
            st.caption(
                f"🌙 Transit Moon: Natal H{_today_pf['moon_h']} | "
                f"♃ Jupiter: Natal H{_today_pf['jup_h']} | "
                f"Market: {_today_pf['day_signal']} | "
                f"Last computed: {_pf_at}"
            )

        # Live Planet positions table
        with st.expander("🪐 Live Transit Positions (Swiss Ephemeris)", expanded=False):
            try:
                _live_pos = _get_live_transit_positions()
                _lp_rows  = []
                for _pn, _pk in [("Moon",swe.MOON),("Sun",swe.SUN),("Mercury",swe.MERCURY),
                                  ("Venus",swe.VENUS),("Mars",swe.MARS),
                                  ("Jupiter",swe.JUPITER),("Saturn",swe.SATURN)]:
                    _lon = _live_pos.get(_pn, 0)
                    _nak_i = int(_lon / (360/27))
                    _h = (_nc["lagna_idx"] and (int(_lon/30) - _nc["lagna_idx"]) % 12 + 1) or "?"
                    _lp_rows.append({
                        "Planet":    _pn,
                        "Lon (°)":   round(_lon, 3),
                        "Sign":      _RASHIS[int(_lon/30)],
                        "Nakshatra": _NAKS[_nak_i],
                        "KP Sub":    _kp_sublord_from_lon(_lon),
                        "Natal H":   _h,
                    })
                st.dataframe(pd.DataFrame(_lp_rows), use_container_width=True)
                st.caption("Positions are sidereal (Lahiri ayanamsa). Natal H = house from your lagna.")
            except Exception as _lpe:
                st.warning(f"Live positions error: {_lpe}")

    # ── 30-DAY PERSONAL FORECAST TABLE ───────────────────────────────
    st.markdown("### 📅 30-Day Personal Finance Calendar")
    st.caption(
        "Future dates use market-open (9:15 AM) Moon position, refreshed every hour.  "
        "Moon Nak column shows which nakshatra Moon is in at market open that day.  "
        "Green = Profit days  |  Red = Loss days  |  Orange = Caution"
    )

    _pf30_rows = []
    _pf30_start = datetime.now(IST).date()
    _pf30_cnt   = 0
    for _pf30_di in range(1, 65):
        if _pf30_cnt >= 30:
            break
        _pf30_d = _pf30_start + timedelta(days=_pf30_di)
        if _pf30_d.weekday() >= 5 or _pf30_d in _get_nse_holidays():
            continue
        try:
            _r = _personal_finance_score(_pf30_d)
        except Exception:
            continue
        _pf30_rows.append({
            "Date":       _pf30_d.strftime("%a %d %b"),
            "Signal":     _r["label"],
            "Score":      _r["score"],
            "Conv %":     _r["conviction"],
            "Dasha":      _r["dasha"],
            "Moon Nak":   _r.get("moon_nak", "?"),
            "KP Sub":     _r.get("moon_sub", "?"),
            "Moon H":     f"H{_r['moon_h']}",
            "Jup H":      f"H{_r['jup_h']}",
            "Market":     _r["day_signal"].replace("🟢","").replace("🔴","").replace("🟡","").strip(),
        })
        _pf30_cnt += 1

    if _pf30_rows:
        _pf30_df = pd.DataFrame(_pf30_rows)

        def _pf30_sig_color(val):
            v = str(val)
            if "HIGH PROFIT" in v: return "background-color:#003300;color:#00ff88;font-weight:700"
            if "PROFIT"      in v: return "background-color:#0a2b0a;color:#69f0ae;font-weight:600"
            if "NEUTRAL"     in v: return "background-color:#2b2200;color:#ffd600"
            if "CAUTION"     in v: return "background-color:#2b1400;color:#ffab40"
            if "LOSS"        in v: return "background-color:#300000;color:#ff5252;font-weight:700"
            return ""

        def _pf30_score_color(val):
            try:
                v = float(val)
                if v >= 30:  return "color:#00e676;font-weight:700"
                if v >= 12:  return "color:#69f0ae"
                if v >= 0:   return "color:#ffd600"
        ...
        st.dataframe(
            _pf30_df.style
                .map(_pf30_sig_color,   subset=["Signal"])
                .map(_pf30_score_color, subset=["Score"])
                .set_properties(**{"font-size": "12px", "text-align": "left"})
                .set_table_styles([{"selector": "th", "props": [
                    ("font-weight","bold"),("background-color","#1a1a2e"),("color","white")
                ]}]),
        ...
        _pf_profit = sum(1 for r in _pf30_rows if r["Score"] >= 12)
        _pf_loss   = sum(1 for r in _pf30_rows if r["Score"] < -15)
        _pf_neut   = len(_pf30_rows) - _pf_profit - _pf_loss
        st.caption(
            f"30-day: 🟢 **{_pf_profit}** Profit | 🔴 **{_pf_loss}** Loss | 🟡 **{_pf_neut}** Neutral  "
            f"— Active dasha: {_maha}/{_antar}  "
            f"— Future dates cached 1 h (slow planets barely move intraday)"
        )
    else:
        st.info("Could not compute 30-day personal forecast.")
...
with tabs[18]:
    ...
    _ALERT_CATEGORIES_UI = [
        ...
        ("⚡ O=L BULLISH GAINERS",                "OEH_OEL_LONG"),
        ("⚡ O=H BEARISH LOSERS",                 "OEH_OEL_SHORT"),
        ("🚀 YH 1.5% Breakout",                   "YH15"),
        ("🟢 4-BAR Reversal Buy",                 "FOUR_BAR_BUY"),
        ...
        # Personal Finance Daily Forecast
        ("🌟 Personal Finance Forecast (Daily Startup)", "PERSONAL_FORECAST"),
    ]
    ...
    _ROUTE_CATEGORIES = [
        ...
        ("🏖️ NSE Holiday Alerts",                 "HOLIDAY_ALERT"),
        ("🔥 HeatMap Reversal (Early Detection)", "HEATMAP_REVERSAL"),
        ("🌟 Personal Finance Forecast",          "PERSONAL_FORECAST"),
        ("📡 Ch2: BOS UP (Raw)",                  "tg2_BOS_UP_RAW"),
        ("📡 Ch2: BOS DOWN (Raw)",                "tg2_BOS_DOWN_RAW"),
        ("📡 Ch2: CHoCH UP (Raw)",                "tg2_CHOCH_UP_RAW"),
        ("📡 Ch2: CHoCH DOWN (Raw)",              "tg2_CHOCH_DOWN_RAW"),
        ("📡 Ch2: SMC+OI Confluence (Raw)",       "tg2_SMC_OI_CONFLUENCE_RAW"),
    ]
