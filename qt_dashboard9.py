...
QMainWindow, QDialog { background: #0b0f1a; }
QWidget { background: #0b0f1a; color: #c8d8f0; }
...
QTabBar::tab:hover:!selected { color: #90b8e0; background: #0d1525; }
...
QTableWidget {
    ...
    color: #c8d8f0;
    font-size: 12px;
    selection-background-color: #1a2d50;
    outline: none;
}
QTableWidget::item {
    padding: 5px 10px;
...
QDoubleSpinBox:hover, QLineEdit:hover { border-color: #2979ff; }
QDoubleSpinBox:focus, QLineEdit:focus { border-color: #2979ff; background: #131d2e; }
...
QPushButton:pressed { background: #0d1f40; }
QPushButton:disabled { color: #243040; border-color: #151f30; }
...
QScrollBar::handle:vertical:hover { background: #2979ff; }
...
QLabel { color: #c8d8f0; background: transparent; }
...
QCheckBox { color: #8aaccc; font-size: 11px; spacing: 6px; }
QCheckBox::indicator {
...
# Greens: bullish / profit / positive
C_GREEN   = "#26de81"   # bright emerald — BUY signals, gains
C_GREEN2  = "#20bf6b"   # slightly deeper — confirmed bull
# Reds: bearish / loss / negative
C_RED     = "#fc5c65"   # vivid coral red — SELL signals, losses
C_RED2    = "#eb3b5a"   # deeper red — confirmed bear
# Accents
C_YELLOW  = "#f7b731"   # amber — ATM, warnings, neutral important
C_BLUE    = "#2979ff"   # electric blue — accent, active, selected
C_CYAN    = "#45aaf2"   # sky blue — secondary info, OI values
C_PURPLE  = "#a55eea"   # violet — CHoCH, special events
C_ORANGE  = "#fd9644"   # orange — approaching levels, near-alerts
# Backgrounds
C_BG      = "#0b0f1a"   # page background
C_BG_CARD = "#0f1520"   # card / panel background
C_BG_ROW  = "#111826"   # table row alternate
C_BG_BULL = "#051a0d"   # bull row tint
C_BG_BEAR = "#1a0508"   # bear row tint
C_BG_ATM  = "#0d1a2e"   # ATM strike row
# Borders & dims
C_BORDER  = "#1a2540"   # standard border
C_BORDER2 = "#223060"   # emphasized border
C_DIM     = "#3d6080"   # dimmed / secondary text
C_MUTED   = "#7a9bb8"   # muted / label text
C_TEXT    = "#c8d8f0"   # primary text
C_TEXT2   = "#e8f4ff"   # bright text / headings
# Glow helpers
C_GLOW_G  = "#26de8133"
C_GLOW_R  = "#fc5c6533"
C_GLOW_B  = "#2979ff33"
# Legacy compat
C_BG_CARD_OLD = "#0d1829"
...
def section_title(text):
    """Consistent section header with accent left-border."""
    lbl = QLabel(f"  {text}")
...
def stat_pill(label: str, value: str, color: str, width: int = 150) -> QFrame:
    """
    Clean stat pill widget used across all panels.
    label = small dim caption, value = large bold colored number/text.
    """
    frame = QFrame()
    ...
    lbl_cap.setStyleSheet(f"color:{C_DIM}; background:transparent; letter-spacing:1px;")
...
def mini_badge(label: str, value: str, color: str) -> QFrame:
    ...
    cap.setStyleSheet(f"color:{C_DIM}; background:transparent; letter-spacing:0.5px;")
...
# Per-day dedup so the same signal doesn't spam every 60 seconds
_tg_dedup: dict = {}   # {key: date_str}

def _tg_send(msg: str, dedup_key: str = None) -> bool:
    """Blocking Telegram send. Call from a background thread."""
...
def _tg_bg(msg: str, dedup_key: str = None):
    """Non-blocking — fires in a daemon thread so UI never freezes."""
    import threading
    threading.Thread(target=_tg_send, args=(msg, dedup_key), daemon=True).start()
...
def _alert_oi(data: dict):
    ...
    icon = "🟢" if is_bull else ("🔴" if "BEAR" in direction else "⚠️")
    msg = (
    ...
    _tg_bg(msg)   # direction change — no dedup (fire every change)
...
def _alert_smc(data: dict):
    ...
    # Alert when signal or score changes significantly (dedup within same day+signal)
    today  = date.today().isoformat()
    ...
    is_bull= "BUY" in signal or "BULL" in signal
    icon   = "🟢" if is_bull else ("🔴" if ("SELL" in signal or "BEAR" in signal) else "⚠️")
    reason_lines = "\n".join(f"  • {r}" for r in reasons) if reasons else "  —"
    msg = (
        f"{icon} <b>SMC+OI Confluence: {signal}</b>  ⏰ {_tg_ts()}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Score: <b>{score:+d}</b>  |  Confluence: <b>{conf}%</b>\n"
        f"LTF Trend: <b>{lk}</b>  |  HTF Trend: <b>{hk}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Reasons:\n{reason_lines}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 Action: <b>{action}</b>\n"
    ...
    _tg_bg(msg, dedup_key=key)
    print(f"[TG] SMC alert → {signal} (score {score:+d})")
...
class OIPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.chain_tbl.setStyleSheet("""
            ...
            QTableWidget::item { padding: 5px 10px; border-bottom: 1px solid #0f1828; }
            QHeaderView::section {
                background: #080c16;
                color: #3d6080;
                ...
                letter-spacing: 1px;
                padding: 6px 10px;
                border: none; border-bottom: 1px solid #1a2540;
            }
        """)
...
class SMCPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.reasons_tbl.setStyleSheet("QTableWidget{alternate-background-color:#0d1525;}")
...
class BosPanel(QWidget):
    ...
    def _make_tbl(self, cols):
        ...
        t.setShowGrid(False)
        t.setStyleSheet("QTableWidget{alternate-background-color:#0d1525;background:#0b0f1a;} QTableWidget::item{padding:5px 10px;border-bottom:1px solid #0f1828;}")
        return t
...
class DbPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.sym_tbl.setStyleSheet("QTableWidget{alternate-background-color:#0d1525;background:#0b0f1a;}")
...
class AutoRefreshBar(QWidget):
    ...
    def __init__(self, interval_secs=AUTO_REFRESH_SECS):
        ...
        self.setStyleSheet(f"background:#060a14;border-top:1px solid #1a2540;")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        brand.setFont(QFont("Segoe UI", 13, QFont.Weight.Bold))
        brand.setStyleSheet("color:#2979ff; letter-spacing:3px; background:transparent;")
        hl.addWidget(brand)

        dot = QLabel("  ●")
        dot.setFont(QFont("Segoe UI", 8))
        dot.setStyleSheet(f"color:{C_DIM}; background:transparent;")
        hl.addWidget(dot)

        sub = QLabel("  TRADING DASHBOARD")
        sub.setFont(QFont("Segoe UI", 9))
        sub.setStyleSheet(f"color:{C_DIM}; letter-spacing:2px; background:transparent;")
        hl.addWidget(sub)
        ...
        self.lbl_spot.setStyleSheet(f"color:{C_TEXT2}; background:transparent;")
        ...
        self.lbl_chg.setStyleSheet(f"color:{C_DIM}; background:transparent;")
        ...
        self.lbl_clock.setStyleSheet(f"color:{C_MUTED}; background:transparent; letter-spacing:0.5px;")
        ...
        # Phase 3 panels
        if SCREENER_OK:
            # Pass kite_getter so no circular import is needed
    ...
    def _on_kite_connected(self):
        ...
        self.lbl_kite.setStyleSheet(f"""
            color:{C_GREEN}; background:{C_GREEN}14;
        ...
        # Ensure screener has fresh kite reference (in case it was created before connect)
        if SCREENER_OK and hasattr(self.screener_panel, 'set_kite_getter'):
            self.screener_panel.set_kite_getter(
                lambda: (
                    KiteSession.get().kite
                    if (KiteSession.get().ok and KiteSession.get().kite is not None)
                    else None
        ...
        # Delay screener scan slightly so Kite session is fully ready
    ...
    def _mark_refreshed(self, ts: str = ""):
        ...
        self.lbl_last_refresh.setStyleSheet(f"color:{C_GREEN};background:transparent;font-size:10px;font-weight:700;")
        # Fade back after 4 seconds
        QTimer.singleShot(4000, lambda: self.lbl_last_refresh.setStyleSheet(
            f"color:{C_DIM};background:transparent;font-size:10px;"))
        # AutoRefreshBar
    ...
    def _tick_clock(self):
        ...
        if mo:
            ...
            self.lbl_market.setStyleSheet(f"""
                color:{C_RED}; background:{C_RED}14;
                border:1px solid {C_RED}35;
                border-radius:12px; padding:3px 14px;
                font-size:10px; font-weight:700;
            """)
