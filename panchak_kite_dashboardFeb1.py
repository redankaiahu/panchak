...
def fetch_yesterday_ohlc(token):
    ...
    if not bars:
        return None, None
    b = bars[0]
    return round(b["open"], 2), round(b["high"],2), round(b["low"],2), round(b["close"],2)
...
def live_data():
    ...
    for s in SYMBOLS:
        ...
        tk = get_token(s)
        yo, yh, yl, yc = fetch_yesterday_ohlc(tk)
...
st_autorefresh(interval=8880_000, key="refresh")
...
for _, r in yh15_df.iterrows():
    ...
    if key not in st.session_state.alert_keys:
        alert_time = datetime.now(IST)   # 🔒 ONLY HERE
...
for _, r in df.iterrows():
    ...
    # =====================================================
    # 🟢 GREEN STRUCTURE (Delayed Break Above YH)
    # =====================================================
    if (
        r["LIVE_OPEN"] > r["YEST_CLOSE"] and
        r["LIVE_OPEN"] < r["YEST_HIGH"] and
        first["high"] <= r["YEST_HIGH"]   # first candle no breakout
    ):
        # Check if ANY later 15-min candle breaks YH
        ...
        if breakout_candle and r["LTP"] > r["YEST_HIGH"]:
            ...
            post_high = max(c["high"] for c in candles if c["date"] >= breakout_candle["date"])
    ...
    # =====================================================
    # 🔴 RED STRUCTURE (Delayed Break Below YL)
    # =====================================================
    if (
        r["LIVE_OPEN"] > r["YEST_LOW"] and
        r["LIVE_OPEN"] < r["YEST_CLOSE"] and
        first["low"] >= r["YEST_LOW"]   # first candle no breakdown
    ):
        ...
        # 🔴 RED STRUCTURE
         ...
         #   post_low = min(c["low"] for c in candles if c["date"] >= breakdown_candle["date"])
        ...
        # 🔴 RED STRUCTURE (Improved Quality)

        if breakdown_candle and r["LTP"] < r["YEST_LOW"]:

            # Get index of breakdown candle
            idx = next(
            ...
            if idx is not None and idx > 0:
                ...
                # Condition 1: Previous candle must be RED
                prev_is_red = prev_candle["close"] < prev_candle["open"]

                # Condition 2: Breakdown candle must be RED
                current_is_red = current_candle["close"] < current_candle["open"]

                # Condition 3: Not fully covered by next candle
                follow_through = False
            if idx + 1 < len(candles):
                next_candle = candles[idx + 1]
                follow_through = next_candle["close"] < current_candle["close"]

            if prev_is_red and current_is_red and follow_through:
...
#green_structure_df = pd.DataFrame(green_rows)
...
# =========================================================
# 🕘 HOURLY OPENING RANGE – WITH RANGE % & POST BREAK %
# =========================================================
...
for _, r in df.iterrows():
    ...
    data = get_hourly_opening_range(tk)
    if not data:
        continue

    hour_high = data["1H_HIGH"]
    hour_low  = data["1H_LOW"]

    # 1️⃣ First Hour Range %
    range_pct = round(
        ((hour_high - hour_low) / hour_low) * 100,
        2
    )
    ...
    if r["LTP"] > hour_high:
        ...
        current_type = "🟢 1H BREAKOUT"

        if data["BREAK_HIGH_TIME"]:
            break_time = data["BREAK_HIGH_TIME"].strftime("%H:%M")

        post_break_pct = round(
            ((r["LTP"] - hour_high) / hour_high) * 100,
            2
        )
    ...
    # 🔴 CURRENT BREAKDOWN
        ...
        current_type = "🔴 1H BREAKDOWN"

        if data["BREAK_LOW_TIME"]:
            break_time = data["BREAK_LOW_TIME"].strftime("%H:%M")

        post_break_pct = round(
            ((hour_low - r["LTP"]) / hour_low) * 100,
            2
        )

    # Only append if currently outside range
    if current_type:
...
new_hourly = detect_new_entries(
    "HOURLY_BREAK",
    hourly_break_df["Symbol"].tolist()
)

notify_all(
    "HOURLY_BREAK",
    "🕘 1H Opening Range Break",
    new_hourly,
    ltp_map
)
...
st.dataframe(
    #alerts_df.drop(columns=["TS"]),
    alerts_df.drop(columns=["TS"], errors="ignore"),
...
# =================================================
        ...
        ###################     Auto-expire old alerts (e.g. after 30 min)
cutoff = datetime.now(IST) - timedelta(minutes=60)
st.session_state.alerts = [
    a for a in st.session_state.alerts if a["TS"] >= cutoff
]
