...
def parse_signal(message_text):
    ...
    # ✅ Additional parsing for formats like "NIFTY 24000 CE"
    if not symbol or not strike or not option_type:
        direct_match = re.search(r'(NIFTY|BANKNIFTY|FINNIFTY|MIDCPNIFTY|[A-Z]+)\s*(\d+)\s*(CE|PE)', msg)
        if direct_match:
            symbol = direct_match.group(1).strip()
            strike = direct_match.group(2).strip()
            option_type = direct_match.group(3).strip()

    # Entry from Range first
    range_match = re.search(r'(\d+(?:\.\d+)?)[-/](\d+(?:\.\d+)?)', msg)
    if range_match:
