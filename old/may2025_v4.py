...
def get_last_week_candle(historical_data, holidays):
    # Step 1: Clean and prepare the index
    historical_data.index = pd.to_datetime(historical_data.index)
    historical_data = historical_data.sort_index()

    # Step 2: Remove weekends
    filtered = historical_data[~historical_data.index.weekday.isin([5, 6])]

    # Step 3: Remove holidays
    if holidays:
        holiday_dates = set(h.date() for h in holidays)
        filtered = filtered[~filtered.index.normalize().isin(pd.to_datetime(list(holiday_dates)))]

    # Step 4: Determine today's date and current weekday
    today = datetime.today().date()
    weekday = today.weekday()  # Monday = 0, Sunday = 6

    # Step 5: Build set of dates in this week (Mon to today)
    start_of_week = today - timedelta(days=weekday)
    dates_to_exclude = {start_of_week + timedelta(days=i) for i in range(weekday + 1)}

    # Align filtering to DataFrame index using .normalize() for date matching
    filtered = filtered[~filtered.index.normalize().isin(dates_to_exclude)]

    print("📉 Filtered Data (after removing this week's trading days):")
    print(filtered.tail(10))

    # Step 6: Take the last 5 valid trading days before this week
    last_5_days = filtered.tail(5)
    print("🗓️ Last 5 Days Used for Weekly Candle:")
    print(last_5_days)

    if last_5_days.empty or len(last_5_days) < 3:
        print("❌ Not enough data for weekly candle.")
        return None

    # Step 7: Build the weekly candle
    weekly_open = last_5_days['open'].iloc[0]
    weekly_high = last_5_days['high'].max()
    weekly_low = last_5_days['low'].min()
    weekly_close = last_5_days['close'].iloc[-1]

    print("✅ Weekly Candle Dates Used:", list(last_5_days.index.normalize().date))
    ...
    if feed_message.get("t") == "tk":  # Token data for OHLC
        ...
        if instrument:
            ...
            if historical_data:
                ...
                YCCon = yesterday_close - 5             # yesterday close for sellcondition1
                YH3Con = three_days_ago_high + 5
                YH15 = yesterday_high + (yesterday_high * 0.011)
                YH15Con = YH15 + (YH15 * 0.0075)
                OL = yesterday_close - 5
                OH = yesterday_close + 5
...
def should_place_order():
    current_time = datetime.now().time()
    cutoff_time_start = datetime.strptime('06:18:05', '%H:%M:%S').time()  # After 9:21 AM
