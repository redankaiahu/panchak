        ...
        if MODE == "paper":
            ...
            try:
                ...
                if trade_data.get('buy_above'):
                    ...
                    filled_qty = int(order_history.get('Fillshares', 0))
                    if filled_qty == 0:
                        print(f"❌ Entry order not filled. Skipping SL & Target orders.")
                        return
                    TICK_SIZE = 0.05
                    # Determine SL and target based on transaction type
                    entry_price = trade_data['entry']
                    if entry_price < 5:
                        stop_loss_price = round_to_tick(entry_price * 0.70)  # 30% SL
                    elif entry_price < 10:
                         stop_loss_price = round_to_tick(entry_price * 0.75)  # 25% SL
                    else:
                        stop_loss_price = round_to_tick(entry_price * 0.80)  # 20% SL
                    trigger_price = round_to_tick(stop_loss_price + 0.10)
                    target_price = round_to_tick(entry_price * 2.0)      # 100% profit    #round_to_two_decimals(target_price_cal)
                    ...
                    threading.Thread(target=monitor_and_cancel_opposite, args=(sl_order_id, target_order_id), daemon=True).start()
                    ...
                    if entry_price < 5:
                        ...
                        stop_loss_price = round_to_tick(entry_price * 0.80)  # 20% SL
                    trigger_price = round_to_tick(stop_loss_price + 0.10)
                    target_price = round_to_tick(entry_price * 2.0)      # 100% profit   
...
def monitor_and_cancel_opposite(sl_order_id, tgt_order_id):
    try:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏳ Starting order monitor thread...")
        while True:
            time.sleep(10)
            sl_status = alice.get_order_history(sl_order_id)
            tgt_status = alice.get_order_history(tgt_order_id)

            if sl_status['Status'].lower() == 'complete':
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚨 SL hit. Cancelling Target order...")
                try:
                    alice.cancel_order(tgt_order_id)
                    print("✅ Target order cancelled.")
                except Exception as e:
                    print("⚠️ Failed to cancel Target:", e)
                break

            if tgt_status['Status'].lower() == 'complete':
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🏁 Target hit. Cancelling SL order...")
                try:
                    alice.cancel_order(sl_order_id)
                    print("✅ SL order cancelled.")
                except Exception as e:
                    print("⚠️ Failed to cancel SL:", e)
                break
    except Exception as mon_err:
        print("Monitor thread error:", mon_err)
