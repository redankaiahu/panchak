...
import logging
import time as time_module  # Standard time module for sleep
from pya3 import *
...
import requests
import pickle
from datetime import time as datetime_time  # Datetime time class
    ...
    if feed_message.get("t") == "tk":  # Token data for OHLC
        ...
        while True:
            ...
            if current_time < datetime_time(9, 30):
                print(f"Waiting for 9:30 AM to process data. Current time: {current_time}")
            ...
            #if historical_data_one.empty or len(historical_data_one) < 15:
                #print("Data not yet complete. Waiting for full 15-minute data...")
                #time.sleep(10)  # Wait and re-fetch
                #time_module.sleep(10)
                #continue
            ...
            if not open_orders:
                ...
                for symbol, details in orders_by_symbol.items():
                    # Proceed if at least a trigger pending order is available
...
def should_place_order():
    ...
    cutoff_time_end = datetime.strptime('15:35:00', '%H:%M:%S').time()    # Before 2:50 PM
