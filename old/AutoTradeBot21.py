...
def parse_signal(message_text):
    ...
    target = None
    buy_above = False
    cmp_value = None  # ✅ To store CMP separately

    msg = clean_message(message_text)

    # 1. Try Stock Name and Strike parsing (OPTION TRADE style)
    stock_match = re.search(r'STOCK NAME\s*-?\s*([A-Z\s]+)', msg)
    if stock_match:
        symbol = stock_match.group(1).strip().replace(' ', '')
    ...
    # 2. Try direct format parsing (eg. BANKNIFTY 54000 PE, #LT 3300 CE)
    if not symbol or not strike or not option_type:
        direct_match = re.search(r'#?([A-Z]+)\s*(\d+)\s*(CE|PE)', msg)
        if direct_match:
            symbol = direct_match.group(1).strip().replace(' ', '')
            strike = direct_match.group(2).strip()
            option_type = direct_match.group(3).strip()

    # 3. Also NIFTY/BANKNIFTY direct
    if not symbol or not strike or not option_type:
        nifty_match = re.search(r'(NIFTY|BANKNIFTY|FINNIFTY|MIDCPNIFTY)\s*(\d+)\s*(CE|PE)', msg)
        if nifty_match:
            symbol = nifty_match.group(1).strip().replace(' ', '')
            strike = nifty_match.group(2).strip()
            option_type = nifty_match.group(3).strip()

    # 4. Parse Range Level Entry
    ...
    # 7. Parse @ entry (NIFTY 24250 CE @ 106)
    if not cmp_value:
    ...
    if entry_from_buy_above:
        ...
        try:
            ...
            if abs(strike_val - cmp_value) / strike_val > 0.05:
                # CMP far from strike → it is Option CMP
                entry = cmp_value
            else:
                # CMP near strike → ignore CMP, entry should have been set already
                pass
        except Exception as e:
            print(f"Error comparing CMP vs Strike: {e}")
    elif cmp_value:
        entry = cmp_value

    # Final Symbol Clean-up: Remove if "CMP" stuck inside symbol
