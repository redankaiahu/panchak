...
def feed_data(message):
    ...
    if feed_message.get("t") == "tk":  # Token data for OHLC
        ...
        if instrument:
            ...
            if symbol_name and ltp is not None:
                ltp_by_symbol[symbol_name] = ltp
            else:
                print(f"Invalid LTP or symbol for {symbol_name}. Skipping.")
            ...
            if not open_orders:
                ...
                for order in open_orders:
                    ...
                    if status in ['open', 'trigger pending', 'complete']:
                        if symbol1 not in orders_by_symbol:
                            orders_by_symbol[symbol1] = {
                                'open': None,
                        ...
                        if status == 'open' and not orders_by_symbol[symbol1]['open']:
                            orders_by_symbol[symbol1]['open'] = {
                                'Prc': order.get('Prc'),
                                'OrderID': sl_order_id,
                                'Qty': qty
                            }
                        elif status == 'trigger pending' and not orders_by_symbol[symbol1]['trigger_pending']:
                            orders_by_symbol[symbol1]['trigger_pending'] = {
                            ...
                            orders_by_symbol[symbol1]['complete'].append({
                ...
                for symbol1, details in orders_by_symbol.items():
                    if details['open'] and details['trigger_pending'] and details['complete']:
                        ...
                        ltp1 = None
                        if symbol1 in ltp_by_symbol:
                            ltp1 = ltp_by_symbol[symbol1]
                            if printed_symbols.get(symbol1) != ltp1:
                                ...
                                #modify_stoploss_orders_data(symbol1, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty )
                                printed_symbols[symbol1] = ltp1                            
                                executor1.submit( modify_stoploss_orders_data,symbol1, ltp1, stoploss_price, target_price, trigger_price, executed_price, trantype, sl_order_id, qty)
                            else:
                                if symbol_name not in ltp_by_symbol:    # Track processed or skipped symbols
                                    if not printed_symbols.get(f"skipped_{symbol1}"):
                                        print(f"Symbol {symbol1} already processed. Skipping further processing.")
                                        printed_symbols[f"skipped_{symbol1}"] = True 
                                    continue  # Skip further processing for this symbol 
                        else:
                            print(f"Symbol {symbol1} not found in ltp_by_symbol. Skipping.")
            ...
            try:
                with ThreadPoolExecutor(max_workers=75) as executor:
                    futures = [
                        ...
                        #executor.submit(modify_stoploss_orders_data,symbol1,ltp1, stoploss_price, target_price, trigger_price, executed_price, trantype, sl_order_id, qty)
                        ]
...
def modify_stoploss_orders_data(symbol1, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    try:
        ...
        print(f"Symbol: {symbol},ltp :{ltp},trantype:{trantype},executed_price:{executed_price},target:{target_price},stoploss:{stoploss_price},trigger:{trigger_price}\n")
