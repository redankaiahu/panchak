...
def parse_ocr_image_for_trade(img_bytes):
    try:
        ...
        for line in lines:
            ...
            # Try to dynamically extract symbol if line ends in NSE FO or similar
            if not symbol and re.search(r"(NSE|NSF)[^\w]*FO", line.upper()):
                parts = line.upper().split()
                if len(parts) >= 2:
                    symbol_candidate = parts[0].strip("₹")
                    if symbol_candidate.isalpha() and 2 <= len(symbol_candidate) <= 8:
                        symbol = symbol_candidate

            if not strike or not option_type:
                match = re.search(r"(\d{3,5})\s*(CE|PE|CC|CF)", line.upper())
                if match:
                    strike = match.group(1)
                    option_type = match.group(2).replace("CC", "CE").replace("CF", "CE")
        ...
        # Fallback: 4-number row (OHLC guess)
        for line in lines:
        ...
        # Final fallback: max of reasonable floats
