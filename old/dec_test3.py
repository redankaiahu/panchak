    ...
    #except Exception as e:
     #   logging.error(f"Error in feed_data: {e}")
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(evaluate_buy_condition_1, historical_data, ltp,open_price,high_price,low_price,volume,ORBL5,ORBH5,ORBL15,ORBH15, condition1, condition2, condition3, condition4, condition5,ORBL15_one,ORBH15_one,ORBL15_two,ORBH15_two,ORBL15_thr,ORBH15_thr,ORBL15_four,ORBH15_four,ORBH5Con, ORBL5Con, ORBH15Con, ORBL15Con, ORBHP, ORBLP, SMA_5, ema_200, ema_50, ema_20, ema_13) ]
...
def monitor_symbol(symbol):
    try:
        # Check if symbol exists in tick_data
        if symbol not in tick_data:
            logging.error(f"No live data found for symbol: {symbol}")
            return
        live_data = tick_data[symbol]
        ltp = live_data["ltp"]
        open_price = live_data["open_price"]
        high_price = live_data["high_price"]
        low_price = live_data["low_price"]
        volume = live_data["volume"]
        instrument = live_data["instrument"]
        #symbol_name = live_data["symbol_name"]
        symbol_name = instrument.symbol
        ORBL5 = live_data["ORBL5"]
        ORBH5 = live_data["ORBH5"]
        ORBL15 = live_data["ORBL15"]
        ORBH15 = live_data["ORBH15"]
        condition1 = live_data["condition1"]
        condition2 = live_data["condition2"]
        condition3 = live_data["condition3"]
        condition4 = live_data["condition4"]
        condition5 = live_data["condition5"]
        ORBL15_one = live_data["ORBL15_one"]
        ORBH15_one = live_data["ORBH15_one"]
        ORBL15_two = live_data["ORBL15_two"]
        ORBH15_two = live_data["ORBH15_two"]
        ORBL15_thr = live_data["ORBL15_thr"]
        ORBH15_thr = live_data["ORBH15_thr"]
        ORBL15_four = live_data["ORBL15_four"]
        ORBH15_four = live_data["ORBH15_four"]
        ORBH5Con = live_data["ORBH5Con"]
        ORBL5Con = live_data["ORBL5Con"]
        ORBH15Con = live_data["ORBH15Con"]
        ORBL15Con = live_data["ORBL15Con"]
        ORBHP = live_data["ORBHP"]
        ORBLP = live_data["ORBLP"]
        SMA_5 = live_data["SMA_5"]
        ema_200 = live_data["ema_200"]
        ema_50 = live_data["ema_50"]
        ema_20 = live_data["ema_20"]
        ema_13 = live_data["ema_13"]
        ...
        logging.error(f"Error processing symbol {symbol}: {exc}")
...
def main_monitor(symbols):
    while True:
        if not symbols:
            logging.error("No valid symbols to monitor.")
            break
