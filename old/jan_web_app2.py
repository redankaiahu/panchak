...
def execute_login_script():
    ...
    try:
        ...
        # Extracting and checking the output for success
        stdout = result.stdout
        if result.returncode == 0 and "Logged in successufly" in stdout:
            login_status_message = "Login Successful, welcome REDANKAIAH UPPALA"
            login_status_color = "green"
        else:
            login_status_message = "Login Failed. Check the logs."
            login_status_color = "red"

        # Debugging: log full output for reference
        print("STDOUT:", stdout)
