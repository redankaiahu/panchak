...
def parse_signal(message_text):
    ...
    if stock_match:
        symbol = stock_match.group(1).strip().replace(' ', '')  # IHCL
    ...
    # Entry from Range Level FIRST (most important)
    range_match = re.search(r'(\d+(?:\.\d+)?)[-/](\d+(?:\.\d+)?)(?:\s*RANGE\s*LEVEL|\s*RANGELEVEL|\s*RANGE|\s*RANGE\s+LEVEL)?', msg)
    if range_match:
        low = float(range_match.group(1))
        high = float(range_match.group(2))
        entry = min(low, high)

    # Fallback: Entry from BUY ABOVE
    if not entry:
        buy_above_match = re.search(r'BUY ABOVE\s*(\d+)', msg)
        if buy_above_match:
            entry = float(buy_above_match.group(1))

    # Fallback: Entry from CMP only if Range not found
    if not entry:
    ...
    if target_match:
        targets = re.findall(r'\d+(\.\d+)?', target_match.group(1))
        if targets:
            target = max(map(float, targets))

    # DEBUG Print
    print(f"[DEBUG] symbol={symbol}, strike={strike}, option_type={option_type}, entry={entry}, sl={sl}, target={target}")

    # Only return if enough data
