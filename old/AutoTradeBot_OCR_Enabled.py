...
def parse_ocr_image_for_trade(img_bytes):
    try:
        ...
        high = None
        for line in lines:
            if not symbol and "HAVELLS" in line.upper():
                symbol = "HAVELLS"
            if not strike and re.search(r"(\d{3,5})\s*(CE|PE)", line.upper()):
                match = re.search(r"(\d{3,5})\s*(CE|PE)", line.upper())
                strike = match.group(1)
                option_type = match.group(2)
            if not expiry and re.search(r"(\d{2}-[A-Z]{3}-\d{2})", line.upper()):
                expiry = re.search(r"(\d{2}-[A-Z]{3}-\d{2})", line.upper()).group(1)
            if not high and "HIGH" in line.upper():
                ...
                if idx + 1 < len(lines):
                    try:
                        ...
                        continue
        if symbol and strike and expiry and high:
            return {
                "symbol": symbol,
                "strike": strike,
