...
def execute_start_script():
    ...
    try:
        ...
        log_message(logging.INFO, "Start script executed successfully.")
    except Exception as e:
        start_status_message = f"Error: {e}"
        start_status_color = "red"
        log_message(logging.ERROR, f"Error in starting script: {e}")
...
def exit_all():
    try:
        ...
        log_message(logging.INFO, "All running scripts terminated forcefully.")
        os._exit(0)  # Forcefully exit the Flask app and Python process
    except Exception as e:
