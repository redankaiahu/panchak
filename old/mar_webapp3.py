...
kpastro_script_name = "kpastro_nifty14.py"
...
def index():
    ...
    <body>
        ...
        <div class="button-container">
            <form action="/kpastro" method="get">
                <button type="submit" class="kpastro-btn">KPAstro</button>
            </form>
        </div>
    </body>
    </html>
...
@app.route("/kpastro", methods=["GET"])
def kpastro_page():
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Select Date for KPAstro</title>
    </head>
    <body>
        <h1>Select a Date for KPAstro Analysis</h1>
        <form action="/kpastro_execute" method="post">
...
@app.route("/kpastro_execute", methods=["POST"])
def execute_kpastro():
    selected_date = request.form.get("date")
    if not selected_date:
        return "Error: No date selected", 400
    try:
        os.chdir(script_dir)
        #process = subprocess.Popen(["python3", kpastro_script_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        #output, error = process.communicate(input=selected_date + "\n")
        process = subprocess.Popen(["python3", kpastro_script_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        output, error = process.communicate(input=selected_date.strip() + "\n")

        return render_template_string('''
        <!DOCTYPE html>
        <html>
        <head>
            <title>KPAstro Output</title>
        </head>
        <body>
            <h1>KPAstro Output for {{ date }}</h1>
            <pre>{{ output }}</pre>
            <pre style="color: red;">{{ error }}</pre>
            <a href="/kpastro">Back</a>
        </body>
        </html>
        ''', date=selected_date, output=output, error=error)
    except Exception as e:
        logger.error(f"Error executing KPAstro script: {e}")
...
def exit_all():
    ...
    try:
        for process in processes:
            subprocess.run(["taskkill", "/PID", str(process.pid), "/F", "/T"], shell=True)
        processes.clear()
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=9080, debug=True, use_reloader=False)