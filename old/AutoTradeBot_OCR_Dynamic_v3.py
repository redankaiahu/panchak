...
def parse_ocr_image_for_trade(img_bytes):
    try:
        ...
        for line in lines:
            # Collect all float numbers for fallback
        ...
        # Fallback: Pick the largest reasonable float from all numbers (but not huge like LTPs ₹1000+)
        if not high and all_floats:
            reasonable_floats = [x for x in all_floats if 1.0 <= x <= 300.0]
            if reasonable_floats:
                high = max(reasonable_floats)
