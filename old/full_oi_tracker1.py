...
def show_and_export(symbol, df, ltp, call_oi, put_oi, call_chg, put_chg, pcr, timestamp):
    ...
    # Include Top 2 Resistance and Support
    export_df = pd.DataFrame({
        ...
        'Top Resistance 1': [top_calls.iloc[0]['Strike Price']],
        'Top Resistance 2': [top_calls.iloc[1]['Strike Price']],
        'Top Support 1': [top_puts.iloc[0]['Strike Price']],
        'Top Support 2': [top_puts.iloc[1]['Strike Price']],
    })
    ...
    filename = os.path.join(save_path, f"oi_summary_{symbol}_{today}.csv")

    # Append with headers only once
