...
async def my_event_handler(event):
    try:
        ...
        group_name = sender.username if sender else ""

        if group_name.lower() not in [g.lower() for g in GROUPS_TO_MONITOR]:
            return  # Ignore if not from target groups

        message_text = event.message.message
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Message received from {group_name}: {message_text}")
