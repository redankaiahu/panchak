...
sq_end_time = datetime_time(15, 20)
sq_start_time1 = datetime_time(11, 5)
sq_end_time1 = datetime_time(15, 20)
place_order_time = datetime_time(9, 30)
place_order_start = datetime_time(9, 31)
place_order_end = datetime_time(14, 40)
...
executor2 = ThreadPoolExecutor(max_workers=2)  # Initialize globally
...
def open_count_calculations():
    ...
    try:
        ...
        if not open_positions:
            return 0  # Return zero as no orders are open
        ...
        if not open_orders or not isinstance(open_orders, list):
            return 0  # Return zero if no orders are open or the format is unexpected
...
def periodic_open_count_calculations():
    ...
    while True:
        ...
        print(f"Open orders count: {order_count}")
...
#cancel_unnecessary_orders_thread1 = threading.Thread(target=square_off_thread, daemon=True, name=square_off_thread)
#cancel_unnecessary_orders_thread1.start()

#executor2.submit( square_off_thread )
...
def evaluate_buy_condition_1(data, ltp,open_price,high_price,low_price,volume,ORBL5,ORBH5,ORBL15,ORBH15,condition1,condition2,condition3, condition4, condition5,ORBL15_one,ORBH15_one,ORBL15_two,ORBH15_two,ORBL15_thr,ORBH15_thr,ORBL15_four,ORBH15_four,ORBH5Con, ORBL5Con, ORBH15Con, ORBL15Con, ORBHP, ORBLP,SMA_5, ema_200, ema_50, ema_20, ema_13, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,latest_max_5,latest_max_120,SMA_Volume_5):
    try:
        if place_order_start <= current_time1 <= place_order_end:
            if (
                ...
                #print(f"BUY cond 1 -- {symbol_name}: day's Open: {open_price}, High: {high_price}, Low: {low_price}, LTP: {ltp}, ORBL5: {ORBL5}, ORBH5: {ORBH5}, ORBH15: {ORBH15}, ORBL15: {ORBL15}")
    ...
    try:
        if place_order_start <= current_time1 <= place_order_end:
            if (
                ...
                #volume > data["yesterday_vol"] * 1.5 and
                ...
                #print(f"Intraday_Magic--{symbol_name}: day's Open: {open_price}, High: {high_price}, Low: {low_price}, LTP: {ltp}, ORBL5: {ORBL5}, ORBH5: {ORBH5}, ORBH15: {ORBH15}, ORBL15: {ORBL15}")
                #print(f"Y_Open: {data['yesterday_open']}, High: {data['yesterday_high']}, Low: {data['yesterday_low']}, Close: {data['yesterday_close']}, volume: {volume}, yesterday_vol:{data['yesterday_vol']}, daybefore_vol: {data['daybefore_vol']}\n")
...
def feed_data(message):
    ...
    if feed_message.get("t") == "tk":  # Token data for OHLC
        ...
        if instrument:
            ...
            low_values15 = [low1, low2, low3, low4, low5, low6, low7, low8, low9, low10, low11, low12, low13, low14, low15]
            high_values15 = [high1, high2, high3, high4, high5, high6, high7, high8, high9, high10, high11, high12, high13, high14, high15]
            ...
            ORBL15 = min(low_values15)
            ORBH15 = max(high_values15)
            ...
            ORBH5Con = float(yesterday_high) + 5
            ORBL5Con = float(yesterday_low) - 5
            ORBH15Con = float(yesterday_high) + 5
            ORBL15Con = float(yesterday_low) - 5
            ...
            try:
                with ThreadPoolExecutor(max_workers=75) as executor:
                    futures = [
                        ...
                        #executor.submit(modify_stoploss_orders_data,ltp1,symbol, stoploss_price, target_price, trigger_price, executed_price, trantype, sl_order_id, qty)
                        ]
...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    try:
        ...
        #print(f"Symbol: {symbol},ltp :{ltp},trantype:{trantype},executed_price:{executed_price},target:{target_price},stoploss:{stoploss_price},trigger:{trigger_price},sl_order_id:{sl_order_id}\n")
        #executed_price1 = executed_price + 5
        instrument = alice.get_instrument_by_symbol("NSE", symbol)
        if not instrument:
    ...
    try:
        ...
        # Monitor Stop Loss Order
            ...
            if sl_order_status and sl_order_status['Status'].lower() == 'complete':
                print(f"===========> {symbol} Stop Loss order executed. Canceling Target order: {target_order_id} - TIME: {current_time1}")
            ...
            if target_order_status and target_order_status['Status'].lower() == 'complete':
                print(f"===========> {symbol} Target order executed. Canceling Stop Loss order: {sl_order_id} - TIME: {current_time1}")
...
def place_order_with_prevention( instrument, transaction_type, price,  symbol_name,condition,open_price,high_price,low_price,volume,ORBL5,ORBH5,ORBL15,ORBH15,yesterday_open,yesterday_high,yesterday_low,yesterday_close,ORBL15_one,ORBH15_one,ORBL15_two,ORBH15_two,ORBL15_thr,ORBH15_thr,ORBL15_four,ORBH15_four,ORBH5Con, ORBL5Con, ORBH15Con, ORBL15Con, ORBHP, ORBLP,SMA_5 ):   
    ...
    #if current_time1 < place_order_time:        #if current_time1 < time(9, 30):
     #  print(f"=======>>>>Time is not 9:30 yet -- {condition} - {transaction_type} - {symbol_name} at {price} -  Time: [{current_time}] ")
      # return
    #order_count = 0
    ...
    if order_count >= 12:
       ...
       print(f"=======>>>>Order count limit reached -- not placing order -- {condition} - {transaction_type} - {symbol_name} at {price} -  Time: [{current_time1}] \n")
       return
    ...
    try:
        ...
        #if target_order_id is not None:  # Check if the target order ID is valid
         #   order_count += 1  # Increment the count for each valid order
...
def squareoff_place_order():    
    current_time1 = datetime.now().time()        # Get current time)
    cutoff_time_start = datetime.strptime('15:05:05', '%H:%M:%S').time()  # After 15:05 PM
    cutoff_time_end = datetime.strptime('15:18:00', '%H:%M:%S').time()    # Before 15:18 PM
    
    #if current_time >= cutoff_time_start and current_time <= cutoff_time_end:   # Check if current time is within the allowed time window
    #if cutoff_time_start <= current_time <= cutoff_time_end:
    if sq_start_time <= current_time1 <= sq_end_time:
        return True
    else:
        return False
    #current_time = datetime.now().time()  # Get current time
    #cutoff_time_start = time(15, 5, 5)   # 15:05:05 PM
    #cutoff_time_end = time(15, 18, 0)    # 15:18:00 PM

    # Check if current time is within the allowed time window
    #return cutoff_time_start <= current_time <= cutoff_time_end
    #result = cutoff_time_start <= current_time <= cutoff_time_end
    #print(f"Within cutoff range: {result}")
    #return result
...
def exit_after_TENFourteen():
    ...
    while True:
        ...
        #cutoff_time = datetime.strptime('09:21:00', '%H:%M:%S').time()
        # Define cutoff times
        cutoff_time_start = datetime.strptime('06:00:50', '%H:%M:%S').time()  # After 9:21 AM
        cutoff_time_end = datetime.strptime('15:29:50', '%H:%M:%S').time()    # Before 2:50 PM

        # Check if current time is within the allowed time window
        #if current_time >= cutoff_time_start and current_time <= cutoff_time_end:
        if current_time < cutoff_time_start or current_time > cutoff_time_end:
...
def main_loop():
    ...
    with websocket_lock:
        ...
        while True:
            ...
            elif not squareoff_place_order():
                print("===> Calling square off positions function  <===","current_time:",current_time)
                try:
                    square_off_positions()  # Ensure this function is properly defined
                except Exception as e:
                    print(f"Error in square_off_positions: {e}")
                #sleep_module.sleep(60)  # Sleep for 60 seconds before rechecking
                time_module.sleep(60)  # Correct

                continue  # Recheck again after sleep
...
def handle_exit(signal_received, frame):
    ...
    """
    print("\nKeyboardInterrupt detected. Terminating all threads and exiting...")
    terminate_flag.set()  # Signal all threads to terminate
    sys.exit(0)
...
def example_thread():
    """
    Example thread that runs until terminated.
    """
