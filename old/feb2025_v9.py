    ...
    if feed_message.get("t") == "tk":  # Token data for OHLC
        ...
        if instrument:
            ...
            if historical_data:
                ...
                YH15 = yesterday_high + (yesterday_high * 0.015)
                    ...
                    if details['open'] and details['trigger_pending'] and details['complete']:
                        ...
                        if symbol in ltp_by_symbol:
                            ...
                            if printed_symbols.get(symbol) != ltp1:
                                ...
                                executor1.submit( modify_stoploss_orders_data,symbol, ltp1, stoploss_price,  trigger_price,executed_price, trantype, sl_order_id, qty )
...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price,  trigger_price,executed_price, trantype, sl_order_id, qty ):
    global modified_symbols
    if symbol not in modified_symbols:
        modified_symbols[symbol] = {"0.25%": False,"0.5%": False, "1%": False, "2%": False, "3%": False, "5%": False}
    ...
    try:
        ...
        # LTP move thresholds (from executed price)
        move_0_25_percent = executed_price * 0.0025  # 0.25%
        move_0_5_percent = executed_price * 0.005  # 0.5%
        move_1_percent = executed_price * 0.01  # 1%
        move_2_percent = executed_price * 0.02  # 2%
        move_3_percent = executed_price * 0.03  # 3%
        move_5_percent = executed_price * 0.05  # 5%

        # SL move amounts (from executed price)
        sl_move_below_executed_price = executed_price - (executed_price * 0.0029)  # SL need to move below of executed price
        sl_move_0_002_percent = executed_price * 0.0002  # 0.002% SL move when LTP moves 0.5%
        sl_move_0_5_percent = executed_price * 0.0009  # 0.5% SL move when LTP moves 1%
        sl_move_1_percent = executed_price * 0.01  # 1% SL move when LTP moves 2%
        sl_move_2_percent = executed_price * 0.02  # 2% SL move when LTP moves 3%
        sl_move_3_percent = executed_price * 0.03  # 3% SL move when LTP moves 5%

        # Ordered from highest to lowest to ensure proper SL movement
        move_percentages = [
            ("5%", move_5_percent, sl_move_3_percent),
            ("3%", move_3_percent, sl_move_2_percent),
            ("2%", move_2_percent, sl_move_1_percent),
            ("1%", move_1_percent, sl_move_0_5_percent),
            ("0.5%", move_0_5_percent, sl_move_0_002_percent),
            ("0.25%", move_0_25_percent, sl_move_below_executed_price),
        ]

        new_stop_loss = None
        new_trigger_price = None

        for move_name, move_value, sl_move in move_percentages:
            if not modified_symbols[symbol][move_name]:
                if (trantype == "S" and ltp <= executed_price - move_value) or \
                   (trantype == "B" and ltp >= executed_price + move_value):

                    new_stop_loss = executed_price - sl_move if trantype == "S" else executed_price + sl_move
                    new_trigger_price = new_stop_loss - 0.30 if trantype == "S" else new_stop_loss + 0.30

                    modified_symbols[symbol][move_name] = True  # Mark threshold as triggered
                    break  # Stop checking after first unmet condition
        ...
        TICK_SIZE = 0.05
        new_stop_loss = round(float(round_to_tick(new_stop_loss, TICK_SIZE)), 2)
        new_trigger_price = round(float(round_to_tick(new_trigger_price, TICK_SIZE)), 2)
        ...
        try:
            ...
            print(f"======>> SL order modified successfully for {symbol} at {new_stop_loss} and executed price is: {executed_price} - LTP: {ltp} : {modify_order_response}\n")
        except Exception as e:
