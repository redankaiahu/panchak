...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    try:
        ...
        if new_stop_loss is None or new_trigger_price is None:
            print(f"Unable to calculate new SL for {symbol}. Using current values.")
            new_stop_loss, new_trigger_price = stoploss_price, trigger_price
...
def calculate_new_sl(ltp, executed_price, move_percent, adjustment, direction):
    ...
    # Ensure values are significantly different
    new_stop_loss = round(new_stop_loss, 2)
    new_trigger_price = round(new_trigger_price, 2)
    if abs(new_stop_loss - ltp) < 0.05:  # Adjust threshold as needed
        print(f"Calculated SL ({new_stop_loss}) is too close to LTP ({ltp}). Adjusting.")
        new_stop_loss = ltp + 0.1 if direction == "up" else ltp - 0.1
    return new_stop_loss, new_trigger_price
...
def should_place_order():
    ...
    cutoff_time_end = datetime.strptime('15:30:00', '%H:%M:%S').time()    # Before 2:50 PM
