...
sq_end_time = datetime_time(15, 18)
    ...
    try:
        ...
        if trantype == "S":  # Sell Case
            if ltp <= executed_price - move_1_percent:
                new_stop_loss, new_trigger_price = calculate_new_sl(ltp, executed_price, 0.005, 0.30, "down")
            elif ltp <= executed_price - move_2_percent:
                new_stop_loss, new_trigger_price = calculate_new_sl(ltp, executed_price, 0.01, 0.30, "down")
            elif ltp <= executed_price - move_3_percent:
                new_stop_loss, new_trigger_price = calculate_new_sl(ltp, executed_price, 0.02, 0.30, "down")
        else:  # Buy Case
            if ltp >= executed_price + move_1_percent:
                new_stop_loss, new_trigger_price = calculate_new_sl(ltp, executed_price, 0.005, 0.30, "up")
            elif ltp >= executed_price + move_2_percent:
                new_stop_loss, new_trigger_price = calculate_new_sl(ltp, executed_price, 0.01, 0.30, "up")
            elif ltp >= executed_price + move_3_percent:
                new_stop_loss, new_trigger_price = calculate_new_sl(ltp, executed_price, 0.02, 0.30, "up")
        ...
        if (trantype == "S" and ltp < executed_price) or (trantype == "B" and ltp > executed_price):
            try:
                ...
                print(f"======>>  SL order modified successfully for {symbol}-stoploss_price:{stoploss_price}-new_stop_loss:{new_stop_loss} - {transaction_type} : {modify_order_response}\n")
...
def calculate_new_sl(ltp, executed_price, move_percent, adjustment, direction):
    """
    Calculate new stop-loss based on movement percent and adjustment.
    """
    adjustment_factor = 1 + move_percent if direction == "up" else 1 - move_percent
    new_stop_loss = executed_price * adjustment_factor
    new_trigger_price = new_stop_loss + adjustment if direction == "up" else new_stop_loss - adjustment

    # Round to two decimals
    new_stop_loss = round(new_stop_loss, 2)
    new_trigger_price = round(new_trigger_price, 2)

    return new_stop_loss, new_trigger_price
...
def socket_close():
    global socket_opened
    socket_opened = False
    print(f"WebSocket connection closed at {datetime.now().time()}, restarting...")
    
    try:
        if alice is not None:
            alice.stop_websocket()  # Stop the WebSocket gracefully
            print("WebSocket stopped successfully.")
        else:
            print("WebSocket instance is None. Skipping stop.")
    except AttributeError as e:
        print(f"Error: WebSocket object not initialized - {e}")
    except Exception as e:
...
websocket_restart_lock = threading.Lock()
def restart_websocket():
    global socket_opened
    with websocket_restart_lock:
        if not socket_opened:
            print(f"Restarting WebSocket at {datetime.now().time()}...")
            try:
                # Safely stop the WebSocket if initialized
                if alice is not None:
                    alice.stop_websocket()
                    print("WebSocket stopped successfully.")
                else:
                    print("WebSocket instance is None. Skipping stop step.")
                ...
                print(f"Error: WebSocket object is not initialized - {e}")
            except Exception as e:
                print(f"Unexpected error while restarting WebSocket: {e}")
...
def monitor_websocket():
    ...
    while True:
        if not socket_opened:
            print("WebSocket not open, restarting...")
            restart_websocket()
...
def validate_websocket():
    global socket_opened
    if alice is None or not socket_opened:
        print("WebSocket is not active or AliceBlue instance is None.")
        return False
    return True
...
def should_place_order():
    ...
    cutoff_time_end = datetime.strptime('23:30:00', '%H:%M:%S').time()    # Before 2:50 PM
