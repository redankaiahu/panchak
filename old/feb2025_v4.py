...
sq_start_time1 = datetime_time(9, 30)
...
printed_symbols = {}
modified_symbols = {}
executor1 = ThreadPoolExecutor(max_workers=100)  # Initialize globally
...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    if symbol not in modified_symbols:
        modified_symbols[symbol] = {"0.3%": False,"0.5%": False, "1%": False, "1.5%": False, "2%": False, "2.3%": False, "3%": False}
    ...
    try:
        ...
        sl_move_1_5_percent = executed_price * 0.015
        sl_move_1_percent = executed_price * 0.01  # 1% SL move when LTP moves 2%
        sl_move_2_percent = executed_price * 0.02
        sl_move_2_2_percent = executed_price * 0.025  # 2% SL move when LTP moves 3%

        # Ordered from highest to lowest to ensure proper SL movement
        move_percentages = [
            ("3%", move_3_percent, sl_move_2_2_percent),
            ("2.3%", move_2_3_percent, sl_move_2_percent),
            ("2%", move_2_percent, sl_move_1_5_percent),
            ("1.5%", move_1_5_percent, sl_move_1_percent),
            ("1%", move_1_percent, sl_move_0_5_percent),
            ("0.5%", move_0_5_percent, sl_move_0_002_percent),
            ("0.3%", move_0_3_percent, sl_move_0_001_percent),
        ]
...
# Main loop
def main_loop():
    cancel_unnecessary_orders_thread1 = threading.Thread(target=cancel_unnecessary_orders1, daemon=True, name=cancel_unnecessary_orders)
    cancel_unnecessary_orders_thread1.start()
