...
def get_last_week_candle(historical_data, holidays):
    ...
    # Step 2: Remove holidays (match only the date part)
    if holidays:
        holiday_dates = set([h.date() for h in holidays])
        #filtered_data = filtered_data[~pd.Series(filtered_data.index.date).isin(holiday_dates)]
        # Fix line
        filtered_data = filtered_data[~pd.Series(filtered_data.index.date, index=filtered_data.index).isin(holiday_dates)]



    # Step 3: Determine today's weekday (Mon=0, Fri=4)
    today = datetime.today()
    weekday_index = today.weekday()

    # Step 4: Get past 5 trading days (excluding today)
    recent_data = filtered_data[filtered_data.index < today]
    last_5_days = recent_data[-5:]

    # Step 5: Validate
    if last_5_days.empty or last_5_days.shape[0] < 3:
        print("❌ Not enough data for weekly candle.")
        return None

    # Step 6: Calculate candle
