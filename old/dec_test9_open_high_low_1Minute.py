...
def open_count_calculations():
    ...
    try:
        ...
        if isinstance(open_orders, dict):  # Convert dict to list
            open_orders = [open_orders]
        elif not isinstance(open_orders, list):
            print(f"Unexpected open_orders type: {type(open_orders)}. Expected list or dict.")
            return 0
...
def socket_error(msg):
    ...
    threading.Thread(target=main_loop).start()  # Restart WebSocket
...
def main_loop():
    ...
    while True:
        ...
        try:
            ...
            time.sleep(60)  # Delay before restarting    
