    ...
    try:
        ...
        if isinstance(historical_data, pd.DataFrame) and not historical_data.empty:
            ...
            if historical_data.shape[0] >= 3:
                ...
                historical_data['datetime'] = pd.to_datetime(historical_data.index)
                historical_data = historical_data.sort_index()  # Just in case it's not sorted
                #historical_data.set_index('datetime', inplace=True)
                weekly_data = historical_data.resample('W-FRI').agg({
                    'open': 'first',
                    'high': 'max',
                    'low': 'min',
                    'close': 'last'
                ...
                last_week_candle = weekly_data.iloc[-1]
                weekly_open = float(last_week_candle['open'])
                weekly_high = float(last_week_candle['high'])
                weekly_low = float(last_week_candle['low'])
                weekly_close = float(last_week_candle['close'])
                print(historical_data.index.dtype)
                print(f"\n📊 Weekly Candle Data for {symbol}:")
