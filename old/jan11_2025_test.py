...
def load_orders_placed():
    print("in load_orders_placed loop")
    global orders_placed
...
def save_orders_placed():
    print("in save_orders_placed loop")
    global orders_placed
    file_path = get_orders_file_path()
...
def cancel_unnecessary_orders():
    print("in cancel_unnecessary_orders loop")
    current_time1 = datetime.now().time()
    try:
...
def cancel_unnecessary_orders1():
    print("in cancel_unnecessary_orders1 loop")
    current_time1 = datetime.now().time()
...
def square_off_positions():
    print("in square_off_positions loop")
    global order_count
    try:
...
def square_off_thread():
    print("in square_off_thread loop")
    current_time1 = datetime.now().time()
...
def evaluate_buy_condition_1(data,ltp,open_price,high_price,low_price,volume, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,Y_H_C_d,Y_H_L_d,Y_C_L_d,YHCon,YLCon):
    try:
        ...
        if (
            ...
            place_order_with_prevention( instrument, TransactionType.Buy, ltp, ltp, symbol_name,open_price,high_price,low_price,volume,yesterday_open,yesterday_high,yesterday_low,yesterday_close )
            return True
...
def evaluate_sell_condition(data,ltp,open_price,high_price,low_price,volume, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,Y_H_C_d,Y_H_L_d,Y_C_L_d,YHCon,YLCon):
    try:
        ...
        if (
            ...
            #print(f"Y_Open: {data['yesterday_open']}, High: {data['yesterday_high']}, Low: {data['yesterday_low']}, Close: {data['yesterday_close']}, volume: {volume}, yesterday_vol:{data['yesterday_vol']}, daybefore_vol: {data['daybefore_vol']}\n")
            place_order_with_prevention( instrument, TransactionType.Sell, ltp, symbol_name,open_price,high_price,low_price,volume,yesterday_open,yesterday_high,yesterday_low,yesterday_close )
            return True
    except KeyError as e:
...
def feed_data(message):
    ...
    if feed_message.get("t") == "tk":  # Token data for OHLC
        ...
        if instrument:
            ...
            open_orders = alice.get_order_history('')
            #print("in getting details for modify_stoploss_orders_data loop")
            #print(f"DEBUG: open_orders type: {type(open_orders)}, value: {open_orders}")
            if not open_orders:
                print("No orders found. Skipping processing.")
            

            # Check if open_orders is a dictionary indicating an error
            #if isinstance(open_orders, dict) and open_orders.get('stat') == 'Not_Ok':
             #   print(f"Error from get_order_history: {open_orders.get('emsg')}")
              #  return

            # Check if open_orders is a valid list
            #if not isinstance(open_orders, list):
             #   print(f"Error: Expected list, but got {type(open_orders)}. Skipping processing.")
              #  return
                ...
                for symbol, details in orders_by_symbol.items():
                    if details['trigger_pending']:
                        stoploss_price = details['trigger_pending']['Prc']
                        trigger_price = details['trigger_pending']['Trgprc']
                        sl_order_id = details['trigger_pending'].get('OrderID')  # Ensure valid OrderID
                        qty = details['trigger_pending']['Qty']

                        if not sl_order_id:
                            print(f"DEBUG: Missing SL Order ID for {symbol}. Skipping.")
                            continue

                        # Handle missing "open" order gracefully
                        target_price = details['open']['Prc'] if details['open'] else None
                        #if not target_price:
                         #   print(f"INFO: Missing 'open' order for {symbol}. Using only 'trigger pending' details.")

                        # Fetch executed price from complete orders
                        executed_price = ( details['complete'][0]['Avgprc'] if details['complete'] else None )
                        trantype = ( details['complete'][0]['Trantype'] if details['complete'] else None )
...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    print("in  modify_stoploss_orders_data loop")
    ...
    try:
        ...
        if trantype == "S":  # Sell Case
            ...
            if ltp <= executed_price - move_2_percent:
            ...
            if ltp >= executed_price + move_2_percent:
                new_stop_loss = executed_price + (executed_price * 0.01)  # Move to 1% below executed price
                new_trigger_price = new_stop_loss + 0.30
            elif ltp >= executed_price + move_3_percent:
                new_stop_loss = executed_price + (executed_price * 0.02)  # Move to 2% below executed price
                new_trigger_price = new_stop_loss + 0.30
            else:
                return  # No change required for current LTP
        ...
        if (trantype == "S" and ltp < executed_price1) or (trantype == "B" and ltp > executed_price1):
            try:
                modify_order_response = alice.modify_order(
                transaction_type=transaction_type,  #TransactionType.Sell,#transaction_type,
                instrument=instrument,
...
def place_order_with_prevention( instrument, transaction_type, price,  symbol_name,open_price,high_price,low_price,volume,yesterday_open,yesterday_high,yesterday_low,yesterday_close ):   
    global orders_placed, order_tracking, order_count, sl_modify_tracking
    current_time1 = datetime.now().time()
    print("in place_order_with_prevention loop")
    ...
    try:
        ...
        print("-> -> -> -> -> -> -> -> -> -> -> -> -> -> -> -> -> -> -> ->-> -> -> -> -> -> -> ->-> -> -> -> -> -> -> ->")
        print(f" Order placed for {symbol_name}: {transaction_type} at {price} - origOrderId: {order_number} at Time: ... [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ")
        print(f" day's Open: {open_price}, High: {high_price}, Low: {low_price}, LTP: {ltp}")
...
def create_subscription_list(symbols):
    print("in create_subscription_list loop")
...
def load_symbols_from_txt(file_path):
    print("in load_symbols_from_txt loop")
    try:
...
def should_place_order():
    print("in should_place_order loop")
    # Get current time
    current_time = datetime.now().time()
    ...
    # Define cutoff times
    cutoff_time_start = datetime.strptime('08:15:05', '%H:%M:%S').time()  # After 9:21 AM
