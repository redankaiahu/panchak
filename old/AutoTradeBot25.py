...
ltp_by_symbol = {}  # Filled by WebSocket handler globally
...
# Simulate get_ltp using tick data dictionary
def get_ltp_simulated(instrument):
    symbol = instrument.symbol if hasattr(instrument, 'symbol') else None
    if symbol and symbol in ltp_by_symbol:
        return {'ltp': ltp_by_symbol[symbol]}
    else:
        return {'ltp': None}
...
def place_trade(trade_data):
    try:
        ...
        trading_symbol, lot_size = get_trading_symbol_from_nfo(
            ...
            def execute_limit_order():
                try:
                    order = alice.place_order(
                        ...
                        product_type=ProductType.Delivery,  #ProductType.Normal,
                    ...
                    def monitor_and_switch():
                        time.sleep(60)
                        orderbook = alice.get_order_history(order['NOrdNo'])
                        status = orderbook['Status'].lower()
                        if status in ['open', 'trigger pending']:
                            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏳ Order still unfilled. Switching to MARKET.")
                            try:
                                alice.cancel_order(order['NOrdNo'])
                                market_order = alice.place_order(
                                    transaction_type=TransactionType.Buy,
                                    instrument=instrument,
                                    quantity=quantity,
                                ...
                                print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Switched to MARKET Order.")
                            except Exception as me:
                                print(f"⚠️ Error while switching to market order: {me}")
                    threading.Thread(target=monitor_and_switch).start()
                except Exception as oe:
                    print(f"⚠️ Error placing LIVE order: {oe}")

            if trade_data.get('buy_above'):
                def wait_and_place_trigger_order():
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏳ Waiting for CMP >= {trade_data['entry']} to trigger order...")
                    try:
                        while True:
                            ltp_data = get_ltp_simulated(instrument)
                            ltp = ltp_data.get('ltp') if ltp_data else None
                            print(f"[DEBUG] LTP of {trading_symbol} = {ltp}")
                            if ltp and ltp >= trade_data['entry']:
                                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔔 Trigger hit! Placing order...")
                                execute_limit_order()
                                break
                            time.sleep(5)
                    except Exception as le:
                        print(f"⚠️ Error in trigger loop: {le}")

                threading.Thread(target=wait_and_place_trigger_order).start()
            else:
                execute_limit_order()
