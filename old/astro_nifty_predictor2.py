...
def get_full_astro_prediction(date_obj, time_str="09:15:00", lat=19.0760, lon=72.8777):
    ...
    # Prediction logic and reason
    reasons = []
    if lagna_lord in malefics:
        reasons.append(f"Lagna Lord ({lagna_lord}) is malefic")
    if sub_lord in malefics:
        reasons.append(f"Sub Lord ({sub_lord}) is malefic")
    if nak_name in bearish_nakshatras:
        reasons.append(f"Nakshatra ({nak_name}) is bearish")

    if lagna_lord in benefics and sub_lord in benefics and nak_name not in bearish_nakshatras:
        prediction = 'UP'
        reasons.append("Lagna Lord and Sub Lord are benefics & Nakshatra is not bearish")
    elif lagna_lord in malefics or sub_lord in malefics or nak_name in bearish_nakshatras:
        prediction = 'DOWN'
    else:
        prediction = 'SIDEWAYS/UNCLEAR'
        if not reasons:
            reasons.append("Mixed planetary influences")
...
# --- EXECUTION ---
def run_mode(mode='daily'):
    base_date = datetime.date(2025, 3, 18)
    if mode == 'daily':
        dates = [base_date]
    elif mode == 'weekly':
