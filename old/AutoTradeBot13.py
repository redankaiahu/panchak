...
def parse_signal(message_text):
    ...
    # Extract Target (take the first target)
    target_match = re.search(r'TARGETS?-?\s*([\d/]+)', msg)
    if target_match:
        first_target = target_match.group(1).split('/')[0]
...
def place_trade(trade_data):
    try:
        ...
        # 🔥 Use final expiry-based trading symbol fetch
