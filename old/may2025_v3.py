...
def get_last_week_candle(historical_data, holidays):
    ...
    #filtered_data = historical_data[~historical_data.index.weekday.isin([5, 6])]  #original
    filtered_data = historical_data[~historical_data.index.weekday.isin([5, 6])]

    # Step 2: Remove holidays (match date only)
    if holidays:
        holiday_dates = set(h.date() for h in holidays)
        filtered_data = filtered_data[~filtered_data.index.normalize().isin(pd.to_datetime(list(holiday_dates)))]

    # Step 3: Exclude today’s data (if today is trading day)
    today = datetime.today().date()
    filtered_data = filtered_data[filtered_data.index.date < today]
    print("filtered data :", filtered_data)

    # Step 4: Get last 5 valid trading days
    last_5_days = filtered_data.tail(5)
    print("last_5_days :", last_5_days)

    if last_5_days.empty or len(last_5_days) < 3:
        print("❌ Not enough data for weekly candle.")
        return None

    # Step 5: Construct weekly OHLC
