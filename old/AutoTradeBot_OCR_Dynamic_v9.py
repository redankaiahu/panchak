...
def parse_ocr_image_for_trade(img_bytes):
    try:
        ...
        if not high and all_floats:
            reasonable = [x for x in all_floats if 1 <= x <= 200]
            if reasonable:
                high = max(reasonable)
        ...
        if symbol and strike and expiry and high and option_type:
            ...
            if key in processed_symbols:
                log_event(f"⏭️ Duplicate skipped: {key}")
                print(f"⏭️ Already processed today: {key}")
                send_telegram_alert(f"⏭️ Skipped duplicate trade for {key}")
                return None
