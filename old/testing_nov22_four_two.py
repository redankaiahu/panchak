...
sq_end_time = datetime_time(15, 30)
    ...
    try:
        ...
        if open_positions:
            for pos in open_positions:
                ...
                abs_qty = abs(qty)  # Use absolute quantity for placing orders
...
def place_order_with_prevention( instrument, transaction_type, price,  stop_loss, symbol_name,condition,open_price,high_price,low_price,ORBL5,ORBH5,yesterday_open,yesterday_high,yesterday_low,yesterday_close,ORBL15_one,ORBH15_one,ORBL15_two,ORBH15_two,ORBL15_thr,ORBH15_thr,ORBL15_four,ORBH15_four,ORBH5Con, ORBL5Con, ORBH15Con, ORBL15Con, ORBHP, ORBLP ):   
    ...
    try:
        ...
        if transaction_type == TransactionType.Buy:
            stop_loss_price_cal = round_to_tick(executed_price - 4.3, TICK_SIZE)
            #target_price_cal = round_to_tick(executed_price + 10, TICK_SIZE)
            target_price_call = executed_price * 1.05
            target_price_cal = math.floor(target_price_call * 10) / 10
            #stop_loss_price = round_to_two_decimals(stop_loss_price_cal)
            stop_loss_price = float(ORBL5)
            target_price = round_to_two_decimals(target_price_cal)
            #stop_loss_price = executed_price - 4.3
            trigger_price = stop_loss_price + 0.50
            #target_price = executed_price + 5
            sl_transaction_type = TransactionType.Sell
        else:  # Sell order
            stop_loss_price_cal = round_to_tick(executed_price + 4.3, TICK_SIZE)
            #target_price_cal = round_to_tick(executed_price - 10, TICK_SIZE)
            target_price_call = executed_price * 0.95
            target_price_cal = math.floor(target_price_call * 10) / 10
            #stop_loss_price = round_to_two_decimals(stop_loss_price_cal)
            stop_loss_price = float(ORBH5)
            target_price = round_to_two_decimals(target_price_cal)
            #stop_loss_price = executed_price + 4.3
            trigger_price = stop_loss_price - 0.50
            #target_price = executed_price - 5
...
def should_place_order():
    ...
    # Define cutoff times
    cutoff_time_start = datetime.strptime('09:21:00', '%H:%M:%S').time()  # After 9:21 AM
...
def main_loop():
    ...
    while True:
        ...
        if not should_place_order():
            print("It's NOT a Shopping zone before 9:20 AM and after 14:50 PM . Sleeping for 60 seconds to recheck time stamps .","current_time:",current_time)
        ...
        while not socket_opened:
            time.sleep(1)
        print("WebSocket is open. Staying active for 1 hour...")
