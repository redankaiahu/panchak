...
def parse_signal(message_text):
    ...
    # CASE 2: Fallback - Simple Signals

    if not symbol:
        simple_match = re.search(r'#?([A-Z]+)\s*(\d+)\s*(CE|PE)', msg)
        if simple_match:
            symbol = simple_match.group(1).strip().upper()
            strike = simple_match.group(2).strip()
            option_type = simple_match.group(3).strip().upper()
    
    if not symbol:
        nifty_match = re.search(r'(NIFTY|BANKNIFTY)\s*(\d+)\s*(CE|PE)', msg)
        if nifty_match:
            symbol = nifty_match.group(1).strip().upper()
            strike = nifty_match.group(2).strip()
            option_type = nifty_match.group(3).strip().upper()

    # CASE 3: Entry from BUY ABOVE
    if not entry:
        buy_above_match = re.search(r'BUY ABOVE\s*(\d+)', msg)
        if buy_above_match:
            entry = float(buy_above_match.group(1))

    # CASE 4: Entry from CMP
    if not entry:
        cmp_match = re.search(r'CMP\s*-?\s*(\d+(\.\d+)?)', msg)
        if cmp_match:
            entry = float(cmp_match.group(1))

    # CASE 5: Entry from @ (eg. Nifty 24250 CE @ 106)
    if not entry:
        at_price_match = re.search(r'@\s*(\d+)', msg)
        if at_price_match:
            entry = float(at_price_match.group(1))
