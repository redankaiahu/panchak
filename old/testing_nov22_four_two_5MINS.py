...
def square_off_positions():
    ...
    try:
        ...
        if open_positions:
            for pos in open_positions:
                if pos.get('Pcode') != 'MIS':
                    print(f"Skipping {pos['Symbol']} as it is not an MIS position")
                    continue
                symbol = pos['Symbol']
                qty = int(pos['Netqty'])
...
def should_place_order():
    # Get current time
    current_time = datetime.now().time()
    cutoff_time_start = datetime.strptime('09:20:01', '%H:%M:%S').time()  # After 9:21 AM
    cutoff_time_end = datetime.strptime('10:14:50', '%H:%M:%S').time()    # Before 2:50 PM
...
def main_loop():
    ...
    while True:
        ...
        market_close_time = now.replace(hour=10, minute=14, second=59, microsecond=0)
