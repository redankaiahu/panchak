...
login_script = "fully_auto_login_alice.py"
start_script = "dec18_thr_Print3_test.py"
...
def login_script():
    try:
        ...
        os.chdir(script_dir)
        result = subprocess.run(["python3", login_script], shell=True, text=True, capture_output=True)
        if "Logged in successufly" in result.stdout:
            return '''
            <!DOCTYPE html>
            <html>
            <body>
...
def start_script():
    try:
        ...
        os.chdir(script_dir)
        subprocess.Popen(["python3", start_script], shell=True)
