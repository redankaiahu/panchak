...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price,executed_price, trantype, sl_order_id, qty ):
    ...
    if symbol not in modified_symbols:
        modified_symbols[symbol] = {"0.5%": False, "0.75%": False, "1%": False, "2%": False, "3%": False}
    ...
    try:
        ...
        move_0_5_percent = executed_price * 0.002
        move_1_percent = executed_price * 0.005
        move_2_percent = executed_price * 0.01
        move_3_percent = executed_price * 0.02
        # Determine the new stop-loss and trigger price based on movement
        new_stop_loss = None
        new_trigger_price = None
        move_percentages = [
            ("0.5%", move_0_5_percent),
            ("1%", move_1_percent),
            ("2%", move_2_percent),
            ("3%", move_3_percent),
        ]

        for move_name, move_value in move_percentages:
            if not modified_symbols[symbol][move_name]:
                if (trantype == "S" and ltp <= executed_price - move_value) or \
                   (trantype == "B" and ltp >= executed_price + move_value):
                    new_stop_loss = executed_price - move_value if trantype == "S" else executed_price + move_value
                    new_trigger_price = new_stop_loss - 0.30 if trantype == "S" else new_stop_loss + 0.30
                    modified_symbols[symbol][move_name] = True
                    break  # Exit loop after modifying SL for the first unmet condition
