...
def get_trading_symbol(symbol, strike, option_type, nfo_df):
    today = datetime.now().date()

    # Filter matching symbol, strike, and option type
    filtered = nfo_df[
    ...
    if filtered.empty:
        print(f"⚠️ No matching contracts found for {symbol} {strike} {option_type}.")
        return None, None

    # Convert expiry to datetime
    filtered['Expiry Date'] = pd.to_datetime(filtered['Expiry Date'], format='%d-%m-%Y')

    # Filter only future expiry contracts
    future_contracts = filtered[filtered['Expiry Date'].dt.date >= today]

    if future_contracts.empty:
        print(f"⚠️ No future expiry found for {symbol} {strike} {option_type}.")
        return None, None

    # Pick the nearest expiry contract
    next_contract = future_contracts.sort_values('Expiry Date').iloc[0]

    return next_contract['Trading Symbol'], next_contract['Lot Size']
...
def place_trade(trade_data):
    try:
        ...
        full_symbol, quantity = get_trading_symbol(
            trade_data['symbol'], trade_data['strike'], trade_data['option_type'], nfo_df
        )

        if not full_symbol:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Cannot place order: No valid trading symbol found.")
            return

        if MODE == "paper":
            print(f"[{datetime.now().strftime('%H:%M:%S')}] [PAPER MODE] Simulating Buy {full_symbol} at {trade_data['entry']} Qty={quantity}")
        else:
            try:
                instrument = alice.get_instrument_by_symbol('NFO', full_symbol)
                if isinstance(instrument, dict):
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Cannot place LIVE order: {full_symbol} not found in contract master.")
                    return
                ...
                # Start Timer to Auto Switch to MARKET
                order_id = order['Nstordno']
                threading.Timer(60, auto_switch_to_market, args=[order_id, full_symbol]).start()
