...
try:
    ...
    _KITE_OK = False
    class KiteConnect:  # stub so rest of file doesn't crash on name lookup
...
def _build_cps_tg_message(r: dict) -> str:
    ...
    return (
        ...
        f"🔓 Key level: <b>₹{resist:,.2f}</b> (broke {'above' if is_bull else 'below'})\n"
...
_LIVE_1H_CACHE_KEY   = "_live_1h_candles"    # st.session_state key
_LIVE_1H_HIST_KEY    = "_live_1h_hist_done"  # flag: historical fetch done
_LIVE_1H_LOCK        = __import__("threading").Lock()
_LIVE_1H_HIST_N      = 20                    # how many completed 1H candles to prefetch


def _fetch_hist_1h_batch(symbols_batch, result_dict):
    """
    Background thread: fetch last N completed 1H candles for each symbol.
    Populates result_dict[symbol] = list of candle dicts.
    Rate-limited: 0.12s between calls.
    """
    import time as _t
    _today = datetime.now(IST).date()
    _from  = _today - timedelta(days=10)   # 10 days covers ~70+ 1H candles
    ...
    for sym in symbols_batch:
        try:
            ...
            for b in bars:
                _raw_dt = b.get("date", "")
                _dt_str = str(_raw_dt)[:19] if _raw_dt else ""
                candles.append({
                    "datetime": _dt_str,
            ...
            pass
        _t.sleep(0.12)   # Kite rate limit: ~8 calls/sec


def _ensure_hist_1h_loaded(symbols):
    """
    Ensure historical 1H candles are loaded for all symbols.
    Fires ONE background thread if not already loaded this session.
    """
    if st.session_state.get(_LIVE_1H_HIST_KEY):
        return   # already fetched this session

    if _LIVE_1H_CACHE_KEY not in st.session_state:
        st.session_state[_LIVE_1H_CACHE_KEY] = {}

    def _do_fetch():
        _cache = st.session_state.get(_LIVE_1H_CACHE_KEY, {})
        # Fetch in batches of 10 to avoid blocking
        _batch = [s for s in symbols if s not in _cache]
        if _batch:
            _fetch_hist_1h_batch(_batch, _cache)
            # Merge back (thread-safe: we only add new keys)
            with _LIVE_1H_LOCK:
                existing = st.session_state.get(_LIVE_1H_CACHE_KEY, {})
                existing.update(_cache)
                st.session_state[_LIVE_1H_CACHE_KEY] = existing
        st.session_state[_LIVE_1H_HIST_KEY] = True

    __import__("threading").Thread(target=_do_fetch, daemon=True).start()
...
def _load_tg_dedup():
    try:
        with open(_tg_dedup_path(), "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}
...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    if _up_rows and not _is_tg_disabled("TOP_HIGH"):
        ...
        for r in _up_rows:
            ...
            _lines.append(
                f"  🟢 <b>{r['sym']}</b>  LTP: <b>{r['ltp']}</b>"
                f"  TOP_HIGH: {r['top_high']}"
                f"  +{r['gain']}{_chg}"
            )
        _msg = (
            "🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩\n"
    ...
    if _special_up and st.session_state.get("special_stock_alerts", True):
        ...
        _SEP = "\u2501" * 22
        _sp_tg_msg = "\n".join([
            "\U0001f6a8 <b>\u2b50 SPECIAL STOCKS \u2014 TOP_HIGH BREAK</b>",
            f"\u23f0 {_now_str}",
            _SEP,
            _sp_lines,
            _SEP,
            "\U0001f514 All-3 Alert (Toast + Email + Telegram)",
    ...
    if _special_down and st.session_state.get("special_stock_alerts", True):
        ...
        _SEP = "\u2501" * 22
        _sp_tg_msg = "\n".join([
            "\U0001f6a8 <b>\u2b50 SPECIAL STOCKS \u2014 TOP_LOW BREAK</b>",
            f"\u23f0 {_now_str}",
            _SEP,
            _sp_lines,
            _SEP,
...
def _vedic_day_analysis(d_date, hour_ist=9, minute_ist=15):
    ...
    try:
        ...
        if _sun_in_pisces_late and _days_to_aries <= 10:
            G += 3
            signals.append((
                f"☀️ Sun approaching Aries (exaltation) in ~{_days_to_aries} days "
                f"— market front-runs the bull run (STRONGEST signal, astrologer-confirmed)",
                "G"
            ))
        elif sun_rashi == "Aries":
            G += 3
            signals.append(("☀️ Sun in Aries (exaltation) — strongest bullish month of year", "G"))
    except Exception:
        pass
    ...
    try:
        if saturn_rashi in ("Aquarius", "Capricorn"):
            G += 2
            signals.append((
                f"♄ Saturn in own sign {saturn_rashi} — excellent condition, "
    ...
    try:
        if venus_rashi == "Pisces" and dig.get("Venus") != "EXALTED":
            ...
            G += 2
            signals.append(("♀ Venus in Pisces (exaltation) — FMCG/banks/auto very bullish", "G"))
        elif venus_rashi == "Taurus":
            G += 2
            signals.append(("♀ Venus in own sign Taurus — strong financial sector", "G"))
        elif venus_rashi == "Libra":
            G += 1
            signals.append(("♀ Venus in own sign Libra — balanced bullish tendency", "G"))
    except Exception:
        pass
    ...
    try:
        if mars_rashi in ("Aries", "Scorpio") and dig.get("Mars") != "EXALTED":
            ...
            signals.append((
                f"♂ Mars in own sign {mars_rashi} — aggressive directional move; "
    ...
    try:
        if jupiter_rashi in ("Sagittarius", "Pisces") and dig.get("Jupiter") not in ("EXALTED","OWN"):
            G += 2
            signals.append((
                f"♃ Jupiter in own sign {jupiter_rashi} — wealth expansion, "
                f"banking/finance sector strong",
                "G"
            ))
        elif jupiter_rashi == "Cancer" and dig.get("Jupiter") != "EXALTED":
            G += 3
            signals.append(("♃ Jupiter exalted in Cancer — maximum wealth signal", "G"))
    except Exception:
        pass
    ...
    if _jupiter_exalted and G - B < 0:
        ...
        B -= _floor_shift
        signals.append((
            "♃ JUPITER EXALTED FLOOR: Even with bearish nak, Jupiter in Cancer "
...
def _nr_amplifiers(v: dict, root: int) -> list:
    ...
    # Sun-Saturn
    if ss<3:  amps.append((-3,f"☀-♄ Sun-Saturn EXACT {ss:.1f}° — Nifty+Gold all fall"))
    elif ss<8:amps.append((-2,f"☀-♄ Sun-Saturn {ss:.1f}° — strong bearish"))
    # Moon-Saturn Vish Yoga
    if ms<8:  amps.append((-2,f"🌙-♄ Moon-Saturn {ms:.1f}° Vish Yoga — sharp fall risk"))
    # Moon-Ketu
    if mk<12: amps.append((-2,f"🌙-Ketu {mk:.1f}° — banks/Reliance weak"))
    # Gajakesari (bullish)
    if mj<10: amps.append((+2,f"🌙-♃ Gajakesari {mj:.1f}° — strong rally amplifier"))
    elif mj<15:amps.append((+1,f"🌙-♃ Moon-Jupiter {mj:.1f}° — mild rally support"))
    # Venus in Aries = bottom zone
    if ven_r=="Aries": amps.append((+3,"♀ Venus in Aries — BOTTOM ZONE, heavy long bias"))
    # Mercury own sign = bearish for price
    if mer_r in ("Gemini","Virgo"): amps.append((-1,f"☿ Mercury in {mer_r} — reduces share prices"))
    # Retrograde
    if p in retro: amps.append((-1,f"🔄 Root planet {p} retrograde — energy weakened"))
    if "Saturn" in retro and root!=8: amps.append((+1,"♄ Saturn retrograde — restriction easing"))
    # Saturn in Aries = destroys market confidence
    if sat_r=="Aries": amps.append((-2,"♄ Saturn in Aries → market confidence destroyed"))
    # Moon in Saturn signs
    if moon_r in ("Aquarius","Capricorn"): amps.append((-1,f"🌙 Moon in {moon_r} — psychological drag"))
    return amps
...
NSE_HOLIDAYS = {
    ...
    date(2026,11,24),  # Prakash Gurpurb
...
def _get_cache_date() -> date:
    ...
    # NSE Holiday — use previous trading day (NSE_HOLIDAYS defined later,
    # so check is deferred; safe to skip here since _TODAY_STR is recomputed)
    try:
        ...
        pass  # NSE_HOLIDAYS not yet defined — will recompute later
...
def is_market_hours():
    ...
    # NSE Holiday — no alerts on exchange holidays
    if today in NSE_HOLIDAYS:
        return False

    return time(9, 15) <= now < time(15, 30)   # strict < 15:30 — no alerts at/after close
...
def notify_all(category, title, symbols, ltp_map=None):
    ...
    if not _is_tg_disabled(category):
        ...
        _border  = "🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩" if any(
            x in category for x in ("HIGH","UP","GAIN","GREEN","REVERSAL","STRONG")
        ) else "🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥"
        _sym_lines = "\n".join(
            f"  • <b>{s}</b>  LTP: {ltp_map.get(s,'') if ltp_map else ''}"
        ...
        _tg_syms_key = "_".join(str(s) for s in list(symbols)[:5])
        send_telegram_bg(
            _tg_notify_msg,
...
def fire_bt_st_target_alerts(df_live, ltp_map_ref=None):
    ...
    for _r in _bt_hits:
        ...
        _msg = (
            ...
            f"Time   : {_now_str}\n"
            f"BT Lvl : {_r['bt']:,.2f}\n"
            f"TOP HI : {_r['top_high']:,.2f}\n"
            f"LTP    : {_r['ltp']:,.2f}\n"
            f"Above BT  : +{_r['pts_above_bt']:,.2f} pts\n"
            f"Above TOP : +{_r['pts_above_th']:,.2f} pts"
            f"{_gain_txt}{_chg_txt}\n"
            f"STRONG BULL — price above BT\n"
    ...
    for _r in _st_hits:
        ...
        _msg = (
            ...
            f"Time   : {_now_str}\n"
            f"ST Lvl : {_r['st']:,.2f}\n"
            f"TOP LO : {_r['top_low']:,.2f}\n"
            f"LTP    : {_r['ltp']:,.2f}\n"
            f"Below ST  : -{_r['pts_below_st']:,.2f} pts\n"
            f"Below TOP : -{_r['pts_below_tl']:,.2f} pts"
            f"{_gain_txt}{_chg_txt}\n"
            f"BEAR MOMENTUM — price below ST\n"
...
def fire_combined_engine_tg_alert():
    ...
    try:
        ...
        if not _smc_data and _SMC_ENGINE_OK:
            _oi_ref = st.session_state.get("oi_intel") or {}
    ...
    try:
        _oi_data = st.session_state.get("oi_intel") or {}
        if not _oi_data:
            _oi_data = fetch_nifty_oi_intelligence() or {}
        _oi_dir     = str(_oi_data.get("direction", "—"))
        _oi_pcr     = float(_oi_data.get("pcr", 0) or 0)
        _oi_maxpain = int(_oi_data.get("max_pain", 0) or 0)
        _oi_cwall   = int(_oi_data.get("nearest_call_wall", 0) or 0)
        _oi_pfloor  = int(_oi_data.get("nearest_put_floor", 0) or 0)
        _oi_advice  = str(_oi_data.get("advice", "—"))
        _oi_spot    = int(_oi_data.get("spot", 0) or 0)
        _oi_atm     = int(_oi_data.get("atm", 0) or 0)
    except Exception:
        _oi_dir = "—"; _oi_pcr = 0.0; _oi_maxpain = 0
        _oi_cwall = 0; _oi_pfloor = 0; _oi_advice = "—"
        _oi_spot = 0; _oi_atm = 0
    ...
    msg = (
        ...
        f"   {_oi_advice}\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🌙 <b>Astro Engine:</b>  {_astro_sig} ({_astro_int})\n"
...
# Combines: Heatmap weighted score + SMC + OI + Astro + KP sub-lord
# Fires during market hours only. Dedup: one alert per 15-min slot.
# ═══════════════════════════════════════════════════════════════════════
...
def _fire_heatmap_confluence_alert():
    ...
    try:
        # Use same weight tables as the INDICES tab
        _N50_WT_a = {
        ...
        # Build live change map from df + live_map
        _chg_a = {}
        if "Symbol" in df.columns:
            for _, _r in df.iterrows():
                _s = str(_r.get("Symbol",""))
                if _s: _chg_a[_s] = float(_r.get("CHANGE_%", 0) or 0)
        for _s, _lv in live_map.items():
            if _s not in _chg_a:
                _chg_a[_s] = float(_lv.get("CHANGE_%", 0) or 0)

        # Nifty50 weighted score
        _n50_bull_wt = _n50_bear_wt = _n50_impact = 0.0
        ...
        _n50_total_wt = _n50_bull_wt + _n50_bear_wt + max(_n50_flat * 1.0, 0.01)
        _n50_bull_pct = _n50_bull_wt / (_n50_bull_wt + _n50_bear_wt + 0.01) * 100

        # BankNifty weighted score
        _bnk_bull_wt = _bnk_bear_wt = _bnk_impact = 0.0
        ...
        _n50_ltp  = int(ltp_map.get("NIFTY",     0) or 0)
        _bnk_ltp  = int(ltp_map.get("BANKNIFTY", 0) or 0)
        _n50_pts  = round(_n50_impact / 100 * (_n50_ltp or 24315), 0)
        _bnk_pts  = round(_bnk_impact / 100 * (_bnk_ltp or 56086), 0)

    except Exception as _hm_err:
        _n50_sig = "—"; _bnk_sig = "—"
        _n50_bull = _n50_bear = _n50_flat = 0
        _n50_bull_pct = _bnk_bull_pct = 50.0
        _n50_impact = _bnk_impact = 0.0
        _n50_pts = _bnk_pts = 0
        _n50_top_gainers = _n50_top_losers = []
        _bnk_top_gainers = _bnk_top_losers = []
    ...
    # ── C. OI signal ────────────────────────────────────────────────
    try:
        _oi_d    = st.session_state.get("oi_intel") or {}
    ...
    _max_conv = 8  # max possible score (2+2+2+1+1)
    ...
    _tg_msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 <b>HEATMAP — Nifty 50</b> {_n50_sig}\n"
        f"   {_n50_bull}▲ stocks ({_n50_bull_wt:.0f}% wt)  {_n50_bear}▼ ({_n50_bear_wt:.0f}% wt)  {_n50_flat}—\n"
        f"   Wt Impact: <b>{_n50_impact:+.2f}%  ≈ {_n50_pts:+.0f} pts</b>\n"
        f"   🔼 Gainers: {_gainers_n50}\n"
        f"   🔽 Losers : {_losers_n50}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🏦 <b>HEATMAP — BankNifty</b> {_bnk_sig}\n"
        f"   {_bnk_bull}▲ ({_bnk_bull_wt:.0f}% wt)  {_bnk_bear}▼ ({_bnk_bear_wt:.0f}% wt)  {_bnk_flat}—\n"
        f"   Wt Impact: <b>{_bnk_impact:+.2f}%  ≈ {_bnk_pts:+.0f} pts</b>\n"
        f"   🔼 Gainers: {_gainers_bnk}\n"
        f"   🔽 Losers : {_losers_bnk}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🧠 <b>SMC Structure:</b>  {_smc_sig} (score {_smc_sc:+d})\n"
        f"   15m: {_smc_ltf}  |  HTF: {_smc_htf}\n"
        f"   Action: {_smc_act}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📉 <b>OI Engine:</b>  {_oi_dir}  |  PCR: {_oi_pcr:.2f}\n"
        f"   MaxPain: {_oi_mp:,}  Call Wall: {_oi_cw:,}  Put Floor: {_oi_pf:,}\n"
        f"   {_oi_adv}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🌙 <b>Astro:</b>  {_as_sig} ({_as_int})\n"
        f"   Nak: {_as_nak}  |  Tithi: {_as_tit}  |  Score: {_as_sc:+d}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔑 <b>KP:</b>  Sub-lord: {_kp_sub}  |  Time: {_kp_time}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📌 <b>Why this verdict:</b>\n{_reasons_txt}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚠️ <i>HeatMap Confluence Alert. NOT financial advice.</i>\n"
        f"📊📊📊📊📊📊📊📊📊📊"
    )
...
<table style="width:100%;border-collapse:collapse;font-size:13px">
  ...
  <tr><td style="color:#888;padding:4px 0">🌙 Astro</td><td>{_as_sig} ({_as_int}) | {_as_nak} | {_as_tit}</td></tr>
...
INDEX_SYMBOLS = {
    ...
    # Kite uses "NSE:<tradingsymbol>" — these are the EXACT strings Kite expects
...
def fetch_indices_live():
    _index_rows = []
    # Build {display_name: "NSE:<kite_tradingsymbol>"} for batch quote
    index_kite_symbols = {
        name: f"NSE:{kite_ts}"
    ...
    for index_name, (short_key, kite_ts) in INDEX_SYMBOLS.items():
        ...
        try:
            ...
            _, yh, yl, yc, _ = fetch_yesterday_ohlc(tk)
            _index_rows.append({
                "Index": index_name,
                "OPEN": round(o, 2),
                "HIGH": round(h, 2),
...
def fetch_indices_now():
    ...
    all_keys = [f"NSE:{kite_ts}" for _, (short, kite_ts) in INDEX_SYMBOLS.items()]
    try:
        all_quotes = kite.quote(all_keys)
    ...
    if not result.empty:
        try:
            ...
            print(f"[INDICES] Saved {len(result)} rows → {INDICES_CACHE_CSV}")
        except Exception as _sv_e:
            print(f"[INDICES] Save failed: {_sv_e}")
    else:
        print(f"[INDICES] Empty result — CSV not saved")
    return result
...
if indices_df.empty:
    ...
    if _fb is not None:
        indices_df = _fb
        print(f"[INDICES] Loaded from fallback CSV: {len(indices_df)} rows")
    else:
        print(f"[INDICES] No fallback CSV found at {INDICES_CACHE_CSV}")
...
def fetch_nifty_oi_intelligence():
    ...
    try:
        ...
        try:
            ...
            if "expiry" in nifty_fut.columns:
                nifty_fut["expiry_dt"] = pd.to_datetime(nifty_fut["expiry"], errors="coerce")
        ...
        if "expiry" not in nifty_opts.columns:
            return None
        nifty_opts["expiry_dt"] = pd.to_datetime(nifty_opts["expiry"], errors="coerce")
        today_dt = pd.Timestamp.now()
...
# ── Inside Bar auto-load (module level — runs every refresh so Tab 22 is ready) ─
# Pre-populate session_state BEFORE user clicks Tab 22.
# Uses a background thread so it doesn't block the 60s UI refresh.
_IB_SS_KEY_AUTO = f"inside_bar_scan_{date.today()}"
_IB_LOADING_KEY = "_ib_auto_loading"

def _ib_bg_load():
    """Background thread: load inside bar data into session_state."""
    global _IB_BG_DONE
    try:
        # Re-import session_state inside thread (thread-safe read)
        result = _load_inside_bars()
        st.session_state[_IB_SS_KEY_AUTO] = result
    except Exception as _ib_err:
        print(f"[IB-Auto] load error: {_ib_err}")
    finally:
        _IB_BG_DONE = True

if "_IB_BG_DONE" not in dir():
    _IB_BG_DONE = False

if (_IB_SS_KEY_AUTO not in st.session_state and
        not _IB_BG_DONE and
        not st.session_state.get(_IB_LOADING_KEY, False) and
        is_market_hours() and
        kite is not None):
    st.session_state[_IB_LOADING_KEY] = True
    import threading as _ib_th
    _ib_th.Thread(target=_ib_bg_load, daemon=True).start()

# ── MACD auto-load (module level — runs every refresh so Tab 21 is ready) ───────
_MACD_SS_KEY_AUTO = f"macd_scan_{date.today()}"
_MACD_LOADING_KEY = "_macd_auto_loading"

def _macd_bg_load():
    """Background thread: load MACD data into session_state."""
    global _MACD_BG_DONE
    try:
        result = _load_macd_all()
        st.session_state[_MACD_SS_KEY_AUTO] = result
    except Exception as _macd_err:
        print(f"[MACD-Auto] load error: {_macd_err}")
    finally:
        _MACD_BG_DONE = True

if "_MACD_BG_DONE" not in dir():
    _MACD_BG_DONE = False

if (_MACD_SS_KEY_AUTO not in st.session_state and
        not _MACD_BG_DONE and
        not st.session_state.get(_MACD_LOADING_KEY, False) and
        is_market_hours() and
        kite is not None):
    st.session_state[_MACD_LOADING_KEY] = True
    import threading as _macd_th
    _macd_th.Thread(target=_macd_bg_load, daemon=True).start()
...
def _send_tg_structure_alert(category, title, df_in, ltp_map_ref):
    ...
    try:
        ...
        _today_str = datetime.now(IST).strftime("%Y%m%d")
        _now_str   = datetime.now(IST).strftime("%H:%M IST")
        is_bull    = ("GREEN" in category or "ABOVE" in category or
                      ("BREAK" in category and "BELOW" not in category))
        ...
        for s, _ in new_syms[:15]:
            try:
                ltp = ltp_map_ref.get(s, "")
                row = df_in[df_in["Symbol"] == s]
                chg = ""
                brk_time = ""
                extra = ""
                if not row.empty:
                    if "CHANGE_%" in row.columns:
                        chg = f"  {row.iloc[0]['CHANGE_%']:+.2f}%"
                    if "BREAK_TIME" in row.columns:
                        brk_time = f"  ⏱{row.iloc[0]['BREAK_TIME']}"
                    if is_bull and "POST_BREAK_GAIN_%" in row.columns:
                        extra = f"  +{row.iloc[0]['POST_BREAK_GAIN_%']:.2f}% after break"
                    elif not is_bull and "POST_BREAK_DROP_%" in row.columns:
                        extra = f"  -{row.iloc[0]['POST_BREAK_DROP_%']:.2f}% after break"
                lines.append(f"  {icon} <b>{s}</b>  LTP: {ltp}{chg}{brk_time}{extra}")
            except Exception:
...
# ═══════════════════════════════════════════════════════════════════════
# ── Inside Bar scanner function (module-level for auto-load) ─────────────────
def _load_inside_bars():
    rows = []
    today_d = date.today()
    # Use last trading day to avoid partial intraday candle
    ...
    for sym in STOCKS:
        try:
            ...
            pc     = parent["close"]   # parent close
            cc     = child["close"]    # child close  (= yest close)
            co     = child["open"]     # child open   (= yest open)
            if ch <= ph and cl >= pl:
                rng_parent = round(ph - pl, 2)
                rng_child  = round(ch - cl, 2)
                comp_pct   = round(rng_child / rng_parent * 100, 1) if rng_parent > 0 else 0
                # How close is child high to parent high? (proximity %)
                # Lower = child high is tighter against parent high = coiling at resistance
                ch_to_ph_pct  = round((ph - ch) / ph * 100, 2) if ph > 0 else 99.0
                # How close is child low to parent low?
                cl_to_pl_pct  = round((cl - pl) / pl * 100, 2) if pl > 0 else 99.0
                # Child open vs Parent High — key Siemens-style metric
                # Open below PH AND child high near PH = coiling right at resistance
                open_vs_ph_pct = round((ph - co) / ph * 100, 2) if ph > 0 else 99.0
                # Parent candle bias (bullish/bearish/doji)
                ...
                # Open (child) is below Parent High  AND
                # Child High is within 1.5% of Parent High  AND
                # Compression <= 70% (not a flat bar)
                high_conv = (
                    co < ph and
                    ch_to_ph_pct <= 1.5 and
                    comp_pct <= 70
                )
                # LTP from live df
                try:
                    _ltp_rows = df[df["Symbol"] == sym]["LTP"]
                    ltp = float(_ltp_rows.iloc[0]) if not _ltp_rows.empty else float(cc)
                except Exception:
                    ltp = float(cc)
                # Live open from df
                try:
                    _lo_rows = df[df["Symbol"] == sym]["LIVE_OPEN"]
                    live_open = float(_lo_rows.iloc[0]) if not _lo_rows.empty else 0.0
                except Exception:
                    live_open = 0.0
                # VOL% from df
                try:
                    _vp_rows = df[df["Symbol"] == sym]["VOL_%"]
                    vol_pct = round(float(_vp_rows.iloc[0]), 1) if not _vp_rows.empty else 0.0
                except Exception:
                    vol_pct = 0.0
                rows.append({
                    "Symbol":           sym,
                    "LTP":              round(ltp, 2),
                    "Open":             round(live_open, 2),   # today's live open
                    "Child High":       round(ch, 2),          # = Yest High
                    "Child Low":        round(cl, 2),          # = Yest Low
                    "Child Open":       round(co, 2),          # = Yest Open
                    "Child Close":      round(cc, 2),          # = Yest Close
                    "Parent High":      round(ph, 2),
                    "Parent Low":       round(pl, 2),
                    "Parent Close":     round(pc, 2),
                    "Parent Bias":      parent_bias,
                    "Child Bias":       child_bias,
                    "Compression %":    comp_pct,
                    "CH→PH Gap %":      ch_to_ph_pct,          # child high distance from parent high
                    "Open<PH Gap %":    open_vs_ph_pct,        # yest open distance below parent high
                    "VOL %":            vol_pct,
                    "Entry Long >":     round(ph, 2),
                    "Entry Short <":    round(pl, 2),
                    "SL Long":          round(pl, 2),
                    "SL Short":         round(ph, 2),
                    "Broke High?":      "✅ YES" if ltp > ph else "⏳ No",
                    "Broke Low?":       "✅ YES" if ltp < pl else "⏳ No",
                    "High Conv":        "🔥 YES" if high_conv else "",
                    "Status":           "🟢 BREAKOUT UP" if ltp > ph else
                                        "🔴 BREAKDOWN"   if ltp < pl else
                                        "⏳ Consolidating",
                })
        except Exception:
            continue
    df_out = pd.DataFrame(rows)
    # Sort: High Conviction first, then by Compression %
    if not df_out.empty:
...
# ── MACD scanner function (module-level for auto-load) ───────────────────────
def _load_macd_all():
    rows = []
    today_d = date.today()
    # Use last completed trading day to avoid partial intraday candle
    _ltd = today_d - timedelta(days=1)
    while _ltd.weekday() >= 5 or _ltd in NSE_HOLIDAYS:
        _ltd -= timedelta(days=1)
    sd = _ltd - timedelta(days=90)
    for sym in SYMBOLS[:80]:   # top 80 — enough for signal diversity
        try:
            tok = get_token(sym)
            if not tok:
                continue
            ...
            def _ema_ser(p, n):
                ...
                e = [sum(p[:n]) / n]
                for x in p[n:]:
                    e.append(x * k2 + e[-1] * (1 - k2))
                return e

            ef = _ema_ser(closes, 12)
            es = _ema_ser(closes, 26)
            off = len(ef) - len(es)
            macd_l = [f - s for f, s in zip(ef[off:], es)]
            if len(macd_l) < 9:
                continue
            sig_l = _ema_ser(macd_l, 9)
            hist = macd_l[-1] - sig_l[-1]
            prev_hist = macd_l[-2] - sig_l[-2] if len(macd_l) > 1 else 0

            crossover = ""
            if prev_hist <= 0 and hist > 0:
                crossover = "🟢 BULL CROSS"
            elif prev_hist >= 0 and hist < 0:
                crossover = "🔴 BEAR CROSS"

            rows.append({
                "Symbol":     sym,
                "MACD":       round(macd_l[-1], 3),
                "Signal":     round(sig_l[-1],  3),
                "Histogram":  round(hist, 3),
                "Prev_Hist":  round(prev_hist, 3),
                "Crossover":  crossover,
                "Close":      round(closes[-1], 2),
                "Trend":      "📈 Bull" if hist > 0 else "📉 Bear",
            })
        except Exception:
            continue
    return pd.DataFrame(rows)
...
with tabs[0]:
    ...
    try:
        ...
        _BENEF_C   = {"Sun","Jupiter","Moon","Mars","Venus"}
        _MALEF_C   = {"Saturn","Rahu","Ketu","Mercury"}
        ...
        with _wf1:
            ...
            if _wall_rows:
                st.dataframe(pd.DataFrame(_wall_rows).style
                    ...
                    width='stretch', height=150, hide_index=True)
            else:
...
| 📊 NIFTY OI Intelligence | Spot + PCR + direction + walls + setup | Every 15-min slot |
    ...
    if _snap:
        ...
        _transit_warn = (
            ...
            if _hrs_next < 3 else
            f"🌙 Next nak: <b>{_next_nak}</b> ({_next_lord}★) in {_hrs_next}h"
        )
...
<div class="{_card_cls}">
  ...
  <div style="display:flex;gap:20px;flex-wrap:wrap;margin-top:8px">
    <span>🌙 <b>Moon Nak:</b> {_snap["moon_nak"]}
  ...
  <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:6px;font-size:12px">
    <span>☀️ <b>Sun:</b> {_snap["sun_rashi"]}/{_snap["sun_nak"]}
      &nbsp;<span style="color:#aaa">sub:{_sun_sub}</span></span>
    <span>♂ <b>Mars:</b> {_snap["mars_rashi"]}/{_snap["mars_nak"]}</span>
    <span>♄ <b>Saturn:</b> {_snap["saturn_rashi"]}</span>
    <span>♃ <b>Jupiter:</b> {_snap["jupiter_rashi"]}
      &nbsp;<span style="color:{_dig_col(_jup_dig)}">{_jup_dig}</span>
      &nbsp;<span style="color:#FFD700">{_snap.get("jupiter_murthy","—")} Murthy</span></span>
    <span>♀ <b>Venus:</b> {_snap["venus_rashi"]}</span>
    <span>☿ <b>Mercury:</b> {_snap["mercury_rashi"]}</span>
    <span>☊ <b>Rahu:</b> {_snap["rahu_rashi"]}</span>
  </div>
  ...
  <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:6px;font-size:12px;color:#aaa">
    ...
    <span>☀-♄ Sun-Saturn: <b style="color:{'#FF4444' if _snap['sun_saturn_deg']<8 else '#aaa'}">{_snap["sun_saturn_deg"]}°</b></span>
    <span>🌙-⚫ Moon-Ketu: <b style="color:{'#FF4444' if _snap['moon_ketu_deg']<15 else '#aaa'}">{_snap["moon_ketu_deg"]}°</b></span>
    <span>🌙-♃ Moon-Jup: <b style="color:{'#00C851' if _snap['moon_jupiter_deg']<10 else '#aaa'}">{_snap["moon_jupiter_deg"]}°</b></span>
...
def _apply_fmt(df, row_style_fn=None):
    """Format all numeric cols to 2dp + colour GAIN + CHANGE/CHANGE_%"""
    fmt = {}
...
with tabs[2]:
    ...
    st.dataframe(_pv_styled, width='stretch', height=7800)
    ...
    st.dataframe(_apply_fmt(TOP_HIGH_df), width='stretch', height=7000)
...
with tabs[4]:
    ...
    st.dataframe(_apply_fmt(TOP_LOW_df), width='stretch', height=7000)
...
def build_piind_scanner():
    ...
    for sym, live in live_map.items():
        ...
        if pillars_met == 3 and pillar_d_long:
            long_rows.append({**base, "Setup": "🎯 PI-IND STYLE", "Pillars": "A+B+C ✅",
...
def _fire_mtf_alerts():
    ...
    for _direction, _df, _icon, _label in [
        ...
        for _stock_key, _r in _new_rows:
            ...
            try:
                _lines.append(
                    f"  {_icon} <b>{_s}</b>  LTP:₹{float(_l):,.2f}  Chg:{float(_c):+.1f}%  "
                    f"RelStr:{float(_rs):+.1f}%  Vol:{float(_v):+.0f}%\n"
                    f"    Daily:{_db}  Retest:{_rt}  Score:{_sc}\n"
                    f"    Entry:{_en}  SL:{_sl}"
                )
            except Exception:
                _lines.append(f"  {_icon} <b>{_s}</b>  Score:{_sc}")

        # Send via channel router
        _msg = (
            f"{'🟢'*8 if _label=='LONG' else '🔴'*8}\n"
            f"{_icon} <b>PI-IND STYLE SETUP — {_label} (NEW ENTRIES)</b>\n"
            f"⏰ {_now_str}   Nifty: {_nifty_c:+.2f}%\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"3 Pillars: Retest ✅ + Relative Strength ✅ + TF Align ✅\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            + "\n".join(_lines) +
            f"\n━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚠️ <i>NOT financial advice.</i>\n"
...
with tabs[16]:
    ...
    def _hm_cell_html(sym, wt, chg, ltp, lh, ll, yh, yl, sect):
        ...
        impact = wt * chg / 100
        # Day position: is stock near day high or day low?
        if lh > ll > 0:
            hl_pct = (ltp - ll) / (lh - ll) * 100 if lh != ll else 50
            hl_txt = "🔼 Near Day Hi" if hl_pct > 75 else ("🔽 Near Day Lo" if hl_pct < 25 else "↔ Mid Range")
        else:
            hl_txt = ""
        return (
            f'<div class="hm-cell" style="background:{bg};border-color:{border}" '
            f'title="{sym} | Wt:{wt}% | Chg:{chg:+.2f}% | Impact:{impact:+.4f}%">'
            f'<div class="hm-sym">{sym}</div>'
            f'<div class="hm-chg">{arrow} {chg:+.2f}%</div>'
            f'<div class="hm-meta">{sect} · {wt}%</div>'
            f'<div class="hm-hl">{hl_txt}</div>'
            f'</div>'
        )
...
<div class="mom-wrap">
  ...
  <div style="margin-top:5px;font-size:11px;color:{sig_col}">
    ...
    # ════════════════ INDEX CARDS ════════════════
    _ni_row = indices_df[indices_df["Index"] == "NIFTY 50"] if "Index" in indices_df.columns else pd.DataFrame()
    _bnk_row = indices_df[indices_df["Index"] == "BANK NIFTY"] if "Index" in indices_df.columns else pd.DataFrame()
    _ni_ltp  = float(_ni_row.iloc[0]["LTP"])  if not _ni_row.empty and "LTP"      in _ni_row.columns else 24315.0
    _ni_chg  = float(_ni_row.iloc[0]["CHANGE_%"]) if not _ni_row.empty and "CHANGE_%" in _ni_row.columns else 0.0
    _bnk_ltp = float(_bnk_row.iloc[0]["LTP"]) if not _bnk_row.empty and "LTP"      in _bnk_row.columns else 56086.0
    _bnk_chg = float(_bnk_row.iloc[0]["CHANGE_%"]) if not _bnk_row.empty and "CHANGE_%" in _bnk_row.columns else 0.0
    _ni_col  = "#4ade80" if _ni_chg  >= 0 else "#f87171"
    _bnk_col = "#4ade80" if _bnk_chg >= 0 else "#f87171"

    # ── Live data status ────────────────────────────────────────────────
    _hm_live_count = sum(1 for v in _hm_chg_map.values() if v != 0)
    _hm_total_syms = len(_N50_WT) + len(_BNK_WT)
    _hm_data_ok    = _hm_live_count >= 10
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px">
      <div class="idx-card-hm">
        <div style="font-size:10px;color:#666;font-weight:600;letter-spacing:.06em">NIFTY 50</div>
        <div style="font-size:24px;font-weight:800;color:#fff">{_ni_ltp:,.2f}</div>
        <div style="font-size:13px;font-weight:700;color:{_ni_col}">{_ni_chg:+.2f}%</div>
        <div style="font-size:10px;color:#555;margin-top:2px">Wt impact: {_n50_score['total_impact']:+.3f}% ≈ {_n50_score['total_impact']/100*_ni_ltp:+.0f} pts</div>
      </div>
      <div class="idx-card-hm">
        <div style="font-size:10px;color:#666;font-weight:600;letter-spacing:.06em">BANK NIFTY</div>
        <div style="font-size:24px;font-weight:800;color:#fff">{_bnk_ltp:,.2f}</div>
        <div style="font-size:13px;font-weight:700;color:{_bnk_col}">{_bnk_chg:+.2f}%</div>
        <div style="font-size:10px;color:#555;margin-top:2px">Wt impact: {_bnk_score['total_impact']:+.3f}% ≈ {_bnk_score['total_impact']/100*_bnk_ltp:+.0f} pts</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ════════════════ LEGEND ════════════════
    st.markdown("""
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:8px;font-size:10px;color:#888">
      <b style="color:#aaa">Heatmap:</b>
      <span style="background:#14532d;color:#fff;padding:1px 7px;border-radius:3px">&gt;+2%</span>
      <span style="background:#166534;color:#fff;padding:1px 7px;border-radius:3px">+1–2%</span>
      <span style="background:#1a4a2a;color:#fff;padding:1px 7px;border-radius:3px">+0.25–1%</span>
      <span style="background:#1e293b;color:#fff;padding:1px 7px;border-radius:3px">flat</span>
      <span style="background:#4a0f0f;color:#fff;padding:1px 7px;border-radius:3px">-0.25–-1%</span>
      <span style="background:#7f1d1d;color:#fff;padding:1px 7px;border-radius:3px">-1–-2%</span>
      <span style="background:#450a0a;color:#fff;padding:1px 7px;border-radius:3px">&lt;-2%</span>
      <span style="color:#555">| Bigger wt = bigger cell | Hover for impact</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with _hm_sort_col1:
        _hm_sort = st.radio(
            ...
            horizontal=True, key="hm_sort_mode",
    ...
    _hm_title_suffix = (
        "Top Gainers → Losers" if _hm_sort_mode == "gainers"
        else "Top Losers → Gainers" if _hm_sort_mode == "losers"
        else "Sorted by Weightage"
    )

    st.markdown(f'<div style="font-size:11px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">📊 Nifty 50 — All 50 Stocks ({_hm_title_suffix})</div>', unsafe_allow_html=True)
    st.markdown(_render_heatmap_html(_N50_WT, _N50_SECT, _live_df_hm, _hm_sort_mode), unsafe_allow_html=True)

    # ════════════════ BANKNIFTY HEATMAP ════════════════
    st.markdown(f'<div style="font-size:11px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px;margin-top:4px">🏦 Bank Nifty — All 14 Banks ({_hm_title_suffix})</div>', unsafe_allow_html=True)
    ...
    with _hm_c1:
        st.markdown(
            ...
            + _render_impact_bars(_n50_score["impacts"], "Nifty 50", _ni_ltp)
            + '</div>',
            unsafe_allow_html=True
        )
    with _hm_c2:
        st.markdown(
            '<div style="background:#0f0f0f;border:1px solid #1e1e1e;border-radius:8px;padding:10px 12px">'
            + _render_impact_bars(_bnk_score["impacts"], "BankNifty", _bnk_ltp)
            + '</div>',
            unsafe_allow_html=True
        )
    ...
    def _hm_signal_box(score, label, ltp):
        is_long  = score["bull_wt"] > score["bear_wt"] and score["total_impact"] > 0
        is_short = score["bear_wt"] > score["bull_wt"] and score["total_impact"] < 0
        ...
        return f"""
<div class="sig-box" style="background:{bg};border:1px solid {col}30">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
    <b style="color:{col};font-size:13px">{label}</b>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px">
    <div style="background:#0f0f0f;border-radius:5px;padding:5px 8px;text-align:center">
      <div style="color:#4ade80;font-size:15px;font-weight:700">{score['bull']}</div>
      <div style="color:#666;font-size:9px">Stocks Up</div>
    </div>
    <div style="background:#0f0f0f;border-radius:5px;padding:5px 8px;text-align:center">
      <div style="color:{col};font-size:15px;font-weight:700">{score['bull_wt']:.0f}%</div>
      <div style="color:#666;font-size:9px">Bull Weight</div>
    </div>
    <div style="background:#0f0f0f;border-radius:5px;padding:5px 8px;text-align:center">
      <div style="color:#f87171;font-size:15px;font-weight:700">{score['bear']}</div>
      <div style="color:#666;font-size:9px">Stocks Down</div>
    </div>
  </div>
  <div style="font-size:10px;color:#666">Top drivers: {top3_txt}</div>
  <div style="font-size:11px;color:{col};margin-top:4px">
    Wt Impact: <b>{score['total_impact']:+.3f}%</b> = <b>{score['total_impact']/100*ltp:+.0f} pts</b>
    &nbsp;|&nbsp; Bull {score['bull_wt']:.1f}% vs Bear {score['bear_wt']:.1f}% of index weight
  </div>
</div>"""
    ...
    with st.expander("📋 Raw Index Data (OHLC Table)", expanded=False):
        ...
        st.dataframe(indices_df[_idx_display_cols] if _idx_display_cols else indices_df,
...
with tabs[18]:
    ...
    _ALERT_CATEGORIES_UI = [
        ...
        ("🌙 Advance Astro Alert", "ASTRO_ADVANCE"),
        ("📐 1H BOS/CHoCH",         "BOS_1H"),
    ...
    _TG_ONLY_CATS = {
        ...
        "EXPIRY_ALERT", "HOLIDAY_ALERT", "MTF_SEQ", "HEATMAP_ALERT",
    ...
    for _cdi in range(0, _cal_horizon_days + 5):   # +5 buffer so last day is always included
        ...
        if _cal_jup_exalt and _sig == "BEARISH":
            ...
            if "Amavasya" in _tithi_str or "Krishna Pratipada" in _tithi_str:
                ...
                _rule = f"♃ Jupiter Exalted floor active — {_rule} — expect dip-buy, no crash"
            elif _int in ("EXTREME", "HIGH"):
                # Strong bearish nak → downgrade to CAUTION
                _sig = "BEARISH"; _int = "MEDIUM"
                _rule = f"♃ Jupiter Exalted: {_rule} — floor under market, sell rallies only"
            else:
                # Mild bearish → Mixed
                _sig = "MIXED"; _int = "LOW"
                _rule = f"♃ Jupiter Exalted: {_rule} — market supported, watch for bounce"
...
<div style="max-width:700px;margin:auto;background:#111;border-radius:10px;overflow:hidden">
  ...
  <div style="background:#0a0a0a;padding:10px 20px">
    ...
    if _cal_events_filtered:
        ...
        _sm5.metric("☀️ Sun transits",  _ns)
        ...
        st.dataframe(
            ...
            height=min(max(400, len(_cal_events_filtered) * 38 + 60), 3600),
        )
...
# ═══════════════════════════════════════════════════════════════════════
# 🌙 TAB 19 — KP PANCHANG TRADING WINDOWS
# ═══════════════════════════════════════════════════════════════════════
with tabs[1]:
    render_kp_tab()
...
with tabs[21]:
    ...
    # ── Load MACD data from background worker's live_data CSV + compute on the fly ──
    _macd_rows = []
    try:
        _live_csv = os.path.join(CACHE_DIR, f"live_data_{date.today().strftime('%Y%m%d')}.csv")
        if os.path.exists(_live_csv):
            _ld = pd.read_csv(_live_csv, encoding='utf-8')
        else:
            _ld = df[["Symbol", "LTP", "CHANGE_%"]].copy() if "Symbol" in df.columns else pd.DataFrame()

        # Compute MACD from historical daily data for top stocks
        def _fast_ema(closes, n):
            if len(closes) < n:
                return None
            k = 2 / (n + 1)
            v = sum(closes[:n]) / n
        ...
        _macd_ss_key = f"macd_scan_{date.today()}"
        if _macd_ss_key not in st.session_state:
            with st.spinner("Computing MACD for all stocks… (first load, ~30s)"):
                st.session_state[_macd_ss_key] = _load_macd_all()
        _macd_df = st.session_state[_macd_ss_key]
        if st.button("🔄 Refresh MACD", key="macd_refresh_btn"):
            st.session_state.pop(_macd_ss_key, None)
            st.rerun()
        ...
        if _macd_df.empty:
            ...
            if not _kite_ok:
                ...
                st.warning(
                    ...
                    "- Click **🔄 Refresh MACD** button above\n\n"
            ...
            if not _bull_cross.empty:
                st.dataframe(
                    _bull_cross.sort_values("Histogram", ascending=False)
                    [["Symbol","Close","MACD","Signal","Histogram","Crossover","Trend"]],
                    width='stretch', hide_index=True
                )
            else:
            ...
            if not _bear_cross.empty:
                st.dataframe(
                    _bear_cross.sort_values("Histogram", ascending=True)
                    [["Symbol","Close","MACD","Signal","Histogram","Crossover","Trend"]],
                    width='stretch', hide_index=True
                )
            else:
            ...
            def _macd_style(val):
                ...
                return ""
            st.dataframe(
                _macd_display[["Symbol","Close","MACD","Signal","Histogram","Crossover","Trend"]],
                width='stretch', hide_index=True,
                height=min(800, 60 + len(_macd_display) * 35)
            )
    except Exception as _e:
...
with tabs[22]:
    ...
    _ib_ss_key = f"inside_bar_scan_{date.today()}"
    if _ib_ss_key not in st.session_state:
        # First load: show spinner (takes ~30s — Kite daily candle fetch)
        # Subsequent loads: already in session_state (cached for the day)
        with st.spinner("🔍 Scanning Inside Bar patterns… (first load, takes ~30s)"):
            st.session_state[_ib_ss_key] = _load_inside_bars()
    _ib_df = st.session_state[_ib_ss_key].copy()   # copy so we can update live cols

    # ── Refresh live columns (LTP, VOL%, Status) on every page load ──────────
    # The structural data (Parent High/Low, Compression etc.) is cached daily.
    # LTP, VOL%, and Status are updated from the live df on every 60s refresh
    # so the table display and alert logic always use current market prices.
    if not _ib_df.empty and not df.empty:
        try:
            _live_map    = df.set_index("Symbol")[["LTP","VOL_%","LIVE_OPEN"]].to_dict(orient="index")
            _ib_df["LTP"]    = _ib_df["Symbol"].map(lambda s: _live_map.get(s, {}).get("LTP",    _ib_df.loc[_ib_df["Symbol"]==s,"LTP"].iloc[0]    if (s in _ib_df["Symbol"].values) else 0))
            _ib_df["VOL %"]  = _ib_df["Symbol"].map(lambda s: _live_map.get(s, {}).get("VOL_%",  0))
            _ib_df["Open"]   = _ib_df["Symbol"].map(lambda s: _live_map.get(s, {}).get("LIVE_OPEN", 0))
            # Recompute Status from live LTP
            _ib_df["Status"] = _ib_df.apply(
                lambda r: "🟢 BREAKOUT UP" if r["LTP"] > r["Parent High"]
                else ("🔴 BREAKDOWN" if r["LTP"] < r["Parent Low"]
                else "⏳ Consolidating"), axis=1
            )
            _ib_df["Broke High?"] = _ib_df.apply(lambda r: "✅ YES" if r["LTP"] > r["Parent High"] else "⏳ No", axis=1)
            _ib_df["Broke Low?"]  = _ib_df.apply(lambda r: "✅ YES" if r["LTP"] < r["Parent Low"]  else "⏳ No", axis=1)
        except Exception as _ib_live_err:
            pass   # fallback to cached values — no crash

    # ── Fire Telegram alerts — LIVE LTP check on EVERY refresh ─────────────
    # KEY FIX: LTP is read from live df/ltp_map on each 60s refresh cycle,
    # NOT from the cached session_state (which is stale from scan time).
    # This means the alert fires the MOMENT price crosses the level,
    # not minutes later when the tab was first opened.
    # Proximity pre-alert fires when within 0.3% of level (≈ 15pts on 5000 stock).
    if not _ib_df.empty and is_market_hours():
        _today_str = date.today().strftime("%Y%m%d")
        _IB_PROX_PCT = 0.3    # % within which pre-alert fires
        _IB_PROX_SLOT = datetime.now(IST).strftime("%H%M")[:-1]  # 10-min slot for prox dedup
        ...
        for _, _ib_row in _ib_df.iterrows():
            ...
            # ── LIVE LTP from df (refreshed every 60s) — NOT cached ──────
            try:
            ...
            if _ib_ltp > _ib_ph:
                ...
                _ib_msg = (
                    ...
                    f"📌 <b>{_ib_sym}</b>\n"
                    f"💰 LTP: <b>₹{_ib_ltp:,.2f}</b>  -{_pts_below:.2f} below Parent Low ₹{_ib_pl:,.2f}\n"
                    f"📊 Yest: O:{_ib_co}  H:{_ib_ch}  L:{_ib_cl_price}  C:{_ib_cc}\n"
                    f"📐 Compression: {_ib_comp:.0f}%  |  Vol: {_ib_vol:+.0f}%\n"
                    f"🎯 Entry Short &lt; ₹{_ib_pl:,.2f}  |  SL: ₹{_ib_ph:,.2f}"
                )
                send_alert_routed("INSIDE_BAR", _ib_msg, dedup_key=_ib_dedup)
                ...
                if _dist_up <= _IB_PROX_PCT and not _ib_hc_flag == "" and _ib_hc_flag:
                    ...
                    _prox_key = f"IB_PROX_UP_{_ib_sym}_{_today_str}_{_IB_PROX_SLOT}"
                    ...
                    _prox_msg = (
                        ...
                        f"Parent High: <b>₹{_ib_ph:,.2f}</b>  |  LTP: <b>₹{_ib_ltp:,.2f}</b>\n"
                        f"📐 Compression: {_ib_comp:.0f}%  |  SL if long: ₹{_ib_pl:,.2f}\n"
                        f"⚠️ <i>NOT financial advice.</i>"
                    )
                    send_alert_routed("INSIDE_BAR", _prox_msg, dedup_key=_prox_key)

                elif _dist_dn <= _IB_PROX_PCT and _ib_hc_flag:
                    _prox_key = f"IB_PROX_DN_{_ib_sym}_{_today_str}_{_IB_PROX_SLOT}"
                    ...
                    _prox_msg = (
                        ...
                        f"Parent Low: <b>₹{_ib_pl:,.2f}</b>  |  LTP: <b>₹{_ib_ltp:,.2f}</b>\n"
                        f"📐 Compression: {_ib_comp:.0f}%  |  SL if short: ₹{_ib_ph:,.2f}\n"
                        f"⚠️ <i>NOT financial advice.</i>"
                    )
                    send_alert_routed("INSIDE_BAR", _prox_msg, dedup_key=_prox_key)
...
with tabs[23]:
    ...
    with _gc1:
        # Auto-populate from live Nifty LTP if available
        try:
            _nifty_live_row = df[df["Symbol"] == "NIFTY"]
            _nifty_default  = float(_nifty_live_row.iloc[0]["LTP"]) if not _nifty_live_row.empty else 24000.0
        except Exception:
            _nifty_default = 24000.0
        _gann_price = st.number_input(
            "Enter Price / Level", value=_nifty_default, step=50.0,
    ...
    with _gc2:
        st.markdown(f"**Square of Nine levels for ₹{_gann_price:,.2f}** (increment: {_gann_inc})")
    ...
    # ── Gann Seasonal / Annual Dates ─────────────────────────────────
    st.markdown("### 📅 Gann Seasonal & Annual Dates")
    st.caption(
        "W.D. Gann identified specific calendar dates as high-probability reversal windows. "
        "These are not guarantees — treat as confluence zones."
    )

    _year = date.today().year
    _gann_dates = [
        (f"Jan 10 ({_year})",    date(_year,  1, 10), "Seasonal — Winter low"),
        (f"Jan 19-20 ({_year})", date(_year,  1, 19), "Annual — 19-20 Jan reversal"),
        (f"Feb 3-5 ({_year})",   date(_year,  2,  3), "Seasonal — February high/low"),
        (f"Mar 20-22 ({_year})", date(_year,  3, 20), "Spring Equinox — major reversal"),
        (f"Apr 7-9 ({_year})",   date(_year,  4,  7), "Seasonal — April high"),
        (f"May 5-7 ({_year})",   date(_year,  5,  5), "Seasonal — May reversal"),
        (f"Jun 10-14 ({_year})", date(_year,  6, 10), "Seasonal — June high/low"),
        (f"Jun 21-22 ({_year})", date(_year,  6, 21), "Summer Solstice — major reversal"),
        (f"Jul 7-10 ({_year})",  date(_year,  7,  7), "Seasonal — July low"),
        (f"Aug 5-8 ({_year})",   date(_year,  8,  5), "Seasonal — August reversal"),
        (f"Sep 3-5 ({_year})",   date(_year,  9,  3), "Seasonal — Sept high/low"),
        (f"Sep 22-23 ({_year})", date(_year,  9, 22), "Autumn Equinox — major reversal"),
        (f"Oct 7-10 ({_year})",  date(_year, 10,  7), "Seasonal — October reversal"),
        (f"Nov 5-9 ({_year})",   date(_year, 11,  5), "Seasonal — November"),
        (f"Dec 3-5 ({_year})",   date(_year, 12,  3), "Seasonal — December"),
        (f"Dec 21-22 ({_year})", date(_year, 12, 21), "Winter Solstice — major reversal"),
    ]

    _gd_rows = []
    _today_g  = date.today()
    for _gd_lbl, _gd_date, _gd_desc in _gann_dates:
        _days_away = (_gd_date - _today_g).days
        _status = (
            "🔴 PAST" if _days_away < -3 else
            "🟡 THIS WEEK" if -3 <= _days_away <= 7 else
            f"⏳ In {_days_away}d"
        )
        _gd_rows.append({
            "Date":        _gd_lbl,
            "Significance": _gd_desc,
            "Days Away":   _days_away,
            "Status":      _status,
        })

    _gd_df = pd.DataFrame(_gd_rows)
    # Highlight upcoming dates
    _gd_upcoming = _gd_df[_gd_df["Days Away"] >= 0].head(5)
    if not _gd_upcoming.empty:
        st.markdown("**⚡ Next 5 Gann Dates:**")
        st.dataframe(_gd_upcoming, width='stretch', hide_index=True)

    st.markdown("**📋 Full Gann Calendar:**")
    st.dataframe(_gd_df, width='stretch', hide_index=True)
    ...
    with _gw2:
        ...
        st.markdown(f"**Gann Wheel levels near ₹{_gw_price:,.2f}**")
...
with tabs[25]:
    ...
    st.caption(
        "Detects 12 high-conviction chart patterns across all stocks. "
        "Each pattern includes Entry, SL, Targets, R:R, Post-breakout % move, and full OHLC context. "
        "**Conviction Score 1–10** — higher = more confluences confirmed."
    )
        ...
        with _cp_c4:
            _cp_patterns = st.multiselect(
                ...
                ["Falling Wedge","Ascending Triangle","Bull Flag","Bull Pennant",
                 "Cup & Handle","Double Bottom",
                 "Rising Wedge","Descending Triangle","Bear Flag","Bear Pennant",
...
🔻 **Falling Wedge** — Both trendlines fall, resistance steeper → explosive upside break. Most reliable bullish reversal.

📐 **Ascending Triangle** — Flat resistance + rising lows. Coiling energy. Break above flat line = target = pattern height.

🚩 **Bull Flag** — Pole up (≥3%) + tight rectangular consolidation. Continuation. T = flag top + pole height.

📌 **Bull Pennant** — Pole up + converging triangle. More explosive than flag.

☕ **Cup & Handle** — U-shaped recovery + small handle pullback. Classic William O'Neil setup. Longer timeframe = more reliable.

🔂 **Double Bottom** — Two equal lows + neckline break. W-pattern. T = neckline + pattern height.
""")
            with _lg2:
                st.markdown("""
**🔴 BEARISH PATTERNS**

📈 **Rising Wedge** — Both lines rise, support steeper → breakdown. Often seen at market tops.

📉 **Descending Triangle** — Flat support + falling highs. Breakdown = measured target below support.

🏴 **Bear Flag** — Pole down + tight rectangular consolidation. Continuation lower.

📍 **Bear Pennant** — Pole down + converging triangle. Continuation.

🔃 **Double Top** — Two equal highs + neckline break. M-pattern. Most reliable bearish reversal.

👤 **Head & Shoulders** — Left shoulder + higher head + right shoulder. Neckline break = measured move.

**Conviction Score:** Volume (2pts) + Pattern size (2pts) + Post-break move confirmed (3pts) + Base score (3pts)
""")
