...
def strategy(token, tick):
    ...
    print(f"Checking {symbol} at {ltp}")
    ltp = tick["last_price"]
...
def run_market_replay_proper(replay_date):
    ...
    # Load Yesterday Data from Local
    load_yesterday_from_local(replay_date)

    # Load EMA from Local
    load_ema_data_from_local(replay_date)

    # Load all minute data into memory
    symbol_minute_data = {}
    ...
    for symbol in SYMBOLS:
        ...
        day_df = df[df["date"].dt.date == replay_date.date()]
