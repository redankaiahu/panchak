...
def strategy(token, tick):
    ...
    # =====================================
    # ENTRY TIME FILTER (LIVE + REPLAY SAFE)
    # =====================================
    if "date" in tick:
...
def run_market_replay_proper(replay_date):
    ...
    for current_time in all_times:
        ...
        for symbol, df in symbol_minute_data.items():
            ...
            "volume": row["volume"],
            "date": current_time   # IMPORTANT for replay time filtering
        }

            latest_prices[symbol] = row["close"]
            latest_highs[symbol] = row["high"]
            latest_lows[symbol] = row["low"]
    ...
    #print("\n✅ Replay Finished")
    #daily_summary()
    print("\n🔔 End of Day – Closing Open Positions (MTM)")
    ...
    for symbol in list(paper_positions.keys()):
        ...
        print(
            f"EOD CLOSE | {symbol} | {side} | "
            f"Entry:{entry} Exit:{ltp} NetPnL:{net:.2f}"
        )

    print("\n✅ Replay Finished")
    daily_pnl = round(daily_pnl, 2)
    daily_summary()
    # Store result for weekly/monthly summary
