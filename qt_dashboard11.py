...
DARK_QSS = """
/* ═══════════════════════════════════════════════════════════════════
   PANCHAK DASHBOARD — ZERODHA KITE INSPIRED LIGHT THEME
   Background: White/Light grey | Accent: Kite Blue | Data: Green/Red
   ═══════════════════════════════════════════════════════════════════ */
...
QTabBar::tab:selected {
    color: #1366D6;
    border-bottom: 2px solid #1366D6;
    background: #FFFFFF;
}
...
QComboBox:hover, QComboBox:focus { border-color: #1366D6; }
...
QDoubleSpinBox:hover, QLineEdit:hover { border-color: #1366D6; }
QDoubleSpinBox:focus, QLineEdit:focus { border-color: #1366D6; }
...
QPushButton:disabled { background: #D1D5DB; color: #9CA3AF; border: none; }
...
QScrollBar::handle:vertical:hover { background: #1366D6; }
...
QCheckBox { color: #1A1A1A; font-size: 11px; spacing: 6px; }
QCheckBox::indicator {
...
# ── Color system: Zerodha Kite Light Theme ────────────────────────────────────
C_GREEN   = "#1D7D3C"   # Kite green — bull / gain
C_GREEN2  = "#15643A"   # deeper green
C_RED     = "#D22027"   # Kite red — bear / loss
C_RED2    = "#A8181D"   # deeper red
C_YELLOW  = "#D97706"   # amber — warning / ATM
C_BLUE    = "#1366D6"   # Kite blue — accent
C_CYAN    = "#0369A1"   # info blue
C_PURPLE  = "#7C3AED"   # CHoCH / special
C_ORANGE  = "#EA580C"   # near levels
# Backgrounds
C_BG      = "#F8F9FA"   # page bg
C_BG_CARD = "#FFFFFF"   # card bg
C_BG_ROW  = "#FAFAFA"   # alternate row
C_BG_BULL = "#F0FBF4"   # bull row tint
C_BG_BEAR = "#FFF5F5"   # bear row tint
C_BG_ATM  = "#EFF6FF"   # ATM highlight
# Borders & text
C_BORDER  = "#E5E7EB"
C_BORDER2 = "#D1D5DB"
C_DIM     = "#9CA3AF"   # dim secondary
C_MUTED   = "#6B7280"   # muted labels
C_TEXT    = "#1A1A1A"   # primary text
C_TEXT2   = "#111827"   # headings
# Light glows (tints)
C_GLOW_G  = "#1D7D3C18"
C_GLOW_R  = "#D2202718"
C_GLOW_B  = "#1366D618"
...
class OIPanel(QWidget):
    ...
    def _populate(self, d):
        ...
        try:
            bg_tint = "#F0FDF4" if "BULL" in dirc else ("#FFF5F5" if "BEAR" in dirc else "#FFFBEB")
...
class DbPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.sym_tbl.setStyleSheet("QTableWidget{alternate-background-color:#FAFAFA;background:#FFFFFF;}")
...
class AutoRefreshBar(QWidget):
    ...
    def __init__(self, interval_secs=AUTO_REFRESH_SECS):
        ...
        self.setStyleSheet("background:#FAFAFA;border-top:1px solid #E5E7EB;")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        brand.setStyleSheet("color:#1366D6; letter-spacing:3px; background:transparent;")
        hl.addWidget(brand)

        dot = QLabel("  ●")
        dot.setFont(QFont("Segoe UI", 8))
        dot.setStyleSheet("color:#D1D5DB; background:transparent;")
        hl.addWidget(dot)

        sub = QLabel("  TRADING DASHBOARD")
        sub.setFont(QFont("Segoe UI", 9))
        sub.setStyleSheet("color:#9CA3AF; letter-spacing:2px; background:transparent;")
        hl.addWidget(sub)
        ...
        self.lbl_spot.setStyleSheet("color:#1A1A1A; background:transparent;")
        ...
        self.lbl_clock.setStyleSheet("color:#374151; background:transparent; letter-spacing:0.5px;")
    ...
    def _on_kite_connected(self):
        ...
        self.lbl_kite.setStyleSheet(f"""
            color:{C_GREEN}; background:#F0FDF4;
    ...
    def _tick_clock(self):
        ...
        if mo:
            ...
            self.lbl_market.setStyleSheet(f"""
                color:{C_MUTED}; background:#F3F4F6;
                border:1px solid #E5E7EB;
                border-radius:4px; padding:3px 14px;
                font-size:10px; font-weight:700;
            """)
