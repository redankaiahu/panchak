...
def strategy_open_high_breakdown(ctx):
    ...
    day_open = f5["open"]
    # Use f5["high"] (first-5-min candle high) for the O=H check, NOT latest_highs.
    # latest_highs includes all ticks from 09:15 onwards including post-09:20 prices
    # which could be ABOVE the open if price bounced. f5["high"] is definitively
    # the highest price in the 09:15-09:19 window — the exact check we need.
    day_high = f5["high"]
    tick      = tick_sizes.get(symbol, 0.05)

    # F5 high must not exceed F5 open by more than 1 tick (true Open=High)
    ...
    sell_signal = (
        149 <= ltp <= 6999              and   # widened lower bound from 299→149
