    ...
    for s in SYMBOLS:
        ...
        try:
            bars = kite.historical_data(
                tk,
                last_trading_day(date.today() - timedelta(days=60)),
                date.today(),
                "day"
            )
        except:
        ...
        dfb = pd.DataFrame(bars)
        if dfb.empty or len(dfb) < 50:
            continue

        dfb["EMA20"] = dfb["close"].ewm(span=20).mean()
        dfb["EMA50"] = dfb["close"].ewm(span=50).mean()

        rows.append({
            "Symbol": s,
            "EMA20": round(dfb.iloc[-1]["EMA20"], 2),
            "EMA50": round(dfb.iloc[-1]["EMA50"], 2),
        })

        time.sleep(0.25)

    # 🔒 CRITICAL: guarantee schema even if empty
    if not rows:
...
with tabs[7]:
    ...
    if ohl_df.empty:
        st.info("No OPEN = HIGH or OPEN = LOW stocks today")
    else:
        st.dataframe(
            ohl_df,
            use_container_width=True,
            height=min(1200, 60 + len(ohl_df) * 35)
        )
