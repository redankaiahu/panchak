        ...
        if open_positions:
            for pos in open_positions:
                ...
                if product_type:  # Ensure product_type is valid
                    ...
                    transaction_type = (TransactionType.Sell if -500 <= qty <= 500 and qty > 0 else (TransactionType.Buy if -100 <= qty <= 100 else ValueError("Quantity must be between -100 and 100")))
...
def get_last_week_candle(historical_data, holidays):
    ...
    recent_data['date_only'] = recent_data.index.normalize().date

     # Step 5: Filter both week data
    last_week_data = recent_data[recent_data['date_only'].isin(last_week_dates)]
    curr_week_data = recent_data[recent_data['date_only'].isin(curr_week_dates)]
    ...
    #if curr_week_data.empty or len(curr_week_data) < 1:
        #print("❌ Not enough data for current week's candle.")
     #   return None

    if last_week_data.empty or len(last_week_data) < 3:
        print("❌ Not enough data for last week's candle.")
        return None
    ...
    weekly_volume = filtered['volume'].sum()
    current_wv = curr_week_data['volume'].sum()  # current week volume
    ...
    return {
        ...
        "weekly_volume": int(weekly_volume),
        "current_wv": int(current_wv)
    }
    ...
    try:
        ...
        if isinstance(historical_data, pd.DataFrame) and not historical_data.empty:
            ...
            if historical_data.shape[0] >= 3:
                ...
                weekly_volume = weekly_candle['weekly_volume']
                current_wv = weekly_candle['current_wv']
...
def evaluate_chartink_trendline_breakout(data, ltp, open_price, high_price, low_price, volume, symbol_name, instrument,
                                          ...
                                          three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close,
                                          three_days_ago_vol, four_days_ago_open, four_days_ago_high, four_days_ago_low,
                                          four_days_ago_close, four_days_ago_vol, five_days_ago_close, Y_H_C_d, Y_H_L_d,
                                          Y_C_L_d, YHCon, YLCon, latest_max_5, latest_max_120, SMA_Volume_5, ema_200,
                                          ema_50, ema_20, ema_13, conditionB1, conditionB2, conditionB3, conditionB4,
                                          conditionI1, conditionI2, conditionSB, conditionS1, conditionS2, conditionS3,
                                          conditionS4, YH3Con, YCCon, weekly_open, weekly_high, weekly_low, weekly_close,weekly_volume,current_wv):
    try:
        # Calculate Max(20, Daily High) from 3 days ago and 4 days ago
        max_20_high_3_days_ago = max([
            three_days_ago_high,
            daybefore_high,
            yesterday_high,
            four_days_ago_high,
            ...
            data["daybefore_high"],
            data["yesterday_high"]
        ])  # Simplified placeholder, ideally use rolling high

        # Define today's close (ltp)
        today_close = ltp

        if (
            # Condition 1: Today's close > 3-day ago Max(20, High)
            today_close > max_20_high_3_days_ago and

            # Condition 2: 4 days ago high < same 3-day-ago max
            four_days_ago_high < max_20_high_3_days_ago and

            # Condition 3: Today's close ≥ 20
            today_close >= 20 and

            # Condition 4: Today's volume ≥ 10,000
            volume >= 10000
        ):
            #place_order_with_prevention(instrument, TransactionType.Buy, ltp, symbol_name,open_price, high_price, low_price, volume,yesterday_open, yesterday_high, yesterday_low, yesterday_close,"Trendline_Breakout", yesterday_vol,weekly_open, weekly_high, weekly_low, weekly_close )
            place_order_with_prevention( instrument, TransactionType.Buy, ltp, symbol_name,open_price,high_price,low_price,volume,yesterday_open,yesterday_high,yesterday_low,yesterday_close,"Trendline_Breakout", yesterday_vol,weekly_open,weekly_high,weekly_low,weekly_close )
            return True
    except KeyError as e:
        print(f"Missing data for Chartink-style trendline breakout condition: {e}")
    return False

def evaluate_buy_condition_chartink_v2(data, ltp, open_price, high_price, low_price, volume, symbol_name, instrument,
                                        yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol,
                                        daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol,
                                        three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close,
                                        three_days_ago_vol, four_days_ago_open, four_days_ago_high, four_days_ago_low,
            ...
            try:
                with ThreadPoolExecutor(max_workers=350) as executor:
                    ...
                    futures = [
                        ...
                        executor.submit(evaluate_chartink_trendline_breakout,historical_data,ltp,open_price,high_price,low_price,volume, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,Y_H_C_d,Y_H_L_d,Y_C_L_d,YHCon,YLCon,latest_max_5,latest_max_120,SMA_Volume_5, ema_200, ema_50, ema_20, ema_13,conditionB1,conditionB2,conditionB3,conditionB4,conditionI1,conditionI2,conditionSB,conditionS1,conditionS2,conditionS3,conditionS4,YCCon,YH15,YH15Con,OL,weekly_open,weekly_high,weekly_low,weekly_close,weekly_volume,current_wv),
                        executor.submit(evaluate_buy_condition_chartink_v2,historical_data,ltp,open_price,high_price,low_price,volume, symbol_name, instrument, yesterday_open, yesterday_high, yesterday_low, yesterday_close, yesterday_vol, daybefore_open, daybefore_high, daybefore_low, daybefore_close, daybefore_vol, three_days_ago_open, three_days_ago_high, three_days_ago_low, three_days_ago_close, three_days_ago_vol,four_days_ago_open, four_days_ago_high, four_days_ago_low, four_days_ago_close, four_days_ago_vol, five_days_ago_close,Y_H_C_d,Y_H_L_d,Y_C_L_d,YHCon,YLCon,latest_max_5,latest_max_120,SMA_Volume_5, ema_200, ema_50, ema_20, ema_13,conditionB1,conditionB2,conditionB3,conditionB4,conditionI1,conditionI2,conditionSB,conditionS1,conditionS2,conditionS3,conditionS4,YH3Con,YCCon,weekly_open,weekly_high,weekly_low,weekly_close,weekly_volume,current_wv)
                        ]
...
def modify_stoploss_orders_data(symbol, ltp1, stoploss_price, target_price, trigger_price, executed_price, trantype, sl_order_ids, qty):
    global modified_symbols
    
    # Ensure sl_order_ids is a list, even if a single order is passed
    if isinstance(sl_order_ids, str) or isinstance(sl_order_ids, int):
        sl_order_ids = [sl_order_ids]  # Convert single value to a list
    ...
    try:
        ...
        move_levels = {
            "0.5%": executed_price * 0.005,
            "1%": executed_price * 0.01,
            "2%": executed_price * 0.02,
            "3%": executed_price * 0.03,
            "4%": executed_price * 0.04,
            "5%": executed_price * 0.05,
        }
        sl_moves = {
            "0.5%": executed_price * 0.0002,
            "1%": executed_price * 0.0009,
            "2%": executed_price * 0.005,
            "3%": executed_price * 0.015,
            "4%": executed_price * 0.025,
            "5%": executed_price * 0.04,
        }

        for sl_order_id in sl_order_ids:
            if sl_order_id not in modified_symbols[symbol]:
                modified_symbols[symbol][sl_order_id] = {"0.5%": False, "1%": False, "1.5%": False, "2%": False, "2.5%": False, "3%": False, "4%": False, "5%": False}

            new_stop_loss = None
            new_trigger_price = None

            for level, move_value in move_levels.items():
                if not modified_symbols[symbol][sl_order_id][level]:
                    if (trantype == "S" and ltp <= executed_price - move_value) or \
                       (trantype == "B" and ltp >= executed_price + move_value):
                        
                        new_stop_loss = executed_price - sl_moves[level] if trantype == "S" else executed_price + sl_moves[level]
                        new_trigger_price = new_stop_loss - 0.30 if trantype == "S" else new_stop_loss + 0.30

                        modified_symbols[symbol][sl_order_id][level] = True
                        break

            if new_stop_loss is None or new_trigger_price is None:
                continue

            # Ensure proper rounding to NSE tick size
            TICK_SIZE = 0.05
            new_stop_loss = round(round(new_stop_loss / TICK_SIZE) * TICK_SIZE, 2)
            new_trigger_price = round(round(new_trigger_price / TICK_SIZE) * TICK_SIZE, 2)
            ...
            try:
                ...
                print(f"======>> SL order modified successfully for {symbol} Order {sl_order_id}: New SL: {new_stop_loss}, Executed: {executed_price}, LTP: {ltp}, Response: {modify_order_response}\n")
            except Exception as e:
                print(f"Error modifying SL order {sl_order_id}: {e}")
    except Exception as e:
        print(f"Error in modify_stoploss_orders_data: {e}")
...
def place_order_with_prevention( instrument, transaction_type, price,  symbol_name,open_price,high_price,low_price,volume,yesterday_open,yesterday_high,yesterday_low,yesterday_close,condition, yesterday_vol,weekly_open,weekly_high,weekly_low,weekly_close ):   
    ...
    try:
        ...
        if transaction_type == TransactionType.Buy:
            ...
            stop_loss_price_call = executed_price * (1 - 0.011)
...
def should_place_order():
    current_time = datetime.now().time()
    cutoff_time_start = datetime.strptime('09:18:05', '%H:%M:%S').time()  # After 9:21 AM
        ...
        try:
            while not terminate_program:
                ...
                if not should_place_order():
                    print("It's NOT a Shopping zone before 09:15 AM and after 14:45 PM. Sleeping for 60 seconds.")
