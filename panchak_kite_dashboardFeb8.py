...
df["VOL_%"] = df["VOL_%"].replace([np.inf, -np.inf], np.nan)

# ================= FILTER =================
strong_close_df = df[
    (df["CHANGE_%"] > 1.5) &              # good momentum
    (df["DIST_FROM_DAY_HIGH_%"] <= 0.5) &   # near day high
    (df["LTP"] >= df["EMA20"])
    # Uncomment below if you want volume filter
    # & (df["VOL_%"] > 30)
]

strong_close_df = strong_close_df.sort_values(
    ["CHANGE_%","VOL_%"],
    ascending=False
)
...
# Make sure VOL_% already exists (if not calculate again safely)
if "VOL_%" not in df.columns:
    df["VOL_%"] = (
        (df["LIVE_VOLUME"] - df["YEST_VOL"]) / df["YEST_VOL"] * 100
    ).round(2)
...
# ================= FILTER =================
strong_red_close_df = df[
    (df["CHANGE_%"] < -1.5) &               # strong downside
    (df["DIST_FROM_DAY_LOW_%"] <= 0.5) &     # near day low
    (df["LTP"] <= df["EMA20"])
    # Uncomment for volume filter
    # & (df["VOL_%"] > 30)
]

strong_red_close_df = strong_red_close_df.sort_values(
    ["CHANGE_%","VOL_%"]
)
...
with tabs[4]:
    ...
    st.markdown("## 🔥 Strong Closing Stocks – Near Day High")

    if strong_close_df.empty:
        st.info("No strong closing stocks near day high")
    else:
        st.dataframe(
            strong_close_df[
                [
    ...
    st.markdown("## 🔴 Strong Closing Stocks – Near Day Low")

    if strong_red_close_df.empty:
        st.info("No strong red closing stocks near day low")
    else:
        st.dataframe(
            strong_red_close_df[
                [
...
with tabs[12]:
    ...
    if fut_df.empty:
        ...
        st.dataframe(
            fut_df[display_cols]
            .sort_values("OI_SCORE", ascending=False),
            use_container_width=True
        )
