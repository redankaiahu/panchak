...
# ==========================================================
# 🔮 FULLY AUTOMATIC PANCHAK ENGINE
# ==========================================================

PANCHAK_CYCLES_2026 = [

    (datetime(2026,2,17,9,5,tzinfo=IST),  datetime(2026,2,21,19,7,tzinfo=IST)),
    (datetime(2026,3,16,18,14,tzinfo=IST), datetime(2026,3,21,2,27,tzinfo=IST)),
    (datetime(2026,4,13,3,44,tzinfo=IST),  datetime(2026,4,17,12,2,tzinfo=IST)),
    (datetime(2026,5,10,12,12,tzinfo=IST), datetime(2026,5,14,22,34,tzinfo=IST)),
    (datetime(2026,6,6,19,3,tzinfo=IST),   datetime(2026,6,11,8,16,tzinfo=IST)),
    (datetime(2026,7,4,0,48,tzinfo=IST),   datetime(2026,7,8,16,0,tzinfo=IST)),
    (datetime(2026,7,31,6,38,tzinfo=IST),  datetime(2026,8,4,21,54,tzinfo=IST)),
    (datetime(2026,8,27,13,35,tzinfo=IST), datetime(2026,9,1,3,23,tzinfo=IST)),
    (datetime(2026,9,23,21,57,tzinfo=IST), datetime(2026,9,28,10,16,tzinfo=IST)),
    (datetime(2026,10,21,7,0,tzinfo=IST),  datetime(2026,10,25,19,22,tzinfo=IST)),
    (datetime(2026,11,17,15,30,tzinfo=IST),datetime(2026,11,22,5,54,tzinfo=IST)),
    (datetime(2026,12,14,22,35,tzinfo=IST),datetime(2026,12,19,15,58,tzinfo=IST)),
...
def get_last_completed_cycle():
    ...
    completed = [
        (s,e) for s,e in PANCHAK_CYCLES_2026 if now > e
    ]
...
# ==========================================================
# 🔁 BUILD PANCHAK LEVELS ONLY AFTER CYCLE ENDS
# ==========================================================

def build_panchak_levels(start_dt, end_dt):
...
def manage_panchak_cycle():

    start, end = get_last_completed_cycle()

    if start is None:
        return pd.read_csv(PANCHAK_DATA_FILE)

    cycle_id = start.strftime("%Y%m%d")

    # Check meta file
    if os.path.exists(PANCHAK_META_FILE):
        meta = pd.read_csv(PANCHAK_META_FILE)
        if meta.loc[0,"cycle_id"] == cycle_id:
            return pd.read_csv(PANCHAK_DATA_FILE)

    # 🔥 Build new Panchak levels
    build_panchak_levels(start, end)

    pd.DataFrame([{
        "cycle_id": cycle_id,
        "start": start.isoformat(),
        "end": end.isoformat()
    }]).to_csv(PANCHAK_META_FILE, index=False)

    return pd.read_csv(PANCHAK_DATA_FILE)

panchak_df = manage_panchak_cycle()
...
# =========================================================
# 🔥 STRONG CLOSING – NEAR DAY HIGH
# =========================================================

strong_high_df = df.loc[
    (df["CHANGE_%"] > 1.5) &
    (df["DIST_FROM_DAY_HIGH_%"] <= 0.5)
].copy()
...
# =========================================================
# 🔴 STRONG CLOSING – NEAR DAY LOW
# =========================================================

strong_low_df = df.loc[
    (df["CHANGE_%"] < -1.5) &
    (df["DIST_FROM_DAY_LOW_%"] <= 0.5)
].copy()
...
with tabs[4]:   # 👈 replace with correct index
    ...
    if table1.empty:
        ...
        st.dataframe(table1.sort_values("CHANGE_%", ascending=False), width="stretch")
    ...
    if table2.empty:
        ...
        st.dataframe(table2.sort_values("CHANGE_%", ascending=False), use_container_width=True)
    ...
    if table3.empty:
        ...
        st.dataframe(table3.sort_values("CHANGE_%"), use_container_width=True)
    ...
    if table4.empty:
        ...
        st.dataframe(table4.sort_values("CHANGE_%"), use_container_width=True)
...
with tabs[5]:   # 👈 replace with correct index
    ...
    if table1.empty:
        ...
        st.dataframe(table1.sort_values("CHANGE_%", ascending=False), use_container_width=True)
