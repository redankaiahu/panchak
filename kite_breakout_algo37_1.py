...
def _seed_highs_lows_from_api():
    """
    Fetches today's minute bars for all symbols and seeds latest_highs/lows
    with the true full-day high/low so far.
    Called after cache load (cache only has morning ORB/F15 range).
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            df = df[df["date"].astype(str) >= str(from_dt)]
            api_high = float(df["high"].max())
            api_low  = float(df["low"].min())
    ...
    print(f"📈 latest_highs/lows seeded from full-day minute data" + ts())
