...
# TRADING MODE
# ==========================================
TRADING_MODE = "PAPER"   # ⚠️  MUST change to "LIVE" manually before live trading

FIXED_QTY = 10                # ⚠️  SET TO 1 FOR LIVE TEST — change to 10 only after full validation
LIVE_MAX_TRADES = 13         # max simultaneous live trades (safety cap)
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if status == "LIVE":
                ...
                # target_oid is None/empty for tsl_only strategies (S9).
                # pandas reads empty CSV cell as NaN → str(NaN)="nan" (truthy string).
                # Must convert back to Python None so oco_monitor treats it as tsl_only.
...
def oco_monitor(symbol):
    ...
    For tsl_only positions (e.g. S9), there is no target order — only SL is watched.
    TSL updates (modify_order) are handled inside this monitor when price moves.
    ...
    while True:
        try:
            ...
            orders = kite.orders()
            sl_id  = trade["sl"]
            tgt_id = trade.get("target")   # None for tsl_only positions

            sl_order = next((o for o in orders if o["order_id"] == sl_id), None)

            if sl_order is None:
                print(Fore.RED + f"⚠️  OCO: SL order not found for {symbol}, exiting monitor" + ts())
                write_log(f"OCO SL order not found for {symbol}")
                break

            # ── SL Hit ──────────────────────────────────────────────────
            if sl_order["status"] == "COMPLETE":
                trade_statistics["sl_hits"] += 1
                # Cancel target order if one exists (non-tsl_only strategies)
                if tgt_id:
                    try:
                        kite.cancel_order(variety="regular", order_id=tgt_id)
                    except Exception as ce:
                        print(Fore.YELLOW + f"⚠️  Could not cancel target for {symbol}: {ce}" + ts())
                msg = f"🔴 {symbol} STOP LOSS HIT" + (" | Target order cancelled" if tgt_id else " | TSL-Only position closed")
            ...
            # ── Target Hit (standard strategies only) ────────────────────
            if tgt_id:
                tgt_order = next((o for o in orders if o["order_id"] == tgt_id), None)
                if tgt_order is None:
                    print(Fore.RED + f"⚠️  OCO: Target order not found for {symbol}, exiting monitor" + ts())
                    write_log(f"OCO target order not found for {symbol}")
                    break
            ...
            # ── Live TSL update for tsl_only positions ───────────────────
            # When price moves enough to trigger a trail step, modify the
            # live SL-M order on Kite to lock in the new stop level.
            if tgt_id is None and symbol in paper_positions:
                ...
                if pos.get("status") == "OPEN":
                    ...
                    if current_sl and current_sl != trade.get("_last_live_sl"):
                        try:
                            ...
                            print(Fore.YELLOW + f"🔄 LIVE TSL UPDATED | {symbol} | New SL: {current_sl}" + ts())
                            write_log(f"LIVE TSL UPDATED {symbol} SL:{current_sl}")
                        except Exception as te:
                            print(Fore.YELLOW + f"⚠️  TSL modify failed for {symbol}: {te}" + ts())
...
def _refresh_orb_cache_loop():
    ...
    while True:
        try:
            ...
            # Guard: only treat as "after market" if we are within market hours (09:15–15:30).
            # If started outside market hours (e.g. 22:00 on AWS for testing),
            # sleep and wait rather than permanently killing the loop.
            if not ("09:15" <= now_time <= "15:30"):
...
def paper_position_manager():
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            # =============================
            # TRAILING LOGIC
            # Use per-position tsl_step if set (e.g. S9 uses 1.5%),
            # otherwise fall back to global TRAIL_STEP_PERCENT.
            # =============================
            ...
            # =============================
            # EXIT CONDITIONS
            # tsl_only positions: exit only on SL hit — no target check.
            # =============================
            ...
            if side == "BUY":
                if ltp <= pos["sl"]:
                    exit_price  = pos["sl"]
                    exit_reason = "SL HIT"
                elif not tsl_only_pos and ltp >= pos["target"]:
                    exit_price  = pos["target"]
...
def _build_first5(symbol, ltp, current_time):
    ...
    if current_time >= "09:20":
        ...
        if c["high"] != 0 and c["low"] != 999999:
            ...
            if not was_ready:
                print(Fore.CYAN +
                    f"📊 F5 CANDLE FROZEN | {symbol} | "
                    f"Open:{c['open']:.2f}  High:{c['high']:.2f}  "
                    f"Low:{c['low']:.2f}  Close:{c['close']:.2f}  "
                    f"Range:{((c['high']-c['low'])/c['open']*100):.2f}%" + ts())
...
def strategy_open_high_breakdown(ctx):
    ...
    # ── Condition A: Open == High (within 0.1% tolerance) ───────────────
    # Use first5 open as the true market open (frozen at first tick of day).
    # Open==High must STILL be valid at signal time — day_high must not have
    # exceeded first5_open by more than 0.1%, meaning price never rallied.
    day_open = f5["open"]
    day_high = latest_highs.get(symbol, f5["high"])

    open_equals_high_pct = abs(day_high - day_open) / day_open * 100
    if open_equals_high_pct > 0.1:
        _scan_record(_SNAME, symbol,
            f"Open≠High: open={day_open:.2f} high={day_high:.2f} diff={open_equals_high_pct:.2f}%")
        return
    ...
    # ── Condition C: Previous day must be GREEN (close > open) ──────────
    yest_open  = yest.get("open", 0)
    yest_close = yest.get("close", 0)
    if yest_open == 0 or yest_close <= yest_open:
        _scan_record(_SNAME, symbol,
            f"Yest not green: yest_open={yest_open:.2f} yest_close={yest_close:.2f}")
        return

    # ── Entry: LTP breaks below first-5 candle low ──────────────────────
    entry_level = f5["low"]
    sl_level    = round(day_high + 1, 2)   # SL = day high (= open) + 1 point
    ...
    # Stale guard: if price already broke >0.5% below first-5 low earlier
    # and has since bounced back — re-test, not fresh breakdown.
...
def auto_square_off():
    ...
    while True:
        ...
        # ── Market hours guard ───────────────────────────────────────────
        # Square-off must only fire during live market hours (09:15–15:30).
        # Without this, starting the algo after 15:15 (e.g. at 22:00 on AWS
        # for testing) immediately triggers square-off because
        # "22:00" >= "15:15" is True in string comparison.
            ...
            def _replay_exit_check(price_for_trail):
                ...
                if side == "BUY":
                    if low <= pos["sl"]:
                        exit_price  = pos["sl"]
                        exit_reason = "SL HIT"
                    elif not pos.get("tsl_only") and high >= pos["target"]:
                        exit_price  = pos["target"]
                        exit_reason = "TARGET HIT"
                elif side == "SELL":
                    if high >= pos["sl"]:
                        exit_price  = pos["sl"]
                        exit_reason = "SL HIT"
                    elif not pos.get("tsl_only") and low <= pos["target"]:
                        exit_price  = pos["target"]
                        exit_reason = "TARGET HIT"
...
def _dashboard_get_json():
    ...
    positions = _read_csv(POSITIONS_CACHE_FILE)
    trades    = _read_csv(paper_trade_log_file)
    log_lines = _read_log(LOG_TXT_FILE)

    # Inject live LTP and unrealized PnL into each open position row
    for p in positions:
        try:
            ...
            net    = gross - (BROKERAGE_PER_ORDER * 2)
            p["ltp"]      = round(ltp, 2)
            p["live_pnl"] = round(net, 2)
        except Exception:
            p["ltp"] = ""
            p["live_pnl"] = ""

    # Use in-memory daily_pnl (closed PnL) — more current than CSV snapshot
    try:    closed_pnl_snap = float(daily_pnl)
    except Exception: closed_pnl_snap = 0.0

    # Compute live open/unrealized PnL from in-memory paper_positions
    open_pnl_snap = 0.0
    try:
        for sym, pos in paper_positions.items():
            ...
            side  = pos["side"]
            points = (ltp - entry) if side == "BUY" else (entry - ltp)
            open_pnl_snap += points * qty
    except Exception:
        pass

    total_pnl_snap = round(closed_pnl_snap + open_pnl_snap, 2)

    open_c = closed_c = wins = losses = 0
    for p in positions:
        try:
            if p.get("status") == "OPEN":  open_c   += 1
            else:                          closed_c += 1
