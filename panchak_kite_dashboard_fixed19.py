...
# ═══════════════════════════════════════════════════════════════════════
with tabs[0]:
    st.header("🔭 ASTRO INTELLIGENCE CENTRE")
    st.caption(
        "Consolidated astro signals from Banerjee (Stock Market Astrology, 2009) · "
        "Vasudev (Vedic Astrology in Money Matters) · KP Krishnamurti system. "
        "**NOT financial advice. Use alongside technical analysis.**"
    )

    # ── LIVE TODAY SNAPSHOT ─────────────────────────────────────────────
    try:
        _as = _vedic_day_analysis(datetime.now(IST).date())

        _col_a, _col_b, _col_c, _col_d, _col_e = st.columns(5)
        _overall_color = "🟢" if "BULLISH" in _as["overall"] else ("🔴" if "BEARISH" in _as["overall"] else "⚠️")
        _col_a.metric("📅 Today Signal",   f"{_overall_color} {_as['overall']}")
        _col_b.metric("🌙 Moon Nakshatra", _as["moon_nak"])
        _col_c.metric("🌙 Moon Rashi",     _as["moon_rashi"])
        _col_d.metric("📆 Tithi",          _as["tithi"].split("(")[0].strip())
        _col_e.metric("⚖️ Score",          f"{_as['net_score']:+d} (B{_as['bearish_pts']}/G{_as['bullish_pts']})")

        st.caption(
            f"☀️ **Sun:** {_as['sun_rashi']} / {_as['sun_nak']}  ·  "
            f"♀ **Venus:** {_as['venus_rashi']}  ·  "
            f"♂ **Mars:** {_as['mars_rashi']} / {_as['mars_nak']}  ·  "
            f"♄ **Saturn:** {_as['saturn_rashi']}  ·  "
            f"♃ **Jupiter:** {_as['jupiter_rashi']}  ·  "
            f"☿ **Mercury:** {_as['mercury_rashi']}  ·  "
            f"☊ **Rahu:** {_as['rahu_rashi']}  ·  "
            f"Mars–Rahu: **{_as['mars_rahu_deg']}°**  ·  "
            f"Sun–Saturn: **{_as['sun_saturn_deg']}°**"
        )
        st.markdown("**Active signals today:**")
        for _st_text, _st_type in _as["signals"]:
            _ico = {"B":"🔴","G":"🟢","T":"⚠️","M":"🟡"}.get(_st_type,"")
            st.markdown(f"&nbsp;&nbsp;{_ico} {_st_text}")
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 1 — NEXT 5-DAY CONFIRMED SIGNAL FORECAST
    # Banerjee Ch8: Nakshatra lord + aspects = buy/sell. Use Moon's nakshatra,
    # check whether nakshatra lord receives benefic or malefic aspects.
    # Vasudev: fertile/barren signs, planet bias.
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("📅 Next 5 Trading Days — Multi-Source Confirmed Signals")
    st.caption(
        "Combines Vasudev (planet/sign bias, fertile-barren signs), Banerjee (nakshatra lord aspects), "
        "and existing 30-rule engine. **Confidence = rules in agreement / total rules checked.**"
    )

    # --- helper: planet aspect quality based on current rashi friendships ---
    # Banerjee Ch7 transit rules: Sun/Mars/Moon/Guru = benefics to fiery/watery signs
    # Budha/Sukra/Sani/Rahu = malefics
    _BENEFIC_PLANETS  = {"Sun","Jupiter","Moon","Mars"}
    _MALEFIC_PLANETS  = {"Saturn","Rahu","Ketu","Mercury","Venus"}  # context-dependent but market general

    # Nakshatra lords (Banerjee Table Ch4)
    _NAK_LORD_MAP = {
    ...
    # Vasudev fertile/barren sign bias
    _FERTILE_SIGNS = {"Aries","Taurus","Leo","Scorpio","Sagittarius"}
    _BARREN_SIGNS  = {"Gemini","Cancer","Libra","Aquarius","Pisces"}

    # Planet market bias from Vasudev Ch5
    _PLANET_BIAS = {
        "Sun":"BULLISH","Moon":"BEARISH","Mars":"BULLISH","Mercury":"BEARISH",
        "Jupiter":"BEARISH","Venus":"BULLISH","Saturn":"BULLISH",
        "Rahu":"BULLISH","Ketu":"BEARISH",
    }

    # Business-cycle recession combo: Sani in fiery sign + Guru aspect = crash
    _FIERY_SIGNS = {"Aries","Leo","Sagittarius"}

    _forecast5 = []
    _check5 = datetime.now(IST).date()
    _found5  = 0

    for _di5 in range(1, 20):
        _fd5 = _check5 + timedelta(days=_di5)
        if _fd5.weekday() >= 5 or _fd5 in _get_nse_holidays():
            continue
        try:
            _r5 = _vedic_day_analysis(_fd5)
        except Exception:
            continue

        # --- Signal Engine 1: Nakshatra lord aspect (Banerjee Ch8) ---
        _nak_lord = _NAK_LORD_MAP.get(_r5["moon_nak"], "")
        _nak_signal = "BULLISH" if _nak_lord in _BENEFIC_PLANETS else ("BEARISH" if _nak_lord in _MALEFIC_PLANETS else "MIXED")

        # --- Signal Engine 2: Moon rashi Vasudev ---
        _mrashi = _r5["moon_rashi"]
        if _mrashi in _FERTILE_SIGNS:
            _rashi_signal = "BULLISH"
        elif _mrashi in _BARREN_SIGNS:
            _rashi_signal = "BEARISH"
        else:
            _rashi_signal = "MIXED"

        # --- Signal Engine 3: Sun rashi Vasudev (monthly bias) ---
        _srashi = _r5["sun_rashi"]
        if _srashi in {"Sagittarius","Leo","Aries","Scorpio","Capricorn"}:
            _sun_signal = "BULLISH"
        elif _srashi in {"Aquarius","Cancer","Gemini"}:
            _sun_signal = "BEARISH"
        else:
            _sun_signal = "MIXED"

        # --- Signal Engine 4: Existing 30-rule engine net score ---
        _net5 = _r5["net_score"]
        if _net5 >= 2:
            _engine_signal = "BULLISH"
        elif _net5 <= -2:
            _engine_signal = "BEARISH"
        else:
            _engine_signal = "MIXED"

        # --- Signal Engine 5: Tithi pattern ---
        _tithi5 = _r5["tithi"]
        if any(x in _tithi5 for x in ["Amavasya","Krishna Pratipada","Chaturdashi"]):
            _tithi_signal = "BEARISH"
        elif any(x in _tithi5 for x in ["Purnima","Shukla Panchami","Shukla Ekadashi","Shukla Tritiya"]):
            _tithi_signal = "BULLISH"
        else:
            _tithi_signal = "MIXED"

        # --- Aggregate: count bull vs bear votes ---
        _votes = [_nak_signal, _rashi_signal, _sun_signal, _engine_signal, _tithi_signal]
        _bull_votes = _votes.count("BULLISH")
        _bear_votes = _votes.count("BEARISH")
        _total_decisive = _bull_votes + _bear_votes

        if _bull_votes >= 4:
            _final = "🟢 STRONG BULLISH"
            _confidence = "Very High"
            _action = "✅ BUY CALLS / Go LONG next morning"
        elif _bull_votes == 3:
            _final = "🟢 BULLISH"
            _confidence = "High"
            _action = "✅ Lean LONG — buy dips"
        elif _bear_votes >= 4:
            _final = "🔴 STRONG BEARISH"
            _confidence = "Very High"
            _action = "❌ BUY PUTS / Go SHORT next morning"
        elif _bear_votes == 3:
            _final = "🔴 BEARISH"
            _confidence = "High"
            _action = "❌ Lean SHORT — sell rallies"
        elif _bull_votes == 2 and _bear_votes <= 1:
            _final = "🟡 MILD BULLISH"
            _confidence = "Medium"
            _action = "⚠️ Wait for technical confirmation"
        elif _bear_votes == 2 and _bull_votes <= 1:
            _final = "🟡 MILD BEARISH"
            _confidence = "Medium"
            _action = "⚠️ Avoid longs, wait for confirmation"
        else:
            _final = "🟡 MIXED / VOLATILE"
            _confidence = "Low"
            _action = "⛔ AVOID — contradictory signals"

        # Conjunction override — extreme warnings
        _override_note = ""
        if _r5["mars_rahu_deg"] < 10:
            _final = "🔴 EXTREME BEARISH"
            _confidence = "Very High"
            _action = "❌ STRONG SHORT — Mars–Rahu < 10° (lower circuit risk)"
            _override_note = f"⚡ Mars–Rahu {_r5['mars_rahu_deg']}°"
        if _r5["sun_saturn_deg"] < 6:
            _final = "🔴 STRONG BEARISH"
            _confidence = "High"
            _action = "❌ SHORT — Sun–Saturn < 6°"
            _override_note += f" | ☀♄ {_r5['sun_saturn_deg']}°"

        _forecast5.append({
            "Date": _fd5.strftime("%a %d %b"),
            "Moon Nak (Lord)": f"{_r5['moon_nak']} ({_nak_lord})",
            "Moon Rashi": _mrashi,
            "Tithi": _r5["tithi"].split("(")[0].strip(),
            "Nak Signal": f"{'🟢' if _nak_signal=='BULLISH' else '🔴' if _nak_signal=='BEARISH' else '🟡'} {_nak_signal}",
            "Rashi Signal": f"{'🟢' if _rashi_signal=='BULLISH' else '🔴' if _rashi_signal=='BEARISH' else '🟡'} {_rashi_signal}",
            "Engine Score": f"{_net5:+d}",
            "Votes B/Bear": f"🟢{_bull_votes} / 🔴{_bear_votes}",
            "FINAL SIGNAL": _final,
            "Confidence": _confidence,
            "Action": _action,
            "Override": _override_note,
        })
        _found5 += 1
        if _found5 >= 5:
            break
    ...
    if _forecast5:
        _df5 = pd.DataFrame(_forecast5)

        def _f5_signal_color(val):
            v = str(val)
            if "STRONG BULLISH" in v or "EXTREME BULLISH" in v: return "background-color:#b8f5d0;color:#0a5c2f;font-weight:700"
            if "BULLISH" in v: return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
            if "EXTREME BEARISH" in v or "STRONG BEARISH" in v: return "background-color:#f5b8b8;color:#7b1c1c;font-weight:700"
            if "BEARISH" in v: return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            if "MIXED" in v or "MILD" in v: return "background-color:#fff9e6;color:#7d5a00"
            return ""

        def _f5_conf_color(val):
            v = str(val)
            if v == "Very High": return "background-color:#1a6b3c;color:white;font-weight:700"
            if v == "High":      return "background-color:#27ae60;color:white;font-weight:600"
            if v == "Medium":    return "background-color:#f39c12;color:white"
            if v == "Low":       return "background-color:#e74c3c;color:white"
            return ""

        st.dataframe(
            _df5.style
                .map(_f5_signal_color, subset=["FINAL SIGNAL","Nak Signal","Rashi Signal"])
                .map(_f5_conf_color,   subset=["Confidence"])
                .set_properties(**{"font-size":"12px","text-align":"left"})
                .set_table_styles([{"selector":"th","props":[("font-weight","bold"),("background-color","#0d1b2a"),("color","white")]}]),
            use_container_width=True,
            height=230,
        )

        # Key notes per day
        with st.expander("📋 Day-by-Day Position Guidance"):
            for _row5 in _forecast5:
                _icon5 = "🟢" if "BULLISH" in _row5["FINAL SIGNAL"] else ("🔴" if "BEARISH" in _row5["FINAL SIGNAL"] else "🟡")
                st.markdown(f"**{_icon5} {_row5['Date']} — {_row5['FINAL SIGNAL']}** (Confidence: {_row5['Confidence']})")
                st.markdown(f"&nbsp;&nbsp;📌 Action: **{_row5['Action']}**")
                st.markdown(f"&nbsp;&nbsp;🌙 {_row5['Moon Nak (Lord)']} · {_row5['Moon Rashi']} · {_row5['Tithi']}")
                st.markdown(f"&nbsp;&nbsp;Votes — Bull: {_row5['Votes B/Bear']} · Engine Score: {_row5['Engine Score']}")
                if _row5["Override"]:
                    st.markdown(f"&nbsp;&nbsp;⚡ **Override active: {_row5['Override']}**")
                st.markdown("---")

    st.divider()

    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 2 — INTRADAY TIMING MAP (Today)
    # Banerjee Ch10: Hora chart. Each planetary hora = 1 hour from sunrise.
    # Benefic aspects = BUY, Malefic aspects = SELL during that hora.
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("⏰ Today's Intraday Timing Map — Hora (Planetary Hours)")
    st.caption(
        "**Source: Banerjee Ch10 + Vasudev Ch.7** — Each hora = 1 hour starting from sunrise. "
        "Planet ruling hora + aspects it receives = BULLISH or BEARISH window. "
        "Sunrise Mumbai ≈ 06:15–06:30 IST (varies by date). Market opens 09:15."
    )

    # Hora chart (Banerjee Fig1 — standard Vedic Hora)
    # Row = hora number (1-24 from sunrise), Col = weekday (0=Sun,...,6=Sat)
    _HORA_TABLE = [
        ["Sun","Venus","Mars","Mercury","Jupiter","Venus","Saturn"],  # Hora 1
    ...
    # Planet → market direction per Banerjee Ch10 summary + Vasudev Ch5
    # When aspected by BENEFICS (Sun/Moon/Jupiter/Mars in friendly rashi) → BUY
    # When aspected by MALEFICS (Saturn/Rahu/Mercury/Ketu) → SELL
    _HORA_PLANET_SIGNAL = {
        "Sun":     {"buy_sectors":"Power, Gold, Media, PSUs",         "bias":"BULLISH", "buy_aspects":"Moon/Mars/Jupiter","sell_aspects":"Venus/Saturn/Rahu"},
        "Moon":    {"buy_sectors":"Crude, Silver, Oil&Gas, Shipping", "bias":"BEARISH", "buy_aspects":"Sun/Jupiter",      "sell_aspects":"Rahu/Saturn"},
        "Mars":    {"buy_sectors":"Infra, Steel, Cement, Pharma",     "bias":"BULLISH", "buy_aspects":"Sun/Moon/Jupiter", "sell_aspects":"Mercury/Rahu/Saturn"},
        "Mercury": {"buy_sectors":"Banking, IT, Telecom, FMCG",       "bias":"BEARISH", "buy_aspects":"Sun/Venus",        "sell_aspects":"Moon/Mars"},
        "Jupiter": {"buy_sectors":"Banking, Gold, FMCG, Exports",     "bias":"MIXED",   "buy_aspects":"Sun/Moon/Mars",    "sell_aspects":"Mercury/Venus/Rahu"},
        "Venus":   {"buy_sectors":"Silver, Jewel, FMCG, Hotels",      "bias":"BULLISH", "buy_aspects":"Mercury/Saturn",   "sell_aspects":"Sun/Moon/Mars"},
        "Saturn":  {"buy_sectors":"Infra, Realty, Metals, Oil&Gas",   "bias":"BULLISH", "buy_aspects":"Mercury/Venus/Rahu","sell_aspects":"Sun/Moon/Mars"},
    }

    # Compute today's hora schedule
    _today_dt    = datetime.now(IST)
    _weekday_idx = _today_dt.weekday()  # 0=Mon…6=Sun
    # Convert to Banerjee weekday: 0=Sun 1=Mon…6=Sat
    _weekday_ban = (_weekday_idx + 1) % 7  # Mon=1,Tue=2,...,Sun=0

    # Mumbai sunrise approx (good enough): 06:15 + 1 min per 2 days from Mar equinox
    # Simple approx: use fixed 06:20 IST for Mumbai
    _sunrise_hour   = 6
    _sunrise_minute = 20
    _sunrise_base   = _today_dt.replace(hour=_sunrise_hour, minute=_sunrise_minute, second=0, microsecond=0)

    _hora_rows = []
    for _hi in range(24):
        _hora_start = _sunrise_base + timedelta(hours=_hi)
        _hora_end   = _hora_start   + timedelta(hours=1)
        _planet     = _HORA_TABLE[_hi][_weekday_ban]
        _pdata      = _HORA_PLANET_SIGNAL[_planet]

        # Only show market hours 09:15 - 15:30
        _hs_t = _hora_start.replace(tzinfo=None) if _hora_start.tzinfo else _hora_start
        if _hora_end.hour < 9 or _hora_start.hour >= 16:
            continue
        if _hora_end.hour == 9 and _hora_end.minute < 15:
            continue

        # Actual clip to market hours
        _mkt_start = _hora_start.strftime("%H:%M")
        _mkt_end   = _hora_end.strftime("%H:%M")

        _bias = _pdata["bias"]
        _is_now = (_hora_start <= _today_dt < _hora_end)

        _hora_rows.append({
            "Hora Window": f"{_mkt_start} – {_mkt_end}",
            "Ruling Planet": f"{'⭐' if _is_now else ''} {_planet}",
            "Default Bias": f"{'🟢' if _bias=='BULLISH' else '🔴' if _bias=='BEARISH' else '🟡'} {_bias}",
            "Buy When Aspected By": _pdata["buy_aspects"],
            "Sell When Aspected By": _pdata["sell_aspects"],
            "Key Sectors to Watch": _pdata["buy_sectors"],
            "NOW": "◀ ACTIVE" if _is_now else "",
        })

    if _hora_rows:
        _df_hora = pd.DataFrame(_hora_rows)

        def _hora_bias_color(val):
            v = str(val)
            if "BULLISH" in v: return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
            if "BEARISH" in v: return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            if "MIXED"   in v: return "background-color:#fff9e6;color:#7d5a00"
            return ""

        def _hora_now_color(val):
            if "ACTIVE" in str(val): return "background-color:#ff6b35;color:white;font-weight:700"
            return ""

        st.dataframe(
            _df_hora.style
                .map(_hora_bias_color, subset=["Default Bias"])
                .map(_hora_now_color,  subset=["NOW"])
                .set_properties(**{"font-size":"12px","text-align":"left"})
        ...
        st.info(
            "🧭 **How to use:** Find today's Moon nakshatra lord above. "
            "If nakshatra lord is a BENEFIC planet (Sun/Moon/Jupiter/Mars) → day bias BULLISH; "
            "enter longs during BULLISH hora windows. "
            "If nakshatra lord is MALEFIC (Saturn/Rahu/Mercury/Ketu) → day bias BEARISH; "
            "sell rallies during BEARISH hora windows. "
            "For intraday: strongest moves occur when HORA planet's bias matches NAKSHATRA bias. "
            "Double-confirmation = high probability trade. (Banerjee Ch8 + Ch10)"
        )

        # Today's intraday text guide
        try:
            _nak5 = _as["moon_nak"]
            _lord5 = _NAK_LORD_MAP.get(_nak5, "Unknown")
            _day_bias = "BULLISH" if _lord5 in _BENEFIC_PLANETS else "BEARISH"
            _icon_db = "🟢" if _day_bias == "BULLISH" else "🔴"

            with st.expander(f"📐 Today's Intraday Guide — Moon in {_nak5} (Lord: {_lord5}) → {_icon_db} {_day_bias} Day"):
                if _day_bias == "BULLISH":
                    st.markdown(f"""
**Day Bias: 🟢 BULLISH** (Nakshatra lord {_lord5} is a benefic planet)

**Strategy:**
- Morning (09:15–10:30): Watch for gap-up confirmation. If market opens positive → BUY CALLS in first 15 min
- Mid-session: Look for BULLISH hora windows (Sun/Mars/Venus hora) → add longs
- Avoid shorts unless you see 3+ bearish signals converge
- Target: 50–75% of daily expected range (Moon degree × 3 = max Nifty points)
- Moon degree at open × 3 = approximate max intraday move today

**Best trade setup:** Buy on first 5-min candle close above 9:15 open. Hold through bullish horas.
                    """)
                else:
                    st.markdown(f"""
**Day Bias: 🔴 BEARISH** (Nakshatra lord {_lord5} is a malefic planet)

**Strategy:**
- Morning (09:15–10:30): Watch for gap-down confirmation. If market opens negative → BUY PUTS / short Nifty
- Mid-session: Look for BEARISH hora windows (Saturn/Mercury hora) → add shorts
- Any rally to VWAP or 15-min EMA = SHORT opportunity
- Avoid longs. If forced to buy, keep strict SL
- Target: 50–75% of daily expected range on downside

**Best trade setup:** Short on first 5-min candle close below 9:15 open. Hold through bearish horas.
                    """)
        except Exception:
            pass
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 3 — BANERJEE SECTOR ALERTS (today's nakshatra-based sector calls)
    # Banerjee Ch8: each nakshatra day → specific sectors BUY or SELL
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("🏭 Today's Sector Astro Alerts — Banerjee Nakshatra × Sector System")
    st.caption("Source: Banerjee Ch8 (nakshatra lord × sector) + Ch4 (graha → sector). "
               "BUY when nakshatra lord is aspected by benefics; SELL when aspected by malefics.")

    _NAK_SECTOR_MAP = {
        "Ashwini":    {"lord":"Ketu",    "sectors":"Power, IT, Leather, Pharma, Crude, Infrastructure, Auto"},
        "Bharani":    {"lord":"Venus",   "sectors":"Silver, Jewelry, Gems, FMCG, Banking, Hotels, Sugar, Agri"},
        "Krittika":   {"lord":"Sun",     "sectors":"Power, Gold, Media, Nuclear, PSUs, Wheat"},
        "Rohini":     {"lord":"Moon",    "sectors":"Crude, Silver, Oil&Gas, Shipping, Milk, Cold Drinks"},
        "Mrigasira":  {"lord":"Mars",    "sectors":"Healthcare, Auto, Pharma, Infra, Cement, Steel, Metals"},
        "Ardra":      {"lord":"Rahu",    "sectors":"Media, Telecom, Power, Realty, Infrastructure, Metals"},
        "Punarvasu":  {"lord":"Jupiter", "sectors":"Banking, Insurance, Jewelry, Gold, FMCG, Textiles, Exports"},
        "Pushya":     {"lord":"Saturn",  "sectors":"Cement, Infrastructure, Iron&Steel, Leather, Crude, Realty"},
        "Ashlesha":   {"lord":"Mercury", "sectors":"Telecom, IT, FMCG, Banking, Aviation, Textiles, Agro"},
        "Magha":      {"lord":"Ketu",    "sectors":"Power, IT, Leather, Pharma, Crude, Infrastructure, Auto"},
        "Purva Phalguni":  {"lord":"Venus",  "sectors":"Silver, Jewelry, Gems, FMCG, Banking, Hotels, Sugar"},
        "Uttara Phalguni": {"lord":"Sun",    "sectors":"Power, Gold, Media, Nuclear, PSUs, Wheat"},
        "Hasta":      {"lord":"Moon",    "sectors":"Crude, Silver, Oil&Gas, Shipping, Milk, Cold Drinks"},
        "Chitra":     {"lord":"Mars",    "sectors":"Healthcare, Auto, Pharma, Infra, Cement, Steel, Metals"},
        "Swati":      {"lord":"Rahu",    "sectors":"Media, Telecom, Power, Realty, Infrastructure, Metals"},
        "Vishakha":   {"lord":"Jupiter", "sectors":"Banking, Insurance, Jewelry, Gold, FMCG, Textiles, Exports"},
        "Anuradha":   {"lord":"Saturn",  "sectors":"Cement, Infrastructure, Iron&Steel, Leather, Crude, Realty"},
        "Jyeshtha":   {"lord":"Mercury", "sectors":"Telecom, IT, FMCG, Banking, Aviation, Textiles, Agro"},
        "Mula":       {"lord":"Ketu",    "sectors":"Power, IT, Leather, Pharma, Crude, Infrastructure, Auto"},
        "Purva Ashadha":  {"lord":"Venus",  "sectors":"Silver, Jewelry, Gems, FMCG, Banking, Hotels, Sugar"},
        "Uttara Ashadha": {"lord":"Sun",    "sectors":"Power, Gold, Media, Nuclear, PSUs, Wheat"},
        "Shravana":   {"lord":"Moon",    "sectors":"Crude, Silver, Oil&Gas, Shipping, Milk, Cold Drinks"},
        "Dhanishtha": {"lord":"Mars",    "sectors":"Healthcare, Auto, Pharma, Infra, Cement, Steel, Metals"},
        "Shatabhisha":{"lord":"Rahu",    "sectors":"Media, Telecom, Power, Realty, Infrastructure, Metals"},
        "Purva Bhadrapada":  {"lord":"Jupiter","sectors":"Banking, Insurance, Jewelry, Gold, FMCG, Textiles, Exports"},
        "Uttara Bhadrapada": {"lord":"Saturn", "sectors":"Cement, Infrastructure, Iron&Steel, Leather, Crude, Realty"},
        "Revati":     {"lord":"Mercury", "sectors":"Telecom, IT, FMCG, Banking, Aviation, Textiles, Agro"},
    }

    try:
        _today_nak  = _as["moon_nak"]
        _sdata      = _NAK_SECTOR_MAP.get(_today_nak, {})
        _slord      = _sdata.get("lord","Unknown")
        _ssectors   = _sdata.get("sectors","—")
        _sis_benefic = _slord in _BENEFIC_PLANETS
        _s_call     = "🟢 BUY" if _sis_benefic else "🔴 SELL / SHORT"
        _s_color    = "#d5f5e3" if _sis_benefic else "#fde8e8"
        _s_text_col = "#1e8449" if _sis_benefic else "#c0392b"

        st.markdown(f"""
<div style="background:{_s_color};padding:15px;border-radius:8px;margin:8px 0;">
<h4 style="color:{_s_text_col};margin:0">Moon in <b>{_today_nak}</b> — Nakshatra Lord: <b>{_slord}</b> → {_s_call}</h4>
<p style="margin:4px 0;color:#333"><b>Sectors to watch:</b> {_ssectors}</p>
<p style="margin:4px 0;color:#555">
{'✅ <b>Buy</b> these sector stocks as the day opens. Nakshatra lord (' + _slord + ') is a BENEFIC planet — investor sentiment positive for above sectors.' if _sis_benefic else '❌ <b>Sell/Short</b> these sector stocks on any rally. Nakshatra lord (' + _slord + ') is a MALEFIC planet — investor sentiment negative for above sectors.'}
</p>
</div>
""", unsafe_allow_html=True)

    except Exception as _e:
        st.warning(f"Sector alert error: {_e}")

    st.divider()

    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 4 — 10-DAY DYNAMIC FORECAST (existing engine, moved here)
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("🔮 10-Day Nifty Astro Forecast — Full Detail")
    st.caption(
        "Computed live from planetary positions (Lahiri ayanamsa, Jean Meeus Ch47). "
        "Combines Moon nakshatra, tithi, conjunction angles, Vasudev sign-bias rules."
    )

    _forecast_rows2 = []
    _check_date2 = datetime.now(IST).date()
    _days_found2 = 0

    for _di2 in range(1, 40):
        _fd2 = _check_date2 + timedelta(days=_di2)
        if _fd2.weekday() >= 5 or _fd2 in _get_nse_holidays():
            continue
        try:
            _r2 = _vedic_day_analysis(_fd2)
        except Exception:
            continue

        _conj2 = []
        if _r2["moon_ketu_deg"]  < 15: _conj2.append(f"🌙-Ketu {_r2['moon_ketu_deg']}°")
        if _r2["mars_rahu_deg"]  < 15: _conj2.append(f"♂-Rahu {_r2['mars_rahu_deg']}°")
        if _r2["sun_saturn_deg"] <  8: _conj2.append(f"☀-♄ {_r2['sun_saturn_deg']}°")

        _sig_parts2 = []
        for _txt2, _typ2 in _r2["signals"]:
            _ico2 = {"B":"🔴","G":"🟢","T":"⚠️","M":"🟡"}.get(_typ2,"")
            _sig_parts2.append(f"{_ico2} {_txt2}")

        _intra2 = ""
        if _r2["moon_nak"] == "Rohini":           _intra2 = "⚠️ TRAP — sell the rally"
        elif _r2["moon_rashi"] == "Taurus":        _intra2 = "🔄 Gap-down → recovery"
        elif _r2["moon_rashi"] == "Sagittarius":   _intra2 = "🟢 Bullish open expected"
        elif "Amavasya" in _r2["tithi"] or "Pratipada" in _r2["tithi"]:
            _intra2 = "🔴 Sell on any rise"

        _forecast_rows2.append({
            "Date":              _fd2.strftime("%a %d %b"),
            "Signal":            _r2["overall"],
            "Intensity":         _r2["intensity"],
            "Moon Nakshatra":    f"{_r2['moon_nak']} ({_r2['moon_nak_lord']})",
            "Moon Rashi":        _r2["moon_rashi"],
            "Tithi":             _r2["tithi"].split("(")[0].strip(),
            "Key Conjunctions":  "  ·  ".join(_conj2) if _conj2 else "—",
            "Intraday Hint":     _intra2 if _intra2 else "—",
            "Score":             f"{_r2['net_score']:+d} (B{_r2['bearish_pts']}/G{_r2['bullish_pts']})",
            "Key Signals":       "  |  ".join(_sig_parts2[:3]) if _sig_parts2 else "No major signals",
        })
        _days_found2 += 1
        if _days_found2 >= 10:
            break
    ...
    if _forecast_rows2:
        _fdf2 = pd.DataFrame(_forecast_rows2)

        _nb2 = sum(1 for r in _forecast_rows2 if "BEARISH" in r["Signal"])
        _ng2 = sum(1 for r in _forecast_rows2 if "BULLISH" in r["Signal"])
        _nt2 = sum(1 for r in _forecast_rows2 if "TRAP"    in r["Signal"])
        _nm2 = sum(1 for r in _forecast_rows2 if "MIXED"   in r["Signal"])
        st.caption(f"**10-day summary:** 🔴 {_nb2} Bearish  ·  🟢 {_ng2} Bullish  ·  ⚠️ {_nt2} Trap  ·  🟡 {_nm2} Mixed")

        def _fc2_signal_color(val):
            v = str(val)
            if "BEARISH" in v and "BULLISH" not in v: return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            if "BULLISH" in v and "BEARISH" not in v: return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
        ...
        st.dataframe(
            _fdf2.style
                .map(_fc2_signal_color, subset=["Signal"])
                .set_properties(**{"font-size":"12px","text-align":"left"})
                .set_table_styles([{"selector":"th","props":[("font-weight","bold"),("background-color","#1a1a2e"),("color","white")]}]),
            use_container_width=True,
            height=400,
        ...
        with st.expander("🔍 Full Detail — All 10 Days"):
            for _row_d2 in _forecast_rows2:
                _ico2 = ("🔴" if "BEARISH" in _row_d2["Signal"] else
                         "🟢" if "BULLISH" in _row_d2["Signal"] else
                         "⚠️" if "TRAP"    in _row_d2["Signal"] else "🟡")
                st.markdown(f"**{_ico2} {_row_d2['Date']} — {_row_d2['Signal']} ({_row_d2['Intensity']})**")
                st.markdown(f"&nbsp;&nbsp;🌙 Moon: **{_row_d2['Moon Nakshatra']}** · Rashi: {_row_d2['Moon Rashi']} · {_row_d2['Tithi']}")
                if _row_d2["Key Conjunctions"] != "—":
                    st.markdown(f"&nbsp;&nbsp;⚡ {_row_d2['Key Conjunctions']}")
                if _row_d2["Intraday Hint"] != "—":
                    st.markdown(f"&nbsp;&nbsp;💡 Intraday: {_row_d2['Intraday Hint']}")
                for _line2 in _row_d2["Key Signals"].split("  |  "):
                    if _line2.strip():
                        st.markdown(f"&nbsp;&nbsp;&nbsp;&nbsp;{_line2.strip()}")
                st.markdown("---")

    st.divider()

    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 5 — BUSINESS CYCLE INDICATOR (Banerjee Ch16)
    # Saturn in fiery sign + Jupiter aspect → RECESSION WARNING
    # Saturn in earthy → RECOVERY, airy → EXPANSION, watery → BOOM
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("📊 Business Cycle Phase Indicator — Banerjee Ch16 (Saturn–Jupiter)")
    st.caption(
        "Banerjee's astrological theory of business cycles. "
        "Saturn's rashi tattva + Jupiter aspect = phase indicator. "
        "Historical proof: Great Depression 1930, Subprime 2008."
    )

    try:
        _snap_bc = _vedic_day_analysis(datetime.now(IST).date())
        _sat_rashi_bc = _snap_bc["saturn_rashi"]
        _jup_rashi_bc = _snap_bc["jupiter_rashi"]

        _EARTHY = {"Taurus","Virgo","Capricorn"}
        _AIRY   = {"Gemini","Libra","Aquarius"}
        _WATERY = {"Cancer","Scorpio","Pisces"}
        _FIERY2 = {"Aries","Leo","Sagittarius"}

        if _sat_rashi_bc in _FIERY2:
            _phase = "⚠️ RECESSION / CRASH RISK"
            _phase_desc = "Saturn in fiery sign. If Jupiter also aspects → ECONOMIC RECESSION. Banerjee: 'Cash is King — stay out of markets.'"
            _phase_color = "#fde8e8"
            _phase_text  = "#c0392b"
            _phase_action = "🚨 DEFENSIVE: Reduce exposure. Hold cash. Buy puts on major indices."
        elif _sat_rashi_bc in _EARTHY:
            _phase = "🌱 RECOVERY / REVIVAL PHASE"
            _phase_desc = "Saturn in earthy sign. Bottom-fishing in fundamentally strong stocks. Fresh investment opportunities."
            _phase_color = "#e8f8ee"
            _phase_text  = "#1e8449"
            _phase_action = "✅ ACCUMULATE: Buy quality stocks at dips. Long-term entry zone."
        elif _sat_rashi_bc in _AIRY:
            _phase = "📈 GROWTH & EXPANSION PHASE"
            _phase_desc = "Saturn in airy sign. GDP growth visible, EPS rising, markets expanding. Good for diversified portfolio."
            _phase_color = "#e8f0fe"
            _phase_text  = "#1a56db"
            _phase_action = "✅ HOLD/BUY: Stay invested. Look for sector rotation opportunities."
        elif _sat_rashi_bc in _WATERY:
            _phase = "🚀 BOOM / OVER-EXPANSION PHASE"
            _phase_desc = "Saturn in watery sign. Markets flooded with investment. Peak approaching — begin profit booking."
            _phase_color = "#fff9e6"
            _phase_text  = "#7d5a00"
            _phase_action = "⚠️ BOOK PROFITS: Reduce high-risk positions. Set trailing stop losses."
        else:
            _phase = "🟡 TRANSITION"
            _phase_desc = "Saturn rashi transitioning."
            _phase_color = "#f5f5f5"
            _phase_text  = "#333"
            _phase_action = "Monitor closely."

        st.markdown(f"""
<div style="background:{_phase_color};padding:15px;border-radius:8px;margin:8px 0;border-left:5px solid {_phase_text};">
<h4 style="color:{_phase_text};margin:0">Current Phase: {_phase}</h4>
<p style="margin:6px 0;color:#333"><b>Saturn in:</b> {_sat_rashi_bc} &nbsp;|&nbsp; <b>Jupiter in:</b> {_jup_rashi_bc}</p>
<p style="margin:4px 0;color:#555">{_phase_desc}</p>
<p style="margin:6px 0;font-weight:600;color:{_phase_text}">📌 {_phase_action}</p>
</div>
""", unsafe_allow_html=True)

        # Recession watch: Saturn in fiery + Jupiter in odd sign
        _ODD_SIGNS = {"Aries","Gemini","Leo","Libra","Sagittarius","Aquarius"}
        if _sat_rashi_bc in _FIERY2 and _jup_rashi_bc in _ODD_SIGNS:
            st.error(
                f"🚨 **RECESSION COMBINATION ACTIVE:** Saturn in {_sat_rashi_bc} (fiery) + "
                f"Jupiter in {_jup_rashi_bc} (odd sign) = Banerjee's fatal combo. "
                "Historical: 1930, 1940, 1978, 1990, 2008 crashes all followed this pattern. "
                "**Recommend: Exit equity, hold cash/gold.**"
            )

    except Exception as _e:
        st.warning(f"Business cycle error: {_e}")

    st.divider()

    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 6 — GOLD & CRUDE SIGNAL (Banerjee Ch17/18 + Vasudev)
    # Gold = Jupiter cycle (Guru-Rahu conjunction = hyperinflation)
    # Crude = Saturn–Rahu/Ketu conjunction
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("🥇 Gold & 🛢️ Crude Oil Astro Signals")
    st.caption("Banerjee Ch17 (gold = Jupiter/Rahu cycle) · Ch18 (crude = Saturn/Ketu cycle) · Vasudev (sign rulerships)")

    try:
        _snap_gc = _vedic_day_analysis(datetime.now(IST).date())

        # Gold signal
        _jup_g   = _snap_gc["jupiter_rashi"]
        _rahu_g  = _snap_gc["rahu_rashi"]
        _sun_g   = _snap_gc["sun_rashi"]
        _mars_g  = _snap_gc["mars_rashi"]

        # Vasudev: Gold ruled by Sun/Mars, Leo/Aries/Scorpio signs
        _gold_bull_signs = {"Leo","Aries","Scorpio","Sagittarius","Capricorn"}
        _gold_signal = []
        if _sun_g in _gold_bull_signs:
            _gold_signal.append(f"☀️ Sun in {_sun_g} (gold-bullish sign)")
        if _mars_g in _gold_bull_signs:
            _gold_signal.append(f"♂ Mars in {_mars_g} (gold-bullish sign)")
        # Banerjee: Jupiter+Rahu within 30° = gold hyperinflation
        # approximate: same or adjacent rashi
        _RASHI_ORDER = ["Aries","Taurus","Gemini","Cancer","Leo","Virgo",
                        "Libra","Scorpio","Sagittarius","Capricorn","Aquarius","Pisces"]
        _j_idx = _RASHI_ORDER.index(_jup_g) if _jup_g in _RASHI_ORDER else -1
        _r_idx = _RASHI_ORDER.index(_rahu_g) if _rahu_g in _RASHI_ORDER else -1
        _jup_rahu_dist = abs(_j_idx - _r_idx) if _j_idx >= 0 and _r_idx >= 0 else 99
        if _jup_rahu_dist <= 1:
            _gold_signal.append(f"⚡ Jupiter–Rahu near conjunction ({_jup_g}/{_rahu_g}) = GOLD HYPERINFLATION SIGNAL (Banerjee)")

        # Crude signal
        _sat_c  = _snap_gc["saturn_rashi"]
        _ketu_rashi = _RASHI_ORDER[(_RASHI_ORDER.index(_rahu_g)+6) % 12] if _rahu_g in _RASHI_ORDER else "Unknown"
        _crude_signal = []
        # Banerjee Ch18: Saturn+Ketu conjunction = crude spike
        _s_idx = _RASHI_ORDER.index(_sat_c) if _sat_c in _RASHI_ORDER else -1
        _k_idx = _RASHI_ORDER.index(_ketu_rashi) if _ketu_rashi in _RASHI_ORDER else -1
        _sat_ketu_dist = abs(_s_idx - _k_idx) if _s_idx >= 0 and _k_idx >= 0 else 99
        if _sat_ketu_dist <= 1:
            _crude_signal.append(f"⚡ Saturn–Ketu near conjunction ({_sat_c}/{_ketu_rashi}) = CRUDE SPIKE SIGNAL (Banerjee)")
        # Mars-Rahu < 15° = crude bullish (already in engine)
        if _snap_gc["mars_rahu_deg"] < 15:
            _crude_signal.append(f"⚡ Mars–Rahu {_snap_gc['mars_rahu_deg']}° = CRUDE BULLISH (equity bearish)")

        _gc1, _gc2 = st.columns(2)
        with _gc1:
            st.markdown("**🥇 Gold Signal**")
            if _gold_signal:
                for _gs in _gold_signal:
                    st.success(_gs)
            else:
                _neg_gold = [s for s in ["Gemini","Cancer","Libra","Aquarius","Pisces"] if _sun_g == s or _mars_g == s]
                if _neg_gold:
                    st.warning(f"🔴 Gold Bearish — Sun/Mars in bearish signs ({_sun_g}/{_mars_g}). Barren signs = dull gold market.")
                else:
                    st.info(f"🟡 Gold Neutral — Sun:{_sun_g} / Mars:{_mars_g} / Jupiter:{_jup_g}")
            st.caption("Strongest GOLD signal: Jupiter–Rahu conjunction · Sun/Mars in Leo/Aries/Scorpio")

        with _gc2:
            st.markdown("**🛢️ Crude Oil Signal**")
            if _crude_signal:
                for _cs in _crude_signal:
                    st.error(_cs)
            else:
                st.info(f"🟡 Crude Neutral — Saturn:{_sat_c} / Ketu:{_ketu_rashi} / Mars–Rahu:{_snap_gc['mars_rahu_deg']}°")
            st.caption("Strongest CRUDE signal: Saturn–Ketu conjunction · Mars–Rahu < 15°")

    except Exception as _e:
        st.warning(f"Gold/crude signal error: {_e}")

    st.divider()

    # ═══════════════════════════════════════════════════════════════════════
    # SECTION 7 — VASUDEV RULES REFERENCE TABLES (collapsible)
    # ═══════════════════════════════════════════════════════════════════════
    st.subheader("📖 Vasudev Reference Tables — Planet & Sign Market Bias")

    with st.expander("🪐 Planet Market Bias (Vasudev Ch.5)"):
        _vasudev_planet_rules_a = [
            {"Planet":"☀️ Sun","Bias":"🟢 BULLISH","Duration":"Monthly","Notes":"Strong bullish. Sun in Leo/Aries/Capricorn = bullish month. Sun in Aquarius/Cancer = bearish month.","Commodity":"Gold, Copper, Wheat"},
            {"Planet":"🌕 Moon","Bias":"🔴 BEARISH","Duration":"Daily","Notes":"Bearish. If conjunct malefic → short bullish spike. Moon degree × 3 = max Nifty pts intraday.","Commodity":"Silver, Milk, Rice"},
            {"Planet":"♂ Mars","Bias":"🟢 BULLISH","Duration":"~45 days","Notes":"Fiery+aggressive. Bullish but short-lived. Mars–Rahu < 15° = extreme bearish for equity, crude bullish.","Commodity":"Gold(VM), Iron, Steel"},
            {"Planet":"☿ Mercury","Bias":"🔴 BEARISH","Duration":"~25 days","Notes":"Bearish (like Moon). Retrograde + bearish aspect on Jupiter = short stock falls.","Commodity":"Silver, Banking"},
            {"Planet":"♃ Jupiter","Bias":"🔴 BEARISH prices / 🟢 BULLISH shares","Duration":"13 months","Notes":"Creates plenty → depresses commodity prices. Retrograde = loses protective power.","Commodity":"Gold, Crude, Silver(VM)"},
            {"Planet":"♀ Venus","Bias":"🟢 BULLISH","Duration":"~25 days","Notes":"Sudden quantitative upsurge. Venus enters Capricorn = Silver TOP. Semi-Square with Sun = sustained Gold/Silver effect.","Commodity":"Silver, Diamonds, Cotton"},
            {"Planet":"♄ Saturn","Bias":"🟢 BULLISH (scarcity)","Duration":"30 months","Notes":"Scarcity → raises prices. Saturn Sextile = market move. In fiery sign + Jupiter aspect = RECESSION.","Commodity":"Iron, Coal, Oil"},
            {"Planet":"☊ Rahu","Bias":"🟢 BULLISH","Duration":"18 months","Notes":"Like Saturn. Rahu period = confusion/illusion phase — false moves.","Commodity":"Electronics, Chemicals"},
            {"Planet":"☋ Ketu","Bias":"🔴 BEARISH","Duration":"18 months","Notes":"South Node. Bearish. Moon–Ketu < 15° = negative signal.","Commodity":"Bearish most sectors"},
        ]
        _va_df_a = pd.DataFrame(_vasudev_planet_rules_a)
        def _vp_col(val):
            v = str(val)
            if "BULLISH" in v and "BEARISH" not in v: return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
            if "BEARISH" in v and "BULLISH" not in v: return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            if "BULLISH" in v and "BEARISH" in v: return "background-color:#fff3cd;color:#7d5a00;font-weight:600"
            return ""
        st.dataframe(_va_df_a.style.map(_vp_col, subset=["Bias"]).set_properties(**{"font-size":"12px"}), use_container_width=True, height=340)

    with st.expander("♈ Zodiac Sign Market Bias (Vasudev Ch.5)"):
        _sign_rules_a = [
            {"Sign":"♈ Aries","Bias":"🟢 Strong BULLISH","Nature":"Fertile","Notes":"Strongest bullish. Gold+Copper."},
            {"Sign":"♉ Taurus","Bias":"🟢 Slow BULLISH","Nature":"Fertile","Notes":"Bullish, slow. Stock exchanges+Silver."},
            {"Sign":"♊ Gemini","Bias":"🔴 BEARISH","Nature":"Barren","Notes":"Bearish+upheavals. Cotton."},
            {"Sign":"♋ Cancer","Bias":"🔴 Strong BEARISH","Nature":"Barren","Notes":"Strongest bearish. Silver+Gold."},
            {"Sign":"♌ Leo","Bias":"🟢 BULLISH","Nature":"Fertile","Notes":"Like Aries. Gold ruled by Leo."},
            {"Sign":"♍ Virgo","Bias":"🟡 Moderate BEARISH","Nature":"Moderate","Notes":"Moderate fall. Steadiness then slow mutation."},
            {"Sign":"♎ Libra","Bias":"🔴 BEARISH","Nature":"Barren","Notes":"Bearish. Short sudden spikes possible."},
            {"Sign":"♏ Scorpio","Bias":"🟢 Slow BULLISH","Nature":"Fertile","Notes":"Bullish, slow pace. Iron+Steel."},
            {"Sign":"♐ Sagittarius","Bias":"🟢 BULLISH + reversals","Nature":"Fertile","Notes":"Bullish with sudden reversals. Gold/Crude/Stocks."},
            {"Sign":"♑ Capricorn","Bias":"🟢 Moderate BULLISH","Nature":"Moderate","Notes":"Moderate bullish. Quick swings. Gold+Stocks."},
            {"Sign":"♒ Aquarius","Bias":"🔴 BEARISH","Nature":"Barren","Notes":"Bearish. Sudden short rises possible."},
            {"Sign":"♓ Pisces","Bias":"🔴 Slow BEARISH","Nature":"Barren","Notes":"Slow+steady bearish. Precious stones."},
        ]
        _sd_a = pd.DataFrame(_sign_rules_a)
        def _vs_col_a(val):
            v = str(val)
            if "Strong BULLISH" in v: return "background-color:#b8f5d0;color:#0a5c2f;font-weight:700"
            if "Slow BULLISH" in v or "Moderate BULLISH" in v: return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
            if "BULLISH" in v: return "background-color:#e8f8ee;color:#1e8449"
            if "Strong BEARISH" in v: return "background-color:#f5b8b8;color:#7b1c1c;font-weight:700"
            if "Slow BEARISH" in v or "Moderate BEARISH" in v: return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            if "BEARISH" in v: return "background-color:#fde8e8;color:#c0392b"
            return "background-color:#fff9e6;color:#7d5a00"
        def _vn_col(val):
            if val == "Fertile":  return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
            if val == "Barren":   return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            return "background-color:#fff9e6;color:#7d5a00"
        st.dataframe(_sd_a.style.map(_vs_col_a,subset=["Bias"]).map(_vn_col,subset=["Nature"]).set_properties(**{"font-size":"12px"}), use_container_width=True, height=430)

    with st.expander("🌑 Eclipse Effects on Markets (Vasudev Ch.13)"):
        _eclipse_refs = [
            {"Planet Aspects Eclipse":"☿ Mercury","Market Effect":"🟢 BULLISH (edible oils, honey)","Notes":"Rulers of country face unfavourable time."},
            {"Planet Aspects Eclipse":"♀ Venus","Market Effect":"🟢 BULLISH (trade/commerce), Inflationary","Notes":"Standing crops destroyed. Inflationary trends."},
            {"Planet Aspects Eclipse":"♂ Mars","Market Effect":"🟢 BULLISH (active buying)","Notes":"War risk, fire, new concerns floated."},
            {"Planet Aspects Eclipse":"♃ Jupiter","Market Effect":"🔴 BEARISH (subdues prices)","Notes":"Plenty → depresses prices. Bearish from market view."},
            {"Planet Aspects Eclipse":"♄ Saturn","Market Effect":"🔴 BEARISH (depression, famine)","Notes":"Famine conditions. Market depression. Metals spike if Saturn IN eclipse sign."},
            {"Planet Aspects Eclipse":"⛢ Uranus","Market Effect":"🟢 BULLISH (unchecked spurts)","Notes":"Strikes, revolutions. Prices go unchecked."},
            {"Planet Aspects Eclipse":"♆ Neptune","Market Effect":"🔴 BEARISH (chaotic, inflationary)","Notes":"Democratic system disturbed. All commodity prices climb steeply."},
            {"Planet Aspects Eclipse":"♇ Pluto","Market Effect":"🔴 EXTREME CRASH","Notes":"Devastating. Stock market ruinous. Essential commodities scarce — prices never recover."},
        ]
        _ecl_df = pd.DataFrame(_eclipse_refs)
        def _ecl_col(val):
            v = str(val)
            if "EXTREME" in v: return "background-color:#d32f2f;color:white;font-weight:700"
            if "BEARISH" in v: return "background-color:#fde8e8;color:#c0392b;font-weight:600"
            if "BULLISH" in v: return "background-color:#d5f5e3;color:#1e8449;font-weight:600"
            return ""
        st.dataframe(_ecl_df.style.map(_ecl_col,subset=["Market Effect"]).set_properties(**{"font-size":"12px"}), use_container_width=True, height=300)

    with st.expander("⚡ Key Aspects Affecting Markets (Vasudev Ch.7)"):
        st.markdown("""
| Aspect | Planets | Market Effect |
|--------|---------|---------------|
| Conjunction & Opposition (0°/180°) | All planets | **MAJOR MOVE** — direction by planet nature |
| Venus–Sun Semi-Square (45–48°) | ♀ + ☀️ | **BULLISH Gold & Silver** — prolonged effect |
| Saturn Sextile (60°) | ♄ + any | Market move |
| Mars Square (90°) | ♂ + any | **SHARP SUDDEN MOVE** — usually bullish, short |
| Saturn Square (90°) | ♄ + any | **BEARISH pressure** on securities |
| Jupiter Trine (120°) | ♃ + any | Mild BULLISH (shares only) |
| Mars–Rahu Conjunction < 15° | ♂ + ☊ | **EXTREME BEARISH equity / CRUDE BULLISH** |
| Jupiter in fiery sign + Guru aspect | ♃ transit | **RECESSION WARNING** |
| Mercury/Neptune retrograde + bearish aspect on Jupiter | ☿/♆ retro | Short-term stock fall |
| Uranus–Jupiter Conjunction | ⛢ + ♃ | MAJOR stock market regulator |
| 4+ planets in same rāśi | Multiple | Neutral/volatile — directionless |
        """)

    with st.expander("📐 Long/Short Term Framework + Banerjee Transit Rules"):
        st.markdown("""
**Transit Speed → Signal Duration (Vasudev):**

| Planet Type | Planets | Signal Duration |
|-------------|---------|-----------------|
| Long-term (months) | Jupiter, Saturn, Rahu, Uranus | Multi-month trends |
| Medium-term | Mars | Weeks |
| Short-term | Sun, Mercury, Venus | Days to 2 weeks |
| Intraday / Daily | Moon | Same-day only |

**Banerjee's Buy/Sell Rule (Ch7):**
- 📗 **BUY** sector when Jupiter transits its ruling sign + aspected by Sun/Moon/Mars
- 📕 **SELL** sector when Saturn or Rahu transits its ruling sign + aspected by Mercury/Venus/Rahu

**Banerjee Transit Rule (Ch7, General):**
- Planet in exalted/own/friendly rāśi = very good results for that sector
- Planet in enemy/depression rāśi = no good results
- Retrograde benefic → intensifies positive results
- Retrograde malefic → intensifies negative results
- Planet in eclipse → cannot produce favorable results
- 4+ planets in one rāśi = directionless/volatile market

**Intraday confirmed signal (Banerjee Ch8):**
- Step 1: Note Moon's nakshatra for today
- Step 2: Find nakshatra lord
- Step 3: Check if lord receives benefic (Sun/Moon/Jupiter/Mars) or malefic (Saturn/Rahu/Mercury) aspects
- Step 4: Benefic = BUY the sectors ruled by that nakshatra lord. Malefic = SELL/SHORT those sectors
- Step 5: Enter during the hora when that planet rules (see Hora table above)
        """)
