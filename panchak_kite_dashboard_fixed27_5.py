...
with tabs[18]:
    ...
    for _nak in _ALL_NAKS:
        if _nak not in _BEARISH_NAK_RULES and _nak not in _BULLISH_NAK_RULES and _nak not in _TRAP_NAK_RULES:
            _lord = _NAK_LORDS_STD.get(_nak, "Mercury")
            _def  = _LORD_SIG.get(_lord, ("BEARISH","MEDIUM","Default."))
            if _def[0] == "BEARISH":
                _BEARISH_NAK_RULES[_nak] = _def
            else:
                _BULLISH_NAK_RULES[_nak] = _def
    ...
    for _r2 in _astro_rules2:
        ...
        for _rashi in ["Aries","Taurus","Gemini","Cancer","Leo","Virgo",
                       "Libra","Scorpio","Sagittarius","Capricorn","Aquarius","Pisces"]:
            if _rashi.lower() in _c2.lower():
                if "BULLISH" in _s2 and "BEARISH" not in _s2:
                    _SUN_RASHI_RULES[_rashi] = ("BULLISH",_i2,_n2)
                elif "BEARISH" in _s2 and "BULLISH" not in _s2:
                    _SUN_RASHI_RULES[_rashi] = ("BEARISH",_i2,_n2)
                else:
                    _SUN_RASHI_RULES[_rashi] = ("MIXED",_i2,_n2)
                break
    _SUN_RASHI_FALLBACK = {
