...
# ── WebSocket warmup guard ───────────────────────────────────────────────────
# Strategies must NOT fire on the very first ticks after WS connects —
# prices are stale snapshots, candles are single-point, SL is often already
# breached before we even place the order.
# We wait WARMUP_SECONDS after connection before allowing any strategy to fire.
WARMUP_SECONDS  = 60          # 1 full minute of live ticks before trading
ws_connect_time = None         # set in on_connect()
ws_ready        = False        # flips True once warmup is done
...
def place_trade(symbol, side, ltp, tick, condition_name, extra_info=None):
    ...
    structure_text = ""
    if extra_info:
        c15  = extra_info.get("c15", {})
        c1h  = extra_info.get("c1h", {})
        structure_text = (
            f"\n   ORB H/L: {extra_info['orb_high']} / {extra_info['orb_low']}"
            f"\n   F15 H/L: {extra_info['f_high']} / {extra_info['f_low']}"
            f"\n   C15 O/H/L/C: {c15.get('open','-')} / {c15.get('high','-')} / {c15.get('low','-')} / {c15.get('close','-')}"
            f"\n   C1H O/H/L/C: {c1h.get('open','-')} / {c1h.get('high','-')} / {c1h.get('low','-')} / {c1h.get('close','-')}"
...
def _extra(ctx):
    ...
    return {
        ...
        "t_vol":      ctx["live_volume"],          # cumulative in replay, live vol_traded live
        "vol_pct":    ctx["vol_percent"],
        "t_open":     f.get("open", 0),
        "t_high":     latest_highs.get(symbol, orb.get("high", 0)),
        "t_low":      latest_lows.get(symbol,  orb.get("low", 0)),
        "c15":        ctx.get("c15", {}),          # current rolling 15-min candle
        "c1h":        ctx.get("c1h", {}),          # current rolling 1-hour candle
    }
...
def strategy_orb(ctx):
    ...
    # ── Stale breakout guard ────────────────────────────────────────────
    # If today's running day high/low has already moved significantly past
    # the ORB level, the breakout happened earlier and we are chasing.
    # e.g. day_low = 1285 vs orb_low = 1294 → 0.70% below → stale SELL
    t_high = ctx.get("t_high", ltp)
    t_low  = ctx.get("t_low",  ltp)
    MAX_STALE_PCT = 0.5   # if day already moved >0.5% past level, skip
    ...
    if buy_signal:
        ...
        if not (299 <= ltp <= 6999):
            ...
            reason = f"BUY stale: day_high already {day_high_past_orb_pct:.2f}% past ORB high — breakout happened earlier"
        elif ltp <= orb_low and day_low_past_orb_pct > MAX_STALE_PCT:
            reason = f"SELL stale: day_low already {day_low_past_orb_pct:.2f}% past ORB low — breakdown happened earlier"
            ...
            def _replay_exit_check(price_for_trail):
                ...
                while move_percent >= (pos["trail_level"] + TRAIL_STEP_PERCENT):
                    ...
                    print(Fore.YELLOW +
                          ...
                          f"Entry:{entry} | LTP:{price_for_trail} | "
                          f"New SL:{pos['sl']}" + ts())
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        # ── WebSocket warmup guard ───────────────────────────────────────
        # Allow price tracking and candle building above to run always,
        # but hold strategy() until WARMUP_SECONDS have elapsed since
        # ws connected. This prevents entries on stale first-tick prices
        # where SL is already breached before the order even places.
        global ws_ready
        if not ws_ready:
            if ws_connect_time and (datetime.now() - ws_connect_time).total_seconds() >= WARMUP_SECONDS:
                ws_ready = True
                print(Fore.GREEN + f"✅ Warmup complete — strategies now active" + ts())
            else:
                continue   # skip strategy, keep building candles/ORB
...
def on_connect(ws, response):
    ...
    print(Fore.YELLOW + f"⏳ Warmup: strategies will fire after {WARMUP_SECONDS}s ({(ws_connect_time + pd.Timedelta(seconds=WARMUP_SECONDS)).strftime('%H:%M:%S')})" + ts())
