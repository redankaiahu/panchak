...
def _vedic_day_analysis(d_date, hour_ist=9, minute_ist=15):
    ...
    jd2 = _jd(datetime(d_date.year, d_date.month, d_date.day, 27, 45, 0)
               if False else
               datetime(d_date.year, d_date.month, d_date.day, 3, 45, 0) + __import__("datetime").timedelta(days=1))
    def _sid2(fn):
    ...
    _HORA_START = {"Sun":0,"Moon":2,"Mars":4,"Mercury":1,"Jupiter":5,"Venus":3,"Saturn":6}
    _hora_idx   = (_HORA_START.get(day_lord, 0) + 3) % 7   # 9:15 = ~3 h after sunrise
    ...
    # ════════════════════════════════════════════════════════════════
    _fertile = {"Aries","Taurus","Leo","Scorpio","Sagittarius"}
    _barren  = {"Gemini","Cancer","Libra","Aquarius","Pisces"}
...
with tabs[18]:
    ...
    # NOTE: _parse_nak_rules(_astro_rules) and (_astro_rules2) are called
    # below, after both lists are defined.
  ...
  <div style="background:#0a0a0a;padding:10px 20px">
    ...
    **Key Rules (most powerful):**
    ...
    with st.expander("📚 Static Reference — Batch 2: Extended Rules (Sun transit, Hora, Commodities) (click to expand)", expanded=False):
        ...
        # Parse both rule sets now that both lists are fully defined
