...
_TG_DEDUP_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CACHE", f"tg_dedup_{datetime.now().strftime('%Y%m%d')}.json")  # FIX-4b
...
def send_telegram2_bg(message: str, dedup_key: str = None):
    ...
    threading.Thread(target=send_telegram2, args=(message, dedup_key), daemon=True).start()
...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    if _special_up and st.session_state.get("special_stock_alerts", True):
        ...
        _sp_key = "SPECIAL_UP_" + "_".join(sorted(_sp_syms[:3])) + "_" + datetime.now(IST).strftime("%Y%m%d")
        send_telegram_bg(_sp_tg_msg, dedup_key=_sp_key)  # gated by special_stock_alerts above
    ...
    if _special_down and st.session_state.get("special_stock_alerts", True):
        ...
        _sp_key = "SPECIAL_DOWN_" + "_".join(sorted(_sp_syms[:3])) + "_" + datetime.now(IST).strftime("%Y%m%d")
        send_telegram_bg(_sp_tg_msg, dedup_key=_sp_key)  # gated by special_stock_alerts above
...
def _load_alert_toggles() -> dict:
    ...
    New keys added in future versions automatically get their default value.
    """
    saved = {}
    try:
        ...
        pass
    # Merge: saved takes priority; missing keys get their default
...
def _save_alert_toggles():
    """
    Write current session_state toggle values to CACHE/alert_toggles.json.
    Called every time a toggle button is clicked (via st.rerun).
    """
    try:
        os.makedirs(os.path.dirname(_ALERT_TOGGLES_FILE), exist_ok=True)
        snapshot = {
            k: bool(st.session_state.get(k, _ALERT_TOGGLE_DEFAULTS.get(k, True)))
            for k in _ALERT_TOGGLE_DEFAULTS
        }
...
if _market_open:
    st_autorefresh(interval=100 * 1000, key="refresh", debounce=False)
...
def _nifty_break_alert_core(sym, ltp, top_high, least_low, slot_label,
    ...
    with _NIFTY_SLOT_LOCK:
        ...
        if not _state.get(_break_key):
            ...
            send_telegram_bg(_break_msg, dedup_key=None)
        ...
        if (
            ...
            send_telegram_bg(_prog_msg, dedup_key=None)
...
def _send_tg_structure_alert(category, title, df_in, ltp_map_ref):
    ...
    try:
        ...
        new_syms = detect_new_entries(f"TG_{category}", syms)
        if not new_syms:
            return
        _now_str = datetime.now(IST).strftime("%H:%M IST")
        ...
        msg = (
            ...
            f"{icon} <b>{title}</b>\n"
            f"⏰ {_now_str}\n"
            f"📋 Stocks ({len(new_syms)}):\n"
            + "\n".join(lines)
            + f"\n⚠️ <i>NOT financial advice.</i>\n{border}"
        )
        # Dedup: category + symbols + 30-min slot (proper HH:MM format)
        _now_dt   = datetime.now(IST)
        _slot_30  = _now_dt.strftime(f"%H:{(_now_dt.minute // 30) * 30:02d}")
        _syms_key = "_".join(new_syms[:4])
        send_telegram_bg(msg, dedup_key=f"STRUCT_{category}_{_syms_key}_{_slot_30}")
...
def _fire_mtf_alerts():
    ...
    for _direction, _df, _icon, _label in [
        ...
        if _hc.empty:
            continue
        _key = f"PIIND_SEQ_{_direction}_{_slot[:3]}0"
        if _seq_email_already_sent(f"PIIND_{_direction}", _key):
            continue
        _seq_mark_email_sent(f"PIIND_{_direction}", _key)

        _lines = []
        for _, _r in _hc.head(10).iterrows():
            ...
            _lines.append(
                f"  {_icon} <b>{_s}</b>  LTP:{_l}  Chg:{_c:+.1f}%  RelStr:{_rs:+.1f}%  Vol:{_v:+.0f}%\n"
                f"    Daily:{_db}  Retest:{_rt}  Score:{_sc}"
            )

        send_telegram_bg(
            f"{'🟢'*8 if _label=='LONG' else '🔴'*8}\n"
            f"{_icon} <b>PI-IND STYLE SETUP — {_label}</b>\n"
            ...
            + "\n".join(_lines) +
            f"\n━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📌 Entry: above resistance retest level\n"
            f"📌 SL: below first-half day low\n"
            f"⚠️ <i>NOT financial advice.</i>\n"
            f"{'🟢'*8 if _label=='LONG' else '🔴'*8}",
            dedup_key=_key
        )
...
with tabs[8]:   # change tab index if required
    ...
    st.dataframe(_apply_fmt(green_structure_df), use_container_width=True)
    #st.divider()

    st.subheader("🔴 Red Open Structure")
    st.dataframe(_apply_fmt(red_structure_df), use_container_width=True)
...
with tabs[22]:
            ...
            try:
                ...
                if ch <= ph and cl >= pl:
                    ...
                    try:
                        ...
                        ltp = float(_ltp_rows.iloc[0]) if not _ltp_rows.empty else float(child["close"])
                    except Exception:
                        ltp = float(child["close"])
                    rows.append({
                        "Symbol":         sym,
                        "LTP":            round(ltp, 2),
                        "Parent High":    round(ph, 2),
                        "Parent Low":     round(pl, 2),
                        "Child High":     round(ch, 2),
                        "Child Low":      round(cl, 2),
                        "Parent Range":   rng_parent,
                        "Child Range":    rng_child,
    ...
    if _ib_df.empty:
        ...
        _ib1.metric("🕯️ Inside Bars Found",    len(_ib_df))
        _ib2.metric("🟢 Broken Up",           len(_ib_df[_ib_df["Status"].str.contains("UP")]))
        _ib3.metric("🔴 Broken Down",          len(_ib_df[_ib_df["Status"].str.contains("DOWN")]))

        st.markdown("---")

        # Active breakouts first
        ...
        if not _ib_breaking.empty:
            ...
            st.dataframe(
                _ib_breaking.sort_values("Compression %")[
                    ["Symbol","LTP","Parent High","Parent Low","Compression %",
                     "Entry Long >","Entry Short <","SL Long","SL Short","Status"]
                ],
                use_container_width=True, hide_index=True
            )

        st.markdown("### ⏳ Consolidating — Watching for Breakout")
        st.caption("Smaller compression % = tighter coil = more explosive potential breakout.")
        if not _ib_pending.empty:
            st.dataframe(
                _ib_pending.sort_values("Compression %")[
                    ["Symbol","LTP","Parent High","Parent Low","Child High","Child Low",
                     "Compression %","Entry Long >","Entry Short <","SL Long","SL Short"]
                ],
                use_container_width=True, hide_index=True,
                height=min(600, 60 + len(_ib_pending) * 35)
            )
        else:
        ...
        st.markdown("""
**Long trade**: Enter above Parent Bar High → Stop-loss below Parent Bar Low  
**Short trade**: Enter below Parent Bar Low → Stop-loss above Parent Bar High  
**Target**: At least 1:1 R/R minimum. Look for nearest resistance/support for exit.  

**Best setups**: Low compression % (tight inside bar) + trend context + volume contraction.  
**Avoid**: Inside bars near major support/resistance zones where reversal is likely.  
**Triple Inside Bar**: Three consecutive narrowing candles — extremely explosive breakout signal.
""")
    ...
    with _t1:
        ...
        if _all_full.empty:
            ...
            _show_cols_full = [c for c in [
                "Setup","Symbol","LTP","CHANGE_%","Nifty_%","RelStr_%","VOL_%",
                "Retest_Hold","Rel_Strength","Daily_Break","1H_Break",
                "15m_FH_High","15m_Pullback","YEST_HIGH","TOP_HIGH",
                "Conviction","Entry","SL"
        ...
        if not _mtf_short_df.empty:
            ...
            st.dataframe(_mtf_short_df, use_container_width=True, hide_index=True,
    ...
    with _t3:
        ...
        if "YEST_HIGH" in df.columns and "LTP" in df.columns:
            ...
            if _db.empty:
                ...
                _db_show = [c for c in ["Symbol","LTP","CHANGE_%","RelStr_%","VOL_%",
                    "AbvYH_pts","AbvYH_%","YEST_HIGH","TOP_HIGH","PI_Setup"] if c in _db.columns]
                st.dataframe(
