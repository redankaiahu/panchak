...
# =========================================================
# Logic:
#   • 1H opening range = high/low of the 09:15–10:14 window
#     (first 4 × 15-min candles: 09:15, 09:30, 09:45, 10:00)
#   • Breakout = LTP > 1H_HIGH AND LTP > EMA20
#   • Breakdown = LTP < 1H_LOW  AND LTP < EMA20
#   • Requires current time > 10:15 (range must be complete)
# =========================================================

hourly_rows = []

_now_hm = datetime.now(IST).strftime("%H:%M")
_orb_range_ready = _now_hm >= "10:15"   # only meaningful after range window closes

for sym in SYMBOLS:

    # ── Get 15-min candles for this symbol from today's cache ──────
    ...
    # ── Filter to the 09:15–10:14 opening range window ────────────
    # Use time string comparison — robust regardless of resample origin
    ...
    if len(orb_candles) < 2:
        # Need at least 2 candles (09:15 + 09:30) before meaningful range
        continue
    ...
    # ── BREAKOUT: LTP above 1H high AND above EMA20 ─────────────────
    if ltp > h1_high and ltp > row["EMA20"]:
    ...
    # ── BREAKDOWN: LTP below 1H low AND below EMA20 ─────────────────
    elif ltp < h1_low and ltp < row["EMA20"]:
...
<div class="{_vcrd3}" style="padding:16px">
  ...
  <div style="margin-top:10px;display:flex;gap:12px;flex-wrap:wrap;font-size:12px;color:#aaa">
    ...
    for _fc10_di in range(1, 40):
        ...
        _fc10_conj = []
        if _fc10_r["moon_ketu_deg"]  < 15: _fc10_conj.append(f"🌙-Ketu {_fc10_r['moon_ketu_deg']}°")
        if _fc10_r["mars_rahu_deg"]  < 15: _fc10_conj.append(f"♂-Rahu {_fc10_r['mars_rahu_deg']}°")
        if _fc10_r["sun_saturn_deg"] <  8: _fc10_conj.append(f"☀-♄ {_fc10_r['sun_saturn_deg']}°")

        _fc10_sig = []
        for _fc10_txt, _fc10_typ in _fc10_r["signals"]:
            _fc10_ico = {"B":"🔴","G":"🟢","T":"⚠️","M":"🟡"}.get(_fc10_typ, "")
            _fc10_sig.append(f"{_fc10_ico} {_fc10_txt}")

        _fc10_intra = ""
        if _fc10_r["moon_nak"] == "Rohini":            _fc10_intra = "⚠️ TRAP — sell rally"
        elif _fc10_r["moon_rashi"] == "Taurus":         _fc10_intra = "🔄 Gap-down → recovery"
        elif _fc10_r["moon_rashi"] == "Sagittarius":    _fc10_intra = "🟢 Bullish open"
        elif "Amavasya" in _fc10_r["tithi"] or "Pratipada" in _fc10_r["tithi"]:
            _fc10_intra = "🔴 Sell on rise"

        _fc10_rows.append({
            "Date":           _fc10_d.strftime("%a %d %b"),
            "Signal":         _fc10_r["overall"],
            "Intensity":      _fc10_r["intensity"],
            "Moon Nakshatra": f"{_fc10_r['moon_nak']} ({_fc10_r['moon_nak_lord']})",
            "Moon Rashi":     _fc10_r["moon_rashi"],
            "Tithi":          _fc10_r["tithi"].split("(")[0].strip(),
            "Key Conjunctions": "  ·  ".join(_fc10_conj) if _fc10_conj else "—",
            "Intraday Hint":  _fc10_intra if _fc10_intra else "—",
            "Score":          f"{_fc10_r['net_score']:+d} (B{_fc10_r['bearish_pts']}/G{_fc10_r['bullish_pts']})",
            "Key Signals":    "  |  ".join(_fc10_sig[:3]) if _fc10_sig else "No major signals",
        })
        _fc10_found += 1
        if _fc10_found >= 10:
            break
    ...
    if _fc10_rows:
        ...
        st.caption(
            ...
            f"·  ⚠️ {_fc10_nt} Trap  ·  🟡 {_fc10_nm} Mixed"
        )

        with st.expander("🔍 Full Signal Details — All 10 Days"):
            for _fc10_row in _fc10_rows:
                _fc10_ico = (
                    "🔴" if "BEARISH" in _fc10_row["Signal"] else
                    "🟢" if "BULLISH" in _fc10_row["Signal"] else
                    "⚠️" if "TRAP"    in _fc10_row["Signal"] else "🟡"
                )
                st.markdown(
                    f"**{_fc10_ico} {_fc10_row['Date']} — "
                    f"{_fc10_row['Signal']} ({_fc10_row['Intensity']})**"
                )
                st.markdown(
                    f"&nbsp;&nbsp;🌙 Moon: **{_fc10_row['Moon Nakshatra']}** · "
                    f"Rashi: {_fc10_row['Moon Rashi']} · {_fc10_row['Tithi']}"
                )
                if _fc10_row["Key Conjunctions"] != "—":
                    st.markdown(f"&nbsp;&nbsp;⚡ {_fc10_row['Key Conjunctions']}")
                for _fc10_line in _fc10_row["Key Signals"].split("  |  "):
                    if _fc10_line.strip():
                        st.markdown(f"&nbsp;&nbsp;&nbsp;&nbsp;{_fc10_line.strip()}")
                st.markdown("---")
    else:
