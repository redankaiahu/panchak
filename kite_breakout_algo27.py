...
def strategy_yl_breakdown(ctx):
    ...
    symbol           = ctx["symbol"]
    ltp              = ctx["ltp"]
    ohlc             = ctx["ohlc"]       # current tick candle (open = live open, low = day low so far)
    yest             = ctx["yest"]
    ema              = ctx["ema"]
    change_pct       = ctx["change_percent"]
    vol_percent      = ctx["vol_percent"]
    current_time     = ctx["current_time"]
    dist_from_yl_pct = ctx["dist_from_yl_pct"]   # +ve = how far below YL
    dist_from_yh_pct = ctx["dist_from_yh_pct"]   # +ve = how far above YH

    ema20 = ema.get("ema20", 0)
    if not ema20:
        return

    # Only trade after first-15 window is settled and before afternoon chop
    if current_time < "09:30":
        return
    if current_time > "13:30":
        return
    ...
    live_open = ohlc.get("open", 0)
    day_low   = ohlc.get("low", 0)
    day_high  = ohlc.get("high", 0)

    if live_open == 0 or day_low == 0 or day_high == 0:
        return

    # ── SELL: YL Breakdown ──────────────────────────────────────────────
    # Condition map (matches the scanner df columns):
    #   LIVE_OPEN  >= YEST_LOW    → no gap-down (stock opened above YL)
    #   LTP        <= YEST_LOW    → has now broken below YL
    #   DIST_FROM_DAY_LOW_%≤ 0.5 → LTP is within 0.5% above day's intraday low
    #   CHANGE_%   in [-1.2, -0.5] → fresh bearish momentum, not exhausted
    #   LTP        <= EMA20       → below medium-term average (downtrend context)
    if (
        299 <= ltp <= 6999 and
        live_open >= yest["low"] and                          # no gap-down open
        ltp <= yest["low"] and                                # broken below YL
        #dist_from_yl_pct <= MAX_OVEREXTENSION_PCT and         # not already way overextended
        #day_low > 0 and ((ltp - day_low) / day_low * 100) <= 0.5 and   # near day low
        #-1.2 <= change_pct <= -0.5 and                        # bearish momentum, not exhausted
        ltp <= ema20                                           # below EMA20 (downtrend context)
    ):
        place_trade(symbol, "SELL", ltp, ctx["tick"], "YL Breakdown SELL", _extra(ctx))

    # ── BUY: YH Breakout ────────────────────────────────────────────────
    # Mirror of the SELL setup:
    #   LIVE_OPEN  <= YEST_HIGH   → no gap-up (stock opened below YH)
    #   LTP        >= YEST_HIGH   → has now broken above YH
    #   DIST_FROM_DAY_HIGH_% ≤ 0.5 → LTP is within 0.5% below day's intraday high
    #   CHANGE_%   in [+0.5, +1.2] → fresh bullish momentum, not exhausted
    #   LTP        >= EMA20       → above medium-term average (uptrend context)
    if (
        299 <= ltp <= 6999 and
        live_open <= yest["high"] and                         # no gap-up open
        ltp >= yest["high"] and                               # broken above YH
        dist_from_yh_pct <= MAX_OVEREXTENSION_PCT and         # not already way overextended
        day_high > 0 and ((day_high - ltp) / day_high * 100) <= 0.5 and  # near day high
        0.5 <= change_pct <= 1.2 and                          # bullish momentum, not exhausted
        ltp >= ema20                                           # above EMA20 (uptrend context)
    ):
