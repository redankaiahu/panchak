...
DAILY_MAX_LOSS_PERCENT = 100       # 👈 Set to 100% to effectively disable %-based early halt
HARD_LOSS_LIMIT = -1500            # 👈 Fixed Rupee Limit remains active
...
GROSS_WIN_TARGET     = 4500     # ₹ — close ALL positions + halt when gross winning (sum of all profitable
...
def paper_position_manager():
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            if trading_enabled:
                # Daily Max Loss check (fixed rupee limit)
...
def position_manager():
    ...
    while True:
        ...
        try:
            ...
            # ── Guard 1: Daily Max Loss (fixed rupee limit) ───────────────
            if trading_enabled and total_pnl_now <= HARD_LOSS_LIMIT:
                trading_enabled = False
                msg = (
                    f"🚨 DAILY MAX LOSS REACHED (LIVE) | PnL: {total_pnl_now:.2f} | "
...
def _pnl_guard_thread():
    ...
    def _close_all_live(reason):
        """Market-sell every open LIVE position and halt."""
        try:
    ...
    while True:
        try:
            ...
            if trading_enabled and _total_pnl <= HARD_LOSS_LIMIT:
                trading_enabled = False
                _msg = (f"🚨 PNL_GUARD DAILY MAX LOSS | Total PnL:₹{_total_pnl:.2f} "
