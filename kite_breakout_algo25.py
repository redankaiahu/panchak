...
def save_trades_cache():
    ...
    try:
        ...
        print(Fore.YELLOW + f"⚠️  Could not save trades cache: {e}")
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if status == "LIVE":
                ...
                print(Fore.YELLOW + f"♻️  Restored LIVE position: {symbol} — OCO monitor restarted")
            else:
        ...
        print(Fore.CYAN + f"📂 Trades cache loaded — {restored} symbols blocked from re-entry")

    except Exception as e:
        print(Fore.YELLOW + f"⚠️  Could not load trades cache: {e}")
...
def place_trade(symbol, side, ltp, tick, condition_name, extra_info=None):
    ...
    if TRADING_MODE == "LIVE":
        ...
        if (open_count + pending_count) >= LIVE_MAX_TRADES:
            print(Fore.YELLOW + f"⚠️  Max live trades ({LIVE_MAX_TRADES}) reached — skipping {symbol}")
            return
    ...
    if side == "BUY" and sl >= ltp:
        print(Fore.RED + f"⚠️  Invalid SL for BUY {symbol}: SL={sl} >= LTP={ltp}")
        return
    if side == "SELL" and sl <= ltp:
        print(Fore.RED + f"⚠️  Invalid SL for SELL {symbol}: SL={sl} <= LTP={ltp}")
        return
    ...
    if TRADING_MODE == "LIVE":
        ...
        try:
            ...
            print(Fore.YELLOW + f"✅ Entry order placed | ID:{entry_order_id}")
            ...
            print(Fore.YELLOW + f"✅ SL order placed | ID:{sl_order_id} | SL:{sl}")
            ...
            print(Fore.YELLOW + f"✅ Target order placed | ID:{target_order_id} | Target:{target}")
            ...
            print(Fore.RED + f"❌ Live order failed for {symbol}: {e}")
            ...
            if entry_order_id and (sl_order_id is None or target_order_id is None):
                print(Fore.RED + f"⚠️  PARTIAL ORDER — emergency exit for {symbol}")
                ...
                try:
                    ...
                    print(Fore.RED + f"🚨 Emergency exit placed for {symbol}")
                    write_log(f"EMERGENCY EXIT PLACED {symbol}")
                except Exception as ex:
                    print(Fore.RED + f"🚨 EMERGENCY EXIT ALSO FAILED for {symbol}: {ex}")
    ...
    while True:
        try:
            ...
            if sl_order is None or tgt_order is None:
                ...
                print(Fore.RED + f"⚠️  OCO: orders not found for {symbol}, exiting monitor")
            ...
            if sl_order["status"] == "COMPLETE":
                ...
                try:
                    ...
                    print(Fore.YELLOW + f"⚠️  Could not cancel target for {symbol}: {ce}")
            ...
            if tgt_order["status"] == "COMPLETE":
                ...
                try:
                    ...
                    print(Fore.YELLOW + f"⚠️  Could not cancel SL for {symbol}: {ce}")
            ...
            print(Fore.RED + f"⚠️  OCO poll error for {symbol} (attempt {consecutive_errors}): {e}")
            write_log(f"OCO error {symbol}: {e}")
            if consecutive_errors >= 10:
                print(Fore.RED + f"❌ OCO monitor giving up on {symbol} after 10 errors")
...
def load_ema_data_from_local(replay_date=None):
    ...
    print("📊 Calculating and saving EMA from local daily data...")
    ...
    print("✅ EMA calculated and saved to local CSV")
...
def position_manager():
    ...
    while True:
        try:
            ...
            print(Fore.RED + f"position_manager error: {e}")
                    ...
                    for symbol, trade in list(trades_taken.items()):
                        ...
                        for oid_key in ("sl", "target"):
                            ...
                            if order and order["status"] in ("OPEN", "TRIGGER PENDING"):
                                try:
                                    kite.cancel_order(variety="regular", order_id=oid)
                                    print(Fore.YELLOW + f"Cancelled pending {oid_key} order for {symbol}")
                                except Exception as ce:
                                    print(Fore.RED + f"Could not cancel {oid_key} for {symbol}: {ce}")
...
def fresh_daily_download():
    ...
    print("⚡ Downloading last 150 days of daily data for all symbols...")
    ...
    for token, symbol in instrument_tokens.items():
        ...
        try:
            ...
            if not daily_data:
                print(Fore.YELLOW + f"⚠️  {symbol}: no data returned")
                failed += 1
                continue
    ...
    print(Fore.GREEN + f"✅ Daily download done — {success} symbols updated, {failed} failed")
...
if __name__ == "__main__":
    ...
    if skip != "y":
        ...
        print(Fore.YELLOW + "⚠️  Using existing local daily data — EMA/Yesterday may be stale")
