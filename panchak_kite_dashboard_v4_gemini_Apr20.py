...
def _load_inside_bars():
    ...
    # Note: STOCKS and get_token and df are assumed defined globally or accessible
    try:
        from __main__ import STOCKS, get_token, df, kite
    except: pass
    ...
    for sym in STOCKS:
        try:
            ...
            if ch <= ph and cl >= pl:
                ...
                high_conv = (co < ph and ch_to_ph_pct <= 1.5 and comp_pct <= 70)
                try:
                ...
                except: live_open = 0.0
                try:
                    _vp_rows = df[df["Symbol"] == sym]["VOL_%"]
                    vol_pct = round(float(_vp_rows.iloc[0]), 1) if not _vp_rows.empty else 0.0
                except: vol_pct = 0.0
                rows.append({
                    "Symbol": sym, "High Conv": high_conv, "Compression %": comp_pct,
                    "PH": ph, "CH": ch, "PL": pl, "CL": cl, "Child Open": co, "Yest Close": cc,
                    "LTP": ltp, "LIVE_OPEN": live_open, "VOL %": vol_pct, "Parent Bias": parent_bias,
                    "Child Bias": child_bias, "Dist to PH": f"{ch_to_ph_pct}%",
                    "Open vs PH": f"{open_vs_ph_pct}%",
                    "Status": "🟢 BREAKOUT" if ltp > ph else "🔴 BREAKDOWN" if ltp < pl else "⏳ Consolidating",
                })
        except: continue
...
def _load_macd_all():
    ...
    try:
        from __main__ import SYMBOLS, get_token, df, kite
    except: pass
    for sym in SYMBOLS[:80]:
        try:
            ...
            try:
                _ltp_m = df[df["Symbol"] == sym]["LTP"]
                ltp = float(_ltp_m.iloc[0]) if not _ltp_m.empty else closes[-1]
            except: ltp = closes[-1]
            try:
                _v_m = df[df["Symbol"] == sym]["VOL_%"]
                vol = float(_v_m.iloc[0]) if not _v_m.empty else 0.0
            except: vol = 0.0
            status = "⚪ Neutral"
            ...
            rows.append({
                "Symbol": sym, "MACD Status": status, "MACD": round(macd_line, 3),
                "Prev": round(macd_prev, 3), "LTP": ltp, "VOL_%": vol, "Close": round(closes[-1], 2)
            })
        except: continue
...
def _fire_heatmap_confluence_alert():
    ...
    try:
        ...
        _n50_ltp = _idx_q.get("NSE:NIFTY 50", {}).get("last_price", 0) or ltp_map.get("NIFTY", 24315)
        _bnk_ltp = _idx_q.get("NSE:NIFTY BANK", {}).get("last_price", 0) or ltp_map.get("BANKNIFTY", 56086)
        _snx_ltp = _idx_q.get("BSE:SENSEX", {}).get("last_price", 0) or ltp_map.get("SENSEX", 80000)
    ...
    try:
        ...
        _bnk_pcr_oi = _oi_bn_d.get("pcr", 0.0)
...
with tabs[0]:
    ...
    with st.expander("🔮 MAGIC LINES & REVERSALS (High Conviction)", expanded=True):
        ...
        with col_m1:
            ...
            # Use dynamic Nifty spot from fresh OI fetch
            nifty_ref = _oi_nifty.get("spot", 24358.60) if _oi_nifty else 24358.60
            gann_levels = get_gann_square_9(nifty_ref)
            st.write(f"**Base Price:** {nifty_ref:,.2f}")
            m_rows = []
