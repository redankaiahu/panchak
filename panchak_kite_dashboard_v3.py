...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    if _up_rows and not _is_tg_disabled("TOP_HIGH"):
        ...
        try:
            ...
            _pstart_key = "?"
        if not _is_tg_disabled("TOP_HIGH"):
    ...
    if _special_up and st.session_state.get("special_stock_alerts", True):
        ...
        if not _is_toast_disabled("TOP_HIGH"):
            if not _is_toast_disabled("TOP_HIGH"): st.toast(f"🟢 SPECIAL TOP_HIGH BREAK: {', '.join(_sp_syms)}", icon="🚨")
    ...
    if _down_rows and not _is_tg_disabled("TOP_LOW"):
        ...
        try:
            ...
            _pstart_key = "?"
        if not _is_tg_disabled("TOP_LOW"):
    ...
    if _special_down and st.session_state.get("special_stock_alerts", True):
        ...
        if not _is_toast_disabled("TOP_LOW"):
            if not _is_toast_disabled("TOP_LOW"): st.toast(f"🔴 SPECIAL TOP_LOW BREAK: {', '.join(_sp_syms)}", icon="🚨")
...
_ALERT_TOGGLE_DEFAULTS = {
    ...
    # Top Gainers / Losers
    "toast_TOP_GAINERS":    True,
    "email_TOP_GAINERS":    True,
    "tg_TOP_GAINERS":       True,
    "toast_TOP_LOSERS":     True,
    "email_TOP_LOSERS":     True,
    "tg_TOP_LOSERS":        True,
    # Strong Options Buy
    "toast_OPTION_STRONG":  True,
    "email_OPTION_STRONG":  True,
    "tg_OPTION_STRONG":     True,
    # Yesterday Green/Red setups
    "toast_YEST_GREEN_BREAK": True,
    "email_YEST_GREEN_BREAK": True,
    "tg_YEST_GREEN_BREAK":    True,
    "toast_YEST_RED_BREAK":   True,
    "email_YEST_RED_BREAK":   True,
    "tg_YEST_RED_BREAK":      True,
    # 1H Opening Range Breakout/Breakdown
    "toast_HOURLY_BREAK_UP":   True,
    "email_HOURLY_BREAK_UP":   True,
    "tg_HOURLY_BREAK_UP":      True,
    "toast_HOURLY_BREAK_DOWN": True,
    "email_HOURLY_BREAK_DOWN": True,
    "tg_HOURLY_BREAK_DOWN":    True,
    # 2-Min EMA Reversal
    "toast_2M_EMA_REVERSAL":  True,
    "email_2M_EMA_REVERSAL":  True,
...
SPECIAL_ALERT_STOCKS = [
    # NIFTY/BANKNIFTY removed — they are indices, not in stocks df
...
if is_market_hours() and st.session_state.get("toast_TOP_HIGH", True): notify_browser("🟢 New TOP LIVE_HIGH", new_TOP_HIGH)
...
if is_market_hours() and st.session_state.get("toast_TOP_LOW", True): notify_browser("🔴 New TOP LIVE_LOW", new_TOP_LOW)
...
# ═══════════════════════════════════════════════════════════════════════
# ⏱️ KP BREAK ALERT — every 15 minutes (TOP_HIGH + TOP_LOW)
...
def fire_kp_break_15m_alert(df_live, ltp_map_ref=None):
    ...
    # Global dedup file (resets each day via _TG_DEDUP_FILE)
    ...
    msg_parts = [
        "🕐🕐🕐🕐🕐🕐🕐🕐🕐🕐",
        f"⏱️ <b>KP BREAK ALERT — {_slot_str} window</b>",
        f"⏰ {_now_str}",
    ...
    msg_parts.append("🕐🕐🕐🕐🕐🕐🕐🕐🕐🕐")
    final_msg = "\n".join(msg_parts)
    # Save dedup BEFORE sending so concurrent reruns don't double-fire
    _dedup[_dedup_key] = datetime.now(IST).isoformat()
    _save_tg_dedup(_dedup)
    send_telegram_bg(final_msg, dedup_key=None)  # dedup already handled above
...
def fire_combined_engine_tg_alert():
    ...
    # ── 1. SMC engine signal ──────────────────────────────────────────
    try:
        _smc_data   = st.session_state.get("smc_data", {}) or {}
    ...
    # ── 2. OI intelligence ───────────────────────────────────────────
    try:
        _oi_data    = fetch_nifty_oi_intelligence()
        _oi_dir     = str(_oi_data.get("direction", "—")) if _oi_data else "—"
        _oi_pcr     = float(_oi_data.get("pcr", 0) or 0) if _oi_data else 0.0
        _oi_maxpain = _oi_data.get("max_pain", 0) if _oi_data else 0
        _oi_cwall   = _oi_data.get("nearest_call_wall", 0) if _oi_data else 0
        _oi_pfloor  = _oi_data.get("nearest_put_floor", 0) if _oi_data else 0
        _oi_advice  = str(_oi_data.get("advice", "—")) if _oi_data else "—"
    except Exception:
        _oi_dir = "—"; _oi_pcr = 0.0; _oi_maxpain = 0
        _oi_cwall = 0; _oi_pfloor = 0; _oi_advice = "—"

    # ── 3. Astro engine signal ────────────────────────────────────────
    # _astro_snap kept in outer scope so step 5 can safely reference it
    _astro_snap  = {}
    _astro_score = 0
    _astro_sig   = "—"
    _moon_nak    = "—"
    ...
    # ── 5. KP Astro sub-lord (safe — _astro_snap always a dict) ─────
    ...
    _bull_count = sum([
        ...
        1 if ("bull" in _kp_time.lower() or "positive" in _kp_time.lower()) else 0,
    ])
    _bear_count = sum([
    ...
    msg = (
        "🧠🧠🧠🧠🧠🧠🧠🧠🧠🧠\n"
        f"<b>COMBINED ENGINE — {_slot_str}</b>\n"
        f"⏰ {_now_str}\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>VERDICT: {_verdict}</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 <b>SMC+OI:</b> {_smc_signal}  (score: {_score_str})\n"
        ...
        f"   Tithi: {_tithi}\n"
        f"🔮 <b>KP Time:</b> {_kp_time}\n"
        f"🔑 <b>KP Astro Sub:</b> {_kp_sub}  Sub-Sub: {_kp_subsub}\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    _save_tg_dedup(_dedup)
    send_telegram_bg(msg, dedup_key=None)  # dedup already handled above
...
if st.session_state.get("toast_DAILY_UP", True): notify_browser("🚀 YH 1.5 STRONG BREAKOUT", new_YH15)
...
if st.session_state.get("toast_2M_EMA_REVERSAL", True): notify_browser("🟢 4-BAR REVERSAL BUY", new_4BAR_BUY)
...
def fire_oi_15m_alerts(events, spot, atm):
    ...
    for _ev in new_events:
        ...
        # Gate: put crumble → tg_PUT_CRUMBLE, general OI15m → tg_OI_INTEL
...
# 5. O=H / O=L Setups (Gainers + Losers)
_send_tg_structure_alert(
    "OEH_OEL_SETUPS",
...
with tabs[18]:
    ...
    _ALERT_CATEGORIES_UI = [
        ...
        ("⏱️ KP Break Alert (15 min)", "KP_BREAK_15M"),
