...
# ================= CACHE FILE PATHS =================
LIVE_CACHE_CSV     = os.path.join(CACHE_DIR, "live_data.csv")
INDICES_CACHE_CSV  = os.path.join(CACHE_DIR, "indices_live.csv")
OI_CACHE_CSV       = os.path.join(CACHE_DIR, "oi_data.csv")
FUT_CACHE_CSV      = os.path.join(CACHE_DIR, "futures_data.csv")
FIVE_MIN_CACHE_CSV = os.path.join(CACHE_DIR, "five_min.csv")
FIFTEEN_M_CACHE_CSV= os.path.join(CACHE_DIR, "fifteen_min.csv")

def _csv_is_fresh(path, max_age_sec=95):
    """Returns True if CSV exists and was written within max_age_sec seconds."""
...
if replay_mode:
    ...
    if base_df.empty and os.path.exists(LIVE_CACHE_CSV):
        st.warning("⚠️ Live fetch failed — showing last saved data")
        base_df = pd.read_csv(LIVE_CACHE_CSV)
    if _oi_prefetch.empty and os.path.exists(OI_CACHE_CSV):
        _oi_prefetch = pd.read_csv(OI_CACHE_CSV)
...
if new_TOP_HIGH:
    threading.Thread(
        target=send_email,
        args=("TOP LIVE_HIGH Breakout Alert", "New TOP LIVE_HIGH:\n" + "\n".join(new_TOP_HIGH)),
        daemon=True
    ).start()
...
def notify_all(category, title, symbols, ltp_map=None):
    ...
    # ----- SEND ONE EMAIL (non-blocking — runs in background thread) -----
    if email_symbols and is_email_allowed():
        def _send_bg(t=title, syms=email_symbols):
            send_email(t, t + "\n\n" + "\n".join(syms))
...
five_df = pd.read_csv(FIVE_MIN_CACHE_CSV) if os.path.exists(FIVE_MIN_CACHE_CSV) else _5min_EMPTY
...
if indices_df.empty and os.path.exists(INDICES_CACHE_CSV):
    indices_df = pd.read_csv(INDICES_CACHE_CSV)
...
def _fetch_15m_background():
    ...
    if result:
        ...
        for sym, bars in result.items():
            for b in bars:
                rows15.append({"Symbol":sym,"date":b["date"],"open":b["open"],
...
if os.path.exists(FIFTEEN_M_CACHE_CSV):
    _df15 = pd.read_csv(FIFTEEN_M_CACHE_CSV)
...
if fut_df.empty and os.path.exists(FUT_CACHE_CSV):
    fut_df = pd.read_csv(FUT_CACHE_CSV)
...
HI_LO_TRACK_CSV = os.path.join(CACHE_DIR, "15m_hilo_track.csv")
VOL_TRACK_CSV   = os.path.join(CACHE_DIR, "15m_vol_track.csv")

def get_current_15m_slot():
    """Returns the current 15-min slot start time as a string e.g. '09:15', '09:30' …"""
    now = datetime.now(IST)
    minutes = (now.hour * 60 + now.minute)
    slot_start_min = (minutes // 15) * 15
    h, m = divmod(slot_start_min, 60)
    return f"{h:02d}:{m:02d}"

def build_15m_tracker():
    """
    CORRECT SEQUENTIAL 15-MIN BREAKOUT LOGIC:

    TABLE A — HIGH BREAKOUT (sequential chain):
      - Candle 1 (first 15m, 9:15): establishes the reference HIGH
      - Candle 2 must break Candle 1 HIGH (Candle2.HIGH > Candle1.HIGH) ✅
      - Candle 3 must break Candle 2 HIGH (Candle3.HIGH > Candle2.HIGH) ✅  etc.
      - Every single candle in the chain must break the previous candle's HIGH
      - AND current LTP must be above the latest broken high level
      - If ANY candle in the chain fails to break the previous high → EXCLUDED
      - Same logic inverted for LOW BREAKDOWN

    TABLE B — VOLUME SURGE + ABOVE YEST HIGH:
      - Current 15m candle volume > previous 15m candle volume
      - AND LTP > YEST_HIGH (confirmed breakout with volume)
    """
    # ── guard ─────────────────────────────────────────────────
    if not os.path.exists(FIVE_MIN_CACHE_CSV):
        return pd.DataFrame(), pd.DataFrame()

    try:
        raw = pd.read_csv(FIVE_MIN_CACHE_CSV)
    ...
    # ── aggregate 5-min → 15-min OHLCV ───────────────────────
    raw = raw.sort_values("datetime")
    raw["slot"] = raw["datetime"].dt.floor("15T").dt.strftime("%H:%M")

    agg_dict = {"HIGH": ("high","max"), "LOW": ("low","min"),
                "OPEN": ("open","first"), "CLOSE": ("close","last")}
    agg = raw.groupby(["Symbol","slot"]).agg(**agg_dict).reset_index()

    # real volume
    if "volume" in raw.columns:
        vol_agg = raw.groupby(["Symbol","slot"])["volume"].sum().reset_index()
        vol_agg.columns = ["Symbol","slot","VOLUME"]
        agg = agg.merge(vol_agg, on=["Symbol","slot"], how="left")
    else:
        agg["VOLUME"] = 0

    agg = agg.sort_values(["Symbol","slot"]).reset_index(drop=True)

    # ── only look at COMPLETED candles (exclude current in-progress slot) ──
    now_slot = get_current_15m_slot()

    # ── live data lookup ──────────────────────────────────────
    live_cols = ["Symbol","LTP","LIVE_OPEN","LIVE_HIGH","LIVE_LOW",
                 "CHANGE_%","YEST_HIGH","YEST_LOW","YEST_CLOSE",
                 "LIVE_VOLUME","TOP_HIGH","TOP_LOW","NEAR","GAIN"]
    live_cols = [c for c in live_cols if c in df.columns]
    live_map  = df[live_cols].set_index("Symbol").to_dict("index")

    high_rows = []   # TABLE A — sequential HIGH breakout
    low_rows  = []   # TABLE A — sequential LOW breakdown
    vol_rows  = []   # TABLE B — volume surge + above YEST_HIGH
    ...
    for sym, g in agg.groupby("Symbol"):
        ...
        if not live:
            continue
        ltp       = live.get("LTP", 0)
        yest_high = live.get("YEST_HIGH", 0)

        # ══ TABLE A — SEQUENTIAL HIGH BREAKOUT ══════════════
        # Rule: every candle N must have HIGH > candle N-1 HIGH
        # Starting from candle 1 (index 0 = first 15m of the day = 9:15)
        chain_high_ok = True
        for i in range(1, len(completed)):
            if completed.iloc[i]["HIGH"] <= completed.iloc[i-1]["HIGH"]:
                chain_high_ok = False
                break

        if chain_high_ok:
            # last completed candle's HIGH is the reference level
            last_candle   = completed.iloc[-1]
            prev_candle   = completed.iloc[-2]
            broken_level  = round(last_candle["HIGH"], 2)
            # LTP must still be above the latest broken high
            if ltp > broken_level:
                high_rows.append({
                    "Symbol":         sym,
                    "LTP":            round(ltp, 2),
                    "CHANGE_%":       live.get("CHANGE_%", 0),
                    "FIRST_SLOT":     completed.iloc[0]["slot"],
                    "FIRST_HIGH":     round(completed.iloc[0]["HIGH"], 2),
                    "PREV_SLOT":      prev_candle["slot"],
                    "PREV_HIGH":      round(prev_candle["HIGH"], 2),
                    "BROKEN_LEVEL":   broken_level,
                    "CANDLES_CHAIN":  len(completed),
                    "ABOVE_LEVEL_%":  round((ltp - broken_level) / broken_level * 100, 2),
        ...
        # ══ TABLE A — SEQUENTIAL LOW BREAKDOWN ══════════════
        # Rule: every candle N must have LOW < candle N-1 LOW
        chain_low_ok = True
        for i in range(1, len(completed)):
            if completed.iloc[i]["LOW"] >= completed.iloc[i-1]["LOW"]:
                chain_low_ok = False
                break
        ...
        if chain_low_ok:
            last_candle  = completed.iloc[-1]
            prev_candle  = completed.iloc[-2]
            broken_level = round(last_candle["LOW"], 2)
            if ltp < broken_level:
                low_rows.append({
                    "Symbol":         sym,
                    "LTP":            round(ltp, 2),
                    "CHANGE_%":       live.get("CHANGE_%", 0),
                    "FIRST_SLOT":     completed.iloc[0]["slot"],
                    "FIRST_LOW":      round(completed.iloc[0]["LOW"], 2),
                    "PREV_SLOT":      prev_candle["slot"],
                    "PREV_LOW":       round(prev_candle["LOW"], 2),
                    "BROKEN_LEVEL":   broken_level,
                    "CANDLES_CHAIN":  len(completed),
                    "BELOW_LEVEL_%":  round((broken_level - ltp) / broken_level * 100, 2),
                    "LIVE_HIGH":      live.get("LIVE_HIGH", 0),
                    "LIVE_LOW":       live.get("LIVE_LOW",  0),
                    "YEST_LOW":       live.get("YEST_LOW", 0),
                    "NEAR":           live.get("NEAR", ""),
                    "GAIN":           live.get("GAIN", ""),
                    "TOP_HIGH":       live.get("TOP_HIGH", 0),
                    "TOP_LOW":        live.get("TOP_LOW",  0),
        ...
        # ══ TABLE B — VOLUME SURGE + ABOVE YEST HIGH ════════
        # Current completed candle volume > previous completed candle volume
        # AND LTP > YEST_HIGH
        if len(completed) >= 2:
            curr_vol = completed.iloc[-1]["VOLUME"]
            prev_vol = completed.iloc[-2]["VOLUME"]
            curr_slot = completed.iloc[-1]["slot"]
            prev_slot = completed.iloc[-2]["slot"]
            if curr_vol > prev_vol and ltp > yest_high and prev_vol > 0:
                vol_surge_pct = round((curr_vol - prev_vol) / prev_vol * 100, 1)
                vol_rows.append({
                    "Symbol":       sym,
                    "LTP":          round(ltp, 2),
                    "CHANGE_%":     live.get("CHANGE_%", 0),
    ...
    # TABLE A: HIGH breakouts (sorted by chain length desc, then % above)
    high_df = pd.DataFrame(high_rows)
    low_df  = pd.DataFrame(low_rows)

    if not high_df.empty:
        high_df = high_df.sort_values(
            ["CANDLES_CHAIN","ABOVE_LEVEL_%"], ascending=[False, False])
    if not low_df.empty:
        low_df  = low_df.sort_values(
            ["CANDLES_CHAIN","BELOW_LEVEL_%"], ascending=[False, False])

    # Combine HIGH + LOW into one hilo_df with a TYPE column
    if not high_df.empty:
        high_df["TYPE"] = "🟢 HIGH BREAK"
    if not low_df.empty:
        low_df["TYPE"]  = "🔴 LOW BREAK"

    hilo_df = pd.concat([high_df, low_df], ignore_index=True) if (not high_df.empty or not low_df.empty) else pd.DataFrame()
    ...
    if not hilo_df.empty:
        hilo_df.to_csv(HI_LO_TRACK_CSV, index=False)
    if not vol_df.empty:
        vol_df.to_csv(VOL_TRACK_CSV, index=False)

    return hilo_df, vol_df

# Build tracker every rerun (uses already-loaded five_df candles)
_hilo_df, _vol_df = build_15m_tracker()
...
with tabs[4]:   # 👈 replace with correct index
    ...
    # ═══════════════════════════════════════════════════════════
    # 🟢 TABLE A1 — SEQUENTIAL 15-MIN HIGH BREAKOUT
    # Every candle's high > previous candle's high (full chain from 9:15)
    # AND LTP currently above the latest broken high
    # ═══════════════════════════════════════════════════════════
    st.markdown("### ⚡ Sequential 15-Min HIGH Breakout")
    st.caption("Every 15m candle broke previous candle's HIGH from 9:15 onwards — LTP still above latest broken level")

    _high_df = _hilo_df[_hilo_df["TYPE"] == "🟢 HIGH BREAK"] if not _hilo_df.empty and "TYPE" in _hilo_df.columns else pd.DataFrame()

    if _high_df.empty:
        st.info("No stocks with sequential 15-min high breakout chain.")
    else:
        _high_show = [c for c in ["Symbol","LTP","CHANGE_%","TYPE","FIRST_SLOT","FIRST_HIGH",
                      "PREV_SLOT","PREV_HIGH","BROKEN_LEVEL","CANDLES_CHAIN",
                      "ABOVE_LEVEL_%","LIVE_HIGH","LIVE_LOW","YEST_HIGH",
                      "NEAR","GAIN","TOP_HIGH","TOP_LOW"] if c in _high_df.columns]
        st.dataframe(
            _high_df[_high_show].style.background_gradient(subset=["ABOVE_LEVEL_%"], cmap="Greens"),
            use_container_width=True
        )
    ...
    # ═══════════════════════════════════════════════════════════
    # 🔴 TABLE A2 — SEQUENTIAL 15-MIN LOW BREAKDOWN
    # Every candle's low < previous candle's low (full chain from 9:15)
    # AND LTP currently below the latest broken low
    # ═══════════════════════════════════════════════════════════
    st.markdown("### 🔻 Sequential 15-Min LOW Breakdown")
    st.caption("Every 15m candle broke previous candle's LOW from 9:15 onwards — LTP still below latest broken level")

    _low_df = _hilo_df[_hilo_df["TYPE"] == "🔴 LOW BREAK"] if not _hilo_df.empty and "TYPE" in _hilo_df.columns else pd.DataFrame()

    if _low_df.empty:
        st.info("No stocks with sequential 15-min low breakdown chain.")
    else:
        _low_show = [c for c in ["Symbol","LTP","CHANGE_%","TYPE","FIRST_SLOT","FIRST_LOW",
                     "PREV_SLOT","PREV_LOW","BROKEN_LEVEL","CANDLES_CHAIN",
                     "BELOW_LEVEL_%","LIVE_HIGH","LIVE_LOW","YEST_LOW",
                     "NEAR","GAIN","TOP_HIGH","TOP_LOW"] if c in _low_df.columns]
        st.dataframe(
            _low_df[_low_show].style.background_gradient(subset=["BELOW_LEVEL_%"], cmap="Reds"),
            use_container_width=True
        )
    ...
    # ═══════════════════════════════════════════════════════════
    # 🔵 TABLE B — 15-MIN VOLUME SURGE + ABOVE YESTERDAY HIGH
    # Current 15m candle volume > previous 15m candle volume
    # AND LTP > YEST_HIGH (confirmed breakout with volume)
    # ═══════════════════════════════════════════════════════════
    st.markdown("### 📈 15-Min Volume Surge + Above Yesterday High")
    st.caption("Current 15m candle volume exceeded previous candle volume AND LTP is above yesterday's high")
    if _vol_df.empty:
        st.info("No stocks currently meeting volume surge + above yesterday high criteria.")
    else:
        _vol_show = [c for c in ["Symbol","LTP","CHANGE_%","PREV_SLOT","PREV_VOL",
                     "CURR_SLOT","CURR_VOL","VOL_SURGE_%","YEST_HIGH","LIVE_HIGH",
                     "LIVE_VOLUME","NEAR","GAIN","TOP_HIGH","TOP_LOW"] if c in _vol_df.columns]
        st.dataframe(
            _vol_df[_vol_show].style.background_gradient(subset=["VOL_SURGE_%"], cmap="Blues"),
            use_container_width=True
        )
