...
def paper_position_manager():
    ...
    global daily_pnl, profit_lock_armed
    ...
    while True:
        ...
        for symbol in list(paper_positions.keys()):
            ...
            # ── Profit Lock check (PAPER mode) ───────────────────────────
            # In LIVE mode this runs inside position_manager(). For PAPER
            # we mirror the same guard here so the lock arms correctly.
            if not profit_lock_armed and daily_pnl >= PROFIT_LOCK_TRIGGER:
                profit_lock_armed = True
                _lock_msg = (
                    f"🔒 PROFIT LOCK ARMED (PAPER) | PnL:{daily_pnl:.2f} | "
                    f"Floor:₹{PROFIT_LOCK_FLOOR}"
                )
                print(Fore.GREEN + _lock_msg + ts())
                write_log(_lock_msg)
                # Tighten SLs: move every open position's SL to entry (breakeven)
                _tick_lock = 0.05
                for _lsym, _lpos in list(paper_positions.items()):
                    if _lpos.get("status") != "OPEN":
                        continue
                    _le   = float(_lpos.get("entry", 0))
                    _lsd  = _lpos.get("side", "BUY")
                    _lbase = _lsym.split("__PYR")[0]
                    _tick_lock = tick_sizes.get(_lbase, 0.05)
                    _new_lock_sl = round(round(_le / _tick_lock) * _tick_lock, 10)
                    _cur_lock_sl = float(_lpos.get("sl", 0))
                    if _lsd == "BUY" and _new_lock_sl > _cur_lock_sl:
                        ...
                        write_log(f"PROFIT LOCK PAPER SL tightened {_lsym} → {_new_lock_sl}")
                    elif _lsd == "SELL" and _new_lock_sl < _cur_lock_sl:
                        _lpos["sl"] = _new_lock_sl
                        _lpos["_sl_floor"] = _new_lock_sl
                        _lpos["trail_level"] = max(_lpos.get("trail_level", 0), _lpos.get("tsl_step", TRAIL_STEP_PERCENT))
                        write_log(f"PROFIT LOCK PAPER SL tightened {_lsym} → {_new_lock_sl}")
                ...
                def _tighten_all_sls():
                    ...
                    try:
                        ...
                        for sym, pos in list(paper_positions.items()):
                            ...
                            # For SELL: SL above entry. Tighten = move toward entry (down).
                            # Only move if current SL is still above entry.
                            # For BUY:  SL below entry. Tighten = move toward entry (up).
                            if side == "SELL":
                                if new_sl >= cur_sl:
                                    continue   # already at breakeven or better
                            else:
                                if new_sl <= cur_sl:
                                    continue
