...
def send_telegram(message: str, dedup_key: str = None, parse_mode: str = "HTML") -> bool:
    ...
    dedup_key: if provided, sent once per 10-min slot per key.
    """
    if not TG_BOT_TOKEN:
        return False
...
def fire_panchak_tg_alerts(df_live, ltp_map=None):
    ...
    for _, row in df_live.iterrows():
        ...
        if pd.notna(top_high) and ltp > 0 and ltp >= float(top_high):
            ...
            # General panchak alert: dedup per panchak period
    ...
    if _special_up and st.session_state.get("special_stock_alerts", True):
        ...
        _sp_key = "SPECIAL_UP_" + "_".join(_sp_syms[:3]) + "_" + datetime.now(IST).strftime("%Y%m%d_%H%M")[:13]
    ...
    if _special_down and st.session_state.get("special_stock_alerts", True):
        ...
        _sp_key = "SPECIAL_DOWN_" + "_".join(_sp_syms[:3]) + "_" + datetime.now(IST).strftime("%Y%m%d_%H%M")[:13]
        send_telegram_bg(_sp_tg_msg, dedup_key=_sp_key)  # gated by special_stock_alerts above
...
def fetch_indices_now():
    ...
    for index_name, sym in INDEX_SYMBOLS.items():
        try:
            # Try both NSE: key and direct lookup
            q = all_quotes.get(f"NSE:{sym}") or all_quotes.get(sym)
            if not q:
                print(f"[INDICES] No quote for {index_name} ({sym})")
                continue
