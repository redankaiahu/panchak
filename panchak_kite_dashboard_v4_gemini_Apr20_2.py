...
def _load_inside_bars():
    ...
    for sym in STOCKS:
        try:
            ...
            if ch <= ph and cl >= pl:
                ...
                try:
                    _lo_rows = df[df["Symbol"] == sym]["Open"] # Fix: use "Open" not "LIVE_OPEN" if needed, but the main df has "Open"
                    if "Open" not in df.columns and "LIVE_OPEN" in df.columns:
                ...
                rows.append({
                    ...
                    "CH→PH Gap %":      ch_to_ph_pct,          # numeric for sorting
    ...
    if not rows:
        return pd.DataFrame(columns=_IB_COLS)
